...
### Build and Run
- `cargo run` - Build and run Labrador locally
- `cargo bundle --bin labrador` - Bundle the main app

### Running with a local backend
To connect Labrador to a local backend instance:
...
- `cargo nextest run -p labrador_completer --features v2` - Run completer tests with v2 features
...
- `./script/run-clang-format.py -r --extensions 'c,h,cpp,m' ./crates/labrador_ui/src/ ./app/src/` - Format C/C++/Obj-C code
...
This is a Rust-based terminal emulator with a custom UI framework.

### Key Components

**UI Framework** (`crates/labrador_ui/`, `crates/labrador_ui_core/`):
...
- Local workspace and Drive-style features (`drive/`)
- Authentication and user management (`auth/`)
- Settings and preferences (`settings/`)
- Workspace and session management (`workspace/`)

**Core Libraries**:
- `crates/labrador_core/` - Core utilities and platform abstractions
- `crates/editor/` - Text editing functionality
- `crates/labrador_ui/` and `crates/labrador_ui_core/` - Custom UI framework
...
5. **Local Data Flow**: Workspace and settings state is coordinated through shared models

### Development Guidelines

**Workspace Structure**:
- This is a Cargo workspace with 60+ member crates
- Main binary is in `app/`, UI framework in `crates/labrador_ui/`
...
**GraphQL**:
- Schema and client code generation from `crates/labrador_graphql_schema/api/schema.graphql`
- TypeScript types generated for frontend integration

### Feature Flags

Labrador uses compile-time feature flags with a small runtime plumbing layer.

How to add a feature flag:
- Add a new variant to `labrador_core/src/features.rs` in the `FeatureFlag` enum
