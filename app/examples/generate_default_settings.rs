...
use labrador_core::features::{FeatureFlag, DEBUG_FLAGS, DOGFOOD_FLAGS, PREVIEW_FLAGS, RELEASE_FLAGS};
use labrador_ui_extras::user_preferences::toml_backed::TomlBackedUserPreferences;
use labrador_ui_extras::user_preferences::UserPreferences as _;
...
/// Without an explicit reference to the `labrador` library, the linker will
/// not include most of the app's object files and the `inventory`
/// submissions they contain.
fn ensure_settings_linked() {
    let _ = std::hint::black_box(labrador::settings::RESTORE_SESSION);
}
