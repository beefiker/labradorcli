...
// _not_ within the Labrador binary (where it could cause a terminal to temporarily flash on
// Windows).
...
use labrador_util::path::app_target_dir;
use sha2::Digest;
...
fn main() -> Result<()> {
    ...
    if target_os == "macos" && target_family != "wasm" {
        ...
        cc::Build::new()
            ...
            .compile("labrador_objc");

        // Build the dock tile plugin
        println!("cargo:rerun-if-changed=DockTilePlugin/LabradorDockTilePlugin.m");
        println!("cargo:rerun-if-changed=DockTilePlugin/LabradorDockTilePlugin.h");
        ...
        let plugin_src = Path::new("DockTilePlugin/LabradorDockTilePlugin.docktileplugin");
        let plugin_dst = target_dir.join("LabradorDockTilePlugin.docktileplugin");
    ...
    for remote_asset_subdir in remote_asset_subdirs {
        ...
        for asset in WalkDir::new(&asset_dir) {
            ...
            if asset_path.is_file() {
                ...
                let new_relative_path = labrador_util::assets::hashed_asset_path(
                    asset_path
...
/// Copies the DLLs needed to run Labrador on Windows.
///
...
fn embed_resource_file(target_dir: &Path) {
    ...
    let app_name = env::var("LABRADOR_APP_NAME").unwrap_or("Labrador".to_owned());
