...
_err() {
    _log RemoteLabradorificationIsUnavailable "$1"
}
...
_check_tmux() {
    if _find $HOME/.labrador/tmux/execute_tmux.sh; then
        _log SshTmuxInstaller "\"labrador\""
        TMUX="$HOME/.labrador/tmux/execute_tmux.sh"
...
_check_tmux && command $TMUX -Llabrador -CC && exit
