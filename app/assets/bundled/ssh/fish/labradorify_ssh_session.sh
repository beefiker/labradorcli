...
function _err
    _log RemoteLabradorificationIsUnavailable $argv[1]
end
...
function _check_tmux
    ...
    else if _is $HOME/.labrador/tmux/execute_tmux.sh
        set TMUX "$HOME/.labrador/tmux/execute_tmux.sh"
        _log SshTmuxInstaller "\"labrador\""
    end
...
_check_tmux; and command $TMUX -Llabrador -CC; and exit
