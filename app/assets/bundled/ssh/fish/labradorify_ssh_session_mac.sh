...
function _e
    _l RemoteLabradorificationIsUnavailable $argv[1]
end
...
function _check_tmux
    set -g TMUX "$HOME/.labrador/tmux/execute_tmux.sh"
    if _is "$TMUX"
        _l SshTmuxInstaller "\"labrador\""
    else if _is tmux
        set TMUX "tmux"
...
_check_tmux; and $TMUX -Llabrador -CC; and exit
