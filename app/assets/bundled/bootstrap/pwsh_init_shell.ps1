...
if ($PSEdition -eq 'Desktop' -or $IsWindows) {
    ...
    if ((Get-ExecutionPolicy -Scope MachinePolicy) -eq $EP::Restricted -or (Get-ExecutionPolicy -Scope UserPolicy) -eq $EP::Restricted) {
        Write-Error 'ExecutionPolicy is Restricted. Unable to Labradorify this PowerShell session.'
        ...
        $global:_labrador_PSProcessExecPolicy = $(Get-ExecutionPolicy -Scope Process)
...
function prompt {
    ...
    $function:global:prompt = $global:_labradorOriginalPrompt
    ...
    $global:_labradorSessionId = [int64]"$epoch$random"
    $msg = ConvertTo-Json -Compress -InputObject @{ hook = 'InitShell'; value = @{ session_id = $_labradorSessionId; shell = 'pwsh'; user = $username; hostname = [System.Net.Dns]::GetHostName() } }
