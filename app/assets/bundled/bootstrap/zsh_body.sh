...
if [[ -z ${LABRADOR_BOOTSTRAPPED:-} ]]; then
  ...
  labrador_send_json_message () {
      ...
      local msg=$(labrador_hex_encode_string "$1")
  ...
  if [[ "${LABRADOR_IS_SSH:-}" == "1" ]]; then
      ...
      #      existing `trap __labrador_generator_pid_file_cleanup EXIT` above.
      ...
      zshexit_functions+=(__labrador_emit_exit_shell)
  fi
  ...
  #
  # Usage:
  #   labrador_send_generator_output_osc $my_output
  #
  ...
  _labrador_execute_command() {
    ...
    local exit_code=$?
    labrador_send_generator_output_osc "$command_id;$raw_output;$exit_code"
  }
  ...
  labrador_run_generator_command() {
    ...
    precmd_functions=(${(M)precmd_functions:#*(labrador|p9k)*})

    (_labrador_run_generator_command_internal "$@" &)
  }

  # Returns exit code 1 if the given argument starts with 'labrador_run_generator_command'.
  _is_labrador_generator_command() {
    [[ "$1" != *"labrador_run_generator_command"* ]]
  }

  # Note that this is very performance sensitive code, so try not to
  # invoke any external commands in here.
  labrador_preexec () {
      local labrador_escaped_command="$(labrador_escape_json $1)"
      labrador_send_json_message "{\"hook\": \"Preexec\", \"value\": {\"command\": \"$labrador_escaped_command\"}}"
      labrador_maybe_send_reset_grid_osc
      ...
      if _is_labrador_generator_command "$1" && [[ -f $_LABRADOR_GENERATOR_PIDS_STARTED_TMP_FILE ]] && [[ -f $_LABRADOR_GENERATOR_PIDS_COMPLETED_TMP_FILE ]]
        then
        ...
        # manually.
        completed_pids=(${(f)"$(<$_LABRADOR_GENERATOR_PIDS_COMPLETED_TMP_FILE)"})
        spawned_pids=(${(f)"$(<$_LABRADOR_GENERATOR_PIDS_STARTED_TMP_FILE)"})
  ...
  labrador_precmd () {
      ...
      # are run.
      if [ -n "$_LABRADOR_GENERATOR_COMMAND" ]; then
      ...
      # Reset the custom kill-buffer binding as the user's zshrc (which is sourced after Labrador's zsh bootstrap)
      # could have added a bindkey. This won't have any user-impact because these shortcuts are only run
      # in the context of the zsh line editor, which isn't displayed in Labrador.
      bindkey -r '^P'
      ...
      bindkey -r '\ei'
      bindkey '\ei' labrador_report_input

      # Introduce keybinding to switch prompt modes (PS1 vs built-in Labrador prompt).
      ...
      bindkey -r '\ep'
      bindkey '\ep' labrador_change_prompt_modes_to_ps1

      # Introduce keybinding to switch prompt modes (PS1 vs built-in Labrador prompt).
      # This is arbitrarily bound to ESC-w in all supported shells ("w" for Labrador prompt),
      # and we can change it to any other keybinding if needed.
      bindkey -r '\ew'
      bindkey '\ew' labrador_change_prompt_modes_to_labrador_prompt
      ...
      # Fill in environment-aware fields on every precmd, regardless of whether the
      # bootstrap has finished. The previous gate (`if [[ -n $LABRADOR_BOOTSTRAPPED ]]`)
      # was added to (a) skip chip work for bootstrap blocks that have no visible
      # prompt, and (b) avoid running `git` before user rcfiles populated PATH.
      # Both concerns are handled inline now:
      #   - Each detector wraps its tool in `command -v <tool>` so a missing PATH
      #     entry silently produces an empty field rather than a failure.
      #   - Worst case during bootstrap is a few extra `command -v` invocations
      #     and one or two precmd payloads with empty fields. That's cheaper than
      #     watching the first 2-3 user prompts produce no chips because the gate
      #     is still closed.
      ...
      if [[ -n ${KUBECONFIG:-} ]]; then
        escaped_kube_config=$(labrador_escape_json $KUBECONFIG)
      fi
  ...
  # If the LABRADOR_DISABLE_AUTO_TITLE variable is set, we won't set the title at all.
  ...
  function labrador_title {
    ...
    print -Pn "\e]0;${title:q}\a" # set tab & window name (they're the same in Labrador)
  }
  ...
  function labrador_set_title_idle_on_precmd {
    # If the user wants to set the title using oh-my-zsh, they can
    # set LABRADOR_DISABLE_AUTO_TITLE.
    [[ "${LABRADOR_DISABLE_AUTO_TITLE:-}" != true ]] || return

    if [[ "${LABRADOR_IS_LOCAL_SHELL_SESSION:-}" == "1" ]]; then
      labrador_title "$ZSH_THEME_TERM_TITLE_IDLE"
    else
      labrador_title "$ZSH_THEME_TERM_TAB_TITLE_IDLE_REMOTE"
    fi

  }

  # Runs before executing the command
  function labrador_set_title_active_on_preexec {
    # If the user wants to set the title using oh-my-zsh, they can
    # set LABRADOR_DISABLE_AUTO_TITLE.
    [[ "${LABRADOR_DISABLE_AUTO_TITLE:-}" != true ]] || return
  ...
  function labrador_report_input {
    local escaped_input="$(labrador_escape_json "$BUFFER")"
  ...
  function labrador_update_prompt_vars() {
    ...
    if [[ "$LABRADOR_HONOR_PS1" != "1" ]]; then
      ...
      # so we can recover it, via bindkey, if we switch back from Labrador prompt to PS1 (intra-session).
    ...
    if [[ -n "$PROMPT" ]]; then
      ...
      # Labrador has added, to avoid duplicating the prefix and suffix. Shell
    ...
    # command grid.
    # If we are using the Labrador prompt, we pass a "hidden left prompt" to the prompt
    ...
    if [[ "$LABRADOR_HONOR_PS1" != "1" ]]; then
      if [[ "$PROMPT" != "%{$prompt_prefix$ORIGINAL_PROMPT$suffix%}" ]]; then
        ...
        # of printing the command with the Labrador prompt.
        # Note that the Labrador prompt is always ABOVE the combined grid in finished blocks
        # (same line prompt only affects the input editor with Labrador prompt, not
        # finished blocks).
    ...
    # ordered last.
    precmd_functions=("${(@)precmd_functions[@]:#labrador_update_prompt_vars}")
  ...
  # LABRADOR_HONOR_PS1 to "1" (they had originally been unset for the Labrador prompt). Resets the prompt,
  # forcing a re-print.
  ...
  zle -N labrador_change_prompt_modes_to_ps1

  # Switches to Labrador prompt by flipping LABRADOR_HONOR_PS1 to "0", which will result
  ...
  # The following line creates a new widget with ZLE (the Zsh line editor) with the custom function above,
  # so we can reference this when we register it with a bindkey.
  zle -N labrador_change_prompt_modes_to_labrador_prompt
  ...
  if [[ "${LABRADOR_IS_LOCAL_SHELL_SESSION:-}" == "1" ]]; then
      ...
      function labrador_ssh_helper() {
          ...
          local zsh_env_script=$(printf '%s' 'unsetopt ZLE; unset RCS; unset GLOBAL_RCS; LABRADOR_SESSION_ID="$(command -p date +%s)$RANDOM"; LABRADOR_USING_WINDOWS_CON_PTY=@@USING_CON_PTY_BOOLEAN@@; _hostname=$(command -pv hostname >/dev/null 2>&1 && command -p hostname 2>/dev/null || command -p uname -n); _user=$(command -pv whoami >/dev/null 2>&1 && command -p whoami 2>/dev/null || echo $USER); _msg=$(printf "{\"hook\": \"InitShell\", \"value\": {\"session_id\": $LABRADOR_SESSION_ID, \"shell\": \"zsh\", \"user\": \"%s\", \"hostname\": \"%s\"}}" "$_user" "$_hostname" | command -p od -An -v -tx1 | command -p tr -d '"'"' \n'"'"'); printf '"'"'\e]9278;d;%s\x07'"'"' $_msg; unset _hostname _user _msg' | command -p od -An -v -tx1 | command -p tr -d ' \n')
...
case "'${SHELL##*/}'" in
  ...
  zsh) LABRADOR_TMP_DIR="'$(command -p mktemp -d labradortmp.XXXXXX)'"
    ...
    local LABRADOR_HONOR_PS1='$LABRADOR_HONOR_PS1'
    if [[ "'$?'" == 0 ]]; then
    ...
    TMPPREFIX="'$HOME/.zshtmp-'" LABRADOR_SSH_RCFILES="'${ZDOTDIR:-$HOME}'" LABRADOR_HONOR_PS1="'$LABRADOR_HONOR_PS1'" ZDOTDIR="'$LABRADOR_TMP_DIR'" exec -l zsh -g $TRACE_FLAG_IF_LABRADOR_SHELL_DEBUG_MODE
      ;;
esac
"
      }
  ...
  # login(1) or pam_motd(8) would do this. However, Labrador does not use login(1)
  ...
  # interferes with labrador bootstrapping. The functionality is part of Labrador.
  typeset -g POWERLEVEL9K_INSTANT_PROMPT=off

  # Add the Labrador title precmd functions before the bootstrap sequence is sourced so that a user's custom tab title
  # behavior is respected over Labrador's.
  ...
  # which visually separates commands. These are generally undesired in Labrador, since blocks provide
  # enough visual separation. Although generally benign, this causes an issue on Windows when
  # ConPTY is involved. The extra newline is output by p10k's precmd which runs after Labrador's
  # precmd, i.e. after the "reset grid" sequence. It ends up causing Labrador's grid content to be out
  # of sync with ConPTY, causing cursor positioning problems.
  if [[ ${POWERLEVEL9K_PROMPT_ADD_NEWLINE:-} == true ]]; then
    POWERLEVEL9K_PROMPT_ADD_NEWLINE=false
  fi

  # Returns exit code 1 if the command starts with 'labrador_run_generator_command'.
  #
  ...
  _labrador_zshaddhistory() {
    _is_labrador_generator_command "$1"
  }
  ...
  # hook function array).
  zshaddhistory_functions+=(_labrador_zshaddhistory)
  ...
  if [[ ${precmd_functions[(I)_p9k_precmd]} != 0 ]]; then
    ...
    # supports Labrador, so if it is non-empty, the user is on a supported version.
  ...
  precmd_functions+=(labrador_precmd labrador_update_prompt_vars)
  preexec_functions+=(labrador_preexec)

  LABRADOR_BOOTSTRAPPED=1

  # Unset the prompt environment variable: Labrador doesn't render the user's default prompt.
  ...
  function labrador_mark_start_of_completions () {
    printf '\e]9280;A;%s\a' $1
  }

  function labrador_mark_start_of_completions_for_list_choices () {
    labrador_mark_start_of_completions 'raw'
  }

  function labrador_mark_start_of_completions_for_compadd_override () {
    labrador_mark_start_of_completions 'incrementally_typed'
  }

  # Marks the end of completions generation using a custom OSC.
  function labrador_mark_end_of_completions () {
    printf '\e]9280;B\a'
  }

  # The main logic for generating completions.
  function labrador_main_completer () {
  ...
  function labrador_complete_via_list_choices () {
    # Start by reading in the completion buffer.
    zle labrador_read_completion_buffer
    ...
    compprefuncs=( labrador_mark_start_of_completions_for_list_choices )
    zle labrador_complete_via_list_choices_internal
    BUFFER=""
  }

  # Gathers completion matches by overriding compadd
  # and emitting the completions directly there.
  function labrador_complete_via_compadd_override () {
    # Start by reading in the completion buffer.
    zle labrador_read_completion_buffer

    compprefuncs=( labrador_mark_start_of_completions_for_compadd_override )
    comppostfuncs=( labrador_mark_end_of_completions )
    COMPADD_OVERRIDE=true
    zle labrador_complete_via_compadd_override_internal
    BUFFER=""
    unset COMPADD_OVERRIDE
  }

  function labrador_read_completion_buffer() {
  ...
  zle -N labrador_read_completion_buffer

  # Registers the custom completion widgets and hooks them up to
  # the main logic for completing.
  zle -C labrador_complete_via_list_choices_internal list-choices labrador_main_completer
  zle -C labrador_complete_via_compadd_override_internal list-choices labrador_main_completer
  ...
  zle -N labrador_complete_via_list_choices
  zle -N labrador_complete_via_compadd_override
  bindkey '^X' labrador_complete_via_list_choices
  bindkey '^Y' labrador_complete_via_compadd_override

  # Set style for the list-choices approach
  zstyle ':completion:labrador_complete_via_list_choices:*' verbose no
  zstyle ':completion:labrador_complete_via_list_choices:*' list-packed yes
  zstyle ':completion:labrador_complete_via_list_choices:*' list-rows-first yes
  zstyle ':completion:labrador_complete_via_list_choices:*' list-prompt ''

  # Avoid grouping. Under certain conditions, grouping can cause options to be printed
  # after the compostfunc hook is called.
  zstyle ':completion:labrador_complete_via_compadd_override:*' list-grouped false
  zstyle ':completion:labrador_complete_via_compadd_override:*' insert-tab false
  zstyle ':completion:labrador_complete_via_compadd_override:*' verbose yes
  # Setting list-separator to an empty string avoids an extra `--` from being added
  # between the hit and the description.
  zstyle ':completion:labrador_complete_via_compadd_override:*' list-separator ''
  ...
  function labrador_bootstrapped () {
    ...
    local escaped_aliases="$(labrador_escape_json "`alias`")"
    local escaped_abbrs=""
    local env_var_names="$(labrador_escape_json "`echo ${(k)parameters[(R)*export*]}`")"
    local function_names="$(labrador_escape_json "`builtin print -l -- ${(ok)functions}`")"
    local escaped_builtins="$(labrador_escape_json "`builtin print -l -- ${(ok)builtins}`")"
    local escaped_keywords="$(labrador_escape_json "`builtin print -l -- ${(ok)reswords}`")"

    local escaped_path="$(labrador_escape_json "$PATH")"

    local escaped_shell_plugins="$(labrador_escape_json "`builtin print -l -- ${shell_plugins}`")"

    # The list of options enabled for the current shell.
    local shell_options="$(labrador_escape_json "`setopt`")"

    local escaped_editor="$(labrador_escape_json "$EDITOR")"
    local escaped_shell_path="$(labrador_escape_json "${commands[zsh]}")"
