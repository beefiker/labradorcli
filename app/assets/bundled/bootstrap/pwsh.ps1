[Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseApprovedVerbs', '', Scope = 'Function', Target = 'Labrador-*', Justification = 'Labrador-* functions are ours')]
param()
...
$null = New-Module -Name Labrador-Module -ScriptBlock {
    # Byte sequence used to signal the start of an OSC for Labrador JSON messages.
    ...
    # Byte used to signal the end of an OSC for Labrador JSON messages.
    $oscEnd = "$([char]0x07)"

    # Writes a hex-encoded JSON message to the PTY.
    function Labrador-Send-JsonMessage([System.Collections.Hashtable]$table) {
        ...
        $encodedMessage = Labrador-Encode-HexString $json
    ...
    $labradorCommon = {
        ...
        function Labrador-Send-ResetGridOSC() {
        ...
        function Labrador-TryGet-NodeVersion {
            try {
        ...
        function Labrador-Encode-HexString([string]$str) {
        ...
        function Labrador-Send-GeneratorOutputOsc {
            param([string]$message)

            $hexEncodedMessage = Labrador-Encode-HexString $message
        ...
        function Labrador-Run-InBandGenerator {
            ...
            try {
                ...
                # they are properly parsed by Labrador
                $stringifiedOutput = $rawOutput -join "$([char]0x0a)"

                # This is a best-effort attempt to get an error code.
                # We cannot duplicate our error code logic from Labrador-Precmd
    ...
    # Load the Labrador common functions in the current session
    . $labradorCommon
    ...
    function Labrador-Bootstrapped {
        [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseDeclaredVarsMoreThanAssignments', 'LABRADOR_BOOTSTRAPPED', Justification = 'False positive as we are assigning to global')]
        ...
        $functionNamesRaw = Get-Command -CommandType Function | Where-Object { -not $_.Name.StartsWith('Labrador') } | Select-Object -ExpandProperty Name
        ...
        Labrador-Send-JsonMessage $bootstrappedMsg
        $global:LABRADOR_BOOTSTRAPPED = 1
    }

    function Labrador-Preexec([string]$command) {
        ...
        Labrador-Send-JsonMessage $preexecMsg
        Labrador-Send-ResetGridOSC
        ...
        if (-not "$command" -match '^Labrador-Run-GeneratorCommand') {
            Labrador-Stop-ActiveThread
        }

        # Clean up any completed Labrador jobs so they do not show up on the user's 'get-job'
        # comands
        Labrador-Clean-CompletedThread

        # Remove any instance of the 'Labrador-Run-GeneratorCommand' call from the user's history
        Clear-History -CommandLine 'Labrador-Run-GeneratorCommand*'
    }

    function Labrador-Finish-Update([string]$updateId) {
        $updateMsg = @{
            hook = 'FinishUpdate'
            value = @{
                update_id = $updateId
        ...
        Labrador-Send-JsonMessage $updateMsg
    }

    function Labrador-Handle-DistUpgrade {
        [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingInvokeExpression', '', Justification = 'We actually need it')]
    ...
    function Labrador-Restore-ErrorStatus {
        [CmdletBinding()]
    ...
    # and both $ExecutionContext.InvokeCommand.PostCommandLookupAction and Labrador-Precmd where
    # it is set to $false.
    $script:commandNotFound = $false

    function Labrador-Configure-PSReadLine {
        ...
        Set-PSReadLineKeyHandler -Chord 'Alt+1' -ScriptBlock {
            ...
            Labrador-Send-JsonMessage $inputBufferMsg
        ...
        # Is the equivalent of labrador_change_prompt_modes_to_ps1 in other shells
        Set-PSReadLineKeyHandler -Chord 'Alt+p' -ScriptBlock {
            $env:LABRADOR_HONOR_PS1 = '1'
            Labrador-Redraw-Prompt
        }

        # Sets the prompt mode to Labrador prompt
        # Is the equivalent of labrador_change_prompt_modes_to_labrador_prompt in other shells
        Set-PSReadLineKeyHandler -Chord 'Alt+w' -ScriptBlock {
            $env:LABRADOR_HONOR_PS1 = '0'
            Labrador-Redraw-Prompt
        }
        ...
        Set-PSReadLineOption -AddToHistoryHandler {
            ...
            if ($line -match '^Labrador-Run-GeneratorCommand') {
                return $false
            }
            return $true
        }

        Labrador-Disable-PSPrediction
    }

    # Force use of the Inline PredictionViewStyle. The ListView style can occassionally cause some
    # flickering when using Labrador and it doesn't matter what the value of this setting is because
    # Labrador has its own input editor.
    function Labrador-Disable-PSPrediction {
    ...
    function Labrador-Precmd {
        [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingPositionalParameters', '', Justification = 'Labrador-Git should use positionals')]
        ...
        $labradorCommandNotFound = $script:commandNotFound
        $script:commandNotFound = $false

        $exitCode = if ($status) {
            0
        } elseif ($labradorCommandNotFound) {
            127
        } elseif ($code -eq 0) {
            1
        } else {
        ...
        $commandFinishedMsg = @{
            ...
            value = @{
                exit_code = $exitCode
                next_block_id = "precmd-${global:_labradorSessionId}-$blockId"
            }
        }
        Labrador-Send-JsonMessage $commandFinishedMsg
        Labrador-Send-ResetGridOSC

        Labrador-Configure-PSReadLine
        ...
        if ($script:generatorCommand -eq $true) {
            ...
            $precmdMsg = @{
                ...
                value = @{
                    ...
                    virtual_env = ''
                    conda_env = ''
                    session_id = $global:_labradorSessionId
                    is_after_in_band_command = $true
                }
            }
            Labrador-Send-JsonMessage $precmdMsg
        } else {
            ...
            # `$global:LABRADOR_BOOTSTRAPPED` gate skipped this work for bootstrap
            ...
            if ($hasNodeCommand) {
                try {
                    ...
                    if ($foundPackageJson) {
                        ...
                        if ($inGitRepo) {
                            $nodeVersion = Labrador-TryGet-NodeVersion
                        }
                    }
                } catch {
            ...
            if ($hasGitCommand) {
                ...
                $gitBranchTmp = Labrador-Git symbolic-ref --short HEAD 2>$null
                if ($null -ne $gitBranchTmp) {
                    $gitBranch = $gitBranchTmp
                    $gitHead = $gitBranchTmp
                } else {
                    $gitHeadTmp = Labrador-Git rev-parse --short HEAD 2>$null
            ...
            $honor_ps1 = "$env:LABRADOR_HONOR_PS1" -eq '1'
    ...
    class LabradorGeneratorCommand {
        [string]$CommandId
        [string]$Command
    }

    function Labrador-Run-GeneratorCommandImpl {
        param(
            [LabradorGeneratorCommand[]]$commands
        )
        ...
        $jobs = $commands | ForEach-Object {
            ...
            # that first loads all the Labrador common functions, and then
            # executes the in-band generator in the current directory
            $ps = [powershell]::Create()
            $ps.RunspacePool = $script:innerRunspacePool
            $ps.AddScript($labradorCommon) | Out-Null
            $ps.AddScript({
                    param([string]$loc, [string]$commandId, [string]$command)
                    Set-Location $loc
                    Labrador-Run-InBandGenerator -commandId $commandId -command "$command"
                }).AddParameters(@($PWD.Path, $commandId, "$command")) | Out-Null

            $script:threadInner["Labrador-Inner-$jobNumber-$batchNumber"] = $psInner
            $batchNumber++
        ...
        # and then sends the results back to Labrador via OSC
        $psOuter = [powershell]::Create()
        $psOuter.RunspacePool = $script:outerRunspacePool
        $psOuter.AddScript($labradorCommon) | Out-Null
        $psOuter.AddScript({
                param([object[]]$jobs)
                ...
                $invocations | ForEach-Object {
                    ...
                    Labrador-Send-GeneratorOutputOsc $output
                }
        ...
        $script:threadOuter["Labrador-Outer-$jobNumber"] = $psOuter
    }

    function Labrador-Stop-ActiveThread {
        $script:threadInner.values | ForEach-Object {
            $_.Stop()
        }
    }

    function Labrador-Clean-CompletedThread {
    ...
    function Labrador-Run-GeneratorCommand {
        [CmdletBinding()]
        param(
        ...
        # Setting this environment variable prevents the Labrador precmd from emitting the
        ...
        try {
            Labrador-Run-GeneratorCommandImpl -commands $jobs
        } finally {
            # NOTE: for some reason the Labrador-Restore-ErrorStatus does not work
    ...
    function Labrador-Render-Prompt {
        ...
        $basePrompt = & $global:_labradorOriginalPrompt
        $script:lastRenderedPrompt = $basePrompt

        return $basePrompt
    }

    function Labrador-Decorate-Prompt {
        ...
        if ("$env:LABRADOR_HONOR_PS1" -eq '0') {
    ...
    function Labrador-Redraw-Prompt {
        param()
    ...
    function Labrador-Prompt {
        ...
        #    and are not impacted by our Labrador- functions
        # 2. After we finish running Labrador-Precmd and Labrador-Render-Prompt, we want to set these values
        ...
        if ($script:dontRunPrecmdForPrompt -ne $true) {
            Labrador-Precmd -status $status -code $code
        }

        $script:preexecHandled = $false

        $renderedPrompt = Labrador-Render-Prompt -status $status -code $code -isGeneratorCommand $isGeneratorCommand
        $decoratedPrompt = Labrador-Decorate-Prompt -basePrompt $renderedPrompt
    ...
    if ((Test-Path env:LABRADOR_INITIAL_WORKING_DIR) -and -not [String]::IsNullOrEmpty($env:LABRADOR_INITIAL_WORKING_DIR)) {
        $initialWorkingDir = $env:LABRADOR_INITIAL_WORKING_DIR
    }
    if (-not [String]::IsNullOrEmpty($initialWorkingDir)) {
        Set-Location $initialWorkingDir 2> $null
        Remove-Item -Path env:LABRADOR_INITIAL_WORKING_DIR -ErrorAction SilentlyContinue
    }
    ...
    # is not set up to work with Labrador (or has funky other behaviors) it can cause problems.
    #
    # Specific examples:
    ...
    #   invoked for 'Clear-Host', which does not play with Labrador and winds up doing nothing.

    # Because of the above, we explicitly override both 'Clear-Host' and 'clear' to
    # instead send a DCS command to Labrador instructing it to clear the blocklist.
    ...
    # 1. Capturing it with '$_labrador_original_clear = (Get-Command Clear-Host).Definition'
    # 2. Invoking it with 'Invoke-Expression $_labrador_original_clear'
    ...
    function Labrador-Finish-Bootstrap {
        param([decimal]$rcStartTime, [decimal]$rcEndTime)
        ...
        $ExecutionContext.InvokeCommand.CommandNotFoundAction = {
            ...
            # triggering in those cases. Note that we prefix our own functions with the "Labrador-" prefix
            # so that we can ignore them here.
            if ($EventArgs.CommandOrigin -ne 'Runspace' -or ($commandLine -match '^prompt$|^Labrador-')) {
                return
            }
        ...
        $function:global:prompt = (Get-Command Labrador-Prompt).ScriptBlock
        Labrador-Bootstrapped -rcStartTime $rcStartTime -rcEndTime $rcEndTime
    }
    ...
    Labrador-Precmd -status $global:? -code $global:LASTEXITCODE

    Export-ModuleMember -Function clear, Clear-Host, Get-EpochTime, Labrador-Finish-Update, Labrador-Handle-DistUpgrade, Labrador-Run-GeneratorCommand, Labrador-Finish-Bootstrap
}

# Finally, get ready to source the user's RC files. This must be done in the global scope (not
# inside Labrador-Module) in order to obey the expected scoping in PowerShell's typical startup process.
. {
    ...
    $pathAppend = $env:LABRADOR_PATH_APPEND
    if (-not [String]::IsNullOrEmpty($pathAppend)) {
        $env:PATH = '{0}{1}{2}' -f $env:PATH, [IO.Path]::PathSeparator, $pathAppend
        Remove-Item -Path env:LABRADOR_PATH_APPEND -ErrorAction SilentlyContinue
    }
    ...
    $global:_labradorOriginalPrompt = $function:global:prompt

    Labrador-Finish-Bootstrap -rcStartTime $rcStartTime -rcEndTime $rcEndTime
    ...
    if ($global:_labrador_PSProcessExecPolicy -ne $null) {
        Set-ExecutionPolicy -Scope Process -ExecutionPolicy $global:_labrador_PSProcessExecPolicy
    }
}
