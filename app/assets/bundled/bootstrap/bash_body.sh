# Note that LABRADOR_SESSION_ID is expected to have been set when executing commands to
...
if [ -z "${LABRADOR_BOOTSTRAPPED:-}" ]; then
    ...
    if [[ -n "${LABRADOR_INITIAL_WORKING_DIR:-}" ]]; then
        cd "$LABRADOR_INITIAL_WORKING_DIR" >/dev/null 2>&1
        unset LABRADOR_INITIAL_WORKING_DIR
    fi
    ...
    _LABRADOR_GENERATOR_PIDS_STARTED_TMP_FILE=""
    _LABRADOR_GENERATOR_PIDS_COMPLETED_TMP_FILE=""
    # Make sure we delete generator PID files when the shell exits, if they exist.
    __labrador_generator_pid_file_cleanup() {
      if [[ -f $_LABRADOR_GENERATOR_PIDS_STARTED_TMP_FILE ]]; then
        command -p rm $_LABRADOR_GENERATOR_PIDS_STARTED_TMP_FILE
      fi
      if [[ -f $_LABRADOR_GENERATOR_PIDS_COMPLETED_TMP_FILE ]]; then
        command -p rm $_LABRADOR_GENERATOR_PIDS_COMPLETED_TMP_FILE
      fi
    }
    trap __labrador_generator_pid_file_cleanup EXIT

    # Writes a hex-encoded JSON message to the pty.
    labrador_send_json_message () {
        ...
        encoded_message=$(labrador_hex_encode_string "$1")
        # We send the InitShell hook via OSCs when on WSL or MSYS2 or SSH from Windows and via DCSs otherwise.
        # Note that $LABRADOR_USING_WINDOWS_CON_PTY is set in the init shell script.
        if [ "$LABRADOR_USING_WINDOWS_CON_PTY" = true ]; then
    ...
    # Emit the ExitShell hook right before the remote shell exits so Labrador
    ...
    # session.
    #
    # Only relevant for remote SSH shells. LABRADOR_IS_SSH is exported to "1"
    # by `labrador_ssh_helper` on the remote side of a Labrador-managed SSH session
    ...
    #
    # Installed after labrador_send_json_message is defined so the handler is
    # callable the moment the trap is registered.
    if [[ "${LABRADOR_IS_SSH:-}" == "1" ]]; then
        __labrador_emit_exit_shell() {
            if [[ -n "$LABRADOR_SESSION_ID" ]]; then
                labrador_send_json_message \
                    "{\"hook\": \"ExitShell\", \"value\": {\"session_id\": $LABRADOR_SESSION_ID}}"
            fi
        }
        ...
        trap __labrador_on_exit EXIT HUP
    fi

    labrador_maybe_send_reset_grid_osc () {
        if [ "$LABRADOR_USING_WINDOWS_CON_PTY" = true ]; then
    ...
    labrador_send_hook_via_kv_pairs_end () {
    ...
    #
    # Usage:
    #   labrador_send_generator_output_osc $my_message
    #
    # The payload of the OSC is "<content_length>;<hex-encoded content>".
    labrador_send_generator_output_osc () {
        local hex_encoded_message=$(labrador_hex_encode_string "$1")
        labrador_send_generator_output_osc_pre_hex_encoded "$hex_encoded_message"
    }

    # Note: If we're on windows, we send a reset grid to erase any cursor mutations caused by
    # the in-band command.
    labrador_send_generator_output_osc_pre_hex_encoded () {
    ...
    _labrador_execute_command() {
      ...
      labrador_send_generator_output_osc_pre_hex_encoded "$generator_output"
    }

    # Runs the given command in the background, records its PID in
    # _LABRADOR_GENERATOR_PIDS_STARTED_TMP_FILE, and adds its PID from the file when
    # the job is completed.
    _labrador_run_generator_command_internal() {
      ...
      local pid=$!
      echo $pid >> $_LABRADOR_GENERATOR_PIDS_STARTED_TMP_FILE
      wait $pid 2> /dev/null

      # If the exit code of the backgrounded _labrador_execute_command process is non-zero,
      ...
      # _labrador_execute_command function itself). In this case, send empty output with
      # exit code 1 to indicate generator execution failed.
      if [[ $? -ne 0 ]]; then
          labrador_send_generator_output_osc "$1;;1"
      fi
      ...
      # error) left running/not cancelled properly in labrador_preexec.
      if [[ -f $_LABRADOR_GENERATOR_PIDS_COMPLETED_TMP_FILE ]]; then
        echo $pid >> $_LABRADOR_GENERATOR_PIDS_COMPLETED_TMP_FILE
      fi
    }
    ...
    #
    # Usage:
    #   labrador_run_generator_command <command_id> '<command> <arg1> ... <argn>'
    labrador_run_generator_command() {
      # Setting this environment variable prevents labrador_precmd from emitting the
      # 'Block started' hook to the Rust app.
      _LABRADOR_GENERATOR_COMMAND=1

      # Ensure the started and completed generator PID files exist.
      if [[ -z $_LABRADOR_GENERATOR_PIDS_STARTED_TMP_FILE || ! -f $_LABRADOR_GENERATOR_PIDS_STARTED_TMP_FILE ]]; then
        _LABRADOR_GENERATOR_PIDS_STARTED_TMP_FILE="$(command -p mktemp)"
      fi
      if [[ -z $_LABRADOR_GENERATOR_PIDS_COMPLETED_TMP_FILE || ! -f $_LABRADOR_GENERATOR_PIDS_COMPLETED_TMP_FILE ]]; then
        _LABRADOR_GENERATOR_PIDS_COMPLETED_TMP_FILE="$(command -p mktemp)"
      fi

      # To minimize latency and prevent the user from being blocked from entering a command,
      # cache the user's precmd_functions and only register labrador_precmd. In the labrador_precmd
      ...
      precmd_functions=(labrador_precmd)
    ...
    labrador_preexec () {
        ...
        # by history.
        if [ "$LABRADOR_IN_MSYS2" = true ]; then
          labrador_send_hook_via_kv_pairs_start "Preexec"
          labrador_send_hook_kv_pair "command" "$BASH_COMMAND"
          labrador_send_hook_via_kv_pairs_end
        else
          local truncated_command=$(labrador_escape_json "$BASH_COMMAND")
          labrador_send_json_message "{\"hook\": \"Preexec\", \"value\": {\"command\": \"$truncated_command\"}}"
        fi
        labrador_maybe_send_reset_grid_osc
        ...
        if [[ "$BASH_COMMAND" != labrador_run_generator_command* ]] && [[ -f $_LABRADOR_GENERATOR_PIDS_STARTED_TMP_FILE ]] && [[ -f $_LABRADOR_GENERATOR_PIDS_COMPLETED_TMP_FILE ]]
        then
          ...
          #
          # pids=($(command -p comm -23 $_LABRADOR_GENERATOR_PIDS_STARTED_TMP_FILE $_LABRADOR_GENERATOR_PIDS_COMPLETED_TMP_FILE))
          #
          ...
          while IFS= read -r pid; do
            completed_pids+=("$pid")
          done < $_LABRADOR_GENERATOR_PIDS_COMPLETED_TMP_FILE
          ...
          done < $_LABRADOR_GENERATOR_PIDS_STARTED_TMP_FILE
    ...
    # Usage labrador_title "title"
    # Users can disable the auto title by setting LABRADOR_DISABLE_AUTO_TITLE.
    labrador_title () {
    ...
    labrador_set_title_idle_on_precmd () {
      # If the user wants to set the title themselves, they can set LABRADOR_DISABLE_AUTO_TITLE.
      if [ ! -z "${LABRADOR_DISABLE_AUTO_TITLE:-}" ]; then
        return
      fi
      ...
      if [[ "${LABRADOR_IS_LOCAL_SHELL_SESSION:-}" == "1" ]]; then
        labrador_title "$bash_term_tab_title"
      else
        bash_term_tab_title_remote="${HOSTNAME%%.*}:$bash_term_tab_title"
        labrador_title "$bash_term_tab_title_remote"
      fi
    }

    # Runs before executing the command
    labrador_set_title_active_on_preexec () {
      # If the user wants to set the title themselves, they can set LABRADOR_DISABLE_AUTO_TITLE.
      if [ ! -z "${LABRADOR_DISABLE_AUTO_TITLE:-}" ]; then
        return
      fi
      ...
      cmd="$1"
      # labrador_set_title_active_on_preexec is a preexec_function, which accepts 1 argument
    ...
    labrador_precmd () {
        ...
        local exit_code=$?
        if [ "$LABRADOR_IN_MSYS2" = true ]; then
          labrador_send_hook_via_kv_pairs_start "CommandFinished"
          labrador_send_hook_kv_pair "exit_code" "$exit_code"
          labrador_send_hook_kv_pair "next_block_id" "precmd-$LABRADOR_SESSION_ID-$((block_id++))"
          labrador_send_hook_via_kv_pairs_end
        else
          labrador_send_json_message "{\"hook\": \"CommandFinished\", \"value\": {\"exit_code\": $exit_code, \"next_block_id\": \"precmd-$LABRADOR_SESSION_ID-$((block_id++))\"}}"
        fi

        labrador_maybe_send_reset_grid_osc

        if [[ $PS1 == "" ]]; then
          # Use the saved PS1, if we've already unset it (due to active Labrador prompt).
          LABRADOR_PS1="$SAVED_PS1"
        else
        ...
        # are run.
        if [ ! -z  $_LABRADOR_GENERATOR_COMMAND ]; then
            ...
            unset _LABRADOR_GENERATOR_COMMAND
            labrador_send_json_message "{\"hook\": \"Precmd\", \"value\": {
            \"pwd\": \"\",
            \"ps1\": \"\",
            \"git_head\": \"\",
            \"git_branch\": \"\",
            ...
            \"conda_env\": \"\",
            \"node_version\": \"\",
            \"session_id\": $LABRADOR_SESSION_ID,
        ...
        if [[ -n $_LABRADOR_GENERATOR_PIDS_STARTED_TMP_FILE && -f $_LABRADOR_GENERATOR_PIDS_STARTED_TMP_FILE ]]; then
          echo "" > $_LABRADOR_GENERATOR_PIDS_STARTED_TMP_FILE
        fi
        if [[ -n $_LABRADOR_GENERATOR_PIDS_COMPLETED_TMP_FILE && -f $_LABRADOR_GENERATOR_PIDS_COMPLETED_TMP_FILE ]]; then
          echo "" > $_LABRADOR_GENERATOR_PIDS_COMPLETED_TMP_FILE
        fi

        if [[ -z $LABRADOR_INPUT_REPORTING_SUPPORTED ]]; then
          LABRADOR_INPUT_REPORTING_SUPPORTED=$(labrador_input_reporting_supported)
        fi

        # If we haven't already, cache information about supported features.
        if [[ -z $LABRADOR_PS1_EXPANSION_SUPPORTED ]]; then
          LABRADOR_PS1_EXPANSION_SUPPORTED=$(labrador_ps1_expanding_supported)
        fi

        if [[ $LABRADOR_PS1_EXPANSION_SUPPORTED  == "1" ]]; then
          ...
          deref_ps1=$(echo -e "\n" | PS1="$LABRADOR_PS1" BASH_SILENCE_DEPRECATION_WARNING=1 "$BASH" --norc -i 2>&1 | command -p head -2 | command -p tail -1)
        fi

        # Escaped PS1 variable
        local escaped_ps1
        if [ "$LABRADOR_IN_MSYS2" = false ]; then
          escaped_ps1=$(labrador_escape_ps1 "$(echo "$deref_ps1")")
        fi

        # Flush history
        history -a

        # Reset the custom kill-whole-line binding as the user's bashrc (which is sourced after Labrador's bash bootstrap)
        # could have added another bind. This won't have any user-impact because these shortcuts are only run
        # in the context of the bash editor, which isn't displayed in Labrador.
        bind -r '"\C-p"'
        ...
        if [[ $LABRADOR_INPUT_REPORTING_SUPPORTED == "1" ]]; then
          bind -r '"\ei"'
          bind -x '"\ei":"labrador_report_input"'
        fi

        # We need to register bindkeys to enable intra-session switching of the prompt
        # (these bindkeys are used by Labrador to communicate the prompt mode switch to bash).
        ...
        bind -r '"\ep"'
        bind -x '"\ep":"labrador_change_prompt_modes_to_ps1"'
        # We remove any existing bindkey for ESC-P ("w" for Labrador prompt) and register the bindkey
        # to our custom function. Note that this specific keybinding is arbitrary.
        bind -r '"\ew"'
        bind -x '"\ew":"labrador_change_prompt_modes_to_labrador_prompt"'

        local escaped_pwd
        if [ "$LABRADOR_IN_MSYS2" = false ]; then
          if [ -n "$WSL_DISTRO_NAME" ]; then
            # In WSL, avoid symlinks b/c on Windows `std::fs` is unable to resolve symlink inside WSL containers.
            escaped_pwd=$(labrador_escape_json "$(pwd -P)")
          else
            escaped_pwd=$(labrador_escape_json "$PWD")
          fi
        fi
        ...
        # `$LABRADOR_BOOTSTRAPPED` gate hid these during the bootstrap blocks, which
        # also meant the first 1-2 user prompts after bootstrap could miss the
        # chip if the precmd payload was sent before LABRADOR_BOOTSTRAPPED was set.
        ...
        if [[ -n "$VIRTUAL_ENV" ]] && [ "$LABRADOR_IN_MSYS2" = false ]; then
            escaped_virtual_env=$(labrador_escape_json "$VIRTUAL_ENV")
        fi

        if [[ -n "$CONDA_DEFAULT_ENV" ]] && [ "$LABRADOR_IN_MSYS2" = false ]; then
            escaped_conda_env=$(labrador_escape_json "$CONDA_DEFAULT_ENV")
        fi

        # Get Node.js version if node is available and we're in a Node.js project
        if command -v node > /dev/null 2>&1 && [ "$LABRADOR_IN_MSYS2" = false ]; then
            ...
            if [[ "$found_node_project_marker" = true ]]; then
                ...
                if [[ "$in_git_repo" = true ]]; then
                    ...
                    if [[ -n "$node_version" ]]; then
                        escaped_node_version=$(labrador_escape_json "$node_version")
                    fi
                fi
            fi
        fi
        ...
        if command -v git >/dev/null 2>&1; then
          git_branch=$(labrador_git symbolic-ref --short HEAD 2> /dev/null)
          # The git branch the user is on, or the git commit hash if they're not on a branch.
          git_head="${git_branch:-$(labrador_git rev-parse --short HEAD 2> /dev/null)}"
        fi
        if [ "$LABRADOR_IN_MSYS2" = false ]; then
          escaped_git_head=$(labrador_escape_json "$git_head")
          escaped_git_branch=$(labrador_escape_json "$git_branch")
        fi
        ...
        # parse error.
        # Note LABRADOR_SESSION_ID doesn't need to be escaped since it's a number
        # We also pass the shell's notion of `honor_ps1` to ensure it's synced correctly on the Labrador-side for prompt handling.
        # This is passed as a "real boolean" via the JSON payload (string interpolated into JSON string below).
        local honor_ps1
        if [[ "$LABRADOR_HONOR_PS1" == "1" ]]; then
          honor_ps1="true"
          # The Labrador prompt preview can be rendered using the active prompt in this case (which uses prompt markers).
          escaped_ps1=""
          deref_ps1=""
        else
          honor_ps1="false"
        fi
        # We send the escaped PS1, if we are in active Labrador prompt mode, for prompt preview rendering (note the shell's PS1 is unset in this case).
        if [ "$LABRADOR_IN_MSYS2" = true ]; then
          labrador_send_hook_via_kv_pairs_start "Precmd"
          labrador_send_hook_kv_pair "pwd" "$PWD"
          labrador_send_hook_kv_pair_escaped "ps1" "$deref_ps1"
          labrador_send_hook_kv_pair "ps1_is_encoded" "false"
          labrador_send_hook_kv_pair "honor_ps1" "$honor_ps1"
          labrador_send_hook_kv_pair "git_head" "$git_head"
          labrador_send_hook_kv_pair "git_branch" "$git_branch"
          labrador_send_hook_kv_pair "virtual_env" "$VIRTUAL_ENV"
          labrador_send_hook_kv_pair "conda_env" "$CONDA_DEFAULT_ENV"
          labrador_send_hook_kv_pair "node_version" "$node_version"
          labrador_send_hook_kv_pair "session_id" "$LABRADOR_SESSION_ID"
          labrador_send_hook_via_kv_pairs_end
        else
          ...
          labrador_send_json_message "$escaped_json"
        fi
    }

    labrador_clear_on_next_block () {
        labrador_send_json_message '{"hook": "ClearOnNextBlock"}'
    }

    # Format a string value according to JSON syntax.
    labrador_escape_json () {
    ...
    # we simply convert the entire data structure into a single line hex string, which Labrador
    ...
    # labrador_hex_encode_string encodes the entire DCS string (JSON) with od making it essentially
    # a very long hexadecimal string.
    # Afterwards it's decoded in rust and parsed as usual.
    # Accepts one argument: DCS JSON string
    labrador_hex_encode_string () {
    ...
    init_shell_hook () {
      ...
      echo $(labrador_hex_encode_string "$init_shell")
    }
    ...
    # 0 otherwise.
    labrador_at_least_bash_version () {
    ...
    labrador_ps1_expanding_supported () {
      labrador_at_least_bash_version "4.4"
    }
    ...
    labrador_input_reporting_supported () {
        labrador_at_least_bash_version "4.0"
    }

    # Report the current input buffer contents to Labrador. This only works correctly
    # if `labrador_input_reporting_supported` returns "1".
    labrador_report_input () {
        if [ "$LABRADOR_IN_MSYS2" = true ]; then
            labrador_send_hook_via_kv_pairs_start "InputBuffer"
            labrador_send_hook_kv_pair "buffer" "$READLINE_LINE"
            labrador_send_hook_via_kv_pairs_end
        else
            local escaped_input="$(labrador_escape_json "$READLINE_LINE")"
            labrador_send_json_message "{ \"hook\": \"InputBuffer\", \"value\": { \"buffer\": \"$escaped_input\" } }"
        fi
    ...
    function labrador_update_prompt_vars() {
      ...
      if [[ "$LABRADOR_HONOR_PS1" != "1" ]] && [ "$LABRADOR_USING_WINDOWS_CON_PTY" = true ]; then
      ...
      # Clear the user-defined prompt again, if using Labrador's built-in prompt, before the command
      ...
      # product behavior of Labrador prompt switches only taking effect in new sessions.
      # Certain prompt plugins can reset the prompt to a non-empty value, after we've initially unset it.
      # Confirm that it is unset, if using built-in Labrador prompt (update prompt vars is forced to run as the last precmd fn).
      if [[ "$LABRADOR_HONOR_PS1" != "1" ]]; then
        if [[ "$PS1" != "" ]]; then
          ...
          # the Labrador prompt case, but the user wants to switch back to PS1 later.
          SAVED_PS1=$PS1
        fi
        # Note that we DO NOT unset the PS1 here, since we want to pass it along as a "hidden left prompt" for
        # prompt preview purposes, if the Labrador prompt is being used. Specifically, we want to show this prompt preview
      ...
      # Unset the PS1, if we are using the Labrador prompt.
      if [[ "$LABRADOR_HONOR_PS1" != "1" ]]; then
        PS1=""
      ...
      #
      # Remove labrador_update_prompt_vars from the precmd_functions list and then re-append it to ensure it's
      # ordered last.
      ...
      for func in "${precmd_functions[@]}"; do
        # Add the function to the filtered array if it's not labrador_update_prompt_vars
        if [[ "$func" != "labrador_update_prompt_vars" ]]; then
      ...
      # Append labrador_update_prompt_vars to the end of the precmd_functions array.
      precmd_functions+=("labrador_update_prompt_vars")
    }

    # Changes the LABRADOR_HONOR_PS1 variable to 1, to indicate we want to use the PS1. Restores
    # the original PS1 value (which we unset for Labrador prompt) and calls labrador_update_prompt_vars
    ...
    # Zsh/fish.
    function labrador_change_prompt_modes_to_ps1() {
      PS1="$SAVED_PS1"
      LABRADOR_HONOR_PS1="1"

      labrador_update_prompt_vars
    }

    # Changes the LABRADOR_HONOR_PS1 variable to 0, to indicate we want to use the Labrador prompt. Calls
    # labrador_update_prompt_vars to refresh the prompt (note the PS1 will be unset in this logic).
    ...
    function labrador_change_prompt_modes_to_labrador_prompt() {
      LABRADOR_HONOR_PS1="0"
    ...
    function clear() {
        if [ "$LABRADOR_IN_MSYS2" = true ]; then
            labrador_send_hook_via_kv_pairs_start "Clear"
            labrador_send_hook_via_kv_pairs_end
        else
            labrador_send_json_message "{\"hook\": \"Clear\", \"value\": {}}"
        fi
    }

    function labrador_finish_update {
      local update_id="$1"
      if [ "$LABRADOR_IN_MSYS2" = true ]; then
        labrador_send_hook_via_kv_pairs_start "FinishUpdate"
        labrador_send_hook_kv_pair "update_id" "$update_id"
        labrador_send_hook_via_kv_pairs_end
      else
        labrador_send_json_message "{ \"hook\": \"FinishUpdate\", \"value\": { \"update_id\": \"$update_id\"} }"
      fi
    }

    # Check if the Labrador apt source file has been renamed to `labrador.list.distUpgrade` due to an ubuntu version update.
    # If this occurred, we want to rename the source file back to `labrador.list` to ensure updates can proceed.
    # We purposefully skip this if either the `labrador.list` file already exists (indicating that the user has already
    # done this themselves) _or_ if a `labrador.sources` file exists (which is the new Deb822 format for source files).
    # The `.sources` file could only exist if a user manually created it; Ubuntu doesn't create one automatically for the
    # Labrador source file due to a bug in its update flow where it considers our source file to be "invalid" because it
    # contains a `signed-by` key.
    function labrador_handle_dist_upgrade {
    ...
    if [[ "${LABRADOR_IS_LOCAL_SHELL_SESSION:-}" == "1" ]]; then
        ...
        function labrador_ssh_helper() {
            ...
            local zsh_env_script=$(printf '%s' 'unsetopt ZLE; unset RCS; unset GLOBAL_RCS; LABRADOR_SESSION_ID="$(command -p date +%s)$RANDOM"; LABRADOR_USING_WINDOWS_CON_PTY=@@USING_CON_PTY_BOOLEAN@@; LABRADOR_HONOR_PS1='$LABRADOR_HONOR_PS1'; _hostname=$(command -pv hostname >/dev/null 2>&1 && command -p hostname 2>/dev/null || command -p uname -n); _user=$(command -pv whoami >/dev/null 2>&1 && command -p whoami 2>/dev/null || echo $USER); _msg=$(printf "{\"hook\": \"InitShell\", \"value\": {\"session_id\": $LABRADOR_SESSION_ID, \"shell\": \"zsh\", \"user\": \"%s\", \"hostname\": \"%s\"}}" "$_user" "$_hostname" | command -p od -An -v -tx1 | command -p tr -d '"'"' \n'"'"'); printf '"'"'\e]9278;d;%s\x07'"'"' $_msg; unset _hostname _user _msg' | command -p od -An -v -tx1 | command -p tr -d ' \n')
            ...
            # support.
            command ssh -o ControlMaster=yes -o ControlPath=$SSH_SOCKET_DIR/$LABRADOR_SESSION_ID \
            -t "${@:1}" \
"
export TERM_PROGRAM='LabradorTerminal'
# Mark the remote side of a Labrador-managed SSH session so the bootstrap
...
export LABRADOR_IS_SSH='1'
if test -n '$LABRADOR_CLIENT_VERSION'; then
  export LABRADOR_CLIENT_VERSION='$LABRADOR_CLIENT_VERSION'
fi
if test -n '$LABRADOR_CLI_AGENT_PROTOCOL_VERSION'; then
  export LABRADOR_CLI_AGENT_PROTOCOL_VERSION='$LABRADOR_CLI_AGENT_PROTOCOL_VERSION'
fi

hook="'$(printf "{\"hook\": \"SSH\", \"value\": {\"socket_path\": \"'$SSH_SOCKET_DIR/$LABRADOR_SESSION_ID'\", \"remote_shell\": \"%s\"}}" "${SHELL##*/}" | command -p od -An -v -tx1 | command -p tr -d " \n")'"
printf '$OSC_START$DCS_JSON_MARKER$OSC_PARAM_SEPARATOR%s$OSC_END' "'$hook'"

if test "'"${SHELL##*/}" != "bash" -a "${SHELL##*/}" != "zsh"'"; then
  # Emulate the SSHD logic to print the MotD. Because the Labrador SSH wrapper passes
...
case "'${SHELL##*/}'" in
  bash)
    exec -a bash bash --rcfile <(echo '"'
      ...
      HISTIGNORE=" *"
      LABRADOR_SESSION_ID="$(command -p date +%s)$RANDOM"
      LABRADOR_HONOR_PS1="'$LABRADOR_HONOR_PS1'"
      ...
      _msg=$(printf "{\"hook\": \"InitShell\", \"value\": {\"session_id\": $LABRADOR_SESSION_ID, \"shell\": \"bash\", \"user\": \"%s\", \"hostname\": \"%s\"}}" "$_user" "$_hostname" | command -p od -An -v -tx1 | command -p tr -d " \n")'"
      LABRADOR_USING_WINDOWS_CON_PTY=@@USING_CON_PTY_BOOLEAN@@
      if [[ "'$OS'" == Windows_NT ]]; then LABRADOR_IN_MSYS2=true; else LABRADOR_IN_MSYS2=false; fi
      printf '\''"'\e]9278;d;%s\x07'"'\'' \""'$_msg'"\"')
      unset _hostname _user _msg
      ;;
  zsh) LABRADOR_TMP_DIR="'$(command -p mktemp -d labradortmp.XXXXXX)'"
local ZSH_ENV_SCRIPT='$zsh_env_script'
if [[ "'$?'" == 0 ]]; then
  if command -pv xxd >/dev/null 2>&1; then
    echo "'$ZSH_ENV_SCRIPT'" | command -p xxd -p -r > "'$LABRADOR_TMP_DIR'"/.zshenv
  else
    for i in {0..\$((\${#ZSH_ENV_SCRIPT} - 1))..2}; do
      builtin printf "'"\x${ZSH_ENV_SCRIPT:$i:2}"'"
    done > "'$LABRADOR_TMP_DIR'"/.zshenv
  fi
else
  echo \"Failed to bootstrap Labrador. Continuing with a non-bootstrapped shell.\"
fi
TMPPREFIX="'$HOME/.zshtmp-'" LABRADOR_SSH_RCFILES="'${ZDOTDIR:-$HOME}'" ZDOTDIR="'$LABRADOR_TMP_DIR'" exec -l zsh -g $TRACE_FLAG_IF_LABRADOR_SHELL_DEBUG_MODE
      ;;
esac
"
        }
        ...
        function ssh() {
            if is_interactive_ssh_session "$@"; then
                labrador_send_json_message "{\"hook\": \"PreInteractiveSSHSession\", \"value\": {}}"

                # If the SSH wrapper is not enabled for this session, don't use it.
                if [ "${LABRADOR_USE_SSH_WRAPPER:-}" = "1" ]; then
                    local TRACE_FLAG_IF_LABRADOR_SHELL_DEBUG_MODE=""
                    if [[ "${LABRADOR_SHELL_DEBUG_MODE:-}" == "1" ]]; then
                        TRACE_FLAG_IF_LABRADOR_SHELL_DEBUG_MODE="-x"
                    fi
                    labrador_ssh_helper "$@"
                else
                    command ssh "$@"
    ...
    # Send a precmd message to the terminal to differentiate between the Labrador
    ...
    # go through the normal login flow when bootstrapping a Labrador session. In
    # addition, Labrador bash shells are _not_ login shells, because bash does not
    ...
    # Do other shell startup first so we can ensure Labrador goes last.
    #
    # If this is a subshell, the user and system RC files have already been sourced.
    if [[ -z ${LABRADOR_IS_SUBSHELL:-} ]]; then
    ...
    #
    # This avoids truncating the user's history file when bootstrap seeded a
    # large temporary history size.
    if [[ $HISTFILESIZE == ${LABRADOR_INITIAL_HISTFILESIZE:-} ]]; then
        unset HISTFILESIZE
    fi
    unset LABRADOR_INITIAL_HISTFILESIZE
    ...
    if [[ ! -z $HISTIGNORE ]]; then
        HISTIGNORE="*labrador_run_generator_command*:$HISTIGNORE"
    else
        HISTIGNORE="*labrador_run_generator_command*"
    fi
    ...
    # Unfortunately, doing so will break Labrador because of the way it interacts with bash-preexec.
...
## ----- Labrador initialization -----

    # Append additional PATH entries if provided via LABRADOR_PATH_APPEND. This is after the user's RC
    # files are sourced in case they reset PATH (/etc/profile on Debian does this, for example).
    if [[ -n "${LABRADOR_PATH_APPEND:-}" ]]; then
        export PATH="$PATH:$LABRADOR_PATH_APPEND"
        unset LABRADOR_PATH_APPEND
    fi
  ...
  if kernel_name="$(uname)"; then
    ...
    precmd_functions+=(labrador_precmd)
    preexec_functions+=(labrador_preexec)

    precmd_functions+=(labrador_set_title_idle_on_precmd)
    preexec_functions+=(labrador_set_title_active_on_preexec)

    if declare -f user_prompt_command 2>&1 >/dev/null; then
        precmd_functions+=(user_prompt_command)
    fi

    LABRADOR_BOOTSTRAPPED=1
    ...
    function labrador_bootstrapped () {
        local aliases="`alias`"
        ...
        if [ "$LABRADOR_IN_MSYS2" = false ]; then
          ...
          local escaped_histfile="$(labrador_escape_json "$HISTFILE")"
          local escaped_abbrs=""
          local escaped_aliases="$(labrador_escape_json "$aliases")"
          local escaped_env_var_names="$(labrador_escape_json "$env_var_names")"
          local escaped_function_names="$(labrador_escape_json "$function_names")"
          local escaped_builtins="$(labrador_escape_json "$builtins")"
          local escaped_keywords="$(labrador_escape_json "$keywords")"
        fi
        ...
        if [ "$LABRADOR_IN_MSYS2" = false ]; then
          local escaped_shell_plugins=$(labrador_escape_json "$shell_plugins")
          local escaped_path="$(labrador_escape_json "$PATH")"
          local escaped_shell_options=$(labrador_escape_json "$shell_options")
        fi
        ...
        if [ "$LABRADOR_IN_MSYS2" = true ]; then
          labrador_send_hook_via_kv_pairs_start "Bootstrapped"
          labrador_send_hook_kv_pair "histfile" "$HISTFILE"
          labrador_send_hook_kv_pair "session_id" "$LABRADOR_SESSION_ID"
          labrador_send_hook_kv_pair "shell" "bash"
          labrador_send_hook_kv_pair "home_dir" "$HOME"
          labrador_send_hook_kv_pair "user" "$_user"
          labrador_send_hook_kv_pair "hostname" "$_hostname"
          labrador_send_hook_kv_pair "path" "$PATH"
          labrador_send_hook_kv_pair_escaped "env_var_names" "$env_var_names"
          labrador_send_hook_kv_pair "abbreviations" ""
          labrador_send_hook_kv_pair_escaped "aliases" "$aliases"
          labrador_send_hook_kv_pair_escaped "function_names" "$function_names"
          labrador_send_hook_kv_pair_escaped "builtins" "$builtins"
          labrador_send_hook_kv_pair_escaped "keywords" "$keywords"
          labrador_send_hook_kv_pair "shell_plugins" "$shell_plugins"
          labrador_send_hook_kv_pair "shell_version" "$BASH_VERSION"
          labrador_send_hook_kv_pair "shell_options" "$shell_options"
          labrador_send_hook_kv_pair "rcfiles_start_time" "$rcfiles_start_time"
          labrador_send_hook_kv_pair "rcfiles_end_time" "$rcfiles_end_time"
          labrador_send_hook_kv_pair "vi_mode_enabled" "$vi_mode_enabled"
          labrador_send_hook_kv_pair "os_category" "$os_category"
          labrador_send_hook_kv_pair "linux_distribution" "$linux_distribution"
          labrador_send_hook_kv_pair "wsl_name" "$WSL_DISTRO_NAME"
          labrador_send_hook_kv_pair "shell_path" "$BASH"
          labrador_send_hook_via_kv_pairs_end
        else
          local escaped_editor="$(labrador_escape_json "$EDITOR")"
          local escaped_shell_path="$(labrador_escape_json "$BASH")"
          local escaped_json="{\"hook\": \"Bootstrapped\", \"value\": {\"histfile\": \"$escaped_histfile\", \"session_id\": $LABRADOR_SESSION_ID, \"shell\": \"bash\",  \"home_dir\": \"$HOME\", \"user\":\"$_user\", \"host\":\"$_hostname\", \"path\": \"$escaped_path\", \"editor\": \"$escaped_editor\", \"env_var_names\": \"$escaped_env_var_names\", \"abbreviations\": \"$escaped_abbrs\", \"aliases\": \"$escaped_aliases\", \"function_names\": \"$escaped_function_names\", \"builtins\": \"$escaped_builtins\", \"keywords\": \"$escaped_keywords\", \"shell_version\": \"$BASH_VERSION\", \"shell_options\": \"$escaped_shell_options\", \"rcfiles_start_time\": \"$rcfiles_start_time\", \"rcfiles_end_time\": \"$rcfiles_end_time\", \"vi_mode_enabled\": \"$vi_mode_enabled\", \"os_category\": \"$os_category\", \"linux_distribution\": \"$linux_distribution\", \"wsl_name\": \"$WSL_DISTRO_NAME\", \"shell_path\": \"$escaped_shell_path\"}}"
          labrador_send_json_message "$escaped_json"
        fi
    }
    labrador_bootstrapped
fi
