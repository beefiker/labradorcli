...
# Disable fish autosuggestions - because input goes through Labrador's editor instead,
...
# _labrador_run_generator_command_internal, which instead end in 'e' (0x65).
...
if test -n "$LABRADOR_INITIAL_WORKING_DIR"
    cd "$LABRADOR_INITIAL_WORKING_DIR" >/dev/null 2>&1
    set -e LABRADOR_INITIAL_WORKING_DIR
end

# Append additional PATH entries if provided via LABRADOR_PATH_APPEND.
if test -n "$LABRADOR_PATH_APPEND"
    set -gx --path PATH "$PATH:$LABRADOR_PATH_APPEND"
    set -e LABRADOR_PATH_APPEND
end

function labrador_send_json_message
    # Sends a message to the controlling terminal as a DSC control sequence.
    set -l escaped_json (labrador_hex_encode_string "$argv")
    if [ "$LABRADOR_USING_WINDOWS_CON_PTY" = true ]
...
function labrador_maybe_send_reset_grid_osc
...
# labrador_hex_encode_string hex-encodes the given string with `od`.
function labrador_hex_encode_string
...
# user command output.
set -g _labrador_generator_pids ''
...
#
# Usage:
#   _labrador_run_generator_command_internal <command_id> '<command>'
#
...
function  _labrador_run_generator_command_internal
    ...
    fish -c "
        set -l labrador_using_windows_con_pty $LABRADOR_USING_WINDOWS_CON_PTY;
        ...
        labrador_maybe_send_reset_grid_osc" 2> /dev/null &
        
    set -l command_pid $last_pid
    set -a _labrador_generator_pids $command_pid

    # Remove the command's PID from _labrador_generator_pids when the command exits.
    function on_command_{$command_pid}_finish --on-process-exit $command_pid --inherit-variable command_pid
        set -g _labrador_generator_pids (string replace $command_pid '' $_labrador_generator_pids)
...
function labrador_run_generator_command
    # Setting this environment variable allows labrador_precmd to detect if a generator
    # command or a user command has just completed.
    set -g _LABRADOR_GENERATOR_COMMAND 1
    _labrador_run_generator_command_internal $argv
end

# Run before a command is executed.
function labrador_preexec --on-event fish_preexec
    set -l command (labrador_escape_json "$argv")
    labrador_send_json_message "{\"hook\": \"Preexec\", \"value\": {\"command\": \"$command\"}}"
    labrador_maybe_send_reset_grid_osc

    # If this preexec is called for user command, kill ongoing generator command jobs.
    if test (! string match -q "labrador_run_generator_command*" $argv[1])
        for pid in $_labrador_generator_pids
...
function labrador_update_prompt_vars
  ...
  if not functions -q labrador_original_fish_prompt
    functions -c fish_prompt labrador_original_fish_prompt
  end

  # Back up the original fish_right_prompt if it exists and not already backed up
  if functions -q fish_right_prompt; and not functions -q labrador_original_fish_right_prompt
    functions -c fish_right_prompt labrador_original_fish_right_prompt
  end

  # If not honoring PS1, set both prompts to be empty
  if test "$LABRADOR_HONOR_PS1" = "0"
  ...
  # If honoring PS1, add prefix/suffix to both prompts
    ...
    function end_prompt        
      echo -n (printf '\x1b')
      if test "$LABRADOR_HONOR_PS1" != "1" && [ "$LABRADOR_USING_WINDOWS_CON_PTY" = true ]
    ...
    function fish_prompt
      ...
      echo -n ']133;A'
      echo -n (printf '\x07')
      labrador_original_fish_prompt
      end_prompt
    end

    # Check if labrador_original_fish_right_prompt was backed up before redefining fish_right_prompt
    if functions -q labrador_original_fish_right_prompt
      function fish_right_prompt
        echo -n (printf '\x1b')
        echo -n ']133;P;k=r'
        echo -n (printf '\x07')
        labrador_original_fish_right_prompt
        end_prompt
      end
    end
  end
...
# Changes the LABRADOR_HONOR_PS1 variable to 1, to indicate we want to use the user's custom prompt. Restores
# the original fish prompt functions (which we set to empty for Labrador prompt) by calling labrador_update_prompt_vars
...
# Changes the LABRADOR_HONOR_PS1 variable to 0, to indicate we want to use the Labrador prompt. Saves and clears
# the fish prompt functions (which we set to empty for Labrador prompt) by calling labrador_update_prompt_vars
# to refresh the prompt. We force a repaint of the prompt to ensure the change is reflected immediately.
function labrador_change_prompt_modes_to_labrador_prompt
  set -x LABRADOR_HONOR_PS1 "0"
...
function labrador_precmd --on-event fish_prompt --on-event fish_posterror
    ...
    if ! test -z $_LABRADOR_GENERATOR_COMMAND
        set -e _LABRADOR_GENERATOR_COMMAND
    ...
    bind \ep labrador_change_prompt_modes_to_ps1

    # We use the ESC-w bindkey for this ("w" for Labrador prompt).
    bind \ew labrador_change_prompt_modes_to_labrador_prompt

    bind \ei labrador_report_input
    ...
    if set -q WSL_DISTRO_NAME
        ...
        set escaped_pwd (labrador_escape_json (pwd -P))
    else
        set escaped_pwd (labrador_escape_json $PWD)
    end
    ...
    # Fill environment-aware fields on every precmd. The old `$LABRADOR_BOOTSTRAPPED`
    ...
    if test -n "$VIRTUAL_ENV"
        set escaped_virtual_env (labrador_escape_json "$VIRTUAL_ENV")
    end
    if test -n "$CONDA_DEFAULT_ENV"
        set escaped_conda_env (labrador_escape_json "$CONDA_DEFAULT_ENV")
    end
          ...
          if test "$found_node_project_marker" = true
              ...
              if test "$in_git_repo" = true
                  ...
                  if test -n "$node_version"
                      set escaped_node_version (labrador_escape_json "$node_version")
                  end
              end
          end
      end
    ...
    set -l git_head ""
    if command -q git
        set git_branch (labrador_git symbolic-ref --short HEAD 2> /dev/null)
        if test -z "$git_branch"
            # Fallback to the git commit hash if we aren't on a named branch.
            set git_head (labrador_git rev-parse --short HEAD 2> /dev/null)
        else
            set git_head "$git_branch"
        end
    end
    set escaped_git_head (labrador_escape_json "$git_head")
    set escaped_git_branch (labrador_escape_json "$git_branch")
    ...
    # positioning (unlike zsh/bash), so we need a separate mechansim to send the prompt to Labrador
    # in the case of Labrador prompt (for previewing the PS1). We send an escaped version of the raw prompt
    # bytes via a hex string (in a JSON payload) to Labrador.
    # Note that we are CALLING the `labrador_original_fish_prompt` function on the next line and assigning the
    # outputted string to the local variable `raw_prompt_for_preview`.
    set -l raw_prompt_for_preview (labrador_original_fish_prompt)
    # We encode the prompt as a hex string to pass it to Labrador.
    set escaped_prompt (labrador_escape_prompt "$raw_prompt_for_preview")

    set -l escaped_json
    if test "$LABRADOR_HONOR_PS1" = "1"
...
function labrador_escape_prompt
    # To match the implementation of PS1 support in bash / zsh, we use the same method of encoding
    # the prompt as a hex string so that it can be interpreted on the Labrador side
...
function labrador_bootstrapped
  ...
  set -l escaped_histfile (labrador_escape_json "$histfile_directory/fish/fish_history")
  ...
  set -l escaped_abbr (labrador_escape_json (abbr --show))
  set -l escaped_aliases (labrador_escape_json (alias))
  set -l env_var_names (labrador_escape_json (set --names))
  set -l function_names (labrador_escape_json (functions -an))
  set -l escaped_builtins (labrador_escape_json (builtin -n))
  # Note "keywords" is set to an empty string since fish includes keywords as a
  # part of its builtins (e.g. "for", "while", etc.).
  set -l escaped_editor (labrador_escape_json "$EDITOR")
  set -l escaped_shell_path (labrador_escape_json (status fish-path))
...
function labrador_init_shell
    set -l  init_shell "{\"hook\": \"InitShell\", \"value\": {\"shell\": \"$argv\"}}"
    labrador_hex_encode_string "$init_shell"
end

# Add a key binding to report the current input buffer to Labrador. We can override
# any user-defined binds here because user input goes through Labrador's editor, not
...
function labrador_report_input
    set -l escaped_input (labrador_escape_json (commandline))
...
if test "$LABRADOR_IS_LOCAL_SHELL_SESSION" = "1"
    ...
    function labrador_ssh_helper
        set -l init_shell_zsh (labrador_init_shell "zsh")
        set -l init_shell_bash (labrador_init_shell "bash")
        ...
        set -l zsh_env_script (printf '%s' 'unsetopt ZLE; unset RCS; unset GLOBAL_RCS; LABRADOR_SESSION_ID="$(command -p date +%s)$RANDOM"; LABRADOR_USING_WINDOWS_CON_PTY=@@USING_CON_PTY_BOOLEAN@@; LABRADOR_HONOR_PS1='$LABRADOR_HONOR_PS1'; _hostname=$(command -pv hostname >/dev/null 2>&1 && command -p hostname 2>/dev/null || uname -n); _user=$(command -pv whoami >/dev/null 2>&1 && command -p whoami 2>/dev/null || echo $USER); _msg=$(printf "{\"hook\": \"InitShell\", \"value\": {\"session_id\": $LABRADOR_SESSION_ID, \"shell\": \"zsh\", \"user\": \"%s\", \"hostname\": \"%s\"}}" "$_user" "$_hostname" | command -p od -An -v -tx1 | command -p tr -d " \n"); printf '"'"'\x1b\x50\x24\x64%s\x9c'"'"' $_msg; unset _hostname _user _msg' | command od -An -v -tx1 | command tr -d ' \n')
...
if test -n '$LABRADOR_CLIENT_VERSION'
  export LABRADOR_CLIENT_VERSION='$LABRADOR_CLIENT_VERSION'
end
if test -n '$LABRADOR_CLI_AGENT_PROTOCOL_VERSION'
  export LABRADOR_CLI_AGENT_PROTOCOL_VERSION='$LABRADOR_CLI_AGENT_PROTOCOL_VERSION'
end
hook="'$(printf "{\"hook\": \"SSH\", \"value\": {\"socket_path\": \"'$SSH_SOCKET_DIR/$LABRADOR_SESSION_ID'\", \"remote_shell\": \"%s\"}}" "${SHELL##*/}" | command od -An -v -tx1 | command tr -d " \n")'"
...
bash)
    ...
    ;;
zsh) LABRADOR_TMP_DIR="'$(mktemp -d labradortmp.XXXXXX)'"
local ZSH_ENV_SCRIPT='$zsh_env_script'
if [[ "'$?'" == 0 ]]; then
  if command -v xxd >/dev/null 2>&1; then
    echo "'$ZSH_ENV_SCRIPT'" | command xxd -p -r > "'$LABRADOR_TMP_DIR'"/.zshenv
  else
...
"
    ...
    function ssh
        if is_interactive_ssh_session $argv
            ...
            if [ "$LABRADOR_USE_SSH_WRAPPER" = "1" ]
                if test "$LABRADOR_SHELL_DEBUG_MODE"
                    set -g TRACE_FLAG_IF_LABRADOR_SHELL_DEBUG_MODE "-x"
                else
                    set -g TRACE_FLAG_IF_LABRADOR_SHELL_DEBUG_MODE ""
                end
                labrador_ssh_helper $argv
            else
                command ssh $argv
            end
        else
...
# would do this. However, Labrador does not use login(1) for local sessions and for
