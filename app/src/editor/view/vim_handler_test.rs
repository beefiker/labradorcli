...
use labrador_ui::{App, ViewHandle};
...
fn test_vim_number_repeat_line_motion() {
    ...
    App::test((), |mut app| async move {
        ...
        let mut presenter = labrador_ui::presenter::Presenter::new(window_id);

        let mut updated = HashSet::new();
        updated.insert(app.root_view_id(window_id).unwrap());
        let invalidation = labrador_ui::WindowInvalidation {
            updated,
            ..Default::default()
        };
...
fn test_vim_delete_word_dge() {
    App::test((), |mut app| async move {
        ...
        let editor = add_editor_vim_normal_mode("echo hello-hi labrador-dev", &mut app);

        editor.update(&mut app, |view, view_ctx| {
            view.select_ranges(
                vec![DisplayPoint::new(0, 24)..DisplayPoint::new(0, 24)],
                view_ctx,
            )
            .unwrap();
        ...
        editor.read(&app, |view, app_ctx| {
            assert_eq!(view.buffer_text(app_ctx), "echo hello-hi labradorev");
            assert_eq!(
                view.selected_ranges(app_ctx),
                vec![DisplayPoint::new(0, 22)..DisplayPoint::new(0, 22)]
            );
        });
...
fn test_vim_change_word_cge() {
    App::test((), |mut app| async move {
        ...
        editor.read(&app, |view, app_ctx| {
            assert_eq!(view.buffer_text(app_ctx), "echo hello-hrev");
            assert_eq!(
                view.selected_ranges(app_ctx),
                vec![DisplayPoint::new(0, 12)..DisplayPoint::new(0, 12)]
            );
        ...
        editor.read(&app, |view, app_ctx| {
            assert_eq!(view.buffer_text(app_ctx), "echhrev");
            assert_eq!(
                view.selected_ranges(app_ctx),
                vec![DisplayPoint::new(0, 3)..DisplayPoint::new(0, 3)]
            );
