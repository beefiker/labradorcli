...
use labrador_ui::elements::{Border, ChildView, Flex, ParentElement};
...
use labrador_ui::{Entity, TypedActionView, View, ViewHandle};
    ...
    fn render(&self, ctx: &AppContext) -> Box<dyn Element> {
        ...
        Hoverable::new(self.autosuggestion_hint_mouse_handle.clone(), |state| {
            ...
            let chevron_down = Container::new(
                ConstrainedBox::new(
                    Icon::ArrowDropDown
                        .to_labrador_ui_icon(Fill::Solid(font_color))
                        .finish(),
                )
