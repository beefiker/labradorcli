...
pub use util::*;
use labrador_ui::integration::PersistedDataMap;
pub use labrador_ui::integration::RUNTIME_TAG_FAILURE_REASON;
use labrador_ui::{App, SingletonEntity as _, WindowId};
    ...
    BlocklistAIHistoryModel::handle(app).update(app, |history_model, _| {
        ...
        let debug_link = conversation
            ...
            .map(|token| {
                ...
                // with staging.labrador.dev instead of localhost:8080.
                token
                    .debug_link()
                    .replace("host.docker.internal:8080", "staging.labrador.dev")
            })
