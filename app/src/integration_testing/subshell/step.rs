...
pub fn assert_subshell_banner_is_showing() -> TestStep {
    TestStep::new("Assert the Labradorify banner is visible")
        .add_assertion(move |app, window_id| {
            ...
            terminal_view.read(app, |view, _ctx| {
                async_assert!(matches!(
                    view.model
                        ...
                        .active_block()
                        .block_banner(),
                    Some(WithinBlockBanner::LabradorifyBanner(..))
                ))
            })
        })
        ...
        move |app, window_id| {
            ...
            terminal_view.read(app, |view, _ctx| {
                ...
                match rich_content_type {
                    Some(RichContentType::LabradorifySuccessBlock) => {}
                    _ => {
                        return AssertionOutcome::failure(
                            "Labradorify success block wasn't added to the blocklist".to_owned(),
                        );
                    }
                }
