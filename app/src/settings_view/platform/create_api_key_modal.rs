...
impl ApiKeyType {
    fn description(&self) -> &'static str {
        match self {
            ApiKeyType::Personal => {
                "This API key is tied to your user and can make requests against your Dwarf account."
            }
            ApiKeyType::Team => {
...
impl CreateApiKeyModal {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let name_editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text("Dwarf API Key", ctx);
            editor
        });
