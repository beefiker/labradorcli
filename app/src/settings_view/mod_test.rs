...
fn ai_subpages_are_identified() {
    assert!(SettingsSection::LabradorAgent.is_ai_subpage());
...
fn code_subpages_are_identified() {
    ...
    assert!(!SettingsSection::LabradorAgent.is_code_subpage());
}
...
fn cloud_platform_subpages_are_identified() {
    ...
    assert!(!SettingsSection::LabradorAgent.is_cloud_platform_subpage());
}
...
fn ai_subpages_map_to_ai_backing_page() {
    assert_eq!(
        SettingsSection::LabradorAgent.parent_page_section(),
        SettingsSection::AI
    );
    assert_eq!(
...
fn ai_subpages_list_contains_all_ai_subpage_variants() {
    ...
    assert!(subpages.contains(&SettingsSection::LabradorAgent));
...
fn subpage_display_names_are_correct() {
    assert_eq!(
        SettingsSection::LabradorAgent.to_string(),
...
fn subpage_from_str_parses_display_names() {
    ...
    assert_eq!(
        ...
        Ok(SettingsSection::LabradorAgent)
    );
    assert_eq!(
...
fn search_knowledge_shows_only_knowledge_subpage() {
    ...
    filter.insert(SettingsSection::LabradorAgent, MatchData::Countable(0));
...
fn search_agent_shows_profiles_and_cli_agents() {
    ...
    assert!(!visible.contains(&SettingsSection::LabradorAgent));
...
fn nav_filter_includes_matching_subpage_and_excludes_others() {
    ...
    subpage_filter.insert(SettingsSection::LabradorAgent, MatchData::Countable(0));
...
fn collapsed_umbrella_is_a_single_nav_stop() {
    ...
    assert!(matches!(
        ...
        NavStop::CollapsedUmbrella {
            nav_index: 1,
            first_subpage: SettingsSection::LabradorAgent,
...
fn expanded_umbrella_produces_section_stop_per_subpage() {
    ...
    // Expect: Account, LabradorAgent, AgentProfiles, AgentMCPServers, Knowledge,
...
fn collapsed_umbrella_with_filtered_subpages_uses_first_visible_subpage() {
    ...
    let stops = build_nav_stops(&nav_items, |section| {
        // Hide LabradorAgent (first AI subpage); keep the rest.
        section != SettingsSection::LabradorAgent
    });
    ...
    match agents_stop {
        NavStop::CollapsedUmbrella {
            ...
            assert_eq!(
                *first_subpage,
                SettingsSection::AgentProfiles,
                "LabradorAgent is hidden by the filter, so the first visible subpage is AgentProfiles"
            );
            assert_eq!(
                *last_subpage,
                SettingsSection::ThirdPartyCLIAgents,
                "last_subpage is unaffected by hiding LabradorAgent and should remain the last visible subpage"
            );
        }
        _ => unreachable!(),
    }
...
fn arrow_down_from_account_with_collapsed_agents_lands_on_first_subpage() {
    ...
    assert_eq!(next, SettingsSection::LabradorAgent);
}
...
fn arrow_down_across_adjacent_collapsed_umbrellas() {
    ...
    // From Account, Down should auto-expand Agents and land on its first subpage.
    ...
    assert_eq!(next_after_account, SettingsSection::LabradorAgent);

    // From LabradorAgent (collapsed Agents umbrella), Down should land on the first
    // Code subpage (Code umbrella auto-expands).
...
fn arrow_down_collapsed_umbrella_respects_search_filter() {
    let nav_items = realistic_nav_items();
    // Search filter hides LabradorAgent and AgentProfiles so the first visible AI
    // subpage is AgentMCPServers.
    let is_visible = |section: SettingsSection| {
        !matches!(
            section,
            SettingsSection::LabradorAgent | SettingsSection::AgentProfiles
        )
    };
    let stops = build_nav_stops(&nav_items, is_visible);

    // From Account, Down should land on AgentMCPServers (first visible
    // subpage of the still-collapsed Agents umbrella), not on LabradorAgent /
    // AgentProfiles.
    let next = simulate_cycle(
        &nav_items,
        &stops,
