...
pub enum InstallOrigin {
    /// Triggered by a user gesture inside Labrador (gallery card click,
    /// reinstall button, programmatic in-app flows, etc.).
    InApp,
    /// Triggered by a `labrador://settings/mcp?autoinstall=...` deeplink; must be
...
impl MCPServersSettingsPageView {
    ...
    ///
    /// This is the single sink for `labrador://settings/mcp?autoinstall=<title>`
