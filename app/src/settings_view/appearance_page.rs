...
use crate::themes::theme::{self, RespectSystemTheme, SelectedSystemThemes, ThemeKind, LabradorTheme};
...
use labrador_util::path::user_friendly_path;
use labrador_ui::elements::{
    Clipped, Empty, FormattedTextElement, MainAxisAlignment, MainAxisSize, Text, Wrap,
};
use labrador_ui::fonts::{FamilyId, FontInfo, Weight};
use labrador_ui::keymap::{ContextPredicate, FixedBinding};
use labrador_ui::platform::{Cursor, FilePickerConfiguration, GraphicsBackend};
...
use labrador_ui::units::IntoPixels;
...
use labrador_ui::{platform::SystemTheme, Action};
use labrador_ui::{
...
pub fn init_actions_from_parent_view<T: Action + Clone>(
    ...
    // Add command palette entry for toggling between Labrador and Classic input modes
    app.register_fixed_bindings(vec![FixedBinding::empty(
        format!(
...
pub struct AppearanceSettingsPageView {
    ...
    /// selected Labrador prompt configuration.
...
impl AppearanceSettingsPageView {
    ...
    pub fn new(ctx: &mut ViewContext<AppearanceSettingsPageView>) -> Self {
        ...
        if ChannelState::channel() != Channel::Integration {
            ...
            {
                let all_system_fonts = labrador_ui::fonts::Cache::handle(ctx)
        ...
        Align::new(
            appearance
                ...
                .link(
                    ...
                    Some("https://docs.labrador.dev/terminal/appearance/custom-themes".to_string()),
                    None,
...
impl ThemeSelectWidget {
    fn render_theme_option(
        ...
        let theme: LabradorTheme = LabradorConfig::as_ref(app).theme_config().theme(&theme_kind);
    ...
    fn render(
        ...
        let label_info = AdditionalInfo {
            ...
            on_click_action: Some(AppearancePageAction::OpenUrl(
                "https://docs.labrador.dev/terminal/appearance/size-opacity-blurring".into(),
            )),
            secondary_text: None,
...
impl SettingsWidget for ToolsPanelStateScopeWidget {
    ...
    fn search_terms(&self) -> &str {
        "left tools panel open closed across tabs file tree project explorer global search labrador drive conversation list"
    }
...
impl SettingsWidget for InputTypeWidget {
    ...
    fn search_terms(&self) -> &str {
        "input type labrador universal classic style prompt terminal ai developer mode interface shell chips ps1"
    }
...
impl SettingsWidget for PromptWidget {
    ...
    fn search_terms(&self) -> &str {
        "prompt ps1 terminal labrador shell custom"
    }
    ...
    fn render(
        ...
        let mut column = Flex::column().with_child(render_body_item::<AppearancePageAction>(
            ...
            Some(AdditionalInfo {
                ...
                on_click_action: Some(AppearancePageAction::OpenUrl(
                    "https://docs.labrador.dev/terminal/more-features/full-screen-apps#padding".into(),
                )),
                secondary_text: None,
                tooltip_override_text: None,
            }),
