...
use crate::{
    ...
    report_if_error,
    terminal::labradorify::settings::LabradorifySettings,
...
pub fn init_actions_from_parent_view<T: Action + Clone>(
    ...
    // Add all of the toggle settings from the Labradorify Page that you want to show up on the Command Palette here.
    ...
    if FeatureFlag::SSHTmuxWrapper.is_enabled() {
        ...
        toggle_binding_pairs.push(ToggleSettingActionPair::new(
            &label,
            builder(SettingsAction::LabradorifyPageToggle(
                LabradorifyPageAction::ToggleTmuxLabradorification,
            )),
            context,
...
/// This page lets users configure when they get asked to labradorify a session. Some shell commands
...
/// related to labradorification.
pub struct LabradorifyPageView {
    page: PageType<Self>,
    ...
    ssh_extension_install_mode_dropdown: ViewHandle<Dropdown<LabradorifyPageAction>>,
}

impl LabradorifyPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        instance.update_button_states(labradorify_settings_handle, ctx);
        instance
    }
    ...
    fn update_button_states(
        &mut self,
        labradorify_settings_handle: ModelHandle<LabradorifySettings>,
        ctx: &mut ViewContext<Self>,
    ) {
        let labradorify_settings = labradorify_settings_handle.as_ref(ctx);
        self.remove_denylisted_command_button_states = labradorify_settings
            .subshell_command_denylist
            .iter()
            .map(|_| Default::default())
            .collect();
        self.remove_added_command_button_states = labradorify_settings
            .added_subshell_commands
            .iter()
            .map(|_| Default::default())
            .collect();
        self.remove_denylisted_ssh_button_states = labradorify_settings
            .ssh_hosts_denylist
            .iter()
            .map(|_| Default::default())
            .collect();
    ...
    /// `LabradorifySettings::ssh_extension_install_mode` value (e.g. after it
    /// was changed from the SSH remote server choice view).
    fn update_dropdown(&mut self, ctx: &mut ViewContext<Self>) {
        let current_mode = *LabradorifySettings::as_ref(ctx)
            .ssh_extension_install_mode
            .value();
        self.ssh_extension_install_mode_dropdown
            .update(ctx, |dropdown, ctx| {
                dropdown.set_selected_by_action(
                    LabradorifyPageAction::SetSshExtensionInstallMode(current_mode),
                    ctx,
                );
            });
    }
    ...
    fn handle_added_command_editor_event(
        ...
        match event {
            SubmittableTextInputEvent::Submit(new_command) => {
                LabradorifySettings::handle(ctx).update(ctx, |labradorify_settings, ctx| {
                    labradorify_settings.add_subshell_command(new_command, ctx);
                });
            }
    ...
    fn handle_denylisted_command_editor_event(
        ...
        match event {
            SubmittableTextInputEvent::Submit(new_command) => {
                LabradorifySettings::handle(ctx).update(ctx, |labradorify_settings, ctx| {
                    labradorify_settings.denylist_subshell_command(new_command, ctx);
                });
            }
            SubmittableTextInputEvent::Escape => ctx.emit(SettingsPageEvent::FocusModal),
        }
    ...
    fn handle_denylisted_ssh_editor_event(
        ...
        match event {
            SubmittableTextInputEvent::Submit(new_command) => {
                LabradorifySettings::handle(ctx).update(ctx, |labradorify_settings, ctx| {
                    labradorify_settings.denylist_ssh_host(new_command, ctx);
                });
            }
            SubmittableTextInputEvent::Escape => ctx.emit(SettingsPageEvent::FocusModal),
        }
    ...
    fn remove_denylisted_command(&self, index: usize, ctx: &mut ViewContext<Self>) {
        LabradorifySettings::handle(ctx).update(ctx, |labradorify, ctx| {
            labradorify.remove_denylisted_subshell_command(index, ctx)
        });
    }

    fn remove_added_command(&self, index: usize, ctx: &mut ViewContext<Self>) {
        LabradorifySettings::handle(ctx).update(ctx, |labradorify, ctx| {
            labradorify.remove_added_subshell_command(index, ctx)
        });
    }

    fn remove_denylisted_ssh_host(&self, index: usize, ctx: &mut ViewContext<Self>) {
        LabradorifySettings::handle(ctx).update(ctx, |labradorify, ctx| {
            labradorify.remove_denylisted_ssh_host(index, ctx)
        });
    }
}

impl Entity for LabradorifyPageView {
...
impl LabradorifyPageView {
    fn create_ssh_extension_install_mode_dropdown(
        ctx: &mut ViewContext<Self>,
    ) -> ViewHandle<Dropdown<LabradorifyPageAction>> {
        let items: Vec<DropdownItem<LabradorifyPageAction>> = SshExtensionInstallMode::iter()
            .map(|mode| {
                DropdownItem::new(
                    mode.display_name(),
                    LabradorifyPageAction::SetSshExtensionInstallMode(mode),
                )
            })
            .collect();
        ...
        let current_mode = *LabradorifySettings::as_ref(ctx)
            .ssh_extension_install_mode
            .value();
        let enable_ssh_labradorification = *LabradorifySettings::as_ref(ctx)
            .enable_ssh_labradorification
            .value();
...
impl View for LabradorifyPageView {
...
pub enum LabradorifyPageAction {
...
impl TypedActionView for LabradorifyPageView {
    type Action = LabradorifyPageAction;

    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        use LabradorifyPageAction::*;
        match action {
            ...
            ToggleSshLabradorification => {
                ...
                let enabled = *LabradorifySettings::as_ref(ctx)
                    .enable_ssh_labradorification
                    .value();
                self.ssh_extension_install_mode_dropdown
                    .update(ctx, |dropdown, ctx| {
            ...
            SetSshExtensionInstallMode(mode) => {
                LabradorifySettings::handle(ctx).update(ctx, |labradorify_settings, ctx| {
                    report_if_error!(labradorify_settings
                        .ssh_extension_install_mode
                        .set_value(*mode, ctx));
                });
            }
            LabradorifyPageAction::RemoveDenylistedSshHost(index) => {
...
impl SettingsPageMeta for LabradorifyPageView {
    fn section() -> SettingsSection {
        SettingsSection::Labradorify
    }
...
impl From<ViewHandle<LabradorifyPageView>> for SettingsPageViewHandle {
    fn from(view_handle: ViewHandle<LabradorifyPageView>) -> Self {
        SettingsPageViewHandle::Labradorify(view_handle)
    }
}
...
impl TitleWidget {
    fn render_top_of_page(&self, appearance: &Appearance, _app: &AppContext) -> Box<dyn Element> {
        let labradorify_description = vec![
            ...
            FormattedTextFragment::hyperlink(
                "Learn more",
                "https://docs.labrador.dev/terminal/labradorify/subshells",
            ),
        ];

        let labradorify_description = FormattedTextElement::new(
            FormattedText::new([FormattedTextLine::Line(labradorify_description)]),
            CONTENT_FONT_SIZE,
        ...
        Flex::column()
            ...
            .with_child(labradorify_description)
            .finish()
    }
}
...
impl SettingsWidget for TitleWidget {
    ...
    fn search_terms(&self) -> &str {
        "ssh subshell labradorify shell bootstrap session"
    }
    ...
    fn render_subshells_section(
        ...
        column.add_child(
            view.build_input_list(
                "Added commands",
                &labradorify_settings.added_subshell_commands,
                &view.remove_added_command_button_states,
                LabradorifyPageAction::RemoveAddedCommand,
        ...
        column.add_child(
            view.build_input_list(
                "Denylisted commands",
                &labradorify_settings.subshell_command_denylist,
                &view.remove_denylisted_command_button_states,
                LabradorifyPageAction::RemoveDenylistedCommand,
...
impl SettingsWidget for SubshellsWidget {
    ...
    fn search_terms(&self) -> &str {
        "labradorify subshell shell bootstrap"
    }
...
impl SettingsWidget for SSHWidget {
    ...
    fn render(
        ...
        let enable_ssh_labradorification = *LabradorifySettings::as_ref(app)
            .enable_ssh_labradorification
            .value();
        ...
        add_setting(
            &mut column,
            &LabradorifySettings::as_ref(app).enable_ssh_labradorification,
            move || {
                render_body_item::<LabradorifyPageAction>(
                    format!("{} SSH Sessions", ChannelState::app_name_verbify()),
                    None,
                    LocalOnlyIconState::for_setting(
                        EnableSshLabradorification::storage_key(),
                        EnableSshLabradorification::sync_to_cloud(),
                    ...
                    appearance,
                    ui_builder
                        .switch(self.enable_ssh_labradorification_switch_state.clone())
                        .check(enable_ssh_labradorification)
                        .build()
                        .on_click(move |ctx, _, _| {
                            ctx.dispatch_typed_action(
                                LabradorifyPageAction::ToggleSshLabradorification,
                            );
                        })
                        .finish(),
                    None,
        ...
        if FeatureFlag::SshRemoteServer.is_enabled() {
            let label_color_override = if !enable_ssh_labradorification {
            ...
            add_setting(
                &mut column,
                &LabradorifySettings::as_ref(app).ssh_extension_install_mode,
                move || {
        ...
        add_setting(
            &mut column,
            &LabradorifySettings::as_ref(app).use_ssh_tmux_wrapper,
            move || {
                let mut column = Flex::column();
                ...
                column.add_child(render_body_item::<LabradorifyPageAction>(
                    ...
                    Some(AdditionalInfo {
                        ...
                        on_click_action: Some(LabradorifyPageAction::OpenUrl(
                            "https://docs.labrador.dev/terminal/labradorify/ssh".into(),
                        )),
                        secondary_text: None,
                    ...
                    appearance,
                    ui_builder
                        .switch(self.tmux_labradorification_switch_state.clone())
                ...
                column.add_child(
                    ui_builder
                        .paragraph(SSH_TMUX_LABRADORIFICATION_DESCRIPTION.to_owned())
                        .with_style(UiComponentStyles {
                ...
                if enable_ssh_labradorification && should_prompt_ssh_tmux_wrapper {
                    ...
                    column.add_child(
                        view.build_input_list(
                            "Denylisted hosts",
                            &labradorify_settings.ssh_hosts_denylist,
                            &view.remove_denylisted_ssh_button_states,
                            LabradorifyPageAction::RemoveDenylistedSshHost,
