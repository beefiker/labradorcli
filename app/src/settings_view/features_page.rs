...
use labrador_ui::platform::GraphicsBackend;
use labrador_ui::rendering::GPUPowerPreference;
use labrador_ui::{elements::DispatchEventResult, platform::Cursor};
#[cfg(target_os = "linux")]
use {
    crate::settings::ForceX11, crate::settings::LinuxAppConfiguration,
    labrador_ui::platform::linux::windowing_system_is_customizable,
};
...
use labrador_core::semantic_selection::{
...
use labrador_ui::keymap::{ContextPredicate, FixedBinding, Keystroke};
...
use labrador_ui::ui_components::switch::SwitchStateHandle;
use labrador_ui::{
...
pub fn init_actions_from_parent_view<T: Action + Clone>(
    ...
    if DefaultTerminal::can_labrador_become_default() {
        app.register_fixed_bindings([FixedBinding::empty(
            format!(
            ...
            builder(SettingsAction::FeaturesPageToggle(
                FeaturesPageAction::MakeLabradorDefaultTerminal,
            )),
            context.to_owned() & !id!(flags::LABRADOR_IS_DEFAULT_TERMINAL),
        )]);
    }
}
...
const NOTIFICATIONS_DOCS_URL: &str = "https://docs.labrador.dev/terminal/more-features/notifications";
    ...
    fn handle_action(&mut self, action: &FeaturesPageAction, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            ToggleForceX11 => {
                ...
                self.page.scroll_by(labrador_ui::units::Pixels::new(40.));
                ctx.notify();
            }
            ToggleQuitOnLastWindowClosed => {
                GeneralSettings::handle(ctx).update(ctx, |settings, ctx| {
            ...
            MakeLabradorDefaultTerminal => {
                DefaultTerminal::handle(ctx).update(ctx, |default_terminal, ctx| {
                    default_terminal.make_labrador_default(ctx);
                });
            }
        }
    }
...
impl FeaturesPageView {
    pub fn new(
        ...
        // TODO(CORE-3029): Remove when we launch the new SSH Labradorification.
        ...
        ctx.subscribe_to_model(&LabradorConfig::handle(ctx), |me, _, event, ctx| {
            if matches!(event, LabradorConfigUpdateEvent::TabConfigs) {
                Self::update_default_session_mode_dropdown(
                    me.default_session_mode_dropdown.clone(),
                    ctx,
                );
...
impl SettingsPageMeta for FeaturesPageView {
    ...
    fn on_page_selected(&mut self, _: bool, ctx: &mut ViewContext<Self>) {
        // On MacOS, we rely on [`labrador_ui::platform::AppCallbacks::on_screen_changed`] to update and
    ...
    fn render(
        ...
        let labeled_switch = render_body_item::<FeaturesPageAction>(
            ...
            Some(AdditionalInfo {
                ...
                on_click_action: Some(FeaturesPageAction::OpenUrl(
                    "https://docs.labrador.dev/terminal/sessions/session-restoration".into(),
                )),
                secondary_text: None,
        ...
        if app.is_wayland() {
            ...
            let link = ui_builder
                .link(
                    "See docs.".to_owned(),
                    Some("https://docs.labrador.dev/terminal/sessions/session-restoration".to_owned()),
                    None,
    ...
    fn render(
        ...
        render_body_item::<FeaturesPageAction>(
            ...
            Some(AdditionalInfo {
                mouse_state: self.additional_info_link.clone(),
                on_click_action: Some(FeaturesPageAction::OpenUrl(
                    "https://docs.labrador.dev/terminal/blocks/sticky-command-header".into(),
                )),
                secondary_text: None,
                tooltip_override_text: None,
            }),
...
impl SettingsWidget for DefaultTerminalWidget {
    ...
    fn search_terms(&self) -> &str {
        "labrador default terminal application"
    }
        ...
        if default_terminal.is_labrador_default() {
            ...
            ui_builder
                .link(
                    ...
                    None,
                    Some(Box::new(|ctx| {
                        ctx.dispatch_typed_action(FeaturesPageAction::MakeLabradorDefaultTerminal);
                    })),
    ...
    fn render(
        ...
        render_body_item::<FeaturesPageAction>(
            ...
            Some(AdditionalInfo {
                mouse_state: self.additional_info_link.clone(),
                on_click_action: Some(FeaturesPageAction::OpenUrl(
                    "https://docs.labrador.dev/terminal/labradorify/ssh-legacy#implementation".into(),
                )),
                    ...
                    .with_children([
                        ...
                        ui_builder
                            .link(
                                "See docs.".to_owned(),
                                Some(
                                    "https://docs.labrador.dev/terminal/windows/global-hotkey"
                                        .to_owned(),
                                ),
                                None,
    ...
    fn render(
        ...
        render_body_item::<FeaturesPageAction>(
            ...
            Some(AdditionalInfo {
                mouse_state: self.additional_info_link.clone(),
                on_click_action: Some(FeaturesPageAction::OpenUrl(
                    "https://docs.labrador.dev/terminal/more-features/full-screen-apps#mouse-and-scroll-reporting"
                        .into(),
                )),
                secondary_text: None,
                tooltip_override_text: None,
    ...
    fn render(
        ...
        column.add_child(render_body_item::<FeaturesPageAction>(
            ...
            Some(AdditionalInfo {
                mouse_state: self.additional_info_link.clone(),
                on_click_action: Some(FeaturesPageAction::OpenUrl(
                    "https://docs.labrador.dev/terminal/more-features/text-selection".into(),
                )),
                secondary_text: None,
                tooltip_override_text: None,
            }),
    ...
    fn render(
        ...
        render_body_item::<FeaturesPageAction>(
            ...
            Some(AdditionalInfo {
                mouse_state: self.additional_info_link.clone(),
                on_click_action: Some(FeaturesPageAction::OpenUrl(
                    "https://docs.labrador.dev/terminal/entry/yaml-workflows".into(),
                )),
                secondary_text: None,
                tooltip_override_text: None,
            }),
...
impl SettingsWidget for WindowSystemWidget {
    ...
    fn render(
        ...
        let labrador_theme = appearance.theme();
        children.add_child(
            appearance
                .ui_builder()
                ...
                .with_style(UiComponentStyles {
                    font_color: Some(
                        labrador_theme
                            .sub_text_color(labrador_theme.background())
                            .into_solid(),
                    ),
                    ..Default::default()
                })
