...
use labradorify_page::{LabradorifyPageAction, LabradorifyPageView};
...
pub enum SettingsSection {
    ...
    /// (LabradorAgent, AgentProfiles, Knowledge, ThirdPartyCLIAgents) share this single
    /// backing page, so this variant is needed as the key in `settings_pages`.
    /// External callers should navigate to a specific subpage (e.g. `LabradorAgent`) instead.
    AI,
...
impl Display for SettingsSection {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            SettingsSection::LabradorAgent => write!(f, "{}", ChannelState::app_name_agent()),
            ...
            SettingsSection::Labradorify => write!(f, "{}", ChannelState::app_name_verbify()),
            _ => write!(f, "{self:?}"),
        }
    }
}
...
impl FromStr for SettingsSection {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            "About" => Ok(Self::About),
            "Account" | "LabradorAgent" => Ok(Self::LabradorAgent),
            "AI" => Ok(Self::AI),
            ...
            "Labradorify" => Ok(Self::Labradorify),
            other if other == ChannelState::app_name_verbify() => {
                Ok(Self::Labradorify)
            }
            other if other == ChannelState::app_name_agent() => Ok(Self::LabradorAgent),
...
pub mod flags {
    ...
    #[deprecated = "Use `SSH_TMUX_WRAPPER_CONTEXT_FLAG` for new ssh shell bootstrapping logic"]
    ...
    pub const LABRADOR_SAME_LINE_PROMPT_FLAG: &str = "Labrador_Same_Line_Prompt_Enabled";
    ...
    /// The user's setting enabled UDI, but we may show a classic input (e.g. ssh/subshell labradorification)
    ...
    pub const LABRADOR_IS_DEFAULT_TERMINAL: &str = "LabradorIsDefaultTerminal";
    ...
    pub const ENABLE_LABRADOR_DRIVE: &str = "EnableLabradorDrive";
...
pub fn init_actions_from_parent_view<T: Action + Clone>(
    ...
    labradorify_page::init_actions_from_parent_view(app, context, builder);
...
impl<T: Action + Clone> ToggleSettingActionPair<T> {
    ...
    pub fn new(
        ...
        use labrador_ui::keymap::macros::id;
...
pub enum SettingsAction {
    ...
    LabradorifyPageToggle(LabradorifyPageAction),
    Tab,
    Split(Direction),
    ToggleMaximizePane,
    Close,
...
macro_rules! update_page {
    ($handle:expr, $update:expr, $ctx:expr) => {
        match $handle {
            ...
            SettingsPageViewHandle::Labradorify(handle) => $ctx.update_view(handle, $update),
...
pub struct SettingsView {
    ...
    /// per `SettingsView` per `LABRADOR.md`'s guidance that inline
...
impl SettingsView {
    pub fn new(page: Option<SettingsSection>, ctx: &mut ViewContext<Self>) -> Self {
        ...
        let labradorify_page_handle = ctx.add_typed_action_view(LabradorifyPageView::new);
        ctx.subscribe_to_view(&labradorify_page_handle, |me, _, event, ctx| {
            me.handle_labradorify_page_event(event, ctx);
        });
        ...
        let mut settings_pages = vec![
            ...
            SettingsPage::new(labradorify_page_handle),
        ];
        ...
        let mut nav_items = vec![
            ...
            SettingsNavItem::Page(SettingsSection::Labradorify),
        ...
        let initial_page = match page {
            Some(SettingsSection::Account) => SettingsSection::LabradorAgent,
            Some(SettingsSection::AI) => SettingsSection::LabradorAgent,
            Some(SettingsSection::Code) => SettingsSection::CodeIndexing,
            Some(SettingsSection::CloudEnvironments)
                if !FeatureFlag::CloudEnvironments.is_enabled() =>
            {
                SettingsSection::LabradorAgent
            }
    ...
    fn handle_labradorify_page_event(
        &mut self,
        event: &SettingsPageEvent,
        ctx: &mut ViewContext<Self>,
    ) {
    ...
    pub fn set_and_refresh_current_page_internal(
        ...
        let section = match section {
            SettingsSection::AI => SettingsSection::LabradorAgent,
        ...
        {
            crate::crash_reporting::set_tag("labrador.settings_page", section.to_string());
        }
    ...
    fn should_render_page(&self, settings_page: &SettingsPage, app: &AppContext) -> bool {
        match &settings_page.view_handle {
            ...
            SettingsPageViewHandle::Labradorify(v) => v.as_ref(app).should_render(app),
                    ...
                    Container::new(
                        ConstrainedBox::new(
                            icons::Icon::SearchSmall
                                .to_labrador_ui_icon(appearance.theme().active_ui_text_color())
                                .finish(),
                        )
                        .with_width(16.)
                        .with_height(16.)
...
impl TypedActionView for SettingsView {
    ...
    fn handle_action(&mut self, action: &SettingsAction, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            SettingsAction::LabradorifyPageToggle(labradorify_action) => {
                if let Some(labradorify_page) = self.settings_page(SettingsSection::Labradorify) {
                    if let SettingsPageViewHandle::Labradorify(view) =
                        &labradorify_page.view_handle
                    {
                        view.update(ctx, |view, ctx| {
                            view.handle_action(labradorify_action, ctx);
                        })
                    }
                }
            }
...
impl BackingView for SettingsView {
    ...
    fn handle_pane_header_overflow_menu_action(
        ...
        ctx: &mut labrador_ui::ViewContext<Self>,
    ) {
        self.handle_action(action, ctx)
    }

    fn close(&mut self, ctx: &mut labrador_ui::ViewContext<Self>) {
        ctx.emit(SettingsViewEvent::Pane(PaneEvent::Close));
    }

    fn focus_contents(&mut self, ctx: &mut labrador_ui::ViewContext<Self>) {
        ctx.focus(&self.search_editor)
    }
