...
use crate::settings::{
    ...
    CanUseLabradorCreditsWithByok, CodeSettings, CodebaseContextEnabled, FileBasedMcpEnabled,
    GitOperationsAutogenEnabled, IncludeAgentCommandsInHistory, IntelligentAutosuggestionsEnabled,
    LabradorDriveContextEnabled, MemoryEnabled, NLDInTerminalEnabled,
...
use labrador_ui::ui_components::slider::SliderStateHandle;
use labrador_ui::{
    elements::{
...
pub enum AISubpage {
    /// The main Labrador Agent page: global AI toggle + Active AI + Input + Other sections.
    LabradorAgent,
...
impl AISubpage {
    pub fn from_section(section: SettingsSection) -> Option<Self> {
        match section {
            SettingsSection::LabradorAgent => Some(Self::LabradorAgent),
...
pub fn init_actions_from_parent_view<T: Action + Clone>(
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        ...
        .with_group(bindings::BindingGroup::LabradorAi)],
        app,
    );
    ...
    );
    {
        ...
        let mode_bindings: Vec<FixedBinding> = ThinkingDisplayMode::iter()
            .map(|mode| {
                ...
                .with_group(bindings::BindingGroup::LabradorAi.as_str())
            })
            .collect();
...
impl AISettingsPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        // third-party coding agents rather than Labrador's own AI.
        Self::update_editor_interaction_state(
...
impl View for AISettingsPageView {
    ...
    fn render(&self, app: &labrador_ui::AppContext) -> Box<dyn labrador_ui::Element> {
...
pub enum AISettingsPageAction {
    ...
    ToggleVoiceInput,
    ToggleCanUseLabradorCreditsWithByok,
...
impl From<&AISettingsPageAction> for LoginGatedFeature {
    fn from(val: &AISettingsPageAction) -> LoginGatedFeature {
        ...
        match val {
            AttemptLoginGatedUpgrade => "AI Usage",
            _ => "Unknown reason",
        }
    }
}
...
impl TypedActionView for AISettingsPageView {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            AISettingsPageAction::ToggleCanUseLabradorCreditsWithByok => {
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(settings
            ...
            AISettingsPageAction::ToggleLabradorDriveContext => {
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    let _ = settings
            ...
            AISettingsPageAction::ToggleAgentAttribution => {
                // The updated value syncs to the server automatically via
...
impl SettingsWidget for GlobalAIWidget {
    ...
    fn search_terms(&self) -> &str {
        "oz labrador agent global ai a.i. active next command prompt code diffs suggestion suggested suggestions \
    ...
    fn render_codebase_context_outline_generation_setting(
        ...
        let codebase_context_description = vec![
            ...
            FormattedTextFragment::hyperlink(
                "Learn more",
                "https://docs.labrador.dev/agent-platform/capabilities/codebase-context",
            ),
        ];
        ...
        let subtext = {
            let subtext_fragments = vec![
                ...
                FormattedTextFragment::hyperlink(
                    ...
                    "https://docs.labrador.dev/agent-platform/capabilities/mcp",
                ),
            ];
            ...
            static AUTODETECTION_DESCRIPTION_FRAGMENTS: LazyLock<Vec<FormattedTextFragment>> =
                LazyLock::new(|| {
                    vec![
                        ...
                        FormattedTextFragment::hyperlink(
                            "Let us know",
                            "https://labradordev.typeform.com/to/offrTIpq",
                        ),
                    ]
                });
                        ...
                        > = LazyLock::new(|| {
                            vec![
                                ...
                                FormattedTextFragment::hyperlink(
                                    ...
                                    "https://docs.labrador.dev/agent-platform/capabilities/mcp#file-based-mcp-servers",
                                ),
                            ]
                        });
                        Container::new(
...
struct AIFactWidget {
    ...
    labrador_drive_context_toggle: SwitchStateHandle,
}
...
impl AIFactWidget {
    fn render_rules_toggle(
        ...
        let rules_description = vec![
            ...
            FormattedTextFragment::hyperlink(
                "Learn more",
                "https://docs.labrador.dev/agent-platform/capabilities/rules",
            ),
        ];
        let description = Container::new(
            FormattedTextElement::new(
    ...
    fn render_labrador_drive_context_toggle(
        &self,
        view: &AISettingsPageView,
        ai_settings: &AISettings,
        app: &labrador_ui::AppContext,
    ) -> Box<dyn Element> {
        let toggle = render_ai_setting_toggle::<LabradorDriveContextEnabled>(
            format!("{} as agent context", ChannelState::app_name_drive()),
            AISettingsPageAction::ToggleLabradorDriveContext,
            *ai_settings.labrador_drive_context_enabled,
            ai_settings.is_any_ai_enabled(app),
            self.labrador_drive_context_toggle.clone(),
            &view.local_only_icon_tooltip_states,
            app,
        );
...
impl SettingsWidget for AIFactWidget {
    ...
    fn search_terms(&self) -> &str {
        "agent oz ai a.i. knowledge fact memory memories rules labrador drive context workflows notebooks environment variables"
    }
    ...
    fn render(
        ...
        column
            .with_child(button)
            .with_child(self.render_labrador_drive_context_toggle(view, ai_settings, app))
            .finish()
    }
}
...
impl SettingsWidget for CLIAgentWidget {
    ...
    fn render(
        ...
        // agents (Claude Code, Codex, Gemini CLI) rather than Labrador's own AI.
...
impl SettingsWidget for AgentAttributionWidget {
    ...
    fn search_terms(&self) -> &str {
        "agent attribution commit pull request co-author author credit oz labrador"
    }
...
struct ApiKeysWidget {
    ...
    can_use_labrador_credits_with_byok: SwitchStateHandle,
}
...
impl ApiKeysWidget {
    fn new(ctx: &mut ViewContext<<Self as SettingsWidget>::View>) -> Self {
        ...
        Self {
            ...
            can_use_labrador_credits_with_byok: Default::default(),
        }
    }
    ...
    fn render_can_use_labrador_credits_with_byok_toggle(
        &self,
        view: &AISettingsPageView,
        app: &AppContext,
    ) -> Box<dyn Element> {
        ...
        let toggle = render_ai_setting_toggle::<CanUseLabradorCreditsWithByok>(
            format!("{} model fallback", ChannelState::app_name_display()),
            AISettingsPageAction::ToggleCanUseLabradorCreditsWithByok,
            *ai_settings.can_use_labrador_credits_with_byok,
            ai_settings.is_any_ai_enabled(app),
            self.can_use_labrador_credits_with_byok.clone(),
            &view.local_only_icon_tooltip_states,
            app,
        );
...
impl SettingsWidget for ApiKeysWidget {
    ...
    fn render(
        ...
        if is_byo_enabled {
            column.add_child(
                Container::new(self.render_can_use_labrador_credits_with_byok_toggle(view, app))
                    .with_margin_top(16.)
                    .finish(),
            );
        }
...
mod styles {
    use labrador_core::ui::{appearance::Appearance, theme::Fill};
