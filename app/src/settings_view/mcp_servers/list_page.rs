...
use labrador_core::ui::{appearance::AppearanceEvent, theme::color::internal_colors, Icon};
use labrador_ui::{
    elements::{
...
impl MCPServersListPageView {
    ...
    fn file_based_root_chip_text(root_path: &PathBuf) -> Option<String> {
        ...
        // If the path is the Labrador data directory (e.g. ~/.labrador), set the text to
        // "global". The Labrador provider stores its data directory as the root path rather than the
        // home directory, unlike other providers that store the home directory directly.
        if root_path == &crate::labrador_managed_paths_watcher::labrador_data_dir() {
            return Some("global".to_string());
        }
