...
use super::{
    ...
    labradorify_page::LabradorifyPageView,
    SettingsSection,
};
use crate::{
...
use labrador_core::{settings::SyncToCloud, ui::theme::color::internal_colors};
use labrador_ui::{
    elements::{
...
pub enum SettingsPageViewHandle {
    ...
    Labradorify(ViewHandle<LabradorifyPageView>),
...
impl SettingsPageViewHandle {
    pub fn child_view(&self) -> Box<dyn Element> {
        ...
        match self {
            ...
            Labradorify(view_handle) => ChildView::new(view_handle).finish(),
        ...
        Container::new(
            Flex::row()
                ...
                .with_child(
                    ConstrainedBox::new(
                        Icon::ChevronRight
                            .to_labrador_ui_icon(appearance.theme().main_text_color(icon_bg))
                            .finish(),
                    )
                    .with_width(16.)
                    .with_height(16.)
...
pub fn render_body_item_label_internal<T: Clone + Action>(
    ...
    if let Some(icon) = label_icon {
        label.add_child(
            Container::new(
                ConstrainedBox::new(icon.to_labrador_ui_icon(label_color).finish())
                    .with_width(16.)
                    .with_height(16.)
                    .finish(),
            )
                ...
                Some(
                    appearance
                        ...
                        .with_style(UiComponentStyles {
                            font_color: Some(
                                labrador_theme
                                    .sub_text_color(labrador_theme.surface_2())
                                    .into_solid(),
                            ),
                            margin: Some(Coords {
                                left: 8.,
    ...
    let label = if let Some(secondary_text) = secondary_text {
        ...
        let secondary_text_child = appearance
            ...
            .with_style(UiComponentStyles {
                font_color: Some(
                    color_override
                        .unwrap_or(labrador_theme.sub_text_color(labrador_theme.surface_2()))
                        .into_solid(),
                ),
                margin: Some(Coords {
                    top: 4.,
...
/// [`SettingsWidget`]s.
pub(super) enum PageType<V: labrador_ui::View> {
...
impl<V: labrador_ui::View> PageType<V> {
    /// A page where the contents cannot be separated for showing search results. If any part
    /// matches the search query, the whole page must show. The whole page is one big
    /// [`SettingsWidget`].
...
pub(super) enum FilteredPageType<'a, V: labrador_ui::View> {
    Monolith {
...
pub(super) struct Category<V: labrador_ui::View> {
    title: String,
    subtitle: Option<String>,
    widgets: Vec<Box<dyn SettingsWidget<View = V>>>,
}

impl<V: labrador_ui::View> Category<V> {
    pub(super) fn new(
...
pub(super) struct FilteredCategory<'a, V: labrador_ui::View> {
