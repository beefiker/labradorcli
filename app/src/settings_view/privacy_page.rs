...
use labrador_ui::ui_components::button::{ButtonVariant, TextAndIcon, TextAndIconAlignment};
...
static SAFE_MODE_DESCRIPTION: LazyLock<String> = LazyLock::new(|| {
    format!(
        "When this setting is enabled, Labrador will scan blocks, the contents of \
...
const TELEMETRY_DOCS_URL: &str =
    "https://docs.labrador.dev/support-and-community/privacy-and-security/privacy#what-telemetry-data-does-labrador-collect-and-why";
...
fn data_management_description() -> String {
    ...
    You will no longer be able to use Labrador.",
...
impl SecretRedactionWidget {
    ...
    fn add_button(text: impl Into<Cow<'static, str>>, appearance: &Appearance) -> TextAndIcon {
        TextAndIcon::new(
            ...
            text,
            Icon::Plus.to_labrador_ui_icon(appearance.theme().active_ui_text_color()),
            MainAxisSize::Min,
...
impl SettingsWidget for AppAnalyticsWidget {
    ...
    fn should_render(&self, app: &AppContext) -> bool {
        // Builds without a telemetry config (e.g. Labrador OSS) cannot ship
...
impl SettingsWidget for CrashReportsWidget {
    ...
    fn should_render(&self, app: &AppContext) -> bool {
        // Builds without a crash reporting config (e.g. Labrador OSS) cannot ship
            ...
            .with_child(
                ui_builder
                    .paragraph(
                        ...
                        from Labrador to external servers to ensure you feel comfortable that your \
...
fn description_text_color(theme: &LabradorTheme) -> labrador_core::ui::theme::Fill {
