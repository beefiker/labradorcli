...
fn test_tokenizer_special_chars() {
    // Test string includes terms with hyphen, underscore, forward slash, backslash, and colon.
    let test_string = "test-cli/launch_command:run C:\\\\Program_Files\\\\Test\\\\core-engine.dll check_status:/dev/test_drive-0";
    let tokens = token_stream_helper(test_string);

    assert_eq!(tokens.len(), 25);
    assert_token(&tokens[0], 0, "test-cli/launch_command:run", 0, 27);
    assert_token(&tokens[1], 1, "test", 0, 4);
    ...
    assert_token(
        &tokens[7],
        7,
        "C:\\\\Program_Files\\\\Test\\\\core-engine",
        28,
        64,
    );
    assert_token(&tokens[15], 15, "dll", 65, 68);
    assert_token(&tokens[16], 16, "check_status:/dev/test_drive-0", 69, 99);
}
...
fn test_searcher() {
    ...
    let search_strings = ["run test on web server", "run test-on-web server"];
    ...
    let result = searcher.search_full_doc("test on web").unwrap();
    assert_eq!(
        result.len(),
        2,
    ...
    assert_eq!(
        result[0].values.name, "run test-on-web server",
...
fn test_searcher_scores() {
    ...
    let search_strings = ["run test on web server", "run test_on_web:server"];
    ...
    let result = searcher.search_full_doc("test").unwrap();
    assert_eq!(
        result.len(),
        2,
        "both search strings should match with the custom tokenizer"
...
fn test_searcher_async() {
    ...
    let search_strings = [
        ...
        "Run test-on-web",
        "Run fresh test-local and clear test-dev permissions",
    ...
    assert_eq!(
        result[0].name, "Run fresh test-local and clear test-dev permissions",
