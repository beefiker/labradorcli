...
use labrador_ui::keymap::{DescriptionContext, Keystroke};
...
impl SearchItemIcon for BindingGroup {
    ...
    fn icon_color(&self, appearance: &Appearance) -> ColorU {
        match self {
            ...
            Self::LabradorAi if !FeatureFlag::AgentMode.is_enabled() => {
                ColorU::from_u32(colors::LABRADOR_AI)
            }
            Self::LabradorAi => appearance.theme().foreground().into_solid(),
