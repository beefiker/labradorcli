use crate::{
    ai_assistant::{
        ai_assistant_feature_name, execution_context::LabradorAiExecutionContext,
...
#[derive(Clone, Debug)]
pub enum LabradorAISearchItem {
    /// Translates the query within command search.
    Translate,

    /// Opens Labrador AI with the query.
    Open,
}

impl LabradorAISearchItem {
    fn item_body_text(&self) -> String {
        match self {
            LabradorAISearchItem::Translate => {
                format!(
                    "Translate into shell command using {}",
                    ai_assistant_feature_name()
                )
            }
            LabradorAISearchItem::Open => {
                format!(
...
impl SearchItem for LabradorAISearchItem {
    ...
    fn render_icon(
        ...
        // Since the Labrador AI logo color is hardcoded, let's find the best
        ...
        let icon = if FeatureFlag::AgentMode.is_enabled() {
            ...
            UIIcon::AiAssistant.to_labrador_ui_icon(color.into()).finish()
        };
    ...
    fn accept_result(&self) -> CommandSearchItemAction {
        match self {
            LabradorAISearchItem::Translate => CommandSearchItemAction::TranslateUsingLabradorAI,
            LabradorAISearchItem::Open => CommandSearchItemAction::OpenLabradorAI,
        }
    }
...
/// The Labrador AI data source provides a synchronous item that opens/translates
/// using Labrador AI when selected.
pub struct LabradorAIDataSource;

impl LabradorAIDataSource {
    pub fn new(
...
impl SyncDataSource for LabradorAIDataSource {
    type Action = CommandSearchItemAction;
    ...
    fn run_query(
        ...
        if query.filters.is_empty() {
            Ok(vec![LabradorAISearchItem::Translate.into()])
        } else {
            // Since the query matched, the `#` filter must be applied in this case.
            Ok(vec![LabradorAISearchItem::Open.into()])
        }
    }
}
...
impl AsyncDataSource for LabradorAIDataSource {
    type Action = CommandSearchItemAction;
