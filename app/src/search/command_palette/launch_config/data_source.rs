...
impl DataSource {
    ...
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
        if labrador_core::features::FeatureFlag::UseTantivySearch.is_enabled() {
    ...
    fn new_fuzzy(ctx: &mut ModelContext<Self>) -> Self {
        ctx.subscribe_to_model(&LabradorConfig::handle(ctx), Self::handle_config_event);
    ...
    fn handle_config_event(&mut self, event: &LabradorConfigUpdateEvent, ctx: &mut ModelContext<Self>) {
        if matches!(event, LabradorConfigUpdateEvent::LaunchConfigs) {
...
impl LaunchConfigSearcher for FuzzyLaunchConfigSearcher {
    ...
    fn refresh_search_index(&mut self, app: &AppContext) {
        self.configs = LabradorConfig::as_ref(app)
            .launch_configs()
            .iter()
