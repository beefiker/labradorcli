...
use labrador_ui::elements::EventHandler;
...
use labrador_ui::keymap::BindingId;
...
impl labrador_ui::View for View {
...
impl View {
    ...
    pub fn is_mode_enabled(&self, mode: PaletteMode, app: &AppContext) -> bool {
        ...
        matches!(
            ...
            (PaletteMode::Command, QueryFilter::Actions)
                ...
                | (PaletteMode::LabradorDrive, QueryFilter::Drive)
        )
    }
    ...
    fn render_palette_list(&self, theme: &LabradorTheme, app: &AppContext) -> Box<dyn Element> {
    ...
    /// Dispatches `action` to the correct window and [`labrador_ui::View`] by using the current state of
    /// the [`BindingSource`] model.
    fn dispatch_typed_action_on_view(
        &self,
        action: &dyn labrador_ui::Action,
        ctx: &mut ViewContext<Self>,
    ) {
