...
use labrador_completer::signatures::CommandRegistry;
use labrador_core::features::FeatureFlag;
use labrador_util::path::{EscapeChar, ShellFamily};
...
impl SessionContext {
    async fn list_directory_entries_internal(
        ...
        match self.session.session_type() {
            ...
            SessionType::LabradorifiedRemote { .. } => {
                let env_vars = self
                    .session
                    .path()
                    .as_deref()
...
impl GeneratorContext for SessionContext {
    async fn execute_command_at_pwd(
        ...
        // because if the user opened Labrador through a parent process that didn't have the PATH var set
...
impl CompletionContext for SessionContext {
    ...
    #[cfg(feature = "completions_v2")]
    fn js_context(&self) -> Option<&dyn labrador_completer::completer::JsExecutionContext> {
        self.js_ctx
            .as_ref()
            .map(|ctx| -> &dyn labrador_completer::completer::JsExecutionContext { ctx })
    }
...
fn ls_script_for_dir(directory: &TypedPath) -> Option<String> {
    ...
    let escaped_dir = labrador_util::path::ShellFamily::Posix.shell_escape(dir_str);
