//! One-time macOS config directory migrations.
//!
...
use labrador_core::paths::{data_dir, LABRADOR_CONFIG_DIR};
...
/// Migrates Preview's config directory from the shared `.labrador` location to
/// `.labrador-preview` by creating symlinks from each top-level entry in
/// `.labrador` into the new directory.
///
...
/// - `~/.labrador-preview` already exists.
/// - `~/.labrador` does not exist.
pub(crate) fn migrate_preview_config_dir_if_needed() {
    ...
    let old_dir = home.join(LABRADOR_CONFIG_DIR);
...
pub(crate) fn migrate_config_dir_via_symlinks(old_dir: &Path, new_dir: &Path) {
    migrate_config_dir_via_symlinks_with_exclusions(old_dir, new_dir, MIGRATION_EXCLUDED_FILES);
}

fn migrate_config_dir_via_symlinks_with_exclusions(
    old_dir: &Path,
    new_dir: &Path,
    excluded_files: &[&str],
) {
    ...
    // migration itself or by ensure_labrador_watch_roots_exist on a subsequent
