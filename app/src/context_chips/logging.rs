...
impl PromptChipLogger {
    ...
    fn init_runtime() -> Self {
        if !labrador_core::channel::ChannelState::enable_debug_features() {
...
pub(crate) fn log_file_path() -> anyhow::Result<PathBuf> {
    let log_directory = labrador_logging::log_directory()?;
    let channel_logfile_name = labrador_core::channel::ChannelState::logfile_name();
