...
use crate::settings::LabradorPromptSeparator;
...
fn test_prompt_settings() {
    App::test((), |mut app| async move {
        ...
        prompt.update(&mut app, |prompt, ctx| {
            prompt
                .update(new_chips.clone(), false, LabradorPromptSeparator::None, ctx)
        ...
        session_settings.read(&app, |settings, _| {
            assert_eq!(
                ...
                PromptConfiguration::from_chips(new_chips, false, LabradorPromptSeparator::None).into()
            );
        });
