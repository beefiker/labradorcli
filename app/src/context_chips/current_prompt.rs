...
use crate::settings::{InputSettings, LabradorPromptSeparator};
...
use labrador_completer::completer::{CommandExitStatus, CommandOutput};
...
#[cfg(feature = "local_fs")]
use labrador_ui::WeakModelHandle;
use labrador_ui::{
    r#async::{SpawnedFutureHandle, Timer},
    AppContext, ViewHandle,
};
use labrador_ui::{Entity, ModelAsRef, ModelContext, ModelHandle, SingletonEntity};
...
pub struct CurrentPrompt {
    ...
    /// The separator to use as a trailing character at the end of Labrador prompt, if any.
    separator: LabradorPromptSeparator,
...
impl CurrentPrompt {
    ...
    pub fn subscribe_to_input_editor(
        ...
        ctx.subscribe_to_view(&editor, move |me, _, ctx| {
            // CurrentPrompt exists and this fn is called even if we're not using Labrador prompt.
    ...
    /// Whether same line prompt is enabled for the Labrador prompt.
    pub fn same_line_prompt_enabled(&self) -> bool {
        self.same_line_prompt_enabled
    }

    /// The separator for the current Labrador prompt.
    pub fn separator(&self) -> LabradorPromptSeparator {
        self.separator
    }
    ...
    ) -> (Option<labrador_completer::completer::CommandOutput>, bool) {
