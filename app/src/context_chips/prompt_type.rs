...
use crate::{
    ...
    settings::LabradorPromptSeparator,
    terminal::{
...
/// The type of Labrador prompt being used
#[derive(Clone)]
pub enum PromptType {
    /// A Labrador prompt that refreshes chip values on its own. Typical for local sessions.
    Dynamic { prompt: ModelHandle<CurrentPrompt> },
    /// A Labrador prompt that does not change unless explicitly overwritten. Used for viewers of shared sessions.
...
impl PromptType {
    ...
    /// The separator for the Labrador prompt.
    pub fn separator(&self, ctx: &AppContext) -> LabradorPromptSeparator {
        match self {
