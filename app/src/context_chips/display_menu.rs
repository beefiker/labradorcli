...
use labrador_core::ui::{appearance::Appearance, builder::MIN_FONT_SIZE, theme::Fill};
...
use labrador_ui::{
    elements::{
        ...
        ScrollStateHandle, Scrollable, ScrollableElement, ScrollbarWidth, Stack, Text, UniformList,
        UniformListState,
    },
    keymap::FixedBinding,
            ...
            Hoverable::new(footer_option.mouse_state.clone(), move |mouse_state| {
                ...
                if let Some(icon) = footer_option.action_item.icon(app) {
                    updated_text.add_child(
                        Container::new(
                            ConstrainedBox::new(
                                icon.to_labrador_ui_icon(Fill::Solid(text_color)).finish(),
                            )
                    ...
                    .map(|index| {
                        ...
                        if let Some(icon) = item.icon(app) {
                            left_side.add_child(
                                Container::new(
                                    ConstrainedBox::new(
                                        icon.to_labrador_ui_icon(Fill::Solid(main_text)).finish(),
                                    )
                                    .with_height(icon_size)
                                    .with_width(icon_size)
                                    .finish(),
