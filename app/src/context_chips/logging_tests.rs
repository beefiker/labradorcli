...
fn test_prompt_chip_log_filename_uses_channel_logfile_stem() {
    assert_eq!(
        prompt_chip_log_filename("labrador_dev.log"),
        "labrador_dev.prompt_chips.log"
    );
    assert_eq!(
        prompt_chip_log_filename("labrador_local"),
        "labrador_local.prompt_chips.log"
    );
}
...
fn test_format_log_entry_preserves_stdout_and_stderr_sections() {
    let output = CommandOutput {
        stdout: b"https://github.com/beefiker/labrador-internal/pull/123\n".to_vec(),
        stderr: b"warning output\n".to_vec(),
        status: CommandExitStatus::Success,
        exit_code: Some(labrador_core::command::ExitCode::from(0)),
    };
    ...
    assert!(entry.contains(
        "stdout:\n<<<STDOUT\nhttps://github.com/beefiker/labrador-internal/pull/123\n>>>STDOUT"
    ));
    assert!(entry.contains("stderr:\n<<<STDERR\nwarning output\n>>>STDERR"));
}
