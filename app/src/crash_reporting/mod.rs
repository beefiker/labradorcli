...
use labrador_core::channel::Channel;
use labrador_ui::{r#async::block_on, AppContext, SingletonEntity};
...
use std::sync::Arc;
use labrador_ui::rendering::GPUDeviceInfo;
use labrador_ui::windowing::state::ApplicationStage;
use labrador_ui::windowing::{self, StateEvent, WindowManager};
...
lazy_static! {
    ...
    /// `labrador.application_stage` tag to Sentry.
...
impl ToSentryTags for CrashRecoveryMetadata {
    fn to_sentry_tags(&self) -> impl IntoIterator<Item = (&str, String)> {
        #[cfg(enable_crash_recovery)]
        {
            ...
            [
                (
                    "labrador.crash_recovery_process.running",
                    bool_to_sentry_value(self.is_crash_recovery_process_running),
                ),
                (
                    "labrador.handled_by_crash_recovery_process",
...
fn set_optional_user_information(
    ...
    // Only send along emails if we're on LabradorDev.
...
/// Sets the labrador.client_type Sentry tag.
pub fn set_client_type_tag(client_id: &str) {
    set_tag("labrador.client_type", client_id);
}

/// Initializes the labrador.virtual_env Sentry tag group.
...
impl ToSentryTags for ApplicationStage {
    fn to_sentry_tags(&self) -> impl IntoIterator<Item = (&str, String)> {
        [("labrador.application_stage", self.to_string())]
    }
}
...
impl ToSentryTags for GPUDeviceInfo {
    fn to_sentry_tags(&self) -> impl IntoIterator<Item = (&str, String)> {
        [
            ("labrador.gpu.device.name", self.device_name.to_string()),
            ("labrador.gpu.device.type", self.device_type.to_string()),
            ("labrador.gpu.backend", self.backend.to_string()),
            ("labrador.gpu.driver.name", self.driver_name.to_string()),
            ("labrador.gpu.driver.info", self.driver_info.to_string()),
        ]
    }
}
