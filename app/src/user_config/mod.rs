...
use crate::themes::theme::LabradorThemeConfig;
...
# https://docs.labrador.dev/terminal/sessions/launch-configurations
#
...
#  - tabs:
#      - layout:
#          cwd: /Users/labrador-user/project
#          commands:
#            - exec: code .
",
        labrador_core::paths::home_relative_path(&crate::user_config::launch_configs_dir())
    );
}

#[derive(Clone)]
pub enum LabradorConfigUpdateEvent {
    Themes,
    LaunchConfigs,
...
#[derive(Default)]
pub struct LabradorConfig {
    launch_configs: Vec<LaunchConfig>,
    tab_configs: Vec<TabConfig>,
    #[cfg_attr(target_family = "wasm", allow(dead_code))]
    tab_config_errors: Vec<TabConfigError>,
    theme_config: LabradorThemeConfig,
}

/// Platform-independent parts of LabradorConfig.
///
...
impl LabradorConfig {
    ...
    pub fn mock(_ctx: &mut ModelContext<Self>) -> Self {
        Self {
            theme_config: LabradorThemeConfig::new(),
            ..Default::default()
        }
    }
    ...
    pub fn theme_config(&self) -> &LabradorThemeConfig {
        &self.theme_config
    }

    /// Saving the newly created launch configuration to the LabradorConfig that we currently
    /// have.
    pub fn append_launch_config(
        ...
        if !self.launch_configs.contains(launch_config) {
            ...
            ctx.emit(LabradorConfigUpdateEvent::LaunchConfigs);
        }
    }
    ...
    pub fn update_theme_config(
        ...
        ctx.emit(LabradorConfigUpdateEvent::Themes);
    }
    ...
    pub fn remove_tab_config_by_path(&mut self, path: &Path, ctx: &mut ModelContext<Self>) {
        ...
        if self.tab_configs.len() != before {
            ctx.emit(LabradorConfigUpdateEvent::TabConfigs);
        }
    }
}
...
pub fn themes_dir() -> PathBuf {
    labrador_core::paths::themes_dir()
}
...
/// These are shipped with Labrador and user-editable (Labrador does not overwrite modifications).
#[cfg_attr(target_family = "wasm", expect(dead_code))]
pub fn default_tab_configs_dir() -> PathBuf {
    base_dir().join("default_tab_configs")
}

/// Returns whether the path points to a tab config TOML file under one of Labrador's
...
/// Ensures `~/.labrador/default_tab_configs/worktree.toml` exists, creating it
...
impl Entity for LabradorConfig {
    type Event = LabradorConfigUpdateEvent;
}

impl SingletonEntity for LabradorConfig {}
