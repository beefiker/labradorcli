...
use labrador_ui::{ModelContext, SingletonEntity};
...
use crate::labrador_managed_paths_watcher::{
    repository_update_touches_path, repository_update_touches_prefix, LabradorManagedPathsWatcher,
    LabradorManagedPathsWatcherEvent,
};
...
use super::{
    launch_configs_dir, tab_configs_dir, themes_dir, LabradorConfigUpdateEvent, LAUNCH_CONFIG_COMMENT,
};

impl super::LabradorConfig {
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
        ...
        ctx.subscribe_to_model(
            ...
            Self::handle_labrador_managed_paths_event,
        );
    ...
    fn handle_labrador_managed_paths_event(
        &mut self,
        event: &LabradorManagedPathsWatcherEvent,
        ctx: &mut ModelContext<Self>,
    ) {
        let LabradorManagedPathsWatcherEvent::FilesChanged(update) = event;
        ...
        if FeatureFlag::TabConfigs.is_enabled() && update_touches_dir(update, &tab_configs_dir()) {
            ...
            let _ = ctx.spawn(
                ...
                |me, (configs, errors), ctx| {
                    ...
                    ctx.emit(LabradorConfigUpdateEvent::TabConfigs);
                    if !errors.is_empty() {
                        ctx.emit(LabradorConfigUpdateEvent::TabConfigErrors(errors));
                    }
                },
            );
        }
        ...
        {
            ctx.emit(LabradorConfigUpdateEvent::Settings);
        }
    }
...
pub fn load_theme_configs(theme_path: &Path) -> LabradorThemeConfig {
    let mut theme_configs = LabradorThemeConfig::new();
