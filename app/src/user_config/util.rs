...
use crate::themes::theme::{ThemeKind, LabradorTheme, LabradorThemeConfig};
...
pub(super) fn parse_single_theme_dir_entry(item: &DirEntry) -> Option<(ThemeKind, LabradorTheme)> {
    parse_single_item_file(item, |file_name, mut theme: LabradorTheme| {
        ...
        let theme_kind = if let Some(name) = theme.name() {
            LabradorThemeConfig::file_to_theme(name, item.path().into())
        } else {
