//! File type detection utilities for determining if files can be opened in Labrador.

#[cfg(feature = "local_fs")]
use crate::util::file::external_editor::{settings::EditorChoice, Editor, EditorSettings};
use serde::{Deserialize, Serialize};
use std::path::Path;
pub use labrador_util::file_type::{is_binary_file, is_file_content_binary, is_markdown_file};
...
/// The type of file that can be opened in Labrador. The in-product treatment for "opening" a file
/// depends on its type.
...
pub enum FileTarget {
    /// Open in Labrador's Markdown viewer.
    MarkdownViewer(EditorLayout),
    /// Open in Labrador's Code Editor.
...
/// Determines if a file can be opened in Labrador and returns its type.
/// Returns `None` if the file is binary and should not be opened.
pub fn is_file_openable_in_labrador(path: &Path) -> Option<OpenableFileType> {
...
/// Only use this for UI elements that must explicitly open a file in Labrador (i.e. "Open in New Tab").
/// Prefer `resolve_file_target` for all other cases to respect users' preferences.
/// This would also force any binary file to be opened in Labrador's Code Editor, so you should likely check
/// `is_file_openable_in_labrador` before rendering any such UI Elements.
#[cfg(feature = "local_fs")]
pub fn resolve_file_target_to_open_in_labrador(
    path: &Path,
    settings: &EditorSettings,
    layout: Option<EditorLayout>,
) -> FileTarget {
    let openable_file_type = is_file_openable_in_labrador(path);
...
pub fn resolve_file_target_with_editor_choice(
    ...
    let is_openable_in_labrador = is_file_openable_in_labrador(path);
    let is_markdown = matches!(is_openable_in_labrador, Some(OpenableFileType::Markdown));
    let layout = layout.unwrap_or(default_layout);
    let is_openable_in_labrador = is_openable_in_labrador.is_some();
    ...
    // 2. Labrador Code Editor (explicit user preference)
    if is_openable_in_labrador && matches!(editor_choice, EditorChoice::Labrador) {
    ...
    match editor_choice {
        ...
        EditorChoice::Labrador | EditorChoice::EnvEditor => unreachable!("Already matched above"),
    }
}
...
mod tests {
    ...
    fn test_binary_files_not_openable() {
        assert!(is_file_openable_in_labrador(Path::new("image.png")).is_none());
        assert!(is_file_openable_in_labrador(Path::new("video.mp4")).is_none());
        assert!(is_file_openable_in_labrador(Path::new("binary.exe")).is_none());
        assert!(is_file_openable_in_labrador(Path::new("archive.zip")).is_none());
    }

    #[test]
    #[cfg(feature = "local_fs")]
    fn test_open_code_panels_file_editor_default_is_labrador() {
        use crate::util::file::external_editor::settings::OpenCodePanelsFileEditor;

        assert_eq!(
            OpenCodePanelsFileEditor::default_value(),
            EditorChoice::Labrador
        );
    }
    ...
    #[test]
    #[cfg(feature = "local_fs")]
    fn test_resolve_file_target_labrador_uses_default_layout() {
        let target = resolve_file_target_with_editor_choice(
            Path::new("data.txt"),
            EditorChoice::Labrador,
            true, /* prefer_markdown_viewer */
    ...
    fn test_markdown_files() {
        assert_eq!(
            is_file_openable_in_labrador(Path::new("README.md")),
            Some(OpenableFileType::Markdown)
        );
        assert_eq!(
            is_file_openable_in_labrador(Path::new("doc.markdown")),
            Some(OpenableFileType::Markdown)
        );
        assert_eq!(
            is_file_openable_in_labrador(Path::new("README")),
            Some(OpenableFileType::Markdown)
        );
        assert_eq!(
            is_file_openable_in_labrador(Path::new("CHANGELOG")),
            Some(OpenableFileType::Markdown)
        );
    }
    ...
    fn test_code_files() {
        assert_eq!(
            is_file_openable_in_labrador(Path::new("main.rs")),
            Some(OpenableFileType::Code)
        );
        assert_eq!(
            is_file_openable_in_labrador(Path::new("app.js")),
            Some(OpenableFileType::Code)
        );
        assert_eq!(
            is_file_openable_in_labrador(Path::new("script.py")),
            Some(OpenableFileType::Code)
        );
        assert_eq!(
            is_file_openable_in_labrador(Path::new("config.json")),
            Some(OpenableFileType::Code)
        );
    }
    ...
    fn test_text_files() {
        ...
        assert_eq!(
            is_file_openable_in_labrador(Path::new("data.txt")),
            Some(OpenableFileType::Text)
        );
        assert_eq!(
            is_file_openable_in_labrador(Path::new("data.csv")),
            Some(OpenableFileType::Text)
        );
        assert_eq!(
            is_file_openable_in_labrador(Path::new("file.svg")),
            Some(OpenableFileType::Text)
        );
    }
