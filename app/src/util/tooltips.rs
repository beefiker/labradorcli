...
///
/// This checks:
/// - Whether Labrador is already the default editor (skip if so)
/// - Whether this file is openable in Labrador (skips binary files and directories)
/// - Whether Labrador is an OS-level default editor (skips Markdown files)
#[cfg(feature = "local_fs")]
pub fn should_show_open_in_labrador_link(path: &Path, app: &AppContext) -> bool {
    use crate::{
    ...
    if matches!(*settings.open_file_editor, EditorChoice::Labrador) {
        return false;
    }
...
pub fn should_show_open_in_labrador_link(_path: &std::path::Path, _app: &AppContext) -> bool {
    false
}
