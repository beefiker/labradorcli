...
use labrador_ui::keymap::{BindingId, IsBindingValid};
...
pub enum CustomAction {
    ...
    #[cfg(windows)]
    WindowsCopy,
    /// Also applies to legacy Labrador AI (toggles the panel)
    NewAgentModePane,
    /// Also applies to legacy Labrador AI (attaches the selection to the panel editor)
...
lazy_static! {
    ...
    /// know about particular Labrador specific actions, so it deals with all actions
    /// as plain isizes.  Within Labrador though we want to deal with them as the enum type.
    ...
    pub static ref MAC_PTY_NON_COMPLIANT_ACTIONS: HashSet<&'static str> = HashSet::from_iter(["terminal:labradorify_subshell", "terminal:open_block_list_context_menu_via_keybinding"]);
...
pub fn custom_tag_to_keystroke(custom: CustomTag) -> Option<Keystroke> {
    match custom.into() {
        ...
        CustomAction::ToggleLabradorDrive => {
            if OperatingSystem::get().is_mac() {
                Keystroke::parse("cmd-\\").ok()
            } else {
                Keystroke::parse("ctrl-shift-|").ok()
...
/// [`labrador_ui::keymap::FixedBinding`] or [`EditableBinding`].
