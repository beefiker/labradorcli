...
//! depends on the platform. Labrador must use this information to avoid rendering UI elements
//! underneath them.
...
mod linux_only {
    ...
    pub(super) use labrador_ui::elements::{
...
mod windows_only {
    ...
    pub(super) use pathfinder_color::ColorU;
    pub(super) use pathfinder_geometry::vector::vec2f;
    pub(super) use labrador_core::ui::theme;
    pub(super) use labrador_ui::elements::{
...
use labrador_ui::{AppContext, Element, WindowId};
...
/// Data Labrador needs to avoid rendering anything below the traffic lights.
#[derive(Clone, Debug)]
...
impl TrafficLightData {
    ...
    /// by labrador_ui. However, native traffic light buttons (e.g. on macOS) don't scale with zoom, so
    ...
    fn render_windows_close_button_icon(icon_color: ColorU) -> Box<dyn Element> {
        ConstrainedBox::new(
            IconComponent::X
                .to_labrador_ui_icon(theme::Fill::Solid(icon_color))
                .finish(),
        )
        .with_height(16.)
        .with_width(16.)
