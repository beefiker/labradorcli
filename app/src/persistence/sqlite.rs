...
use labrador_ui::platform::FullscreenState;
...
use super::model::{
    self, NewActiveMCPServer, NewApp, NewCommand, NewTab, NewTeam, NewWindow, NewWorkspace,
    NewWorkspaceMetadata, NewWorkspaceTeam, Project, Tab, Window,
    WorkspaceMetadata as WorkspaceMetadataModel, AI_FACT_PANE_KIND, CODE_PANE_KIND,
    EXECUTION_PROFILE_EDITOR_PANE_KIND, MCP_SERVER_PANE_KIND, SETTINGS_PANE_KIND,
    TERMINAL_PANE_KIND, WELCOME_PANE_KIND,
};
use super::schema;
use super::{
...
use crate::persistence::model::{
    NewTeamSettings, ProjectRules, CODE_REVIEW_PANE_KIND, GET_STARTED_PANE_KIND,
};
...
const LABRADOR_SQLITE_FILE_NAME: &str = "labrador.sqlite";
...
unsafe fn init_logging() {
    ...
    extern "C-unwind" fn log_callback(_data: *mut c_void, err_code: c_int, msg: *const c_char) {
        ...
        let level = match (primary_error_code, err_code) {
            ...
            // This is mostly outside of Labrador's control (e.g. the user or some system program is
...
pub(super) fn init_db() -> Result<SqliteConnection> {
    ...
    if !db_path.exists() {
        for old_db_path in database_migration_file_paths(&db_path) {
            if migrate_sqlite_database(&old_db_path, &db_path) {
                break;
            }
        }
    }
...
fn database_migration_file_paths(db_path: &Path) -> Vec<PathBuf> {
    let mut paths = Vec::new();
    paths.push(labrador_core::paths::state_dir().join(LABRADOR_SQLITE_FILE_NAME));
    paths.retain(|path| path != db_path);
    paths.dedup();
    paths
}

fn migrate_sqlite_database(old_db_path: &Path, db_path: &Path) -> bool {
    ...
    match std::fs::rename(old_db_path, db_path) {
        Ok(_) => {
            safe_info!(
                safe: ("Migrated SQLite database into Labrador location"),
                full: ("Migrated SQLite database from `{}` to `{}`", old_db_path.display(), db_path.display())
            );

            migrate_sqlite_sidecar(
                old_db_path.with_extension("sqlite-wal"),
                db_path.with_extension("sqlite-wal"),
                "WAL",
            );
            migrate_sqlite_sidecar(
                old_db_path.with_extension("sqlite-shm"),
        ...
        Err(err) => {
            report_error!(anyhow::Error::new(err)
                .context("Failed to migrate SQLite database into Labrador location"));
            false
        }
    }
}

fn migrate_sqlite_sidecar(old_path: PathBuf, new_path: PathBuf, label: &str) {
    if let Err(err) = std::fs::rename(&old_path, &new_path) {
        if err.kind() != std::io::ErrorKind::NotFound {
            report_error!(anyhow::Error::new(err).context(format!(
                "Failed to migrate SQLite {label} into Labrador location"
            )));
        }
    } else {
        log::info!("Migrated SQLite {label} into Labrador location");
    }
}
...
pub fn database_file_path() -> PathBuf {
    labrador_core::paths::secure_state_dir()
        .unwrap_or_else(labrador_core::paths::state_dir)
        .join(LABRADOR_SQLITE_FILE_NAME)
}
...
fn handle_model_event(event: ModelEvent, connection: &mut SqliteConnection) -> anyhow::Result<()> {
    match event {
        ...
        ModelEvent::UpsertCurrentUserInformation {
            user_information: _,
        } => Ok(()),
    ...
    conn.transaction::<(), Error, _>(|conn| {
        ...
        for (idx, window) in app_state.windows.iter().enumerate() {
            ...
            let new_window = NewWindow {
                ...
                labrador_ai_width: window.labrador_ai_width,
                voltron_width: window.voltron_width,
                labrador_drive_index_width: window.labrador_drive_index_width,
...
///
/// One notable exception is the case where there may be two Labrador apps running
...
fn insert_command(
    ...
    conn.transaction::<(), Error, _>(|conn| {
        ...
        let new_command = NewCommand {
            ...
            session_id: command_metadata.session_id.map(|s| u64::from(s) as i64),
