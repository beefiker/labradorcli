...
that are bundled in our app. When the Labrador app starts, we upgrade the schema to the latest version in a transaction.
...
Make sure you have run `/script/bootstrap` at least once before. That runs
`script/install_cargo_build_deps`, which installs `diesel_cli` via
`cargo binstall`. Use that repository-managed setup instead of installing Diesel
CLI manually, so local migration tooling stays consistent across machines.
...
```
cd <labrador repo>
diesel migration run --database-url="/Users/$USER/Library/Application Support/dev.labrador.LabradorLocal/labrador.sqlite"
```
This will run the migration on the same Labrador that runs when you run the app locally. This automatically generates or updates the `app/src/persistence/schema.rs`. We do not make manual edits to `schema.rs`.

You can also print the schema from a database that already has the migration with:
```
diesel print-schema --database-url="/Users/$USER/Library/Application Support/dev.labrador.LabradorLocal/labrador.sqlite"
```
...
```
diesel migration revert --database-url="/Users/$USER/Library/Application Support/dev.labrador.LabradorLocal/labrador.sqlite"
diesel migration redo --database-url="/Users/$USER/Library/Application Support/dev.labrador.LabradorLocal/labrador.sqlite"
```
