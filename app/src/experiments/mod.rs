//! A framework for running A/B tests within Labrador.
//!
//! Before starting, read the internal usage guide. It explains important
//! constraints that are required for proper use of the framework and are not
...
pub trait Experiment<T: Experiment<T>>: FromStr {
        ...
        <T as FromStr>::Err: fmt::Debug,
    {
        ...
        if let Some(group) = assigned_group.as_ref() {
            ...
            {
                let tag_name = format!("labrador.experiments.{}", Self::name());
