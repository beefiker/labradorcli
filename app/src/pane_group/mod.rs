...
use labrador_terminal::shell::{ShellName, ShellType};
...
use labrador_ui::{SingletonEntity, ViewContext};
...
///    MSYS2/WSL on Windows) that a raw `PATH` lookup would miss when Labrador is
...
const LABRADOR_SHELL_COMPATIBILITY_DOCS: &str =
    "https://docs.labrador.dev/getting-started/supported-shells";
...
pub enum Event {
    ...
    /// tell the workspace to open a file within Labrador.
    OpenFileInLabrador {
...
impl PaneGroup {
    ...
    /// is used for intra-session prompt switching between Labrador prompt and PS1.
                        ...
                        .update(app, |terminal_manager, ctx| {
                                ...
                                .downcast_ref::<local_tty::TerminalManager>()
                            {
                                if honor_ps1 {
                                    ...
                                    manager.send_switch_to_labrador_prompt_bindkey(ctx);
                                }
                            }
                        });
                }
            ...
            Banner::<PaneGroupAction>::new_permanently_dismissible(
                BannerTextContent::formatted_text(vec![
                    ...
                    FormattedTextFragment::hyperlink(
                        "Learn more",
                        LABRADOR_SHELL_COMPATIBILITY_DOCS,
                    ),
                ]),
            )
        });
    ...
    fn load_data_into_transcript_viewer(
        ...
        // Insert the conversation ended tombstone (includes Open in Labrador button on WASM).
