...
use url::Url;
use labrador_multi_agent_api as multi_agent_api;
...
fn handle_terminal_view_event(
    ...
    if group.pane_contents.contains_key(&pane_id) {
        match event {
            ...
            Event::OpenFileInLabrador { path, session } => {
                ctx.emit(pane_group::Event::OpenFileInLabrador {
                    path: path.clone(),
                    session: session.clone(),
                });
            }
            #[cfg(feature = "local_fs")]
            Event::PreviewCodeInLabrador { source } => {
                ctx.emit(pane_group::Event::PreviewCodeInLabrador {
                    source: source.clone(),
                });
            }
            #[cfg(feature = "local_fs")]
            Event::OpenCodeInLabrador { source, layout } => {
                ctx.emit(pane_group::Event::OpenCodeInLabrador {
                    source: source.clone(),
                    layout: *layout,
            ...
            Event::OpenLabradorDriveObjectInPane(_uid) => {
                // Labrador Drive object panes have been removed from this fork.
            }
