...
use crate::{
    server::telemetry::OpenedLabradorAISource, terminal::model::terminal_model::BlockIndex,
};
use lazy_static::lazy_static;
use pathfinder_color::ColorU;
use serde::{Deserialize, Serialize};
use labrador_core::{channel::ChannelState, command::ExitCode};
...
impl From<&AskAIType> for OpenedLabradorAISource {
    fn from(value: &AskAIType) -> Self {
        match value {
            AskAIType::FromAICommandSearch { .. } => OpenedLabradorAISource::FromAICommandSearch,
            AskAIType::FromBlock { .. } | AskAIType::FromBlocks { .. } => {
                OpenedLabradorAISource::HelpWithBlock
            }
            AskAIType::FromTextSelection { .. } => OpenedLabradorAISource::HelpWithTextSelection,
        }
    }
}
