...
use labrador_ui::keymap::{EditableBinding, FixedBinding};
use labrador_ui::platform::Cursor;
use labrador_ui::presenter::ChildView;
...
use labrador_ui::{elements::Element, AppContext, Entity, TypedActionView, View, ViewContext};
use labrador_ui::{FocusContext, ModelHandle, SingletonEntity, ViewHandle};
...
use crate::terminal::resizable_data::{ModalType, ResizableData, DEFAULT_LABRADOR_AI_WIDTH};
...
use labrador_ui::elements::ParentElement;
use labrador_ui::elements::Resizable;
use labrador_ui::elements::ResizableStateHandle;

use super::execution_context::LabradorAiExecutionContext;
...
use super::{
    ai_assistant_feature_name, ask_ai_assistant_text, AskAIType, AI_ASSISTANT_LOGO_COLOR,
    AI_ASSISTANT_SVG_PATH, PROMPT_CHARACTER_LIMIT,
};
...
fn zero_state_help_text() -> String {
    format!(
        "Shift + ctrl + space a block or text selection to ask {}.",
...
pub fn init(app: &mut AppContext) {
    ...
    app.register_fixed_bindings([FixedBinding::custom(
        ...
        &format!("Close {}", ai_assistant_feature_name()),
        id!("AIAssistantPanel"),
    )]);
    ...
    app.register_editable_bindings([
        EditableBinding::new(
            ...
            format!("Focus Terminal Input From {}", ai_assistant_feature_name()),
        ...
        EditableBinding::new(
            ...
            format!("Restart {}", ai_assistant_feature_name()),
...
impl AIAssistantPanelView {
    pub fn new(
        ...
        let resizable_state_handle = match resizable_data_handle
            .as_ref(ctx)
            .get_handle(ctx.window_id(), ModalType::LabradorAIWidth)
        {
            Some(handle) => handle,
            None => {
                log::error!("Couldn't retrieve labrador ai resizable state handle.");
                resizable_state_handle(DEFAULT_LABRADOR_AI_WIDTH)
            }
        };
    ...
    fn on_active_session_change(
        ...
        let ai_execution_context = active_session_handle
            ...
            .session(window_id)
            .as_ref()
            .map(LabradorAiExecutionContext::new);
        ...
        let mut header = Flex::row()
            ...
            .with_child(
                Container::new(
                    appearance
                        .ui_builder()
                        .wrappable_text(ai_assistant_feature_name(), false)
                ...
                Flex::row()
                    ...
                    .with_child(
                        Shrinkable::new(
                            ...
                            appearance
                                .ui_builder()
                                .wrappable_text(zero_state_help_text(), true)
                                .with_style(UiComponentStyles {
                                    font_family_id: Some(appearance.ui_font_family()),
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        stack.add_positioned_overlay_child(
            ...
            OffsetPositioning::offset_from_parent(
                ...
                labrador_ui::elements::ParentOffsetBounds::Unbounded,
