...
use labrador_core::{channel::ChannelState, ui::theme::Fill};
...
use crate::settings::{FontSettings, LabradorPromptSeparator};
...
struct MouseStateHandles {
    ...
    restore_default_labrador_prompt_handle: MouseStateHandle,
    labrador_prompt_mouse_state_handle: MouseStateHandle,
...
pub struct EditorModal {
    ...
    /// Dropdown to select the separator for the Labrador prompt, in the case of
    /// same line prompt. This separator is added at the end of the Labrador prompt.
    labrador_prompt_separator_dropdown: ViewHandle<Dropdown<EditorModalAction>>,
    /// The separator currently selected for the Labrador prompt.
    labrador_prompt_separator: LabradorPromptSeparator,
...
impl PromptType {
    fn labrador_prompt_from_settings(app: &AppContext) -> PromptType {
        let session_settings = SessionSettings::as_ref(app);
        if matches!(*session_settings.saved_prompt, PromptSelection::Default) {
            PromptType::LabradorDefault
        } else {
            PromptType::Labrador
        }
    }
    ...
    fn from_settings(app: &AppContext) -> PromptType {
        ...
        if *session_settings.honor_ps1 {
            ...
            Self::labrador_prompt_from_settings(app)
        }
    }
}
...
pub enum EditorModalAction {
    ...
    SetLabradorPromptSeparator { separator: LabradorPromptSeparator },
}
...
impl EditorModal {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let labrador_prompt_separator = match SessionSettings::as_ref(ctx).saved_prompt.value() {
            ...
            _ => LabradorPromptSeparator::None,
        };
        let labrador_prompt_separator_label = labrador_prompt_separator.dropdown_item_label().to_owned();

        let labrador_prompt_separator_dropdown = ctx.add_typed_action_view(|ctx| {
            let mut dropdown = Dropdown::new(ctx);
            dropdown.set_top_bar_max_width(DROPDOWN_WIDTH);
            dropdown.set_menu_width(DROPDOWN_WIDTH, ctx);
            let items = vec![
                DropdownItem::new(
                    LabradorPromptSeparator::None.dropdown_item_label(),
                    EditorModalAction::SetLabradorPromptSeparator {
                        separator: LabradorPromptSeparator::None,
                    },
                ),
                DropdownItem::new(
                    LabradorPromptSeparator::PercentSign.dropdown_item_label(),
                    EditorModalAction::SetLabradorPromptSeparator {
                        separator: LabradorPromptSeparator::PercentSign,
                    },
                ),
                DropdownItem::new(
                    LabradorPromptSeparator::DollarSign.dropdown_item_label(),
                    EditorModalAction::SetLabradorPromptSeparator {
                        separator: LabradorPromptSeparator::DollarSign,
                    },
                ),
                DropdownItem::new(
                    LabradorPromptSeparator::ChevronSymbol.dropdown_item_label(),
                    EditorModalAction::SetLabradorPromptSeparator {
                        separator: LabradorPromptSeparator::ChevronSymbol,
                    },
                ),
            ];
            ...
            dropdown.set_selected_by_name(labrador_prompt_separator_label, ctx);
            dropdown
        });
        ...
        Self {
            ...
            same_line_prompt_enabled,
            labrador_prompt_separator_dropdown,
            labrador_prompt_separator,
        }
    }
    ...
    /// Updates the state of the Labrador prompt separator dropdown to be enabled/disabled based on the current state of the modal.
    fn update_labrador_separator_dropdown_state(&mut self, ctx: &mut ViewContext<Self>) {
        // If we are using the Labrador prompt and SLP is enabled, then we enable the dropdown. Otherwise, disable it.
        if self.prompt_type != PromptType::PS1 && self.same_line_prompt_enabled {
            self.labrador_prompt_separator_dropdown
    ...
    fn save_prompt_to_settings(&mut self, ctx: &mut ViewContext<Self>) {
        if self.is_dirty {
            match self.prompt_type {
                PromptType::PS1 => {
                    // TODO: we need to stop the Labrador prompt generators from running at this point
                ...
                PromptType::Labrador => {
                    ...
                    Prompt::handle(ctx).update(ctx, |prompt, ctx| {
                        report_if_error!(prompt.update(
                            ...
                            self.labrador_prompt_separator,
                            ctx
                        ));
                    });
                }
            ...
            let _prompt_info = match self.prompt_type {
                ...
                PromptType::LabradorDefault => PromptChoice::Default,
                PromptType::Labrador => PromptChoice::Custom {
                    builtin_chips: self
                        .chip_configurator
                        .used_chips
                        .iter()
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            Self::Action::Chip(chip_action) => {
                ...
                if mutated {
                    self.is_dirty = true;
                    self.prompt_type = PromptType::Labrador;
                }
                ctx.notify();
            }
            Self::Action::UsePS1 => {
                ...
                // Disable the Labrador separator dropdown (only applies to Labrador prompt).
                self.update_labrador_separator_dropdown_state(ctx);
                ctx.notify();
            }
            Self::Action::UseLabradorPrompt => {
                self.is_dirty = true;
                self.prompt_type = PromptType::labrador_prompt_from_settings(ctx);
                // Enable the Labrador separator dropdown, if SLP is on.
                self.update_labrador_separator_dropdown_state(ctx);
                ctx.notify();
            }
            Self::Action::ResetLabradorPrompt => {
                self.is_dirty = true;
                self.prompt_type = PromptType::LabradorDefault;
                ...
                self.labrador_prompt_separator = default_prompt.separator();
                // Disable the Labrador separator dropdown, since SLP is off for the default Labrador prompt.
                self.update_labrador_separator_dropdown_state(ctx);
            ...
            Self::Action::ToggleSameLinePrompt => {
                ...
                // In case we had previously picked default Labrador prompt, but now the user toggled
            ...
            Self::Action::SetLabradorPromptSeparator { separator } => {
                self.is_dirty = true;
                self.labrador_prompt_separator = *separator;
                ctx.notify();
            }
        }
        ctx.notify();
...
impl EditorModal {
    ...
    fn render_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        appearance
            ...
            .with_style(UiComponentStyles {
                ...
                font_weight: Some(labrador_ui::fonts::Weight::Bold),
                ..Default::default()
            })
            .build()
            .finish()
    ...
    fn render_unused_chips(&self, appearance: &Appearance) -> Box<dyn Element> {
        self.chip_configurator.render_unused_chips_bank(
            EditorModalAction::UseLabradorPrompt,
    ...
    fn render_restore_default_labrador_prompt_button(
        &self,
        appearance: &Appearance,
    ) -> Box<dyn Element> {
        let button = Hoverable::new(
            self.mouse_state_handles
                .restore_default_labrador_prompt_handle
                .clone(),
            |_state| {
                appearance
                    .ui_builder()
        ...
        .on_click(|ctx, _, _| ctx.dispatch_typed_action(EditorModalAction::ResetLabradorPrompt))
        .with_cursor(Cursor::PointingHand);

        if matches!(self.prompt_type, PromptType::LabradorDefault) && !self.is_dirty {
    ...
    // TODO: consider supporting SLP with the new Labrador prompt.
    #[allow(dead_code)]
    fn render_same_line_prompt_section(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        Flex::row()
            ...
            .with_child(
                Container::new(ChildView::new(&self.labrador_prompt_separator_dropdown).finish())
    ...
    fn render_labrador_prompt_section(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let header_row = Flex::row()
            .with_child(
                appearance
                    ...
                    .with_style(UiComponentStyles {
                        font_size: Some(MODAL_CONTENT_FONT_SIZE),
                        font_weight: Some(labrador_ui::fonts::Weight::Semibold),
                        ..Default::default()
                    })
                    .build()
                    .finish(),
            )
            .with_child(self.render_restore_default_labrador_prompt_button(appearance))
        ...
        self.render_prompt_section(
            appearance,
            matches!(self.prompt_type, PromptType::Labrador | PromptType::LabradorDefault),
            header_row,
            None,
            body,
            self.mouse_state_handles
                .labrador_prompt_mouse_state_handle
                .clone(),
            EditorModalAction::UseLabradorPrompt,
        )
    }
    ...
    fn render_buttons(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        // - the Labrador prompt is used but there are no chips selected
        let save_disabled = !self.is_dirty
            || (matches!(self.prompt_type, PromptType::Labrador)
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let modal = Container::new(
            ConstrainedBox::new(
                column
                    .with_child(
                        Container::new(self.render_labrador_prompt_section(appearance))
                            .with_margin_bottom(MARGIN_BETWEEN_MODAL_SECTIONS)
                            .finish(),
                    )
                    .with_child(
