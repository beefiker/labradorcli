...
/// Start the Labrador UI headless app with all daemon singleton models.
///
...
pub(super) fn run_daemon_app(
    server_model_init: impl FnOnce(&mut labrador_ui::ModelContext<server_model::ServerModel>) -> server_model::ServerModel
        + 'static,
) -> anyhow::Result<()> {
    use labrador_ui::platform::app::AppCallbacks;
    use labrador_ui::platform::AppBuilder;
    ...
    AppBuilder::new_headless(AppCallbacks::default(), Box::new(()), None).run(|ctx| {
        ...
        ctx.add_singleton_model(labrador_files::FileModel::new);
...
#[cfg(not(target_family = "wasm"))]
pub fn wire_auth_token_rotation(ctx: &mut labrador_ui::AppContext) {
