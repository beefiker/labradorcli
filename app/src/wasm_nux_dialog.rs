...
use labrador_ui::elements::{Align, CrossAxisAlignment, Flex};
use labrador_ui::ui_components::{
...
pub enum WasmNUXDialogAction {
    ...
    OpenNativeAndClose,
    /// Open the Labrador download page
    OpenDownloadDesktopAppLink,
    /// Open a link to learn more about Labrador
    LearnMore,
}
...
/// * Download Labrador if they haven't already
...
pub struct WasmNUXDialog {
    ...
    download_labrador_mouse_state: MouseStateHandle,
...
impl WasmNUXDialog {
    pub fn new() -> Self {
        Self {
            ...
            download_labrador_mouse_state: Default::default(),
    ...
    /// behavior: if Labrador is installed, redirect to it; otherwise stay on the web.
...
impl View for WasmNUXDialog {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // 1. The user doesn't have Labrador installed - treat them as a potential new user and encourage downloading Labrador.
        // 2. The user has Labrador installed, but clicked through to the web - ask if they want to always default to web.
        ...
        let dialog = if self.requested_download {
            ...
            Dialog::new(format!("Download {app_name} Desktop?"), None, dialog_styles)
                ...
                .with_bottom_row_child(Self::render_dialog_button(
                    ...
                    &self.download_labrador_mouse_state,
                    appearance,
                ))
        } else {
            let app_name = ChannelState::app_name_display();
...
impl TypedActionView for WasmNUXDialog {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            WasmNUXDialogAction::OpenNativeAndClose => {
                // We intentionally do not set the native preference here, in case the user hasn't actually installed Labrador.
                // If they have, on subsequent loads, we'll detect that Labrador is installed and redirect to the desktop.
                ctx.emit(WasmNUXDialogEvent::Close);
                ...
                if let Some(url) = web_intent_parser::parse_web_intent_from_current_url() {
                    ...
                    crate::platform::wasm::emit_event(
                        crate::platform::wasm::LabradorEvent::OpenOnNative {
            ...
            WasmNUXDialogAction::OpenDownloadDesktopAppLink => {
                ctx.open_url("https://app.labrador.dev/get_labrador");
                self.requested_download = true;
                ctx.notify();
            }
            WasmNUXDialogAction::LearnMore => {
                ctx.open_url("https://www.labrador.dev");
            }
        }
    }
}
