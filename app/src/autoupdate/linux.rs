...
use labrador_terminal::shell::ShellType;
use labrador_ui::ViewContext;
...
mod appimage {
    ...
    pub(super) async fn download_update_and_cleanup(
        ...
        // Move new AppImage over the one that launched the current Labrador instance.
    ...
    pub(super) fn relaunch(appimage_path: &Path) -> Result<()> {
        ...
        // autoupdate process.
        command.arg(labrador_cli::finish_update_flag());
        ...
        if let Ok(path) = std::env::var("LABRADOR_CHANNEL_VERSIONS_PATH") {
            command.env("LABRADOR_CHANNEL_VERSIONS_PATH", path);
        }

        log::info!("Relaunching Labrador for update...");
        command.spawn()?;
        Ok(())
    }
}
...
mod package_manager {
    ...
    impl labrador_ui::Entity for AutoupdateContextBlock {
        type Event = ();
    }

    impl labrador_ui::View for AutoupdateContextBlock {
        ...
        fn render(&self, app: &labrador_ui::AppContext) -> Box<dyn labrador_ui::Element> {
                ...
                .distribution_update_disabled_repository()
            {
                lines.push(FormattedTextLine::Line(vec![
                    ...
                    FormattedTextFragment::inline_code("labrador_handle_dist_upgrade"),
                    FormattedTextFragment::plain_text(
            ...
            lines.push(FormattedTextLine::Line(vec![
                ...
                FormattedTextFragment::hyperlink(
                    ...
                    "https://github.com/beefiker/labrador/issues/new/choose",
                ),
            ]));
    ...
    pub(super) fn relaunch() -> Result<()> {
        ...
        // Add any arguments that were passed to Labrador, skipping the first
        // argument (the name of the executable) and dropping the flag for
        // finishing an update.
        let finish_update_flag = labrador_cli::finish_update_flag();
        command.args(
            std::env::args()
                .skip(1)
...
/// Returns which method should be used to update Labrador.
#[derive(Debug)]
pub(crate) enum UpdateMethod {
    /// We don't know how to update Labrador.
    Unknown,
    /// Labrador is running as an AppImage and should be updated in-place.
    AppImage(PathBuf),
    /// Labrador can be updated using the given package manager.
...
impl PackageManager {
    pub fn update_command(&self, shell_type: ShellType, update_id: &str) -> String {
        ...
        let base_command = match self {
            PackageManager::Apt {
                ...
                let dist_upgrade_fn = match shell_type {
                    ShellType::Zsh | ShellType::Bash | ShellType::Fish => {
                        "labrador_handle_dist_upgrade"
                    }
                    ShellType::PowerShell => "Labrador-Handle-DistUpgrade",
                };
                // If running with apt, attempt to handle a distribution update that may rename the
                // Labrador source file to `{repo_name}.distUpgrade`.
            ...
            PackageManager::Pacman {
                ...
                let repo_prefix = if !is_repo_configured {
                    let cache_dir = labrador_core::paths::cache_dir();
                    ...
                    format!("mkdir -p {cache_dir_str}{and}\\\ncp /etc/pacman.conf {cache_dir_str}{and}\\\nsudo sh -c \"echo '\n[{repo_name}]\nServer = https://releases.labrador.dev/linux/pacman/\\$repo/\\$arch' >> /etc/pacman.conf\"{and}\\\n")
                } else {
                    String::new()
                };
                let key_prefix = if !is_signing_key_configured {
                    ...
                    format!("sudo pacman-key -r \"linux-maintainers@labrador.dev\" --keyserver hkp://keys.openpgp.org:80{and}\\\nsudo pacman-key --lsign-key \"linux-maintainers@labrador.dev\"{and}\\\n")
                } else {
                    String::new()
                };
        ...
        let finish_update_fn = match shell_type {
            ShellType::Zsh | ShellType::Bash | ShellType::Fish => "labrador_finish_update",
            ShellType::PowerShell => "Labrador-Finish-Update",
        };
        ...
        match output {
            Ok(output) => {
                if !output.status.success() {
                    bail!("Failed to determine which package manager was used to install labrador");
                }
...
/// Returns whether the Labrador apt repository is disabled due to a version update.
/// This occurs if there's a `labrador.list.distUpgrade` file but no `labrador.sources` or
/// `labrador.list` file.
...
///
/// However, Ubuntu incorrectly thinks the Labrador source file is invalid (due to the addition of the
/// `signed-by` key) so it only leaves the `*.distUpgrade` source file. We use the existence of this
/// file to determine whether we need to run the special `labrador_handle_dist_upgrade` function to copy
/// `labrador.list.distUpgrade` back to `labrador.list` to re-enable the repository.
...
fn is_pacman_signing_key_installed() -> bool {
    ...
    let output = match command::blocking::Command::new("gpg")
        .args([
            ...
            "--list-keys",
            "--with-colons",
            "linux-maintainers@labrador.dev",
        ])
        .output()
    {
...
fn package_name(channel: Channel) -> &'static str {
    match channel {
        Channel::Stable => "labrador-terminal",
        Channel::Preview => "labrador-terminal-preview",
        Channel::Dev => "labrador-terminal-dev",
        Channel::Integration => "labrador-terminal-integration",
        Channel::Local => "labrador-terminal-local",
...
fn repo_name(channel: Channel) -> String {
    ...
    let channel_suffix = package_name
        .strip_prefix("labrador-terminal")
        .unwrap_or_default();
    format!("labrador{channel_suffix}")
}
