...
/// Download the Inno Setup install wizard, the same one users run on the first Labrador install, and
...
const UPDATE_LOG_FILENAME: &str = "labrador_update.log";

fn autoupdate_log_file() -> Result<PathBuf> {
    labrador_logging::log_directory().map(|dir| dir.join(UPDATE_LOG_FILENAME))
}
...
pub(super) fn check_and_report_update_errors(ctx: &mut AppContext) {
    ...
    if has_unable_to_close {
        log::warn!(
            "Windows auto-update installer was unable to automatically close all applications"
        );
    }
    ...
    .is_some();
    if has_file_in_use {
        log::warn!("Windows auto-update installer encountered a file-in-use error");
    }

    // Fired when the mutex polling loop timed out and a force-kill was attempted.
    let has_mutex_timeout = memchr::memmem::find(
        &contents_lowercase,
        b"labrador mutex still held after timeout",
    )
    .is_some();
    if has_mutex_timeout {
        log::warn!(
            "Windows auto-update installer timed out waiting for the Labrador mutex to release"
        );
    }
    ...
    if has_forcekill_failed {
        log::warn!("Windows auto-update installer failed to force-kill Labrador");
    }
...
pub(super) fn relaunch() -> Result<()> {
    ...
    // The Inno Setup install wizard will run without user input. It will re-launch Labrador after
    ...
    Command::new(&installer_path)
        .args([
            ...
            &log_arg,
            "/update=1",
            // Do not forcibly kill Labrador via RestartManager. The installer will wait for
            // Labrador to exit naturally by polling the single-instance mutex instead.
            "/NOCLOSEAPPLICATIONS",
