...
use labrador_core::{channel::ChannelState, context_flag::ContextFlag};
use labrador_util::path::user_friendly_path;
use labrador_ui::actions::StandardAction;
use labrador_ui::keymap::{Keystroke, Trigger};
use labrador_ui::platform::menu::{
...
const ENABLE_PTY_RECORDING: &str = "Enable PTY Recording Mode (labrador.pty.recording)";
const DISABLE_PTY_RECORDING: &str = "Disable PTY Recording Mode (labrador.pty.recording)";
...
const SETTINGS_CSV_FILE_NAME: &str = "labrador_default_settings.csv";
...
fn make_new_app_menu(ctx: &AppContext) -> Menu {
    let mut menu_items = vec![updateable_custom_item_without_checkmark(
        CustomAction::ShowAboutLabrador,
        ctx,
    )];
    ...
    menu_items.push(MenuItem::Custom(CustomMenuItem::new(
        ...
        move |_props, ctx| {
            ...
            MenuItemPropertyChanges {
                disabled: Some(
                    !DefaultTerminal::can_labrador_become_default()
                        || default_terminal.is_labrador_default(),
                ),
                ..Default::default()
            }
        },
...
fn make_launch_config_menu_items(ctx: &mut AppContext) -> Vec<MenuItem> {
    ...
    let launch_configs = LabradorConfig::handle(ctx).as_ref(ctx).launch_configs();
...
/// Dispatch event to open a new Labrador window
