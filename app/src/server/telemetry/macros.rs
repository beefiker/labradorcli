//! No-op telemetry-sender macros for the dwarf fork.
//!
//! Upstream Warp shipped a Rudderstack/Amplitude analytics pipeline that fired
//! HTTP requests to Warp's hosted servers whenever any of these macros expanded.
//! dwarf is a local-only CLI terminal and must not phone home, so each macro is
