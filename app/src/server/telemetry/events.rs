...
use labrador_completer::completer::MatchType;
use labrador_core::command::ExitCode;
use labrador_core::telemetry::EnablementState;
use labrador_core::telemetry::TelemetryEvent as TelemetryEventTrait;
use labrador_core::telemetry::TelemetryEventDesc;
use labrador_ui::keymap::Keystroke;
use labrador_ui::notification::{NotificationSendError, RequestPermissionsOutcome};
use labrador_ui::rendering::ThinStrokes;
...
use labrador_core::interval_timer::TimingDataPoint;
...
pub struct BootstrappingInfo {
    ...
    pub labrador_attributed_bootstrap_duration_seconds: Option<f64>,
...
/// Common metadata to include in all Labrador Drive telemetry events that act on a specific object.
...
pub struct OpenedSharingDialogEvent {
    ...
    /// Metadata for the object being shared, if it's a Labrador Drive object.
    #[serde(flatten)]
...
/// How the user opened the Labrador Drive sharing dialog.
...
pub enum SharingDialogSource {
    ...
    CommandPalette,
    /// The Labrador Drive index context menu.
    DriveIndex,
    /// The sharing dialog was auto-opened from shared session creation.
    StartedSessionShare,
    /// The user intented into Labrador with an email address to invite.
    InviteeRequest,
...
impl From<&CommandSearchItemAction> for CommandSearchResultType {
    fn from(action: &CommandSearchItemAction) -> Self {
        ...
        match action {
            ...
            OpenLabradorAI => Self::OpenLabradorAI,
            TranslateUsingLabradorAI => Self::TranslateUsingLabradorAI,
...
#[derive(Clone, Copy, Debug, Serialize, Deserialize)]
pub enum OpenedLabradorAISource {
    GlobalEntryButton,
    HelpWithBlock,
...
#[derive(Clone, Copy, Debug, Serialize, Deserialize)]
pub enum LabradorAIRequestResult {
...
pub enum AgentModeSetupProjectScopedRulesActionType {
    ...
    #[serde(rename = "generate_labrador_md")]
    GenerateLabradorMd,
    #[serde(rename = "skip_rules")]
    SkipRules,
    #[serde(rename = "regenerate_labrador_md")]
    RegenerateLabradorMd,
}
...
pub enum TelemetryEvent {
    ...
    /// The native app was opened while logged out. Since Labrador requires login,
    ...
    /// Opened legacy Labrador AI.
    OpenedLabradorAI {
        source: OpenedLabradorAISource,
    },
    /// Issued legacy Labrador AI request.
    LabradorAIRequestIssued {
        result: LabradorAIRequestResult,
    },
    LabradorAIAction {
        action_type: LabradorAIActionType,
    },
    /// This is purely for static prompts! Do not send user-written prompts with this event.
    UsedLabradorAIPreparedPrompt {
        prompt: &'static str,
    },
    ToggleFocusPaneOnHover {
        enabled: bool,
    },
    LabradorAICharacterLimitExceeded,
    ...
    SshTmuxLabradorifyBannerDisplayed,
    /// A SSH Labradorify Block was accepted
    SshTmuxLabradorifyBlockAccepted,
    /// A SSH Labradorify Block was dismissed
    SshTmuxLabradorifyBlockDismissed,
    LabradorifyFooterShown {
        is_ssh: bool,
    },
    AgentToolbarDismissed,
    LabradorifyFooterAcceptedLabradorify {
        is_ssh: bool,
    },
    /// How long until the labradorify process succeeded
    SshTmuxLabradorificationSuccess {
        tmux_installation: Option<TmuxInstallationState>,
        duration_ms: u64,
    },
    /// An SSH Error block was displayed to the user.
    SshTmuxLabradorificationErrorBlock {
        error: LabradorificationUnavailableReason,
        tmux_installation: Option<TmuxInstallationState>,
    },
    ...
    NeedsReauth,
    LabradorDriveOpened {
        source: LabradorDriveSource,
        is_code_mode_v2: bool,
    },
    // Toggled the legacy Labrador AI side panel.
    ToggleLabradorAI {
        opened: bool,
    },
    ...
    /// NOTE: this event is only meant to be used for LabradorDev.
    PtyThroughput {
    ...
    OpenAndLabradorifyDockerSubshell {
    ...
    /// This is a telemetry event used to help track performance of Agent Predict in Labrador,
    ...
    /// Detected that Labrador is running in an isolated sandbox.
    DetectedIsolationPlatform {
        platform: labrador_isolation_platform::IsolationPlatformType,
    },
    ...
    /// Emitted when a labrador://linear deeplink is opened.
...
impl TelemetryEventTrait for TelemetryEvent {
    ...
    fn event_descs() -> impl Iterator<Item = Box<dyn TelemetryEventDesc>> {
        labrador_core::telemetry::enum_events::<Self>()
    }
}
...
impl TelemetryEvent {
    ...
    pub fn payload(&self) -> Option<Value> {
        match self {
            ...
            TelemetryEvent::OpenedLabradorAI { source } => Some(json!({ "source": source })),
            TelemetryEvent::LabradorAIRequestIssued { result } => Some(json!({ "result": result })),
            TelemetryEvent::LabradorAIAction { action_type } => {
            ...
            TelemetryEvent::CodePanelsFileOpened { entrypoint, target } => {
                let (target, layout, editor) = match target {
                    FileTarget::MarkdownViewer(layout) => {
                        ("labrador_markdown_viewer", Some(*layout), None)
                    }
                    FileTarget::CodeEditor(layout) => ("labrador_code_editor", Some(*layout), None),
            ...
            TelemetryEvent::UsedLabradorAIPreparedPrompt { prompt } => {
            ...
            TelemetryEvent::LabradorifyFooterShown { is_ssh }
            | TelemetryEvent::LabradorifyFooterAcceptedLabradorify { is_ssh } => {
            ...
            TelemetryEvent::ToggleLabradorAI { opened } => Some(json!({ "opened": opened })),
            ...
            TelemetryEvent::OpenAndLabradorifyDockerSubshell { shell_type } => {
            ...
            TelemetryEvent::ToggleSshLabradorification { enabled } => Some(json!({"enabled": enabled})),
            ...
            TelemetryEvent::SshTmuxLabradorificationSuccess {
                duration_ms,
                tmux_installation,
            } => Some(json!({
            ...
            TelemetryEvent::SshTmuxLabradorificationErrorBlock {
                error,
                tmux_installation,
            } => Some(json!({
                "error": error,
            ...
            | TelemetryEvent::LabradorAICharacterLimitExceeded
            ...
            | TelemetryEvent::SshTmuxLabradorifyBlockAccepted
            | TelemetryEvent::SshTmuxLabradorifyBlockDismissed
            ...
            TelemetryEvent::LabradorDriveOpened {
                source,
                is_code_mode_v2,
            } => Some(json!({
                "source": source,
    ...
    pub fn contains_ugc(&self) -> bool {
        match self {
            ...
            | TelemetryEvent::InitialWorkingDirectoryConfigurationChanged { .. }
            | TelemetryEvent::OpenedLabradorAI { .. }
            | TelemetryEvent::LabradorAIRequestIssued { .. }
            | TelemetryEvent::LabradorAIAction { .. }
            | TelemetryEvent::UsedLabradorAIPreparedPrompt { .. }
            ...
            | TelemetryEvent::SshTmuxLabradorifyBlockAccepted
            | TelemetryEvent::SshTmuxLabradorifyBlockDismissed
            | TelemetryEvent::LabradorifyFooterShown { .. }
            | TelemetryEvent::AgentToolbarDismissed
            | TelemetryEvent::LabradorifyFooterAcceptedLabradorify { .. }
            | TelemetryEvent::SshTmuxLabradorificationSuccess { .. }
            | TelemetryEvent::SshTmuxLabradorificationErrorBlock { .. }
            | TelemetryEvent::SshInstallTmuxBlockDisplayed
            | TelemetryEvent::SshInstallTmuxBlockAccepted
            ...
            | TelemetryEvent::NeedsReauth
            | TelemetryEvent::LabradorDriveOpened { .. }
            | TelemetryEvent::ToggleLabradorAI { .. }
            ...
            | TelemetryEvent::OpenAndLabradorifyDockerSubshell { .. }
            | TelemetryEvent::UpdateBlockFilterQuery
            ...
            | TelemetryEvent::MCPServerSpawned { .. }
            | TelemetryEvent::MCPToolCallAccepted { .. }
            | TelemetryEvent::ToggleSshLabradorification { .. }
    ...
    pub fn print_telemetry_events_json() -> anyhow::Result<()> {
        ...
        let events: serde_json::Map<String, Value> = labrador_core::telemetry::all_events()
            .filter_map(|event| {
...
impl TelemetryEventDesc for TelemetryEventDiscriminants {
    fn enablement_state(&self) -> EnablementState {
        ...
        match self {
            ...
            Self::OpenedLabradorAI => EnablementState::Always,
            Self::LabradorAIRequestIssued => EnablementState::Always,
            Self::LabradorAIAction => EnablementState::Always,
            Self::UsedLabradorAIPreparedPrompt => EnablementState::Always,
            Self::LabradorAICharacterLimitExceeded => EnablementState::Always,
            ...
            Self::InputAskLabradorAI => EnablementState::Always,
            ...
            Self::SshTmuxLabradorifyBannerDisplayed => EnablementState::Always,
            ...
            Self::ToggleSshLabradorification => EnablementState::Always,
            ...
            Self::SshTmuxLabradorifyBlockAccepted => EnablementState::Always,
            Self::SshTmuxLabradorifyBlockDismissed => EnablementState::Always,
            Self::LabradorifyFooterShown
            | Self::AgentToolbarDismissed
            | Self::LabradorifyFooterAcceptedLabradorify => EnablementState::Always,
            Self::SshTmuxLabradorificationSuccess => EnablementState::Always,
            Self::SshTmuxLabradorificationErrorBlock => EnablementState::Always,
            ...
            Self::LabradorDriveOpened => EnablementState::Always,
            Self::ToggleLabradorAI => EnablementState::Always,
            ...
            Self::OpenAndLabradorifyDockerSubshell => EnablementState::Always,
    ...
    fn name(&self) -> &'static str {
        match self {
            ...
            Self::OpenedLabradorAI => "Opened Labrador AI",
            Self::LabradorAIRequestIssued => "Labrador AI Request Issued",
            Self::LabradorAIAction => "Labrador AI Action",
            Self::UsedLabradorAIPreparedPrompt => "Used Labrador AI Prepared Prompt",
            Self::LabradorAICharacterLimitExceeded => "Labrador AI Character Limit Exceeded",
            ...
            Self::InputAskLabradorAI => "InputBoxAskLabradorAI",
            ...
            Self::SshTmuxLabradorifyBannerDisplayed => "Show Labradorify SSH Banner",
            ...
            Self::ToggleSshLabradorification => "Toggle SSH Labradorification",
            ...
            Self::SshTmuxLabradorifyBlockAccepted => "SSH Tmux Labradorify Block Accepted",
            Self::SshTmuxLabradorifyBlockDismissed => "SSH Tmux Labradorify Block Dismissed",
            Self::LabradorifyFooterShown => "Labradorify Footer Shown",
            Self::AgentToolbarDismissed => "Agent Toolbar Dismissed",
            Self::LabradorifyFooterAcceptedLabradorify => "Labradorify Footer Accepted Labradorify",
            Self::SshTmuxLabradorificationSuccess => "SSH Tmux Labradorification Succeeded",
            Self::SshTmuxLabradorificationErrorBlock => "SSH Tmux Labradorification Error Block",
            ...
            Self::LabradorDriveOpened => "Labrador Drive Opened",
            Self::ToggleLabradorAI => "Toggle Labrador AI",
            ...
            Self::WebCloudObjectOpenedOnDesktop { .. } => "Labrador Drive object opened on desktop",
            Self::DriveSharingOnboardingBlockShown => {
                "Labrador Drive Sharing onboarding block shown"
            }
            ...
            Self::OpenAndLabradorifyDockerSubshell => "OpenAndLabradorifyDockerSubshell",
    ...
    fn description(&self) -> &'static str {
        match self {
            ...
            Self::AnonymousUserExpirationLockout => {
                "An anonymous user opened Labrador after their conversion deadline and was locked out"
            }
            Self::AnonymousUserLinkedFromBrowser => {
            ...
            Self::BackgroundBlockStarted => {
                "Labrador created a background-output Block (whenever a process has been backgrounded and yields some output)"
            }
            ...
            Self::LoggedOutStartup => "Started Labrador in the logged-out / signed-out state",
            Self::DownloadSource => {
                "Whether Labrador was installed from the home page or through homebrew"
            }
            ...
            Self::NotificationClicked => "Clicked desktop notification sent from Labrador",
            ...
            Self::OpenNewSessionFromFilePath => {
                "Dragged a file, folder, etc. into Labrador to start a session"
            }
            ...
            Self::TeamCreated => "Created a Labrador Drive team",
            Self::TeamJoined => "Joined a Labrador Drive team",
            Self::TeamLeft => "Left a Labrador Drive team",
            Self::TeamLinkCopied => "Copied a Labrador Drive team link",
            Self::RemovedUserFromTeam => "Remove user from Labrador Drive team",
            Self::DeletedWorkflow => "Deleted workflow from Labrador Drive team",
            Self::DeletedNotebook => "Deleted notebook from Labrador Drive team",
            Self::ToggleApprovalsModal => "Opened or closed teams modal",
            Self::SendEmailInvites => "Sent email invites for Labrador Drive team",
            ...
            Self::QuitModalDisabled => {
                "The quit modal dialog has been disabled and will not popup when a user closes Labrador while a session is running"
            }
            ...
            Self::OpenedLabradorAI => "Activated Labrador AI",
            Self::LabradorAIRequestIssued => "Issued a question to Labrador AI",
            Self::LabradorAIAction => "Executed a Labrador AI action: Restart, Copy, Insert into terminal",
            Self::UsedLabradorAIPreparedPrompt => {
                "Used one of the Labrador-provided prompts, like \"Show examples\""
            }
            Self::LabradorAICharacterLimitExceeded => {
                "Attempted to ask a question longer than 1k chars to Labrador AI"
            }
            ...
            Self::InputAskLabradorAI => "Clicked \"Ask Labrador AI\" from the Input Editor's context menu",
            ...
            Self::ShowSubshellBanner => {
                "Displayed the banner asking whether Labrador should Labradorify the current session via Labrador's subshell wrapper"
            }
            Self::SshTmuxLabradorifyBannerDisplayed => {
                "Displayed the banner asking whether Labrador should Labradorify the current SSH session via Labrador's SSH Wrapper"
            }
            Self::DeclineSubshellBootstrap => {
                "Developer declined the Labrador banner to Labradorify the current session"
            }
            Self::TriggerSubshellBootstrap => {
                "Attempted to Labradorify the current session via Labrador's subshell wrapper"
            }
            Self::AddDenylistedSubshellCommand => {
                "Explicitly prevent a command from being Labradorified via Labrador's subshell wrapper"
            }
            Self::RemoveDenylistedSubshellCommand => {
                "Removed a command from the list of commands to IGNORE when trying to Labradorify via Labrador's subshell wrapper"
            }
            Self::AddAddedSubshellCommand => {
                "Added a command to be automatically Labradorified via Labrador's subshell wrapper"
            }
            Self::RemoveAddedSubshellCommand => {
                "Removed a command from the list of commands to automatically Labradorify via Labrador's subshell wrapper"
            }
            Self::ReceivedSubshellRcFileDcs => "Spawned a subshell to be automatically Labradorified",
            Self::ToggleSshTmuxWrapper => {
                "Changed the setting for SSH sessions to prompt for Tmux Wrapper"
            }
            Self::ToggleSshLabradorification => "Changed the setting for SSH sessions to be labradorified",
            ...
            Self::SshTmuxLabradorifyBlockAccepted => "User accepted an ssh tmux labradorify block",
            Self::SshTmuxLabradorifyBlockDismissed => "User dismissed an ssh tmux labradorify block",
            Self::LabradorifyFooterShown => {
                "Displayed the labradorify footer for a detected subshell or SSH session"
            }
            Self::AgentToolbarDismissed => "User dismissed the use-agent toolbar",
            Self::LabradorifyFooterAcceptedLabradorify => "User clicked Labradorify in the labradorify footer",
            Self::SshTmuxLabradorificationSuccess => "Ssh tmux labradorification succeeded",
            Self::SshTmuxLabradorificationErrorBlock => "Ssh tmux labradorification errored out",
            ...
            Self::ShowAliasExpansionBanner => {
                "Displayed the banner asking whether Labrador should automatically expand aliases within the Input Editor"
            }
            ...
            Self::ShowVimKeybindingsBanner => {
                "Displayed the banner asking whether Labrador should enable Vim keybindings in the Input Editor"
            }
            ...
            Self::LabradorDriveOpened => "Opened Labrador Drive panel",
            Self::ToggleLabradorAI => {
                "Toggled Labrador AI--an AI assistant to help you debug errors, look up forgotten commands and more"
            }
            ...
            Self::AutoGenerateMetadataSuccess => {
                "Successfully generated metadata for a workflow using Labrador AI"
            }
            Self::AutoGenerateMetadataError => {
                "Failed to generate metadata for a workflow using Labrador AI"
            }
            Self::UpdateSortingChoice => "Modified the sorting scheme for Labrador Drive objects",
            ...
            Self::DuplicateObject => "Cloned a Labrador Drive object",
            Self::ExportObject => "Exported a Labrador Drive object",
            Self::CommandFileRun => {
                "Opened a .cmd or unix executable file and ran it directly in Labrador"
            }
            ...
            Self::JoinedSharedSession => {
                "When you join another instance of Labrador using shared sessions"
            }
            Self::SharedSessionModalUpgradePressed => {
            ...
            Self::WebCloudObjectOpenedOnDesktop => {
                "Labrador Drive object on the web was opened on the desktop"
            }
            Self::DriveSharingOnboardingBlockShown => {
                "Showed onboarding block for Labrador Drive sharing"
            }
            Self::UnsupportedShell => "Booted Labrador with a shell that isn't supported",
            Self::LogOut => "Logged out of the Labrador client",
            Self::SettingsImportInitiated => "Started the import settings flow for new users",
            Self::InviteTeammates => "Sent emails to invite teammates to join Labrador Drive team",
            Self::CopyObjectToClipboard => "Copied an object to the user's keyboard",
            Self::OpenAndLabradorifyDockerSubshell => {
                "Labradorifying a docker subshell from using the docker extension"
            }
            ...
            Self::GlobalSearchQueryStarted => "Started a global search (ripgrep-backed) search",
            ...
            Self::OpenedSharingDialog => {
                "Opened the sharing settings dialog for a session or Labrador Drive object"
            }
            ...
            #[cfg(windows)]
            Self::AutoupdateMutexTimeout => {
                "The Windows auto-update installer timed out waiting for Labrador to release its mutex; a force-kill was attempted"
            }
            #[cfg(windows)]
            Self::AutoupdateForcekillFailed => {
                "The Windows auto-update installer failed to force-kill Labrador after the mutex timeout"
            }
            ...
            Self::LinearIssueLinkOpened => {
                "User opened a labrador://linear deeplink to work on an issue"
            }
...
labrador_core::register_telemetry_event!(TelemetryEvent);
