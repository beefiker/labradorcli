...
use ai::AIClient;
use auth::{AMBIENT_WORKLOAD_TOKEN_HEADER, AuthClient, CLOUD_AGENT_ID_HEADER};
use base64::Engine;
use base64::prelude::BASE64_URL_SAFE;
use channel_versions::ChannelVersions;
use futures::StreamExt;
use labrador_core::context_flag::ContextFlag;
use labrador_core::errors::{AnyhowErrorExt, ErrorExt, register_error};
use labrador_managed_secrets::client::ManagedSecretsClient;
use labrador_ui::{ModelContext, r#async::BoxFuture};
use prost::Message;
use team::TeamClient;
use url::Url;
...
const EXPERIMENT_ID_HEADER: &str = "X-Labrador-Experiment-Id";

/// We use a server error-code header to allow the server to send more specific
/// error code information, so that the client can discern between different
/// errors with the same HTTP status code.
/// See errors/http_error_codes.go on the server for possible values.
const LABRADOR_ERROR_CODE_HEADER: &str = "X-Labrador-Error-Code";
...
const LABRADOR_ERROR_CODE_OUT_OF_CREDITS: &str = "OUT_OF_CREDITS";

/// Error code indicating the user has reached their cloud agent concurrency limit.
const LABRADOR_ERROR_CODE_AT_CAPACITY: &str = "AT_CLOUD_AGENT_CAPACITY";
...
fn server_error_code(headers: &::http::HeaderMap) -> Option<&str> {
    headers
        .get(LABRADOR_ERROR_CODE_HEADER)
...
/// in the server repo. Those rows need to exist in the DB so the authz user loader
...
impl AIApiError {
    ...
    fn from_response_error(err: reqwest::Error, headers: &::http::HeaderMap) -> Self {
        // For HTTP 429 errors, check the server error-code header to distinguish
    ...
    /// Returns the appropriate error for a 429 response by checking the server error-code header.
    fn error_for_429(headers: &::http::HeaderMap) -> Self {
        if server_error_code(headers) == Some(LABRADOR_ERROR_CODE_OUT_OF_CREDITS) {
...
cfg_if::cfg_if! {
    if #[cfg(target_family = "wasm")] {
        ...
        // we don't leverage WebWorkers for async execution in Labrador Web).
...
/// An API wrapper struct with methods for requests to the server.
///
...
pub struct ServerApi {
    ...
    ambient_workload_token: Arc<Mutex<Option<labrador_isolation_platform::WorkloadToken>>>,
...
impl ServerApi {
    ...
    async fn ambient_agent_headers(&self) -> Result<Vec<(&'static str, String)>> {
        ...
        if let Some(token) = workload_token {
            headers.push((AMBIENT_WORKLOAD_TOKEN_HEADER, token));
        }
        if let Some(id) = task_id {
            headers.push((CLOUD_AGENT_ID_HEADER, id));
        }
        if let Some(source) = agent_source {
            headers.push((AGENT_SOURCE_HEADER, source));
        }
    ...
    fn create_oauth_client() -> self::auth::OAuth2Client {
        ...
        oauth2::basic::BasicClient::new(oauth2::ClientId::new("labrador-cli".to_string()))
    ...
    pub fn send_graphql_request<'a, QF, O: labrador_graphql::client::Operation<QF> + Send + 'a>(
        &'a self,
        operation: O,
        ...
        Box::pin(async move {
            ...
            let options = labrador_graphql::client::RequestOptions {
            ...
            if let Some(errors) = response.errors.as_ref() {
                ...
                // "User not in context: Not found" comes from the server as an error when attempting
    ...
    ///
    /// The stream is served by the RTC server (not the main server pool),
    ...
    async fn error_from_response(response: http_client::Response) -> anyhow::Error {
        let status = response.status();
        let error_code = server_error_code(response.headers());
        let is_at_capacity = error_code == Some(LABRADOR_ERROR_CODE_AT_CAPACITY);
        let is_out_of_credits = error_code == Some(LABRADOR_ERROR_CODE_OUT_OF_CREDITS);
        ...
        match self.get_or_refresh_access_token().await {
            Ok(auth_token) => {
                ...
                request = request
                    ...
                    .header(EXPERIMENT_ID_HEADER, anonymous_id);
        ...
        match response {
            Ok(res) => {
                if res.status().is_success() {
                    ...
                    if server_error_code(res.headers()) == Some(LABRADOR_ERROR_CODE_OUT_OF_CREDITS)
                    {
    ...
    pub async fn generate_multi_agent_output(
        &self,
        request: &labrador_multi_agent_api::Request,
    ) -> std::result::Result<AIOutputStream<labrador_multi_agent_api::ResponseEvent>, Arc<AIApiError>>
    {
        let auth_token = self
            .get_or_refresh_access_token()
            .await
        ...
        let is_passive = request.input.as_ref().is_some_and(|input| {
            matches!(
                input.r#type,
                Some(labrador_multi_agent_api::request::input::Type::GeneratePassiveSuggestions(_))
            )
        });
            ...
            let result = match event {
                Ok(reqwest_eventsource::Event::Message(message_event)) => {
                    match BASE64_URL_SAFE.decode(message_event.data.trim_matches('"')) {
                        Ok(decoded_data) => {
                            let action = labrador_multi_agent_api::ResponseEvent::decode(
    ...
    /// Fetches updated Labrador Channel Versions from Labrador Server. If it is the first such request of
...
impl ServerApiProvider {
    ...
    /// and includes standard Labrador request headers.
...
mod tests {
    use super::{
        LABRADOR_ERROR_CODE_HEADER, LABRADOR_ERROR_CODE_OUT_OF_CREDITS, server_error_code,
    };

    #[test]
    fn server_error_code_reads_labrador_header() {
        let mut headers = ::http::HeaderMap::new();
        headers.insert(
            LABRADOR_ERROR_CODE_HEADER,
            ::http::HeaderValue::from_static(LABRADOR_ERROR_CODE_OUT_OF_CREDITS),
        );

        assert_eq!(
            server_error_code(&headers),
            Some(LABRADOR_ERROR_CODE_OUT_OF_CREDITS)
        );
    }

    #[test]
    fn server_error_code_reads_labrador_header_without_fallbacks() {
        let mut headers = ::http::HeaderMap::new();
        headers.insert(
            LABRADOR_ERROR_CODE_HEADER,
            ::http::HeaderValue::from_static("AT_CLOUD_AGENT_CAPACITY"),
        );

        assert_eq!(server_error_code(&headers), Some("AT_CLOUD_AGENT_CAPACITY"));
    }

    #[test]
    fn server_error_code_returns_none_without_supported_header() {
        assert_eq!(server_error_code(&::http::HeaderMap::new()), None);
    }
}
