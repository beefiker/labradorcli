...
use labrador_core::errors::{AnyhowErrorExt, ErrorExt};
...
use labrador_graphql::queries::get_user::{GetUser, GetUserVariables, UserOutput as GqlUserOutput};
use labrador_graphql::queries::get_user_settings::{GetUserSettings, GetUserSettingsVariables};
use labrador_ui::r#async::BoxFuture;
...
pub const AMBIENT_WORKLOAD_TOKEN_HEADER: &str = "X-Labrador-Ambient-Workload-Token";

/// Header key for the cloud agent task ID attached to requests from ambient agents.
pub const CLOUD_AGENT_ID_HEADER: &str = "X-Labrador-Cloud-Agent-ID";
...
pub trait AuthClient: 'static + Send + Sync {
    /// Creates an anonymous user, who is allowed to use Labrador but may lack the ability
    ...
    /// Queries server for a set of the currently logged-in user's fields.
...
impl AuthClient for ServerApi {
    async fn create_anonymous_user(
        ...
        let variables = CreateAnonymousUserVariables {
            input: labrador_graphql::mutations::create_anonymous_user::CreateAnonymousUserInput {
                anonymous_user_type,
                expiration_type: labrador_graphql::mutations::create_anonymous_user::AnonymousUserExpirationType::NoExpiration,
                referral_code,
            },
    ...
    async fn fetch_new_custom_token(&self) -> Result<MintCustomTokenResult> {
        ...
        let operation =
            labrador_graphql::mutations::mint_custom_token::MintCustomToken::build(variables);
    ...
    async fn fetch_user_properties<'a>(
        ...
        match response.user {
            labrador_graphql::queries::get_user::UserResult::UserOutput(user_output) => {
                Ok(user_output)
            }
            labrador_graphql::queries::get_user::UserResult::Unknown => {
    ...
    async fn get_user_settings(&self) -> Result<Option<SyncedUserSettings>> {
        ...
        match response.user {
            labrador_graphql::queries::get_user_settings::UserResult::UserOutput(user_output) => {
            ...
            labrador_graphql::queries::get_user_settings::UserResult::Unknown => {
    ...
    async fn exchange_device_access_token(
        ...
        let result = self
            ...
            .request_async(
                self.client.as_ref(),
                |delay| labrador_ui::r#async::Timer::after(delay).map(|_| ()),
                Some(timeout),
            )
            .await
    ...
    async fn get_or_create_ambient_workload_token(&self) -> Result<Option<String>> {
        ...
        // Issue a new token.
        let workload_token = match labrador_isolation_platform::issue_workload_token(Some(
        ...
        .await
        {
            Ok(token) => token,
            Err(
                labrador_isolation_platform::IsolationPlatformError::NoIsolationPlatformDetected,
            ) => {
                return Ok(None);
            }
