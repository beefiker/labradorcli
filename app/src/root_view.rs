...
use crate::themes::theme::{AnsiColorIdentifier, ThemeKind, LabradorThemeConfig};
...
use labrador_ui::assets::asset_cache::AssetSource;
use labrador_ui::clipboard::ClipboardContent;
use labrador_ui::fonts::Weight;
use labrador_ui::keymap::{EditableBinding, FixedBinding, Keystroke};
use labrador_ui::text_layout::TextAlignment;
...
use labrador_ui::rendering::OnGPUDeviceSelected;
use labrador_ui::{id, AddWindowOptions, DisplayId, SingletonEntity};
use labrador_ui::{
...
use labrador_ui::{FocusContext, NextNewWindowsHasThisWindowsBoundsUponClose};
        ...
        let safe_repos = self
            ...
            .filter(|repo| {
                ...
                Url::parse(repo).is_ok()
                    ...
                    || labrador_util::path::is_posix_portable_pathname(repo)
                    // Or absolute POSIX paths with portable components (e.g., /Users/me/repo)
                    || repo
                        .strip_prefix('/')
                        .is_some_and(labrador_util::path::is_posix_portable_pathname)
            })
            .join(" ");
...
///
/// This is the canonical way to open a new Labrador window - all other entrypoints should delegate to
/// it if possible.
        ...
        None => {
            ...
            let (id, _) = ctx.add_window(
                AddWindowOptions {
                    ...
                    anchor_new_windows_from_closed_position:
                        labrador_ui::NextNewWindowsHasThisWindowsBoundsUponClose::No,
                    on_gpu_driver_selected: on_gpu_driver_selected_callback(),
                    window_instance: Some(ChannelState::app_id().to_string() + "-hotkey"),
                    ..Default::default()
                },
...
/// This action will show or hide all of Labrador's windows except the quake window
///
/// - If Labrador is active and has any windows, hide those windows.
/// - If Labrador is hidden, show all windows.
/// - If Labrador is active but has 0 normal windows, create a new window with a new session.
fn show_or_hide_non_quake_mode_windows(_: &(), ctx: &mut AppContext) {
    ...
    // Now there is at least one window. If a Labrador window is active, hide the app.
...
pub enum LabradorConfettiPreset {
    Celebration,
    Fireworks,
    Snow,
    Cannon,
...
#[derive(Clone, Copy)]
struct LabradorConfettiRun {
    started_at: instant::Instant,
    preset: LabradorConfettiPreset,
}

struct LabradorConfettiElement {
    run: LabradorConfettiRun,
...
impl LabradorConfettiElement {
    const FRAME: Duration = Duration::from_millis(16);

    fn new(run: LabradorConfettiRun) -> Self {
        Self {
            run,
            size: None,
            origin: None,
    ...
    fn duration_for(preset: LabradorConfettiPreset) -> Duration {
        let seconds = preset
            .bursts()
            .iter()
...
impl LabradorConfettiPreset {
    fn bursts(self) -> Vec<ConfettiBurst> {
        let default = LabradorConfettiElement::default_options();
        match self {
...
impl Element for LabradorConfettiElement {
    fn layout(
        &mut self,
    ...
    fn dispatch_event(
        &mut self,
        _event: &labrador_ui::event::DispatchedEvent,
...
/// In the Labrador fork, login is always considered complete, so we only model
...
pub struct RootView {
    ...
    confetti_run: Option<LabradorConfettiRun>,
}
...
impl RootView {
    pub fn new(
        ...
        // The Labrador fork is local-only: there is no remote login flow. We pick
        ...
        let auth_onboarding_state = if local_agent_terminal_only() {
            ...
            // When OpenLabradorNewSettingsModes is enabled, show onboarding to
            // users who haven't completed it yet (tracked via a local
            // UserPreferences key).
            let has_completed_local_onboarding = FeatureFlag::OpenLabradorNewSettingsModes
                .is_enabled()
                && has_completed_local_onboarding(ctx);
            let should_show_onboarding = FeatureFlag::OpenLabradorNewSettingsModes.is_enabled()
        ...
        .then_some(LabradorConfettiRun {
            started_at: instant::Instant::now(),
            preset: LabradorConfettiPreset::Celebration,
        });
    ...
    fn build_plan_yearly_price_cents(_ctx: &AppContext) -> Option<i32> {
        // Hosted pricing data is unavailable in the Labrador fork; the onboarding
    ...
    fn create_agent_onboarding_view(
        ...
        // Pricing updates no longer exist in the Labrador fork.
    ...
    fn handle_agent_onboarding_event(
        ...
        match event {
            ...
            AgentOnboardingEvent::PrivacySettingsFromTerminalThemeSlideRequested => {
                // The dedicated login / privacy slide was tied to the Labrador Cloud
                // sign-in flow; in the local-only Labrador fork there is no slide
                // to show.
            }
            AgentOnboardingEvent::LoginFromWelcomeRequested => {
                // No remote login flow in the local-only Labrador fork.
            }
    ...
    /// bootstrapping its [`ShellType`], set a flag to automatically bootstrap it when the command's
    ...
    pub fn handle_team_intent_link_action(&mut self, _: &(), ctx: &mut ViewContext<Self>) -> bool {
        // Labrador Drive has been removed; just focus the window.
        let window_id = ctx.window_id();
        if let AuthOnboardingState::Terminal(_) = &self.auth_onboarding_state {
            ctx.windows().show_window_and_focus_app(window_id);
        }
    ...
    fn handle_auth_manager_event(&mut self, event: &AuthManagerEvent, ctx: &mut ViewContext<Self>) {
        ...
        match event {
            AuthManagerEvent::AuthComplete => {
                ...
                // local-only Labrador fork; everything else (auth UI, SSO link,
                // web import) was tied to the removed Labrador Cloud login flow.
                ...
                UserAuthenticationError::UserAccountDisabled(_) => {
                    cfg_if! {
                        if #[cfg(target_family = "wasm")] {
                            ...
                            // to use Labrador. Instead, they can sign in or up with a valid account.
    ...
    fn maybe_stop_active_voice_input(
        &mut self,
        key_code: &labrador_ui::platform::keyboard::KeyCode,
        ctx: &mut ViewContext<Self>,
    ) -> bool {
        use crate::settings::AISettings;
        use voice_input::{VoiceInput, VoiceInputState, VoiceInputToggledFrom};
        use labrador_ui::event::KeyState;
    ...
    fn start_pending_tutorial(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if FeatureFlag::OpenLabradorNewSettingsModes.is_enabled()
...
impl View for RootView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        if let Some(run) = self.confetti_run.filter(|run| {
            run.started_at.elapsed() < LabradorConfettiElement::duration_for(run.preset)
        }) {
            stack.add_positioned_overlay_child(
                LabradorConfettiElement::new(run).finish(),
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "voice_input")] {
                use labrador_ui::elements::{EventHandler, DispatchEventResult};
                EventHandler::new(stack.finish())
                    .on_modifier_state_changed(|ctx, _app, key_code, key_state| {
                        if matches!(key_state, labrador_ui::event::KeyState::Released) {
...
pub enum RootViewAction {
    ...
    ShowConfetti(LabradorConfettiPreset),
}
    ...
    fn handle_action(&mut self, action: &RootViewAction, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            RootViewAction::ShowConfetti(preset) => {
                self.confetti_run = Some(LabradorConfettiRun {
                    started_at: instant::Instant::now(),
                    preset: *preset,
                });
                ctx.notify();
...
impl AuthOnboardingState {
    ...
    fn log_out(&mut self, ctx: &mut ViewContext<RootView>) {
        // Local-only fork has no Labrador Cloud account; logout just refreshes the workspace.
