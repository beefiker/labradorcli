...
impl LLMPreferences {
    ...
    ///
    /// Filtered to local-CLI-runnable models only: labrador is local-auth-only,
    /// so Labrador's hosted-only catalog entries (gemini, glm, paywalled gpt
    ...
    pub fn new_choices_since_last_update(&self) -> Option<Vec<LLMInfo>> {
        self.last_update.as_ref().map(|update| {
            // We don't want to display new choices if they are legacy branded.
