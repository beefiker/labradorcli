...
const CODEX_BIN_ENV_VAR: &str = "LABRADOR_CODEX_BIN";
const CODEX_MODEL_ENV_VAR: &str = "LABRADOR_CODEX_MODEL";
const CLAUDE_BIN_ENV_VAR: &str = "LABRADOR_CLAUDE_BIN";
const CLAUDE_MODEL_ENV_VAR: &str = "LABRADOR_CLAUDE_MODEL";
const LOCAL_AGENT_ENV_VAR: &str = "LABRADOR_LOCAL_AGENT";
const LOCAL_AGENT_DISPLAY_NAME: &str = "Local Agent";
const TOOL_CALL_PREFIX: &str = "LABRADOR_TOOL_CALL";
const TOOL_CALL_PREFIXES: &[&str] = &[TOOL_CALL_PREFIX];
const CLAUDE_DEFAULT_MODEL_ID: &str = "claude-code";
const LOCAL_AGENT_TIMEOUT_ENV_VAR: &str = "LABRADOR_LOCAL_AGENT_TIMEOUT_SECS";
...
fn local_agent_env(name: &str) -> Option<String> {
    env::var(name).ok()
}

fn local_agent_env_hint(name: &str) -> String {
    name.to_owned()
}
...
fn configured_local_agent_runtime() -> Option<LocalAgentRuntime> {
    let value = local_agent_env(LOCAL_AGENT_ENV_VAR)?;
...
pub(super) async fn generate_output(
    ...
    let stream = async_stream::stream! {
        ...
        let mut tool_call_was_synthesized_from_prompt = false;
            ...
            local_cwd_change_target(&inputs, &cleaned_text, working_directory.as_deref())
        {
            ...
            if !already_streamed {
                ...
                tool_call_was_synthesized_from_prompt = true;
            }
        } else if let Some(target_dir) =
            extract_cd_command_from_text(&cleaned_text, working_directory.as_deref())
        {
            // The user's prompt didn't explicitly ask to change directory, but the
            // model emitted a `cd <path>` suggestion in its prose. Surface that as
            // an ask-to-run tool call so the user gets a single click to take the
            // suggestion instead of having to retype the command.
            let command = format!("cd {}", shell_quote_path(&target_dir));
            let already_streamed = streamed_tool_calls.iter().any(|c| c.command == command);
            if !already_streamed {
                tool_calls.push(LocalRunShellCommand::read_only(command));
        ...
        // When the synthesized cwd-change came from the user's explicit ask AND it
        // is the only action, simplify the prose (matches behaviour of the previous
        // post-process pass). We don't do this for cd-suggestions extracted from
        // the model's prose — that prose carries useful surrounding context.
        if tool_call_was_synthesized_from_prompt
...
fn standalone_codex_model() -> Option<String> {
    selected_codex_model(local_agent_env(CODEX_MODEL_ENV_VAR), "")
}

fn standalone_claude_model() -> Option<String> {
    selected_claude_model(
        local_agent_env(CLAUDE_MODEL_ENV_VAR),
...
fn prompt_for_local_am_query_suggestion(request: &GenerateAMQuerySuggestionsRequest) -> String {
    ...
    format!(
        "You are generating a local {app_name} agent follow-up chip.\n\
         ...
         Do not mention Labrador, Oz, credits, cloud agents, accounts, or premium features.\n\
         Do not run anything. Do not include markdown. Do not include explanations.\n\
...
fn run_codex_streaming(
    ...
    async_stream::stream! {
        ...
        let mut child = match command.spawn() {
            Ok(child) => child,
            Err(error) => {
                let codex_bin_env_hint =
                    local_agent_env_hint(CODEX_BIN_ENV_VAR);
                yield Err(format!(
                    "{LOCAL_AGENT_DISPLAY_NAME} failed to start. Tried `{codex_bin}`. Set {codex_bin_env_hint} to your Codex CLI path if it is not on PATH.\n\n```text\n{error}\n```"
                ));
                return;
            }
        };
        ...
        let timeout_env_hint =
            local_agent_env_hint(LOCAL_AGENT_TIMEOUT_ENV_VAR);
        ...
        loop {
            ...
            match futures::future::select(next_fut, &mut timeout).await {
                ...
                futures::future::Either::Right(_) => {
                    ...
                    let _ = child.kill();
                    yield Err(format!(
                        "{LOCAL_AGENT_DISPLAY_NAME} Codex CLI timed out after {}s. Set {timeout_env_hint} to override.",
...
async fn run_claude(
    ...
    let output = command.output().await.map_err(|error| {
        format!(
            "{LOCAL_AGENT_DISPLAY_NAME} failed to start Claude Code. Tried `{claude_bin}`. Set {claude_bin_env_hint} to your Claude Code CLI path if it is not on PATH.\n\n```text\n{error}\n```"
        )
    })?;
...
fn run_claude_streaming(
    ...
    async_stream::stream! {
        ...
        let mut child = match command.spawn() {
            Ok(child) => child,
            Err(error) => {
                let claude_bin_env_hint =
                    local_agent_env_hint(CLAUDE_BIN_ENV_VAR);
                yield Err(format!(
                    "{LOCAL_AGENT_DISPLAY_NAME} failed to start Claude Code. Tried `{claude_bin}`. Set {claude_bin_env_hint} to your Claude Code CLI path if it is not on PATH.\n\n```text\n{error}\n```"
                ));
                return;
        ...
        loop {
            ...
            match futures::future::select(next_fut, &mut timeout).await {
                ...
                futures::future::Either::Right(_) => {
                    ...
                    let _ = child.kill();
                    yield Err(format!(
                        "{LOCAL_AGENT_DISPLAY_NAME} Claude Code timed out after {}s. Set {timeout_env_hint} to override.",
                        timeout_total.as_secs()
                    ));
                    return;
                }
...
fn local_agent_timeout() -> Duration {
    let secs = local_agent_env(LOCAL_AGENT_TIMEOUT_ENV_VAR)
...
fn configured_codex_bin() -> Option<String> {
    local_agent_env(CODEX_BIN_ENV_VAR)
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
}

fn configured_claude_bin() -> Option<String> {
    local_agent_env(CLAUDE_BIN_ENV_VAR)
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
}
...
fn model_for_codex(params: &RequestParams) -> Option<String> {
    selected_codex_model(local_agent_env(CODEX_MODEL_ENV_VAR), params.model.as_str())
}
...
fn model_for_claude(params: &RequestParams) -> Option<String> {
    selected_claude_model(local_agent_env(CLAUDE_MODEL_ENV_VAR), params.model.as_str())
}
...
fn tool_call_contract_prose() -> String {
    ...
    format!(
        "{app_name} terminal tool-call contract:\n\
         ...
         LABRADOR_TOOL_CALL {{\"type\":\"run_shell_command\",\"command\":\"pwd\",\"is_read_only\":true,\"uses_pager\":false,\"is_risky\":false,\"wait_until_complete\":true}}\n\
         ...
         - Do not wrap LABRADOR_TOOL_CALL lines in markdown fences. Keep prose short and do not claim validation from commands that only ran in your local agent subprocess.\n\n"
    )
}
...
/// Per-stream line-buffered filter that separates tool-call markers from
...
/// instead of showing raw marker JSON to the user and replacing it after the
/// stream ends.
#[derive(Default)]
...
impl StreamingToolCallFilter {
    ...
    fn drain_complete_lines(&mut self, result: &mut FilteredDelta) {
        while let Some(nl_pos) = self.pending_line.find('\n') {
            ...
            if let Some(marker) = parse_labrador_marker_line(line_without_newline) {
    ...
    fn flush(mut self) -> FilteredDelta {
        ...
        if let Some(marker) = parse_labrador_marker_line(&self.pending_line) {
            result.tool_calls.push(marker);
            return result;
        }
        if starts_with_tool_call_prefix(&self.pending_line) {
...
/// Parse a single line (no trailing newline) as a tool-call marker.
...
fn parse_labrador_marker_line(line: &str) -> Option<LocalRunShellCommand> {
    let json = tool_call_marker_json(line)?;
    let marker: ToolCallMarker = serde_json::from_str(json).ok()?;
...
#[allow(dead_code)]
fn extract_labrador_tool_calls(output_text: &str) -> (String, Vec<LocalRunShellCommand>) {
    ...
    if !contains_tool_call_prefix(output_text) {
    ...
    for line in output_text.lines() {
        let extracted = tool_call_marker_json(line)
            .and_then(|json| serde_json::from_str::<ToolCallMarker>(json).ok())
            .filter(|marker| {
...
fn tool_call_marker_json(line: &str) -> Option<&str> {
    for prefix in TOOL_CALL_PREFIXES {
        ...
        let rest = line[start + prefix.len()..]
            .trim_start()
            .strip_prefix(':')
            .unwrap_or_else(|| line[start + prefix.len()..].trim_start())
            .trim_start();
        if rest.starts_with('{') {
            return Some(rest);
        }
...
fn contains_tool_call_prefix(text: &str) -> bool {
    TOOL_CALL_PREFIXES
        .iter()
        .any(|prefix| text.contains(prefix))
}

fn starts_with_tool_call_prefix(text: &str) -> bool {
...
/// Pull a `cd <path>` command out of model prose so dwarf can surface it as a
/// one-click ask-to-run tool call even when the user didn't phrase their prompt
/// with an explicit "go to" / "bring me to" intent. Returns the target only when:
///   - there's exactly one `cd <path>` line in the output (multiple suggests a
///     tutorial or unrelated example — don't auto-offer),
///   - the path is absolute or `~/...` (paths the user can take literally),
///   - the path exists on disk,
///   - the path isn't the dwarf terminal's current working directory.
fn extract_cd_command_from_text(
    output_text: &str,
    working_directory: Option<&str>,
) -> Option<PathBuf> {
    let mut found: Option<PathBuf> = None;
    for raw_line in output_text.lines() {
        let Some(target) = parse_cd_command_line(raw_line) else {
            continue;
        };
        let Some(candidate) = candidate_path(target) else {
            continue;
        };
        if !candidate.is_dir() {
            continue;
        ...
        if found.is_some() {
            // More than one cd line — likely a tutorial. Bail.
            return None;
        }
    ...
    if working_directory
        .map(Path::new)
        .is_some_and(|wd| same_directory(wd, &target))
    {
        return None;
    }
    Some(target)
...
/// If `line` is a single `cd <path>` shell command (with optional `$ ` prompt
/// and code-fence padding), return the path argument. Returns `None` for lines
/// that aren't a bare `cd` invocation.
fn parse_cd_command_line(line: &str) -> Option<&str> {
    let stripped = line
        .trim()
        .trim_start_matches('`')
        .trim_end_matches('`')
        ...
        .trim_start_matches('$')
        .trim();
    let rest = stripped.strip_prefix("cd ")?;
    let target = rest.trim();
    if target.is_empty() {
        return None;
    }
    // Reject pipes / chained commands — if the line is `cd /x && pwd`, it's not
    // a clean ask-to-run candidate, the chain belongs in the user's terminal as
    // a multi-step instruction rather than a single offer.
    if target.contains("&&")
        || target.contains("||")
        || target.contains(';')
        || target.contains('|')
    ...
    let target = trim_path_candidate(target);
    Some(target)
}
    ...
    api::ResponseEvent {
        r#type: Some(api::response_event::Type::Finished(
            api::response_event::StreamFinished {
                ...
                request_cost: None,
                conversation_usage_metadata: Some(super::empty_conversation_usage_metadata()),
            },
        )),
    }
}
...
mod tests {
    ...
    static LOCAL_AGENT_ENV_LOCK: Mutex<()> = Mutex::new(());
    ...
    fn local_agent_timeout_defaults_when_env_unset() {
        let _guard = LOCAL_AGENT_ENV_LOCK.lock().unwrap();
    ...
    fn streaming_filter_separates_markers_from_prose() {
        ...
        let r2 = filter.ingest("\":\"run_shell_command\",\"command\":\"ls -la\",\"is_read_only\":true}\nLABRADOR_TOOL_CALL {\"type\":\"run_shell_command\",\"command\":\"cat README.md\",\"is_read_only\":true}\n");
    ...
    fn streaming_filter_drops_partial_marker_on_flush() {
        ...
        // half-baked `LABRADOR_TOOL_CALL {"type` fragment to the user.
        let mut filter = StreamingToolCallFilter::default();
        filter.ingest("Some prose.\nLABRADOR_TOOL_CALL {\"type\":\"run_sh");
        let flushed = filter.flush();
        assert!(flushed.text_chunks.is_empty());
        assert!(flushed.tool_calls.is_empty());
    }
    ...
    fn streaming_filter_handles_multiple_lines_in_one_chunk() {
        let mut filter = StreamingToolCallFilter::default();
        let r = filter.ingest(
            "First line.\nSecond line.\nLABRADOR_TOOL_CALL {\"type\":\"run_shell_command\",\"command\":\"pwd\"}\nTrailing.",
        );
        assert_eq!(
            r.text_chunks,
            vec![
    ...
    #[test]
    fn extract_labrador_tool_calls_passes_through_when_no_marker() {
        // Fast path: no TOOL_CALL_PREFIX anywhere → return verbatim, no extraction.
        let input = "Some response text\nwith multiple lines";
        let (cleaned, calls) = extract_labrador_tool_calls(input);
        assert_eq!(cleaned, input);
        assert!(calls.is_empty());
    }

    #[test]
    fn extract_labrador_tool_calls_splits_marker_from_prose() {
        let input = "I'll check that for you.\n\
            LABRADOR_TOOL_CALL: {\"type\":\"run_shell_command\",\"command\":\"ls\",\"is_read_only\":true}\n\
            Done.";
        let (cleaned, calls) = extract_labrador_tool_calls(input);
    ...
    #[test]
    fn extracts_cd_command_suggestion_from_model_prose() {
        // The user didn't explicitly ask to change directories, but the model
        // emitted a `cd <path>` suggestion. We want the ask-to-run UI for it.
        let target_dir = std::env::temp_dir().join(format!(
            "{}-local-codex-{}",
            ChannelState::app_name(),
            Uuid::new_v4()
        ...
        std::fs::create_dir(&target_dir).unwrap();
        let output = format!(
            "If you want to jump to the frontend monorepo I can see in your workspace, use:\n\ncd {}\n\nIf you meant something else, let me know.",
            target_dir.display()
        );
        assert_eq!(
            extract_cd_command_from_text(&output, None).as_deref(),
            Some(target_dir.as_path())
        );
        std::fs::remove_dir(target_dir).unwrap();
    }
    ...
    #[test]
    fn extracts_cd_command_ignores_multiple_suggestions() {
        // Two `cd` lines in the output — likely a tutorial. Don't auto-offer
        // either; the user can copy the specific one they want.
        let target_dir_a = std::env::temp_dir().join(format!(
            "{}-local-codex-{}",
            ChannelState::app_name(),
            Uuid::new_v4()
        ));
        let target_dir_b = std::env::temp_dir().join(format!(
            "{}-local-codex-{}",
            ChannelState::app_name(),
            Uuid::new_v4()
        ));
        std::fs::create_dir(&target_dir_a).unwrap();
        std::fs::create_dir(&target_dir_b).unwrap();
        let output = format!(
            "Step 1:\ncd {}\nStep 2:\ncd {}\n",
            target_dir_a.display(),
            target_dir_b.display()
        );
        assert!(extract_cd_command_from_text(&output, None).is_none());
        std::fs::remove_dir(target_dir_a).unwrap();
        std::fs::remove_dir(target_dir_b).unwrap();
    }

    #[test]
    fn extracts_cd_command_skips_when_already_in_target() {
        // If dwarf is already in the suggested directory, the cd would be a no-op
        // — don't surface it. Saves the user a useless confirmation.
        let target_dir = std::env::temp_dir().join(format!(
            "{}-local-codex-{}",
            ChannelState::app_name(),
            Uuid::new_v4()
        ...
        std::fs::create_dir(&target_dir).unwrap();
        let output = format!("To stay here just run:\n\ncd {}\n", target_dir.display());
        let wd = target_dir.to_string_lossy().into_owned();
        assert!(extract_cd_command_from_text(&output, Some(&wd)).is_none());
        std::fs::remove_dir(target_dir).unwrap();
    }

    #[test]
    fn extracts_cd_command_ignores_chained_commands() {
        // `cd /x && pwd` is a multi-step instruction, not a clean ask-to-run.
        // Skip — keep this path narrow so we don't suggest weird chains.
        let target_dir = std::env::temp_dir().join(format!(
            "{}-local-codex-{}",
            ChannelState::app_name(),
            Uuid::new_v4()
        ...
        std::fs::create_dir(&target_dir).unwrap();
        let output = format!("Try:\n\ncd {} && pwd\n", target_dir.display());
        assert!(extract_cd_command_from_text(&output, None).is_none());
        std::fs::remove_dir(target_dir).unwrap();
    }

    #[test]
    fn extracts_cd_command_skips_nonexistent_paths() {
        // The model hallucinated a path that doesn't exist — don't offer to run it.
        let output = "Try this:\n\ncd /definitely/not/a/real/path/here\n";
        assert!(extract_cd_command_from_text(output, None).is_none());
    }

    #[test]
    fn parse_cd_command_line_handles_prompt_and_backticks() {
        assert_eq!(parse_cd_command_line("cd /tmp"), Some("/tmp"));
        assert_eq!(parse_cd_command_line("  cd /tmp  "), Some("/tmp"));
        assert_eq!(parse_cd_command_line("$ cd /tmp"), Some("/tmp"));
        assert_eq!(parse_cd_command_line("`cd /tmp`"), Some("/tmp"));
        assert_eq!(parse_cd_command_line("cd ~/foo"), Some("~/foo"));
        // Not a bare cd line.
        assert_eq!(parse_cd_command_line("Use cd to navigate"), None);
        assert_eq!(parse_cd_command_line("cd /tmp && pwd"), None);
    }
    ...
    #[test]
    fn extracts_labrador_tool_call_markers_from_output() {
        let output = "I'll inspect it.\nLABRADOR_TOOL_CALL {\"type\":\"run_shell_command\",\"command\":\"rg foo\",\"is_read_only\":true,\"uses_pager\":false,\"is_risky\":false,\"wait_until_complete\":true}\nThen I'll use the result.";

        let (cleaned, tool_calls) = extract_labrador_tool_calls(output);
    ...
    #[test]
    fn extracts_colon_prefixed_labrador_tool_call_markers() {
        let output =
            "LABRADOR_TOOL_CALL: {\"type\":\"run_shell_command\",\"command\":\"cargo test\"}";
    ...
    fn extracts_local_next_command_json() {
        let output = "```json\n{\"command\":\"cargo test -p labrador local_codex\"}\n```";

        assert_eq!(
            extract_local_next_command(output).as_deref(),
            Some("cargo test -p labrador local_codex")
        );
    }
