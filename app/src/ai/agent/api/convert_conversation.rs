...
use crate::ai_assistant::execution_context::{LabradorAiExecutionContext, LabradorAiOsContext};
...
use labrador_multi_agent_api::ask_user_question_result::answer_item::Answer as AskUserQuestionAnswer;
...
pub(crate) fn convert_input_context(context: Option<&api::InputContext>) -> Arc<[AIAgentContext]> {
    ...
    if let (Some(os), Some(shell)) = (&context.operating_system, &context.shell) {
        result.push(AIAgentContext::ExecutionEnvironment(
            LabradorAiExecutionContext {
                os: LabradorAiOsContext {
...
pub(crate) fn convert_tool_call_result_to_input(
    ...
    use labrador_multi_agent_api::message::tool_call_result::Result as ToolCallResultType;
    ...
    match tool_call_result.result.as_ref() {
        ...
        Some(ToolCallResultType::AskUserQuestion(result)) => {
            let ask_result = match &result.result {
                Some(labrador_multi_agent_api::ask_user_question_result::Result::Success(success)) => {
                ...
                Some(labrador_multi_agent_api::ask_user_question_result::Result::Error(err)) => {
