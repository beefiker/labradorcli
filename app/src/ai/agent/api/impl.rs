...
use super::{
    convert_to::convert_input, set_labrador_credit_usage,
    set_labrador_drive_context_enabled, ConvertToAPITypeError, RequestParams, ResponseStream,
};
...
pub async fn generate_multi_agent_output(
    ...
    if let Some(api_keys) = &mut api_keys {
        set_labrador_credit_usage(api_keys, params.allow_use_of_labrador_credits_with_byok);
    }

    let mut settings = api::request::Settings {
    ...
    set_labrador_drive_context_enabled(&mut settings, params.labrador_drive_context_enabled);
...
fn should_use_local_agent() -> bool {
    ai::local_openai_auth::has_access_token()
        ...
        || std::env::var("LABRADOR_ALLOW_OZ_AGENT").as_deref() != Ok("1")
}
