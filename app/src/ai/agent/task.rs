...
impl Task {
        ...
        match &self.data {
            TaskImpl::Server(server_data) => server_data
                ...
                .and_then(|call| match &call.metadata {
                    ...
                    Some(metadata)
                        if super::api::is_labrador_documentation_search_metadata(metadata) =>
                    {
                        None
                    }
                    Some(_) => None,
    ...
    pub fn is_labrador_documentation_search_subagent(&self) -> bool {
        match &self.data {
            TaskImpl::Server(server_data) => server_data
                .subagent_params
                .as_ref()
                .is_some_and(|params| params.call.is_labrador_documentation_search()),
            TaskImpl::Optimistic(_) => false,
        }
    }
