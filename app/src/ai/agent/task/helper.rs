...
use crate::ai::agent::api::is_labrador_documentation_search_metadata;
...
pub trait SubagentExt {
    ...
    fn is_labrador_documentation_search(&self) -> bool;
...
impl SubagentExt for api::message::tool_call::Subagent {
    ...
    fn is_labrador_documentation_search(&self) -> bool {
        self.metadata.as_ref().is_some_and(|metadata| {
            is_labrador_documentation_search_metadata(metadata)
        })
    }
    ...
    fn type_name(&self) -> &'static str {
        ...
        match &self.metadata {
            ...
            Some(metadata) if is_labrador_documentation_search_metadata(metadata) => {
                "labrador_documentation_search"
            }
            Some(_) => "unknown",
            None => "unknown",
        }
