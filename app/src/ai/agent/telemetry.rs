...
impl ForTelemetry for AIAgentCitation {
    ...
    fn for_telemetry(&self, _ctx: &AppContext) -> Option<Self::Output> {
        match self {
            // Labrador Drive cloud-hosted objects have been removed from this fork;
            // there is no longer anything to look up the citation against.
            Self::LabradorDriveObject { .. } => None,
            Self::LabradorDocumentation { path } => {
                Some(CitationForTelemetry::LabradorDocs { page: path.clone() })
            }
