...
impl AIExecutionProfileInfo {
    ...
    /// The Labrador Drive sync ID of this profile, if it has been synced.
    #[cfg_attr(target_family = "wasm", allow(dead_code))]
...
impl AIExecutionProfilesModel {
    ...
    pub fn set_autosync_plans_to_labrador_drive(
        &mut self,
        profile_id: ClientProfileId,
        enabled: bool,
        ctx: &mut ModelContext<Self>,
        ...
        self.edit_profile_internal(
            profile_id,
            |profile| {
                if profile.autosync_plans_to_labrador_drive != enabled {
                    profile.autosync_plans_to_labrador_drive = enabled;
                    return true;
                }
                false
            },
    ...
    fn edit_profile_internal(
        ...
        // Edit local unsynced/CLI profile data in-place.
        if let DefaultProfileState::Unsynced { id, profile } = &mut self.default_profile_state {
            if *id == profile_id {
