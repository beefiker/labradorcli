...
fn render_upgrade_footer(
    ...
    let info_icon = ConstrainedBox::new(
        labrador_core::ui::Icon::Info
            .to_labrador_ui_icon(text_color)
            .finish(),
    )
    .with_width(16.)
    .with_height(16.)
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            ExecutionProfileEditorViewAction::SetPlanAutoSync { enabled } => {
                AIExecutionProfilesModel::handle(ctx).update(ctx, |profiles_model, ctx| {
                    profiles_model.set_autosync_plans_to_labrador_drive(self.profile_id, *enabled, ctx);
                });
                ctx.notify();
            }
...
impl BackingView for ExecutionProfileEditorView {
    ...
    fn handle_pane_header_overflow_menu_action(
        ...
        _ctx: &mut labrador_ui::ViewContext<Self>,
    ) {
