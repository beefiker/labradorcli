...
#[cfg(feature = "local_fs")]
use labrador_ui::platform::SaveFilePickerConfiguration;
...
pub enum Artifact {
    ...
    Plan {
        document_uid: String,
        /// None until the plan is synced to Labrador Drive.
...
impl TryFrom<labrador_graphql::ai::AIConversationArtifact> for Artifact {
    type Error = ();

    fn try_from(value: labrador_graphql::ai::AIConversationArtifact) -> Result<Self, Self::Error> {
        match value {
            labrador_graphql::ai::AIConversationArtifact::PlanArtifact(plan) => Ok(Artifact::Plan {
                document_uid: plan.document_uid.into_inner(),
                notebook_uid: plan.notebook_uid.map(|id| id.into_inner()),
                title: plan.title,
            }),
            labrador_graphql::ai::AIConversationArtifact::PullRequestArtifact(pr) => {
                let (repo, number) = parse_github_pr_url(&pr.url).unzip();
            ...
            labrador_graphql::ai::AIConversationArtifact::ScreenshotArtifact(screenshot) => {
            ...
            labrador_graphql::ai::AIConversationArtifact::FileArtifact(file) => Ok(Artifact::File {
            ...
            labrador_graphql::ai::AIConversationArtifact::Unknown => Err(()),
        }
    }
}
...
pub fn open_screenshot_lightbox<V: labrador_ui::View>(
    artifact_uids: &[String],
    ctx: &mut labrador_ui::ViewContext<V>,
) {
...
pub fn download_file_artifact<V: labrador_ui::View>(
    artifact_uid: &str,
    ctx: &mut labrador_ui::ViewContext<V>,
) {
    let ai_client = ServerApiProvider::handle(ctx).as_ref(ctx).get_ai_client();
...
fn open_file_download_result<V: labrador_ui::View>(
    artifact_uid: &str,
...
#[cfg(feature = "local_fs")]
fn open_file_download_picker<V: labrador_ui::View>(
    artifact: ArtifactDownloadResponse,
    ctx: &mut labrador_ui::ViewContext<V>,
) {
    ...
    ctx.open_save_file_picker(
        move |path_opt: Option<String>, _me: &mut V, ctx: &mut labrador_ui::ViewContext<V>| {
...
fn show_file_download_toast<V: labrador_ui::View>(
    artifact_uid: &str,
