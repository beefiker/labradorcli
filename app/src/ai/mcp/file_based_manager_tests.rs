...
use crate::labrador_managed_paths_watcher::{labrador_data_dir, LabradorManagedPathsWatcher};
...
use labrador_ui::{App, Entity, ModelHandle, SingletonEntity as _};
use watcher::HomeDirectoryWatcher;

// Helper to initialize dependencies and return FileBasedMCPManager handle
fn setup_app(app: &mut App) -> labrador_ui::ModelHandle<FileBasedMCPManager> {
    app.add_singleton_model(DirectoryWatcher::new);
    app.add_singleton_model(|_| DetectedRepositories::default());
    app.add_singleton_model(RepoMetadataModel::new);
    app.add_singleton_model(HomeDirectoryWatcher::new_for_test);
    app.add_singleton_model(LabradorManagedPathsWatcher::new_for_testing);
...
/// A globally-scoped Labrador installation always auto-spawns, regardless of the
/// `file_based_mcp_enabled` toggle.
#[test]
fn test_global_labrador_server_always_spawns() {
    let _flag_guard = FeatureFlag::FileBasedMcp.override_enabled(true);
    let labrador_root = labrador_data_dir();
    let parsed = parse_mcp_json(r#"{"global-labrador": {"command": "npx", "args": ["labrador"]}}"#);

    App::test((), |mut app| async move {
        let manager = setup_app(&mut app);
        let events = subscribe_events(&mut app, &manager);

        // Toggle is off by default; global Labrador server should still spawn.
        manager.update(&mut app, |m, ctx| {
            m.apply_parsed_servers(labrador_root.clone(), MCPProvider::Labrador, parsed, ctx);
        });
        ...
        events.update(&mut app, |e, _| {
            assert_eq!(
                e.spawned_uuids.len(),
                1,
                "Global Labrador server should auto-spawn regardless of toggle"
            );
        });

        // Flipping the toggle must not despawn the global Labrador server.
        ...
        events.update(&mut app, |e, _| {
            assert!(
                ...
                "Global Labrador server should never be despawned by toggle changes, got: {:?}",
                e.despawned_uuids
            );
        });
    });
...
/// A globally-scoped non-Labrador installation only auto-spawns when the toggle is on.
#[test]
fn test_global_non_labrador_server_respects_toggle() {
    let _flag_guard = FeatureFlag::FileBasedMcp.override_enabled(true);
    ...
    App::test((), |mut app| async move {
        ...
        events.update(&mut app, |e, _| {
            assert!(
                ...
                "Global non-Labrador server must not auto-spawn while toggle is off, got: {:?}",
                e.spawned_uuids
            );
        });
        ...
        // Toggle on: the global non-Labrador server should be spawned.
        set_file_based_mcp_enabled(&mut app, true);
        events.update(&mut app, |e, _| {
            assert_eq!(
                e.spawned_uuids,
                vec![installation_uuid],
                "Global non-Labrador server should spawn when toggle flips on"
            );
        });

        // Toggle off: the global non-Labrador server should be despawned.
        set_file_based_mcp_enabled(&mut app, false);
        events.update(&mut app, |e, _| {
            assert_eq!(
                e.despawned_uuids,
                vec![installation_uuid],
                "Global non-Labrador server should despawn when toggle flips off"
            );
        });
    });
}

/// Project-scoped installations (both Labrador and third-party) never auto-spawn on
/// detection, and the toggle must not spawn or despawn them either.
#[test]
fn test_project_scoped_servers_never_auto_spawn() {
    let _flag_guard = FeatureFlag::FileBasedMcp.override_enabled(true);
    let repo_path = PathBuf::from("/tmp/labrador-test-repo");
    let claude_parsed =
        parse_mcp_json(r#"{"proj-claude": {"command": "npx", "args": ["proj-claude"]}}"#);
    let labrador_parsed =
        parse_mcp_json(r#"{"proj-labrador": {"command": "npx", "args": ["proj-labrador"]}}"#);
...
/// is considered global (and thus gated only by the toggle for non-Labrador providers).
#[test]
fn test_server_referenced_from_both_global_and_project_is_global() {
    ...
    let repo_path = PathBuf::from("/tmp/labrador-test-repo-shared");
