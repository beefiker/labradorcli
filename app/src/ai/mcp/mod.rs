...
pub(crate) fn home_config_file_path(provider: MCPProvider) -> Option<PathBuf> {
    match provider {
        MCPProvider::Labrador => labrador_core::paths::labrador_home_mcp_config_file_path(),
...
impl MCPProvider {
    pub fn display_name(&self) -> String {
        match self {
            MCPProvider::Labrador => ChannelState::app_name_display().to_string(),
    ...
    pub fn home_config_path(&self) -> &'static Path {
        match self {
            MCPProvider::Labrador => Path::new(".labrador/.mcp.json"),
...
pub fn mcp_provider_from_file_path(file_path: &Path) -> Option<MCPProvider> {
    ...
    // This avoids `.mcp.json` shadowing `.labrador/.mcp.json`, for example.
...
mod tests {
    ...
    #[test]
    fn mcp_provider_from_file_path_recognizes_labrador_home_path() {
        if let Some(labrador_home_mcp_config_file_path) =
            labrador_core::paths::labrador_home_mcp_config_file_path()
        {
            assert_eq!(
                mcp_provider_from_file_path(&labrador_home_mcp_config_file_path),
                Some(MCPProvider::Labrador)
            );
        }
    }
}
