...
use crate::{
    ...
    labrador_managed_paths_watcher::labrador_data_dir,
...
impl FileBasedMCPManager {
    ...
    /// - For `MCPProvider::Labrador`: `labrador_data_dir()` (i.e. `~/.labrador/.mcp.json`).
        ...
        self.file_based_servers_by_root
            ...
            .any(|(root_path, provider_map)| {
                provider_map.iter().any(|(provider, hashes)| {
                    ...
                    match provider {
                        MCPProvider::Labrador => root_path == &labrador_root,
    ...
    /// Labrador config (`~/.labrador/.mcp.json`). Global Labrador servers always auto-spawn.
    fn is_global_labrador_server(&self, hash: u64) -> bool {
        let labrador_root = labrador_data_dir();
        self.file_based_servers_by_root
            .get(&labrador_root)
            .and_then(|provider_map| provider_map.get(&MCPProvider::Labrador))
    ...
    fn spawn_file_based_servers(
        ...
        // - Global Labrador: always auto-spawn.
        // - Global non-Labrador: auto-spawn iff the toggle is on.
        ...
        for installation in servers_to_spawn {
            ...
            if self.is_global_labrador_server(hash) || (self.is_global_server(hash) && mcp_enabled)
            {
    ...
    fn handle_file_based_mcp_enabled_change(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        // - Global Labrador servers always spawn regardless of the toggle.
        ...
        let global_third_party_servers: Vec<_> = self
            ...
            .iter()
            .filter(|(hash, _)| {
                self.is_global_server(**hash) && !self.is_global_labrador_server(**hash)
            })
        ...
        if !AISettings::as_ref(ctx).is_file_based_mcp_enabled(ctx) {
            ...
            // Toggle on: spawn global third-party servers (global Labrador servers are
    ...
    /// exception: global Labrador installs are discovered in `~/.labrador/` (Labrador's data
    ...
    /// - Global installations (`~/.labrador/.mcp.json`, `~/.claude.json`, etc.): the
    ///   home directory.
    ///
    ...
    pub fn spawn_root_for_installation(&self, uuid: Uuid) -> Option<PathBuf> {
        ...
        // Global Labrador installs live under `~/.labrador/`, which is internal Labrador state
        // rather than a meaningful working directory. Map them to the home dir so
        // all global installs (Labrador and third-party) share a consistent cwd.
        if discovery_root == labrador_data_dir() {
