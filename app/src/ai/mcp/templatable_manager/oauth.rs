...
/// updates to Labrador's secure storage via a channel.
///
...
/// runtime token auto-refreshes are written back to Labrador's secure storage.
///
...
impl TemplatableMCPServerManager {
    ...
    pub fn handle_oauth_callback(&mut self, url: &Url) -> anyhow::Result<()> {
        ...
        labrador_ui::r#async::block_on(server_info.oauth_result_tx.send(result)).map_err(|_| {
...
pub(crate) fn load_credentials_from_secure_storage<T: DeserializeOwned + Default>(
    ...
    app.secure_storage()
        .read_value(key)
        .inspect_err(|err| {
            if !matches!(err, labrador_ui_extras::secure_storage::Error::NotFound) {
