...
use crate::ai::mcp::{Author, JsonTemplate, MCPGalleryManager, MCPServerUpdate};
...
/// An MCP server integration that Labrador ships with bundled skills for.
...
impl TemplatableMCPServerManager {
    ...
    pub fn get_creator(&self, _template_uuid: Uuid, _app: &AppContext) -> Option<String> {
        None
    }
    ...
    fn spawn_server_impl(
        ...
        if let TransportType::CLIServer(cli_server) = &mut server.transport_type {
            ...
            // (repo root for project-scoped configs, ~/.labrador/ or ~ for globals). This
            ...
            // continue to inherit Labrador's process cwd.
...
fn make_client_info() -> rmcp::model::ClientInfo {
    rmcp::model::ClientInfo {
        ...
        client_info: rmcp::model::Implementation {
            name: labrador_core::channel::ChannelState::app_id().to_string(),
            version: labrador_core::channel::ChannelState::app_version()
                .map(|v| v.to_string())
                .unwrap_or_default(),
            title: None,
            icons: None,
