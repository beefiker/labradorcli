...
/// `.labrador/.mcp.json`), capturing the parent directory component.
...
impl FileMCPWatcher {
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
        ...
        if let Some(mcp_config_path) = labrador_managed_mcp_config_path() {
    ...
    fn watch_home_provider_dir(
        ...
        let Ok(std_path) =
            labrador_util::standardized_path::StandardizedPath::from_local_canonicalized(subdir_path)
        else {
            return;
        };
    ...
    fn handle_labrador_managed_paths_event(
        ...
        let Some(mcp_config_path) = labrador_managed_mcp_config_path() else {
            return;
        };
...
async fn parse_mcp_config_file(
    ...
    let json = match provider {
        ...
        MCPProvider::Claude | MCPProvider::Labrador | MCPProvider::Agents => file_contents,
    };
