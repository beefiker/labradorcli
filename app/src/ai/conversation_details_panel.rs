...
use labrador_cli::skill::SkillSpec;
...
impl ConversationDetailsPanel {
    ...
    fn render_creator_section(&self, appearance: &Appearance) -> Option<Box<dyn Element>> {
        ...
        let avatar = Avatar::new(
            avatar_content,
            labrador_ui::ui_components::components::UiComponentStyles {
                width: Some(20.),
                height: Some(20.),
                border_radius: Some(labrador_ui::elements::CornerRadius::with_all(
                    labrador_ui::elements::Radius::Percentage(50.),
                )),
    ...
    fn render_status_section(&self, appearance: &Appearance) -> Option<Box<dyn Element>> {
        ...
        let status_icon = ConstrainedBox::new(icon.to_labrador_ui_icon(color.into()).finish())
    ...
    fn render_skill_section(&self, appearance: &Appearance) -> Option<Box<dyn Element>> {
        ...
        let icon = render_labrador_icon(20., 3.);
    ...
    fn record_copy(&mut self, kind: CopyButtonKind, ctx: &mut ViewContext<Self>) {
        ...
        ctx.spawn(
            async move {
                labrador_ui::r#async::Timer::after(duration).await;
            },
            |me, _, ctx| {
                ctx.notify();
...
impl View for ConversationDetailsPanel {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        if let Some(CreditsInfo::LocalConversation(credits)) = &self.data.credits {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            ConversationDetailsPanelAction::OpenInOz => {
                // Oz run URLs were tied to the deleted PanelMode::Task variant.
            }
        }
    }
}
