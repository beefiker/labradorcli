...
impl AgentInputFooter {
    ...
    fn select_cli_file(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let file_picker_config = labrador_ui::platform::FilePickerConfiguration::new();
        ...
        Fut: std::future::Future<Output = Result<(), PluginInstallError>> + Send + 'static,
    {
        ...
        let (shell_path, shell_type) = match shell_data {
            ...
            // needing the Labrador plugin to integrate with it.
        ...
        if let Some(agent) = self.cli_agent(app) {
            if let Some(icon) = agent.icon() {
                ...
                left_buttons.add_child(
                    Container::new(
                        ConstrainedBox::new(icon.to_labrador_ui_icon(Fill::Solid(icon_color)).finish())
    ...
    pub fn toggle_cli_voice_input(
        ...
        if let voice_input::VoiceInputToggledFrom::Key { state } = source {
            match (&self.cli_voice_input_state, state) {
                (CLIVoiceInputState::Stopped, labrador_ui::event::KeyState::Released) => return,
                (CLIVoiceInputState::Listening, labrador_ui::event::KeyState::Pressed) => return,
                _ => {}
            }
        }
    ...
    fn sync_fast_forward_button(&self, ctx: &mut ViewContext<Self>) {
        // Read directly from the conversation, same data source as the progress
        ...
        .with_child(
            ConstrainedBox::new(
                Icon::CalloutTriangleBorderDown
                    .to_labrador_ui_icon(Fill::Solid(theme.accent().into_solid()))
                    .finish(),
            )
            .with_width(24.)
            .with_height(24.)
        ...
        .with_child(
            ConstrainedBox::new(
                Icon::CalloutTriangleFillDown
                    .to_labrador_ui_icon(background)
                    .finish(),
            )
            .with_width(24.)
            .with_height(24.)
...
impl TypedActionView for AgentInputFooter {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut labrador_ui::ViewContext<Self>) {
        match action {
            #[cfg(feature = "voice_input")]
...
impl ActionButtonTheme for AgentInputButtonTheme {
    ...
    fn font_properties(&self) -> Option<labrador_ui::fonts::Properties> {
        if crate::features::FeatureFlag::CloudModeInputV2.is_enabled() {
            Some(labrador_ui::fonts::Properties {
                weight: labrador_ui::fonts::Weight::Semibold,
                ..Default::default()
            })
        } else {
            None
...
async fn write_install_log(agent: CLIAgent, err: &PluginInstallError) -> Option<PathBuf> {
    let log_path = env::temp_dir().join("labrador-plugin-install.log");
