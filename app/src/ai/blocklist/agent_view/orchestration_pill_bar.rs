...
use labrador_core::ui::{appearance::Appearance, theme::LabradorTheme};
...
use labrador_ui::{AppContext, Entity, ModelHandle, SingletonEntity, View, ViewContext};
...
fn pill_palette(theme: &LabradorTheme) -> [ColorU; 6] {
    [
        theme.ansi_fg_blue(),
...
fn pill_avatar_color(name: &str, theme: &LabradorTheme) -> ColorU {
...
pub struct OrchestrationPillBar {
    ...
    /// and silently swallowing the click (per the Labrador UI mouse-state rule).
...
fn render_pill(
    ...
    // `crates/labrador_ui_core/src/elements/hoverable.rs`). We can therefore move
    ...
    Hoverable::new(mouse_state, move |hover_state| {
        let (background, text_color) = if is_selected {
            ...
            (
                labrador_core::ui::theme::color::internal_colors::neutral_3(theme),
                labrador_core::ui::theme::color::internal_colors::text_main(theme, theme.background()),
            )
        } else {
            (
                labrador_core::ui::theme::color::internal_colors::neutral_2(theme),
                labrador_core::ui::theme::color::internal_colors::text_main(theme, theme.background()),
            )
        };
...
fn render_avatar_disc(
    ...
    let glyph_element: Box<dyn Element> = match glyph {
        ...
        AvatarGlyph::Icon(icon) => {
            ConstrainedBox::new(icon.to_labrador_ui_icon(theme.background()).finish())
                .with_width(10.)
                .with_height(10.)
                .finish()
        }
...
/// and silently break clicks (per the Labrador UI mouse-state guidance).
pub fn render_orchestration_breadcrumbs(
    ...
    let chevron = ConstrainedBox::new(
        Icon::ChevronRight
            .to_labrador_ui_icon(chevron_color.into())
            .finish(),
    )
    .with_width(16.)
    .with_height(16.)
