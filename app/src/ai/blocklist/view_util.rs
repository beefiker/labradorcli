...
use crate::{
    themes::theme::{AnsiColorIdentifier, Fill, LabradorTheme},
    ui_components::icons::Icon,
};

use labrador_ui::elements::ChildAnchor;
use labrador_ui::elements::Hoverable;
use labrador_ui::elements::OffsetPositioning;
use labrador_ui::elements::ParentAnchor;
use labrador_ui::elements::ParentOffsetBounds;
use labrador_ui::elements::Stack;
...
/// input with AI mode).
pub fn ai_brand_color(theme: &LabradorTheme) -> ColorU {
...
/// exceeded" chip).
pub fn error_color(theme: &LabradorTheme) -> ColorU {
        ...
        if state.is_hovered() {
            ...
            let tool_tip = appearance
                ...
                .with_style(UiComponentStyles {
                    font_size: Some(12.),
                    background: Some(labrador_ui::elements::Fill::Solid(tooltip_background)),
...
fn render_input_icon(icon: Icon, color: Fill, app: &AppContext) -> Box<dyn Element> {
    ...
    ConstrainedBox::new(
        Container::new(icon.to_labrador_ui_icon(color).finish())
    ...
    F: FnMut(&mut EventContext) + 'static,
{
    ...
    let mut label_children = vec![ConstrainedBox::new(icon.to_labrador_ui_icon(color).finish())
