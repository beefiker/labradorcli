...
use labrador_core::assertions::safe_assert;
use labrador_multi_agent_api::{message, Task, ToolType};
...
impl SessionContext {
    ...
    /// Returns the remote host ID if this is a `LabradorifiedRemote` session with
    /// a connected `RemoteServerClient`.
    pub fn host_id(&self) -> Option<&labrador_core::HostId> {
        match &self.session_type {
            Some(SessionType::LabradorifiedRemote { host_id }) => host_id.as_ref(),
    ...
    pub fn is_remote(&self) -> bool {
        matches!(
            self.session_type,
            Some(SessionType::LabradorifiedRemote { .. })
        )
    }
...
pub struct BlocklistAIController {
    ...
    /// conversation, because the ambient agent task driver owns the entire Labrador window working on a task, and any
    ...
    /// Set by the agent driver based on the workspace directory (e.g. `{working_dir}/.labrador/attachments`).
...
impl BlocklistAIController {
            ...
            ResponseStreamEvent::ReceivedEvent(event) => {
                ...
                match event {
                    Ok(event) => {
                        ...
                        match event {
                            labrador_multi_agent_api::response_event::Type::Init(init_event) => {
                                history_model.update(ctx, |history_model, ctx| {
                            ...
                            labrador_multi_agent_api::response_event::Type::Finished(
                                finished_event,
                            ) => {
                            ...
                            labrador_multi_agent_api::response_event::Type::ClientActions(actions) => {
    ...
    pub(super) fn handle_response_stream_finished(
        &mut self,
        stream_id: &ResponseStreamId,
        mut finished_event: labrador_multi_agent_api::response_event::StreamFinished,
        conversation_id: AIConversationId,
        ...
        match finished_event.reason {
            Some(labrador_multi_agent_api::response_event::stream_finished::Reason::Done(_)) | None => {
                history_model.update(ctx, |history_model, ctx| {
            ...
            Some(labrador_multi_agent_api::response_event::stream_finished::Reason::Other(_)) => {
            ...
            Some(labrador_multi_agent_api::response_event::stream_finished::Reason::ContextWindowExceeded(_)) => {
            ...
            Some(labrador_multi_agent_api::response_event::stream_finished::Reason::QuotaLimit(_)) => {
                history_model.update(ctx, |history_model, ctx| {
                    history_model.mark_response_stream_completed_with_error(
            ...
            Some(labrador_multi_agent_api::response_event::stream_finished::Reason::LlmUnavailable(_)) => {
            ...
            Some(labrador_multi_agent_api::response_event::stream_finished::Reason::InvalidApiKey(details)) => {
                use labrador_multi_agent_api::LlmProvider;
            ...
            Some(labrador_multi_agent_api::response_event::stream_finished::Reason::InternalError(
                labrador_multi_agent_api::response_event::stream_finished::InternalError{ message})) => {
            ...
            Some(labrador_multi_agent_api::response_event::stream_finished::Reason::MaxTokenLimit(_)) => {
                let error_message = "Input exceeded context window limit.";
                history_model.update(ctx, |history_model, ctx| {
                    history_model.mark_response_stream_completed_with_error(
                        RenderableAIError::ContextWindowExceeded(error_message.to_owned()),
    ...
    let intended_agent = BlocklistAIHistoryModel::as_ref(app)
        ...
        .and_then(|task| {
            if task.is_root_task() {
                Some(labrador_multi_agent_api::AgentType::Primary)
            } else if task.is_cli_subagent() {
                Some(labrador_multi_agent_api::AgentType::Cli)
            } else {
                None
            }
        });
