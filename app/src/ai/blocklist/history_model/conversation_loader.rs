...
fn box_future<F>(f: F) -> labrador_ui::r#async::BoxFuture<'static, Option<CloudConversationData>>
where
    F: Future<Output = Option<CloudConversationData>> + labrador_ui::r#async::Spawnable,
{
    cfg_if::cfg_if! {
...
impl BlocklistAIHistoryModel {
    ...
    pub fn load_conversation_data(
        ...
        conversation_id: AIConversationId,
        ctx: &AppContext,
    ) -> labrador_ui::r#async::BoxFuture<'static, Option<CloudConversationData>> {
            ...
            .filter_map(|agent_conv| {
                ...
                for task in &agent_conv.tasks {
                    for message in &task.messages {
                        match &message.message {
                            Some(labrador_multi_agent_api::message::Message::UserQuery(_)) => {
                                has_user_query = true;
                            }
                            Some(labrador_multi_agent_api::message::Message::SystemQuery(sys)) => {
                                if let Some(labrador_multi_agent_api::message::system_query::Type::AutoCodeDiff(_)) = &sys.r#type {
                ...
                let initial_query = root_task.map(|task| {
                    ...
                    task.messages.iter().find_map(|msg| {
                        match &msg.message {
                            Some(labrador_multi_agent_api::message::Message::UserQuery(user_query)) => {
                                Some(user_query.query.clone())
                            }
                            Some(labrador_multi_agent_api::message::Message::ToolCall(tool_call))  => {
                                let Some(tool) = &tool_call.tool else {
                                    return None;
                                };

                                if let labrador_multi_agent_api::message::tool_call::Tool::ApplyFileDiffs(diff_suggestion) = tool {
