...
use labrador_ui::elements::get_rich_content_position_id;
use labrador_ui::elements::ClippedScrollStateHandle;
use labrador_ui::elements::TableStateHandle;
use labrador_ui::ui_components::radio_buttons::RadioButtonStateHandle;
...
use labrador_ui::elements::SecretRange;
use labrador_ui::ui_components::button::ButtonVariant;
use labrador_ui::ui_components::button::TextAndIcon;
use labrador_ui::ui_components::button::TextAndIconAlignment;
...
#[cfg(feature = "local_fs")]
use labrador_editor::content::edit::resolve_asset_source_relative_to_directory;
use labrador_editor::{
...
use crate::ai::agent::{
    ...
    CreateDocumentsResult, DocumentToCreate, EditDocumentsResult, LocalCLIToolOutput,
    ProgrammingLanguage, RequestCommandOutputResult, SuggestedLoggingId, SummarizationType,
};
...
impl CollapsibleElementState {
    ...
    fn sync_local_cli_tool_output(&mut self, output: &LocalCLIToolOutput) {
        let has_body = !output.body.trim().is_empty();
        let was_finished = self.last_known_is_finished;

        self.sync_finished_state(output.is_complete);

        if !has_body || self.user_toggled_while_streaming {
            return;
        }

        let should_auto_expand = if output.is_complete {
            !was_finished
        } else {
        ...
        if should_auto_expand {
            self.expansion_state = CollapsibleExpansionState::Expanded {
                is_finished: output.is_complete,
                scroll_pinned_to_bottom: !output.is_complete,
            };
        }
    }
...
pub struct AIBlock {
    ...
    window_id: labrador_ui::WindowId,
...
impl AIBlock {
    ...
    fn handle_updated_output(&mut self, output: &AIAgentOutput, ctx: &mut ViewContext<Self>) {
        ...
        for message in &output.messages {
            ...
            if let AIAgentOutputMessageType::LocalCLIToolOutput(output) = &message.message {
                let entry = self
                    .collapsible_block_states
                    .entry(message.id.clone())
                    .or_insert_with(|| CollapsibleElementState {
                ...
                entry.sync_local_cli_tool_output(output);
            }
                ...
                move |is_selected, app| {
                    ...
                    if is_selected {
                        ...
                        button = button.with_text_and_icon_label(TextAndIcon::new(
                            ...
                            accept_text.clone(),
                            Icon::CornerDownLeft.to_labrador_ui_icon(appearance.theme().foreground()),
                            MainAxisSize::Max,
        ...
        ctx.subscribe_to_view(&view, move |me, view, event, ctx| {
            match event {
                ...
                CodeDiffViewEvent::OpenSkill { reference, path } => {
                    #[cfg(feature = "local_fs")]
                    {
                        ctx.emit(AIBlockEvent::OpenCodeInLabrador {
...
pub enum AIBlockEvent {
    ...
    /// Open a Labrador Text instance with the requested code diff.
    OpenCodeWithDiff {
        view: ViewHandle<CodeDiffView>,
    },
    ...
    OpenDetectedFilePath {
        ...
        line_and_column_num: Option<labrador_util::path::LineAndColumnArg>,
...
pub enum AIBlockAction {
    ...
    /// The `source_view_id` will be `None` if the event is dispatched by the [`labrador_ui::elements::SelectableArea`]
    ...
    CopyDebugId(String),
    /// Open Labrador feedback documentation
    OpenFeedbackDocs,
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            AIBlockAction::OpenFeedbackDocs => {
                ctx.open_url("https://docs.labrador.dev/support-and-community/troubleshooting-and-support/sending-us-feedback");
            }
            ...
            AIBlockAction::OpenCodeInLabrador {
                ...
                for action_id in &use_computer_action_ids {
                    ...
                    images.push(ui_components::lightbox::LightboxImage {
                        source: ui_components::lightbox::LightboxImageSource::Resolved {
                            asset_source: labrador_ui::assets::asset_cache::AssetSource::Raw {
                                id: asset_id,
                            },
                        },
                        description: None,
