...
use labrador_multi_agent_api::response_event::{stream_finished, ClientActions};
use labrador_multi_agent_api::{client_action::Action, message::Message};
use session_sharing_protocol::common::{AgentAttachment, ParticipantId, ServerConversationToken};

use super::response_stream::ResponseStreamId;
use super::{BlocklistAIController, RequestInput};
use crate::ai::agent::api::conversation_usage_metadata;
...
impl BlocklistAIController {
    ...
    pub fn handle_shared_session_response_event(
        &mut self,
        resp: labrador_multi_agent_api::ResponseEvent,
        ctx: &mut ModelContext<Self>,
    ) {
        ...
        match kind {
            labrador_multi_agent_api::response_event::Type::Init(init) => {
            ...
            labrador_multi_agent_api::response_event::Type::Finished(finished) => {
    ...
    fn on_shared_init(
        &mut self,
        init_event: labrador_multi_agent_api::response_event::StreamInit,
        ctx: &mut ModelContext<Self>,
    ) {
    ...
    fn on_shared_client_actions(
        &mut self,
        actions: labrador_multi_agent_api::response_event::ClientActions,
        ctx: &mut ModelContext<Self>,
    ) {
    ...
    fn on_shared_finished(
        &mut self,
        finished: labrador_multi_agent_api::response_event::StreamFinished,
        ctx: &mut ModelContext<Self>,
    ) {
    ...
    pub(super) fn send_cancellation_to_viewers(&mut self, ctx: &mut ModelContext<Self>) {
            ...
            BlocklistAIHistoryModel::as_ref(ctx)
                ...
                .map(|conversation| {
                    let token_usage = conversation
                        ...
                        .collect();
                    let labrador_token_usage = conversation
                        .token_usage()
                        .iter()
                        .filter_map(|u| u.to_proto_labrador_usage())
                        .collect();
                    let byok_token_usage = conversation
                        .token_usage()
                        .iter()
                    ...
                    conversation_usage_metadata(
                        ...
                        conversation.was_summarized(),
                        conversation.credits_spent(),
                        token_usage,
                        Some(conversation.tool_usage_metadata().into()),
                        labrador_token_usage,
                        byok_token_usage,
                    )
                })
        });
        ...
        let finished_event = labrador_multi_agent_api::ResponseEvent {
            r#type: Some(labrador_multi_agent_api::response_event::Type::Finished(
                labrador_multi_agent_api::response_event::StreamFinished {
