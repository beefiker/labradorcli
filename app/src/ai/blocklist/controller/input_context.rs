...
lazy_static! {
    ...
    // Regex to match Labrador Drive objects inserted via at-context. Ex: <notebook:[workflow_id]>
...
pub(super) fn parse_context_attachments(
    ...
    for capture in DRIVE_OBJECT_ATTACHMENT_REGEX.captures_iter(query) {
            ...
            (capture.get(0), capture.get(1), capture.get(2))
        {
            ...
            // Labrador Drive objects (plan/workflow/notebook/rule) are no longer
            // resolvable; emit the reference without a payload.
            let _ = object_type_str;
    ...
    // Pending AI document attachments are no longer supported.
    let _ = context_model.pending_document_id();
...
// `get_object_attachment_payload` removed alongside cloud_object infrastructure.
// References to Labrador Drive objects are emitted without payload data.
