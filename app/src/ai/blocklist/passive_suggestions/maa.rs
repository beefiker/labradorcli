...
use labrador_ui::{Entity, EntityId, ModelContext, ModelHandle, SingletonEntity};

cfg_if::cfg_if! {
    if #[cfg(feature = "local_fs")] {
        use std::{path::PathBuf, time::Duration};
        use crate::ai::blocklist::{read_local_file_context, BlocklistAIPermissions};
        use labrador_terminal::shell::ShellLaunchData;
        ...
        use labrador_ui::r#async::FutureExt as AsyncFutureExt;
...
impl PassiveSuggestionsModel {
    ...
    fn send_request(
        ...
        trigger: PassiveSuggestionTrigger,
        supported_tools: Vec<labrador_multi_agent_api::ToolType>,
        ctx: &mut ModelContext<Self>,
    ) {
    ...
    fn handle_finished_stream(
        ...
        if status.is_done() {
            self.send_request(
                ...
                vec![labrador_multi_agent_api::ToolType::SuggestPrompt],
                ctx,
            );
        }
    }
    ...
    fn handle_user_block_completed(
        ...
        if is_prompt_suggestions_enabled {
            supported_tools.push(labrador_multi_agent_api::ToolType::SuggestPrompt);
        }
                    ...
                    move |me, candidate_paths, ctx| {
                        ...
                        me.pending_file_read_handle =
                            Some(ctx.spawn(read_files(file_locations, current_working_directory, shell), move |me, relevant_files, ctx| {
                                me.pending_file_read_handle = None;
                                supported_tools.push(labrador_multi_agent_api::ToolType::ApplyFileDiffs);
                                me.send_shell_command_completed_request(
                                    conversation_id,
                                    block_context,
                                    relevant_files,
...
enum ExtractedSuggestion {
    ...
    CodeDiff {
        apply_file_diffs: labrador_multi_agent_api::message::tool_call::ApplyFileDiffs,
    },
}
...
fn coalesce_messages_from_client_actions(
    client_actions: &[labrador_multi_agent_api::ClientAction],
) -> Vec<labrador_multi_agent_api::Message> {
    use field_mask::FieldMaskOperation;
    use std::collections::HashMap;
    use labrador_multi_agent_api as api;
    use labrador_multi_agent_api::client_action::Action;
