...
use labrador_ui::r#async::{FutureExt as AsyncFutureExt, SpawnedFutureHandle, Timer};
...
impl PassiveSuggestionsModel {
    ...
    fn maybe_generate_passive_code_diff(
        ...
        let should_skip_for_remote = self
            ...
            .as_ref(ctx)
            .session_type(ctx)
            .map(|session_type| matches!(session_type, SessionType::LabradorifiedRemote { .. }))
            .unwrap_or(true);
        ...
        let _prompt_suggestion_id = query.id;
...
fn build_prompt_suggestions_request(
    ...
    execution_context: LabradorAiExecutionContext,
    terminal_model: &Arc<FairMutex<TerminalModel>>,
