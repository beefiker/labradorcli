...
use super::{
    ...
    view_impl::common::{
        load_output_message, render_switch_control_to_user_button, render_agent_progress_indicator,
        render_agent_progress_indicator_base, ButtonProps, ForceRefreshButtonProps, MaybeShimmeringText,
        AgentProgressIndicatorProps, AgentProgressProps, WAITING_FOR_USER_INPUT_MESSAGE,
    },
};
...
use crate::{
    ...
    terminal::{
        ...
        labradorify::render::LEFT_STRIPE_WIDTH,
...
use labrador_ui::{r#async::Timer, TypedActionView};
...
pub struct BlocklistAIStatusBar {
    ...
    /// the progress indicator while the active block has a recorded LRC snapshot.
    last_read_refresh_handle: Option<SpawnedFutureHandle>,

    latest_response_stream_id: Option<ResponseStreamId>,

    /// Agent tip to display below the progress indicator.
...
impl BlocklistAIStatusBar {
    ...
    fn render_agent_progress_indicator_for_latest_exchange(
        &self,
        app: &AppContext,
        ...
        let should_render_progress = !model.request_type(app).is_passive()
        ...
        let fallback_progress_text = resolve_fallback_progress_message(
            current_is_fallback,
            current_display_name,
            model.as_ref(),
            app,
        );
        let has_fallback_progress_text = fallback_progress_text.is_some();
        let default_progress_text = fallback_progress_text.unwrap_or_else(load_output_message);
        let secondary_element = if has_fallback_progress_text {
            Some(render_fallback_explanation(model.as_ref(), app))
        } else {
            self.render_tip(app)
        };

        Some(render_agent_progress_indicator(
            AgentProgressProps {
                model: model.as_ref(),
...
fn render_agent_tip(tip: &AgentTip, app: &AppContext) -> Box<dyn Element> {
    ...
    labrador_ui::elements::FormattedTextElement::new(
        formatted_text,
    ...
    .register_default_click_handlers_with_action_support(move |link, evt, app| {
        use labrador_ui::elements::HyperlinkLens;
        match link {
...
/// If the current exchange is using a fallback model, returns the progress message to display
...
fn resolve_fallback_progress_message<V: View>(
...
impl View for BlocklistAIStatusBar {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn labrador_ui::Element> {
        let appearance = Appearance::as_ref(app);
        ...
        let status_element =
                    ...
                    .is_none()
            {
                render_agent_progress_indicator_base(
                    AgentProgressIndicatorProps {
                        ...
                        progress_indicator_text: MaybeShimmeringText::Static(
            ...
            } else if let (Some(progress_indicator), true) = (
                self.render_agent_progress_indicator_for_latest_exchange(app),
        ...
        // visible above the progress/status indicator so it doesn't disappear
...
pub enum BlocklistAIStatusBarAction {
    ...
    /// by the inline `Check now` affordance in the progress indicator.
