...
use labrador_ui::{View, ViewContext};
...
use crate::ui_components::{blended_colors, labrador_icon::render_labrador_icon};
use crate::util::link_detection::DetectedLinkType;
use crate::workspace::WorkspaceAction;
use itertools::Itertools;
use labrador_core::ui::color::contrast::{
    foreground_color_with_minimum_contrast, MinimumAllowedContrast,
};
use labrador_core::ui::color::Rgb;
use labrador_core::ui::theme::{Fill, LabradorTheme};
use labrador_ui::elements::{Highlight, HighlightedRange, Text};
...
fn create_secret_gray_highlight() -> Highlight {
    Highlight::new().with_text_style(
        TextStyle::new()
            .with_foreground_color(labrador_ui::color::ColorU::new(128, 128, 128, 255))
...
fn add_slash_command_highlight(
    ...
    if let Some(existing) = existing_highlight {
        ...
        bold_properties.weight = labrador_ui::fonts::Weight::Bold;
                ...
                .get(&location)
            {
                style_ranges = links
                    ...
                    .filter_map(|(range, link)| {
                        // Stylings like [](https://example.com) would cause Labrador to panic. We need to filter that out.
                        if range.is_empty() {
                            return None;
                        }
...
pub fn render_citation(
    ...
    let (icon, name) = match citation {
        AIAgentCitation::LabradorDriveObject { uid: _ } => {
            // Labrador Drive citations are no longer resolvable; render a stub.
            (None, String::from("Untitled"))
        }
        AIAgentCitation::LabradorDocumentation { .. } => {
            let icon = render_labrador_icon(font_size, 2.);
            let name = format!("{} Docs", ChannelState::app_name_display());
            (Some(icon), name)
        }
        AIAgentCitation::WebPage { url } => {
            let icon = Icon::LinkExternal
                .to_labrador_ui_icon(theme.foreground())
                .finish();
            let name = url.clone();
            (Some(icon), name)
        }
...
impl View for AIBlock {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let has_inserted_cloud_mode_user_query_block = false;
    ...
    fn on_focus(&mut self, focus_ctx: &labrador_ui::FocusContext, ctx: &mut ViewContext<Self>) {
