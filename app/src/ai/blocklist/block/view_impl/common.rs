...
use labrador_ui::{
    ...
    elements::{
        ...
        Flex, FormattedTextElement, HeadingFontSizeMultipliers, Hoverable, Image as LabradorImage,
...
use crate::{
    ai::{
        ...
        loading::shimmering_labrador_loading_text,
    },
...
use crate::{
    ai::{
        ...
        blocklist::{
            ...
            code_block::{
                render_code_block_plain, render_code_block_with_labrador_text, CodeBlockOptions,
...
use labrador_editor::content::{
    edit::resolve_asset_source_relative_to_directory, mermaid_diagram::mermaid_asset_source,
};
use labrador_util::path::to_relative_path;
use labrador_ui::elements::shimmering_text::ShimmeringTextStateHandle;
use labrador_ui::elements::{Highlight, HighlightedRange};
...
pub struct AgentProgressProps<'a, V> {
    ...
    /// in the progress indicator. When set, the agent's pending poll future is
    ...
    pub default_progress_text: String,
...
pub fn render_agent_progress_indicator<V: View>(
    props: AgentProgressProps<'_, V>,
    app: &AppContext,
) -> Box<dyn Element> {
            ...
            .map(|action| &action.action)
        {
            ...
            action => {
                    ...
                    && active_block.agent_interaction_metadata().is_some()
                {
                    ...
                    props.default_progress_text.clone()
                }
            }
        }
    };
    ...
    let progress_indicator_text = if !should_render_waiting_icon {
    ...
    render_agent_progress_indicator_base(
...
pub struct AgentProgressIndicatorProps {
    pub icon: Option<Box<dyn Element>>,
    pub progress_indicator_text: MaybeShimmeringText,
...
/// Helper function to render text in the agent progress footer.
...
pub fn render_agent_progress_indicator_base(
    props: AgentProgressIndicatorProps,
    app: &AppContext,
) -> Box<dyn Element> {
    let AgentProgressIndicatorProps {
        icon,
        progress_indicator_text,
        non_shimmering_text,
        non_shimmering_suffix,
    ...
    let text = render_output_status_text(progress_indicator_text, appearance, app);
    ...
    let icon = icon.or_else(|| {
        is_shimmering.then(|| render_labrador_icon(icon_size(app) - STATUS_ICON_SIZE_DELTA, 2.))
    });
...
pub fn render_output_status_text(
    ...
    let internal_element = match label {
        ...
        } => shimmering_labrador_loading_text(
            text.to_string(),
            appearance.monospace_font_size() - 2.,
            shimmering_text_handle,
            app,
...
fn render_hide_responses_button(
    ...
    render_agent_progress_indicator_button(
...
fn render_queue_next_prompt_button(
    ...
    let icon = Container::new(
        ConstrainedBox::new(Icon::ClockPlus.to_labrador_ui_icon(icon_color).finish())
            .with_height(icon_size)
            .with_width(icon_size)
            .finish(),
    )
...
fn render_auto_approve_button(props: ButtonProps, appearance: &Appearance) -> Box<dyn Element> {
    ...
    let icon = Container::new(
        ConstrainedBox::new(
            icon.to_labrador_ui_icon(appearance.theme().active_ui_text_color())
                .finish(),
        )
        .with_height(icon_size)
        .with_width(icon_size)
...
/// `Last seen by agent ...` in the progress indicator. On click, short-circuits the
...
fn render_agent_progress_indicator_button<F>(
    ...
    F: 'static + FnMut(&mut EventContext),
{
    ...
    if !labrador_ui::platform::is_mobile_device() {
...
struct VisualMarkdownBlockOptions<A: 'static> {
    ...
    /// Labrador tooltip primitive so hovering surfaces the CommonMark image title.
...
fn render_visual_markdown_block<A: Action>(
    ...
    let image = LabradorImage::new(asset_source, CacheOption::BySize)
        .contain()
    ...
    // Wrap the rendered image in the standard Labrador tooltip when the source
    ...
    let content = if let Some(tooltip) = options.tooltip.clone() {
        ...
        appearance.ui_builder().tool_tip_on_element(
            ...
            mouse_state,
            content,
            labrador_ui::elements::ParentAnchor::TopMiddle,
            labrador_ui::elements::ChildAnchor::BottomMiddle,
...
fn render_visual_card(
    ...
    let header_icon = ConstrainedBox::new(icon.to_labrador_ui_icon(header_text_color.into()).finish())
        .with_width(16.)
        .with_height(16.)
        .finish();
...
fn visual_section_height(app: &AppContext) -> f32 {
    ...
    appearance.ui_font_size()
        * labrador_ui::elements::DEFAULT_UI_LINE_HEIGHT_RATIO
...
fn render_table_section(
    ...
    let table_element = Table::new(table_handles.state_handle.clone(), 0.0, 0.0)
        ...
        .with_config(TableConfig {
            ...
            row_background: labrador_ui::elements::RowBackground {
...
struct TableCellProps {
    ...
    font_family: labrador_ui::fonts::FamilyId,
    font_size: f32,
    font_weight: Weight,
    text_color: ColorU,
...
pub fn render_code_output_section<A: Action>(
    ...
    match props.editor_view {
        Some(view) => render_code_block_with_labrador_text(
            CodeBlockOptions {
...
pub(crate) fn resolve_absolute_file_path(
    ...
    use labrador_util::path::CleanPathResult;
...
pub fn render_failed_output(props: FailedOutputProps, app: &AppContext) -> Box<dyn Element> {
    ...
    let error_text = match props.error {
        ...
        RenderableAIError::InternalLabradorError => {
            format!(
                "{ERROR_APOLOGY_TEXT}\n\nInternal {} error.",
                ChannelState::app_name_display()
            )
...
fn render_invalid_api_key_error(
    ...
    let alert_icon = ConstrainedBox::new(
        Icon::AlertTriangle
            .to_labrador_ui_icon(error_color(appearance.theme()).into())
            .finish(),
    )
    .with_width(icon_size(app))
    .with_height(icon_size(app))
    ...
    let settings_button = appearance
        .ui_builder()
        .button(
            labrador_ui::ui_components::button::ButtonVariant::Outlined,
            state_handle.clone(),
        )
        ...
        .on_click(move |ctx, _, _| {
            ctx.dispatch_typed_action(WorkspaceAction::ShowSettingsPageWithSearch {
                ...
                section: Some(SettingsSection::LabradorAgent),
            });
        })
        .finish();
...
pub(crate) fn render_debug_footer<V: View>(
    ...
    let submit_button = if !is_dogfood && !is_enterprise_user {
        ...
        Some(
            appearance
                .ui_builder()
                .button(
                    labrador_ui::ui_components::button::ButtonVariant::Text,
    ...
    let copy_button_with_tooltip = appearance.ui_builder().tool_tip_on_element(
        ...
        copy_button,
        labrador_ui::elements::ParentAnchor::TopRight,
        labrador_ui::elements::ChildAnchor::BottomRight,
        vec2f(0., -8.),
    );
...
fn markdown_table_appearance_placeholder(appearance: &Appearance) -> MarkdownTableAppearance {
    ...
    MarkdownTableAppearance {
        ...
        text_color: fg,
        border_color: labrador_core::ui::theme::color::internal_colors::neutral_3(theme),
        cell_padding: 8.0,
        outer_border: true,
        column_dividers: true,
        row_dividers: true,
        header_background: labrador_core::ui::theme::color::internal_colors::neutral_2(theme),
        cell_background: bg,
        alternate_row_background: labrador_core::ui::theme::color::internal_colors::neutral_2(theme),
    }
}
