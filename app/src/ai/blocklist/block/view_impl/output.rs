...
#[allow(unused_imports)]
use labrador_util::path::{common_path, CleanPathResult};
use labrador_ui::elements::new_scrollable::SingleAxisConfig;
        ...
        | AIBlockOutputStatus::Failed { .. } => {
            if let Some(output) = status.output_to_render() {
                ...
                fn open_code_block_action(source: CodeSource) -> AIBlockAction {
                    AIBlockAction::OpenCodeInLabrador { source }
                }
                        ...
                        Ordering::Greater => {
                            output_items.add_child(
                                render_informational_footer(
                                    app,
                                    format!(
                                        "Sorry you had a bad experience with this interaction. We appreciate your feedback. Refunded requests: {request_refunded_count}."
                                    ),
                                )
                                .with_agent_output_item_spacing(app)
                                .finish(),
    ...
    if let Some(skill) = skill {
        if !skill.is_bundled() {
            ...
            let open_button = render_skill_button(
                ...
                skill_icon_override,
                move |ctx| {
                    ctx.dispatch_typed_action(AIBlockAction::OpenCodeInLabrador {
                        source: source.clone(),
                    });
                },
            );
...
fn render_stopped_output(props: Props, app: &AppContext) -> Box<dyn Element> {
        ...
        && FeatureFlag::AIResumeButton.is_enabled()
    {
        ...
        let play_icon = Container::new(
            ConstrainedBox::new(Icon::Play.to_labrador_ui_icon(theme.foreground()).finish())
        ...
        let resume_button = labrador_ui::ui_components::button::Button::new(
...
fn render_references_footer(
    ...
    let title_row = Flex::row()
        ...
        .with_child(
            ConstrainedBox::new(chevron.to_labrador_ui_icon(title_row_color).finish())
...
fn render_collapsible_header(
    ...
    let expandable = Hoverable::new(toggle_mouse_state, move |_is_hovered| {
        Flex::row()
            ...
            .with_child(
                Container::new(
                    ConstrainedBox::new(chevron_icon.to_labrador_ui_icon(text_color.into()).finish())
