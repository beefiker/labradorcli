...
impl ConversationUsageView {
    ...
    /// Handles both category-based fields and legacy token totals.
    fn collect_models_by_category(&self) -> HashMap<String, Vec<(String, bool)>> {
        ...
        for model in &self.usage_info.models {
            for (category, &tokens) in &model.labrador_token_usage_by_category {
                if tokens > 0 {
                    entries_by_category
        ...
        for category in categories {
            ...
            for (i, (model_id, is_byok)) in sorted_models.iter().enumerate() {
                ...
                if *is_byok {
                    model_elements.push(
                        ConstrainedBox::new(
                            Icon::Key.to_labrador_ui_icon(text_color.into()).finish(),
                        )
