...
use labrador_ui::r#async::{Spawnable, Timer};
...
impl ShellCommandExecutor {
    ...
    fn handle_terminal_model_event(&mut self, event: &ModelEvent, _ctx: &mut ModelContext<Self>) {
        ...
        // the shell relays current working directory to Labrador.
    ...
    fn action_result_future(
        ...
        enum WakeReason {
            BlockFinished,
            Timeout,
            /// User clicked `Check now` in the progress indicator, short-circuiting
    ...
    ///
    /// Called by the `Check now` affordance in the progress indicator. No-ops if there
