...
use labrador_cli::mcp::MCPSpec;
use serde_json::{Map, Value};

use crate::ai::mcp::TemplatableMCPServer;

const LABRADOR_ID_KEY: &str = "labrador_id";
...
///
/// Notes:
/// - UUID specs are coerced into `{"<uuid>": {"labrador_id": "<uuid>"}}`.
        ...
        match spec {
            MCPSpec::Uuid(uuid) => {
                ...
                insert_unique(
                    ...
                    Value::Object({
                        let mut obj = Map::new();
                        obj.insert(LABRADOR_ID_KEY.to_string(), Value::String(name));
                        obj
                    }),
                )?;
            }
...
fn validate_server_config(server_name: &str, config: &Value) -> anyhow::Result<()> {
    ...
    let has_labrador_id = obj.contains_key(LABRADOR_ID_KEY);
    let has_command = obj.contains_key("command");
    let has_url = obj.contains_key("url");

    let kind_count = usize::from(has_labrador_id) + usize::from(has_command) + usize::from(has_url);
    if kind_count != 1 {
        anyhow::bail!(
            "MCP server '{server_name}' must have exactly one of: 'labrador_id', 'command', or 'url'"
        );
    }
    ...
    if has_labrador_id {
        let labrador_id = obj
            ...
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "MCP server '{server_name}' field '{LABRADOR_ID_KEY}' must be a string"
                )
            })?;

        uuid::Uuid::parse_str(labrador_id).with_context(|| {
            format!("MCP server '{server_name}' field '{LABRADOR_ID_KEY}' must be a UUID")
        })?;
    }
