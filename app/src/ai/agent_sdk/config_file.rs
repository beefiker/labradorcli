...
use crate::ai::agent_sdk::AgentConfigSnapshot;
...
///
/// Behavior:
/// - Entries with `labrador_id` become `MCPSpec::Uuid`.
...
pub fn mcp_specs_from_mcp_servers(
    ...
    for (name, config) in mcp_servers {
        ...
        let labrador_id = obj.get("labrador_id").and_then(Value::as_str);

        if let Some(labrador_id) = labrador_id {
            let uuid = uuid::Uuid::parse_str(labrador_id).map_err(|_| {
                anyhow::anyhow!("MCP server '{name}' field 'labrador_id' must be a UUID")
            })?;
            uuids.push(uuid);
        } else {
