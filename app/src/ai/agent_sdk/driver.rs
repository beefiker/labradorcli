...
use labrador_cli::agent::{Harness, OutputFormat};
use labrador_cli::share::ShareRequest;
use labrador_core::{
    channel::{Channel, ChannelState},
    report_error, report_if_error, safe_debug, safe_info,
};
use labrador_graphql::ai::AgentTaskState;
use labrador_ui::{Entity, ModelContext, ModelHandle, ModelSpawner, SingletonEntity};
...
/// Signals to Claude child-harness hooks that Labrador already owns the background
...
/// self-managed listener path so older Labrador builds and standalone plugin
...
/// that Labrador and the Claude hook scripts share.
...
/// `AgentDriver` is a model for driving an ambient Labrador agent to completion.
///
...
pub enum AgentDriverError {
    ...
    InvalidRuntimeState,
    #[error(
        "Failed to authenticate with server - please log in via 'oz login', provide an API key via '--api-key <key>', or set the LABRADOR_API_KEY environment variable"
    )]
    NotLoggedIn,
...
impl From<labrador_ui::ModelDropped> for AgentDriverError {
    fn from(_: labrador_ui::ModelDropped) -> Self {
...
impl AgentDriver {
    pub fn new(
        ...
        if labrador_isolation_platform::detect().is_some() {
    ...
    /// installing the Labrador plugin and platform plugin, if applicable.
    ///
    ...
    async fn run_harness(
        ...
        let command_result = loop {
            futures::select! {
                ...
                _ = labrador_ui::r#async::Timer::after(HARNESS_SAVE_INTERVAL).fuse() => {
    ...
    fn handle_terminal_driver_event(
        ...
        match event {
            TerminalDriverEvent::SlowBootstrap => {
                eprintln!(
                    "Warning: Terminal session is slow to bootstrap. See https://docs.labrador.dev/support-and-community/troubleshooting-and-support/known-issues#shells to troubleshoot."
                );
            }
