...
use labrador_isolation_platform::IsolationPlatformError;
#[cfg(not(target_family = "wasm"))]
use labrador_logging::log_file_path;
use labrador_ui::ModelSpawner;
use labrador_ui::{platform::TerminationMode, AppContext, SingletonEntity};

use crate::{auth::AuthStateProvider, server::server_api::ServerApiProvider};
...
/// Placeholder for the deleted `cloud_object::Owner`. The Labrador Drive sync
...
use labrador_cli::agent::{Harness, Prompt, RunAgentArgs};
use labrador_cli::OZ_HARNESS_ENV;
...
fn dispatch_command(
    ...
    match command {
        ...
        CliCommand::Secret(secret_cmd) => {
            if !FeatureFlag::LabradorManagedSecrets.is_enabled() {
...
fn build_merged_config_and_task(
    ...
    let loaded_file = match args.config_file.file() {
...
fn resolve_prompt(prompt: &Prompt, ctx: &AppContext) -> Result<String, AgentDriverError> {
    match prompt {
        ...
        Prompt::SavedPrompt(workflow_id) => {
            ...
            // no longer be resolved against Labrador Drive.
            let _ = ctx;
...
impl labrador_ui::Entity for AgentDriverRunner {
    type Event = ();
}

impl labrador_ui::SingletonEntity for AgentDriverRunner {}
...
impl AgentDriverRunner {
    async fn setup_and_run_driver(
        ...
        // Wait for Labrador Drive to sync before building the task config, since
        // prompt resolution (SavedPrompt -> workflow lookup) and environment
        // resolution (CloudAmbientAgentEnvironment lookup) depend on it.
        if foreground
            .spawn(|_, ctx| common::refresh_labrador_drive(ctx))
            .await?
            .await
            .is_err()
        {
            return Err(AgentDriverError::LabradorDriveSyncFailed);
        }
    ...
    /// options, and updates the Server API provider so that all subsequent requests to the server
    ...
    async fn fetch_secrets_and_attachments(
        ...
        let task_metadata = async { Ok::<Option<()>, anyhow::Error>(None) };
        ...
        let (parent_run_id, task_conversation_id, task_harness): (
            Option<String>,
            Option<String>,
            Option<labrador_cli::agent::Harness>,
...
fn launch_command(
    ...
    let cli_name = labrador_cli::binary_name().unwrap_or_else(|| "labrador".to_string());
        ...
        match event {
            ...
            AuthManagerEvent::NeedsReauth => {
                ...
                let message = if auth_state.is_api_key_authenticated() {
                    "Your API key is invalid. Please provide a valid key via '--api-key' or the LABRADOR_API_KEY environment variable.".to_string()
                } else {
