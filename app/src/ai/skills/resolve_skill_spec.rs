...
//!
//! This module exists primarily for `labrador agent run --skill ...` (and related flows) where we need to
...
fn home_skill_dirs_for_resolution() -> Vec<PathBuf> {
    ...
    for provider in SKILL_PROVIDER_DEFINITIONS.iter() {
        if provider.provider == SkillProvider::Labrador {
            for dir in labrador_managed_skill_dirs() {
