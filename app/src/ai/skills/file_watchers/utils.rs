...
pub fn extract_skill_parent_directory(path: &Path) -> Result<PathBuf, Error> {
    let is_labrador_home_skill = path
        .file_name()
        ...
        && path
            ...
            .is_some_and(|parent| labrador_managed_skill_dirs().iter().any(|dir| parent == dir));
    if is_labrador_home_skill {
...
pub fn is_home_provider_path(path: &Path) -> bool {
    SKILL_PROVIDER_DEFINITIONS.iter().any(|provider| {
        if provider.provider == SkillProvider::Labrador {
            return labrador_managed_skill_dirs().iter().any(|dir| path == dir);
        }
