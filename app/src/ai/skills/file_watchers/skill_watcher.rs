...
use crate::labrador_managed_paths_watcher::{
    filter_repository_update_by_prefix, labrador_managed_skill_dirs, LabradorManagedPathsWatcher,
...
impl SkillWatcher {
    ...
    fn new_internal(
        ...
        // environment, Labrador-hosted or self-hosted). The CloudEnvironmentPrep source filter means
    ...
    fn register_symlink_watches(&mut self, skills: &[ParsedSkill], ctx: &mut ModelContext<Self>) {
        for skill in skills {
            ...
            let Ok(std_dir_path) =
                labrador_util::standardized_path::StandardizedPath::from_local_canonicalized(
                    &canonical_dir,
                )
            else {
                continue;
    ...
    fn handle_labrador_managed_paths_event(
        ...
        for skill_dir in labrador_managed_skill_dirs() {
    ...
    fn watch_home_provider_path(
        ...
        let Ok(std_path) =
            labrador_util::standardized_path::StandardizedPath::from_local_canonicalized(path)
        else {
            return;
        };
