...
fn extract_skill_parent_directory_different_providers() {
    ...
    let providers = [".labrador", ".claude", ".codex", ".cursor", ".gemini"];
...
fn is_home_provider_path_true_for_known_providers() {
    ...
    if let Some(path) = labrador_core::paths::labrador_home_skills_dir() {
        assert!(is_home_provider_path(&path));
    }
...
#[test]
fn extract_skill_parent_directory_returns_home_dir_for_labrador_home_skill() {
    let Some(home_dir) = dirs::home_dir() else {
        eprintln!("Skipping test: home directory not available");
        return;
    };
    let Some(labrador_home_skills_dir) = labrador_core::paths::labrador_home_skills_dir() else {
        eprintln!("Skipping test: Labrador home skills directory not available");
        return;
    };

    let skill_path = labrador_home_skills_dir.join("test-skill").join("SKILL.md");
    let result = extract_skill_parent_directory(&skill_path);
...
fn find_skill_directories_in_tree_finds_root_skills() {
    VirtualFS::test("find_root_skills", |dirs, mut vfs| {
        ...
        let root = Entry::Directory(DirectoryEntry {
            path: labrador_util::standardized_path::StandardizedPath::try_from_local(&repo).unwrap(),
...
fn find_skill_directories_in_tree_empty_repo() {
    VirtualFS::test("find_skills_empty", |dirs, mut vfs| {
        ...
        let src = Entry::Directory(DirectoryEntry {
            path: labrador_util::standardized_path::StandardizedPath::try_from_local(&repo.join("src"))
                .unwrap(),
            children: vec![],
            ignored: false,
            loaded: true,
