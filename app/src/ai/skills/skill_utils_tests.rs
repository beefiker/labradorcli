...
#[test]
fn test_skill_path_from_file_path_labrador_home_skill() {
    ...
    let labrador_home_skill = labrador_home_skills_dir
        .join("my-skill")
        .join("assets")
        .join("image.png");
    let result = skill_path_from_file_path(&labrador_home_skill);
    assert_eq!(
        result,
        Some(labrador_home_skills_dir.join("my-skill").join("SKILL.md"))
    );
}
