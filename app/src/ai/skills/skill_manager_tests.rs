use super::*;
use crate::labrador_managed_paths_watcher::LabradorManagedPathsWatcher;
...
fn get_skills_for_working_directory_scopes_subdirectory_skills() {
    ...
    App::test((), |mut app| async move {
        ...
        let canonical_repo =
            labrador_util::standardized_path::StandardizedPath::from_local_canonicalized(&repo)
                .unwrap();
...
fn cloud_environment_skills_always_included() {
    ...
    App::test((), |mut app| async move {
        ...
        let canonical_repo_a =
            labrador_util::standardized_path::StandardizedPath::from_local_canonicalized(&repo_a)
                .unwrap();
        repo_handle.update(&mut app, |repos, _ctx| {
...
Run `{{labrador_cli_binary_name}}` to connect to {{labrador_server_url}}.
"#,
    )
    .unwrap();
...
fn test_read_bundled_skills_preserves_other_content() {
    ...
    // Create a test skill with both Labrador and non-Labrador variables
    let skill_dir = skills_dir.join("test-skill");
    fs::create_dir_all(&skill_dir).unwrap();
    let skill_file = skill_dir.join("SKILL.md");
    fs::write(
...
Use {{other_var}} and {{labrador_cli_binary_name}} together.
"#,
    )
    .unwrap();
...
fn test_build_bundled_skill_context() {
    ...
    // At least 4 entries: Labrador server/CLI/scheme keys and settings_file_path.
    // settings_schema_path is only present when bundled_resources_dir() returns Some.
    assert!(context.len() >= 4);
    assert!(context.contains_key("labrador_server_url"));
    assert!(context.contains_key("labrador_cli_binary_name"));
    assert!(context.contains_key("labrador_url_scheme"));
    assert!(context.contains_key("settings_file_path"));

    assert_eq!(
        context.get("labrador_server_url").unwrap(),
        &ChannelState::server_root_url().to_string()
    );
    assert_eq!(
        context.get("labrador_cli_binary_name").unwrap(),
        ChannelState::channel().cli_command_name()
    );
    assert_eq!(
        context.get("labrador_url_scheme").unwrap(),
...
fn make_skill(name: &str, provider_dir: &str) -> ParsedSkill {
    ...
    ParsedSkill {
        ...
        .unwrap_or(SkillProvider::Labrador),
        scope: SkillScope::Project,
    }
}
