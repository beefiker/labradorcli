...
fn resolve_from_skill_dirs_by_directory_scan_resolves_home_skill_dir() -> Result<()> {
    ...
    let skill_dir = temp_dir.path().join(".labrador").join("skills");
    ...
    assert!(resolved.instructions.contains("Global Labrador skill"));
...
fn resolve_from_root_path_by_directory_scan_respects_directory_precedence() -> Result<()> {
    ...
    let labrador_skill = root.join(".labrador/skills/my-skill/SKILL.md");
    ...
    write_skill_file(
        ...
        "my-skill",
        "desc",
        "# Claude version\n\nDo not pick this when .labrador exists.",
    )?;
    write_skill_file(
        &codex_skill,
        "my-skill",
    ...
    assert!(!resolved.instructions.contains("Labrador version"));
...
fn parse_org_from_git_url_supports_ssh_and_https() {
    assert_eq!(
        parse_org_from_git_url("git@github.com:beefiker/labrador-internal.git"),
        Some("beefiker".to_string())
    );

    assert_eq!(
        parse_org_from_git_url("https://github.com/beefiker/labrador-internal.git"),
        Some("beefiker".to_string())
    );
}
...
fn resolve_simple_name_uses_directory_precedence() -> Result<()> {
    ...
    // Note: .agents/skills has highest precedence, followed by .labrador, then .claude.
    let agents_skill = root.join(".agents/skills/my-skill/SKILL.md");
    write_skill_file(
        &agents_skill,
        "my-skill",
