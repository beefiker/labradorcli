...
use labrador_ui::prelude::MouseStateHandle;
use labrador_ui::EventContext;
use labrador_ui::{AppContext, Element, SingletonEntity};

use crate::labrador_managed_paths_watcher::labrador_managed_skill_dirs;
...
/// symlinks the same skill under `~/.agents/skills/`, `~/.labrador/skills/`, `~/.claude/skills/`, etc.
///
...
pub fn skill_path_from_file_path(file_path: &Path) -> Option<PathBuf> {
    for definition in SKILL_PROVIDER_DEFINITIONS.iter() {
        let home_skill_dirs = if definition.provider == SkillProvider::Labrador {
            labrador_managed_skill_dirs()
        } else {
