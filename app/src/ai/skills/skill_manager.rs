...
pub struct SkillManager {
    ...
    /// Skills bundled into Labrador, each with activation condition and icon.
...
impl SkillManager {
    ...
    /// Load skill definitions bundled with Labrador.
    async fn load_bundled_skills() -> HashMap<String, BundledSkill> {
        let Some(resources_dir) = labrador_core::paths::bundled_resources_dir() else {
            return HashMap::new();
        };
...
///
/// Supported variables:
/// - `{{labrador_server_url}}` - The server root URL
/// - `{{labrador_cli_binary_name}}` - The CLI binary name
/// - `{{labrador_url_scheme}}` - The URL scheme
...
fn build_bundled_skill_context() -> HashMap<String, String> {
    ...
    let cli_binary_name = ChannelState::channel().cli_command_name().to_owned();
    let url_scheme = ChannelState::url_scheme().to_owned();
    let mut context: HashMap<String, String> = [
        ("labrador_server_url".to_owned(), server_root_url),
        ("labrador_cli_binary_name".to_owned(), cli_binary_name),
        ("labrador_url_scheme".to_owned(), url_scheme),
        (
    ...
    if let Some(schema_path) =
        labrador_core::paths::bundled_resources_dir().map(|dir| dir.join("settings_schema.json"))
    {
        context.insert(
...
/// branded icon; everything else falls back to the Labrador logo.
fn icon_for_bundled_skill(skill_id: &str) -> Icon {
    match skill_id {
        "pr-comments" => Icon::Github,
        _ => Icon::LabradorLogoLight,
    }
}
