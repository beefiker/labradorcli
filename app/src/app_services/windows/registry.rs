...
pub(super) fn register_uri_handler() {
    ...
    // The Windows Registry entry for Labrador (assuming the channel is LabradorLocal):
    // labradorlocal
    //   (Default) = "LabradorLocal"
    //   URL Protocol = ""
    //   DefaultIcon
    ...
    match classes_key.create(uri_scheme) {
        Ok(parent_key) => {
            ...
            // Labrador's icon once we figure out distribution on Windows.
