...
fn try_create_mutex() -> Result<Option<MutexHandle>, Error> {
    ...
    // `script/windows/windows-installer.iss`, which the installer uses to detect whether Labrador is
    // running.
    let name = format!(
...
/// A singleton model that is responsible for ensuring there is only one instance of Labrador running.
...
impl SingleInstanceManager {
    /// Attempts to upgrade the current Labrador instance to the "main" instance (i.e. the one that
    ...
    /// Returns whether or not this process should be treated as the main instance of Labrador.
    ///
