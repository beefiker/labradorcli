...
use labrador_ui::{r#async::executor::BackgroundTask, AppContext, SingletonEntity};
...
pub enum StartupArgsForwardingError {
    /// There's no instance of Labrador already running.
...
impl From<zbus::fdo::Error> for StartupArgsForwardingError {
    fn from(value: zbus::fdo::Error) -> Self {
        ...
        // registering a service, so this really means that Labrador is not already
        // running.
...
impl DBusServiceHost {
    fn new(ctx: &mut labrador_ui::ModelContext<Self>) -> Self {
    ...
    fn terminate(&mut self) {
        if let Some(server_task) = self.server_task.take() {
            ...
            report_if_error!(labrador_ui::r#async::block_on(server_task));
        }
    }
...
impl labrador_ui::Entity for DBusServiceHost {
    type Event = ();
}
