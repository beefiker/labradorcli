...
use labrador_cli::RecoveryMechanism;
...
/// Keep in sync with [`labrador_cli::AppArgs`].
...
pub fn is_crash_recovery_process(args: &labrador_cli::AppArgs) -> bool {
...
impl CrashRecoveryProcess {
    ...
    fn kill(&mut self) {
        ...
        *IS_CRASH_RECOVERY_PROCESS_RUNNING.write() = false;
        self.is_alive = false;
        labrador_logging::on_crash_recovery_process_killed();
    }
...
impl CrashRecovery {
    ...
    #[cfg(test)]
    pub fn register_for_test(app: &mut labrador_ui::App) {
        ...
        app.update(|ctx| {
            ctx.add_singleton_model(|ctx| {
                ...
                let launch_mode = crate::LaunchMode::App {
                    args: labrador_cli::AppArgs::default(),
                    api_key: None,
                };
...
#[cfg(windows)]
fn wait_for_parent_crash(args: &labrador_cli::AppArgs) {
    use windows::Win32::{
...
fn handle_parent_crash(
    ...
    labrador_logging::on_parent_process_crash();
