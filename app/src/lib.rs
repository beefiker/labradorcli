...
mod voice;
mod voltron;
mod labrador_managed_paths_watcher;
#[cfg(target_family = "wasm")]
mod wasm_nux_dialog;
mod window_settings;
mod workspaces;
...
//
// Any modules which we make public outside of the Labrador app crate lose dead code
...
// in the labrador::integration_testing::assertions module (or a sub-module).  These
...
use labrador_cli::GlobalOptions;
use labrador_cli::{agent::AgentCommand, CliCommand};
...
pub use labrador_core::errors::{report_error, report_if_error};
...
use labrador_ui::platform::app::ApproveTerminateResult;
...
use crate::user_config::LabradorConfig;
use crate::vim_registers::VimRegisters;
use crate::labrador_managed_paths_watcher::{ensure_labrador_watch_roots_exist, LabradorManagedPathsWatcher};
...
use url::Url;
use labrador_core::execution_mode::{AppExecutionMode, ExecutionMode};
use labrador_managed_secrets::ManagedSecretManager;
use workspace::sync_inputs::SyncedInputState;

use labrador_ui::{integration::TestDriver, App, AssetProvider, Event};
...
use labrador_logging::LogDestination;

// Re-export the send_telemetry_from_ctx macro at the crate root level
pub use labrador_core::send_telemetry_from_app_ctx;
pub use labrador_core::send_telemetry_from_ctx;

// Re-export the safe logging macros at the crate root level for backwards compatibility
pub use labrador_core::{safe_debug, safe_error, safe_info, safe_warn};

#[cfg(feature = "local_fs")]
use labrador_files::FileModel;
use labrador_ui::platform::TerminationMode;
use labrador_ui::windowing::state::ApplicationStage;
use labrador_ui::{AppContext, SingletonEntity, WindowId};
...
/// Launch mode for how to start up Labrador.
#[allow(clippy::large_enum_variant)]
pub enum LaunchMode {
    /// Run the regular GUI application.
    App {
        args: labrador_cli::AppArgs,
        /// API key for server authentication, if provided via `--api-key` or the configured API
        /// key environment variable.
        /// Only used on dogfood channels.
        api_key: Option<String>,
    },

    /// Run the Labrador command-line SDK.
    CommandLine {
        command: labrador_cli::CliCommand,
...
impl LaunchMode {
    fn args(&self) -> Cow<'_, labrador_cli::AppArgs> {
        match self {
            LaunchMode::App { args, .. } => Cow::Borrowed(args),
            LaunchMode::CommandLine { .. }
            | LaunchMode::Test { .. }
            | LaunchMode::RemoteServerProxy
            | LaunchMode::RemoteServerDaemon => Cow::Owned(labrador_cli::AppArgs::default()),
        }
    }
    ...
    /// Returns `true` if Labrador should run headlessly, without a visible UI.
...
pub fn run() -> Result<()> {
    ...
    let args = labrador_cli::Args::from_env();
    ...
    // Labrador or legacy env-var equivalents) so shipped builds can't be redirected away from
    // their baked-in server URLs. See `Channel::allows_server_url_overrides`.
    ...
    if let Some(command) = args.command() {
        ...
        if command.prints_to_stdout() {
            ...
            labrador_util::windows::attach_to_parent_console();
        }
        match command {
            #[cfg(all(feature = "local_tty", unix))]
            labrador_cli::Command::Worker(labrador_cli::WorkerCommand::TerminalServer(args)) => {
            ...
            #[cfg(feature = "plugin_host")]
            labrador_cli::Command::Worker(labrador_cli::WorkerCommand::PluginHost { .. }) => {
                return crate::run_plugin_host();
            }
            #[cfg(feature = "local_tty")]
            labrador_cli::Command::Worker(labrador_cli::WorkerCommand::MinidumpServer { socket_name }) => {
                cfg_if::cfg_if! {
            ...
            labrador_cli::Command::Worker(labrador_cli::WorkerCommand::RemoteServerProxy(args)) => {
                init_common(&LaunchMode::RemoteServerProxy, None)?;
                return crate::remote_server::run_proxy(args.identity_key.clone());
            }
            #[cfg(not(target_family = "wasm"))]
            labrador_cli::Command::Worker(labrador_cli::WorkerCommand::RemoteServerDaemon(args)) => {
                init_common(&LaunchMode::RemoteServerDaemon, None)?;
                return crate::remote_server::run_daemon(args.identity_key.clone());
            }
            #[cfg(not(target_family = "wasm"))]
            labrador_cli::Command::Worker(labrador_cli::WorkerCommand::RipgrepSearch {
                parent,
                ignore_case,
                multiline,
                pattern,
                ...
                labrador_ripgrep::search::run_search_subprocess(
            ...
            labrador_cli::Command::Worker(worker) => {
                // Need this case to handle platforms where there are no enum variants in
                // labrador_cli::WorkerCommand, as we still need to check Command::Worker.

                // On wasm, specifically, we should fail spectacularly if we get here.
                #[cfg(target_family = "wasm")]
                panic!("Worker process not supported on WASM: {worker:?}")
            }
            labrador_cli::Command::Completions { shell } => {
                return labrador_cli::completions::generate_to_stdout(*shell);
            }
            labrador_cli::Command::CommandLine(cmd) => {
                let (is_sandboxed, computer_use_override) = match cmd.as_ref() {
                    labrador_cli::CliCommand::Agent(labrador_cli::agent::AgentCommand::Run(run_args)) => (
                        run_args.sandboxed,
            ...
            labrador_cli::Command::DumpDebugInfo => {
                return debug_dump::run();
            }
            #[cfg(not(target_family = "wasm"))]
            labrador_cli::Command::PrintTelemetryEvents => {
    ...
    let is_cli_binary = cfg!(feature = "standalone")
        || labrador_cli::binary_name().is_some_and(|name| name.starts_with("oz"))
        || std::env::var_os("LABRADOR_CLI_MODE").is_some();
    if is_cli_binary {
        labrador_cli::Args::clap_command().print_help()?;
        return Ok(());
    }
...
pub fn run_integration_test(driver: TestDriver) -> Result<()> {
    let is_integration_test = std::env::var_os("LABRADOR_INTEGRATION").is_some();
    ...
    cfg_if::cfg_if! {
        if #[cfg(enable_crash_recovery)] {
            if crash_recovery::is_crash_recovery_process(launch_mode.args().as_ref()) {
                labrador_logging::init_for_crash_recovery_process()?;
            } else {
                labrador_logging::init(labrador_logging::LogConfig { is_cli, log_destination })?;
            }
        } else {
            labrador_logging::init(labrador_logging::LogConfig { is_cli, log_destination })?;
        }
...
fn run_internal(mut launch_mode: LaunchMode) -> Result<()> {
    ...
    if let LaunchMode::App { .. } = launch_mode {
        match app_services::linux::pass_startup_args_to_existing_instance(
            ...
            // we only want to run a single instance of Labrador at a time.
            Ok(_) => std::process::exit(0),
            // If Labrador isn't already running, we're good to go.
            ...
            // state where Labrador refuses to run even a first instance.
            Err(err) => {
    ...
    // Sets up a Job Object that we associate with the Labrador process to handle
    ...
    let prefs_for_public_settings: &dyn labrador_ui_extras::user_preferences::UserPreferences =
    ...
    let mut app_builder = if launch_mode.is_headless() {
        labrador_ui::platform::AppBuilder::new_headless(
            app_callbacks(launch_mode.is_integration_test()),
            Box::new(ASSETS),
            launch_mode.take_test_driver(),
        )
    } else {
        labrador_ui::platform::AppBuilder::new(
            app_callbacks(launch_mode.is_integration_test()),
            Box::new(ASSETS),
            launch_mode.take_test_driver(),
        )
    ...
    #[cfg(target_os = "macos")]
    {
        use labrador_ui::platform::mac::AppExt;

        let activate_on_launch = !launch_mode.is_integration_test()
            || std::env::var_os("LABRADOR_UI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS").is_some();
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        use labrador_ui::platform::linux::{self, AppBuilderExt};
        ...
        // Force use of Wayland if the user has passed the Labrador or legacy env var.
    ...
    {
        use labrador_ui::platform::windows::AppBuilderExt;
        ...
        if !is_parallels_vm {
            ...
            use labrador_ui::platform::windows::DXCPath;
    ...
    app_builder.run(move |ctx| {
        ...
        ctx.background_executor()
            .spawn(labrador_logging::rotate_log_files())
            .detach();
...
fn initialize_app(
    ...
    startup_toml_parse_error: Option<labrador_ui_extras::user_preferences::Error>,
    ctx: &mut labrador_ui::AppContext,
    ...
    cfg_if::cfg_if! {
        if #[cfg(feature = "integration_tests")] {
            labrador_ui_extras::secure_storage::register_noop(&data_domain, ctx);
        } else if #[cfg(target_os = "linux")] {
            labrador_ui_extras::secure_storage::register_with_fallback(&data_domain, labrador_core::paths::state_dir(), ctx)
        } else if #[cfg(target_os = "windows")] {
            labrador_ui_extras::secure_storage::register_with_dir(&data_domain, labrador_core::paths::state_dir(), ctx)
        } else {
            labrador_ui_extras::secure_storage::register(&data_domain, ctx);
        }
    }

    // One-time migration for macOS Preview app-authored config directories.
    // This must run before ensure_labrador_watch_roots_exist() creates the new
    // directory.
    #[cfg(target_os = "macos")]
    preview_config_migration::migrate_preview_config_dir_if_needed();

    ensure_labrador_watch_roots_exist();
    ctx.add_singleton_model(LabradorManagedPathsWatcher::new);

    ctx.add_singleton_model(LabradorConfig::new);
    ...
    log::info!(
        "Starting {} with channel state {} and version {:?}",
    ...
    // Rewrite recognized Labrador web URLs (sessions, Drive, settings, home) into local
    ...
    if user_is_logged_in {
        ...
        ctx.on_first_frame_drawn(move |ctx| {
            ...
            GPUState::handle(ctx).update(ctx, |gpu_state, ctx| {
                gpu_state
                    .set_has_lower_power_gpu(labrador_ui::rendering::is_low_power_gpu_available(), ctx);
            });
    ...
    #[cfg(feature = "local_fs")]
    {
        ...
        if FeatureFlag::SettingsImport.is_enabled()
            && ChannelState::channel() != labrador_core::channel::Channel::Integration
        {
    ...
    terminal::ssh::labradorify::init(ctx);
    ...
    if labrador_core::features::FeatureFlag::OrchestrationV2.is_enabled() {
    ...
    ctx.add_singleton_model(|ctx| {
        labrador_core::sync_queue::SyncQueue::<SyncTask>::new_with_rate_limit(
            &ctx.background_executor(),
...
fn app_callbacks(is_integration_test: bool) -> labrador_ui::platform::AppCallbacks {
    labrador_ui::platform::AppCallbacks {
        ...
        on_will_terminate: Some(Box::new(move |ctx| {
            ...
            // autoupdate, to ensure we're not running any extra Labrador processes
...
fn launch(ctx: &mut labrador_ui::AppContext, app_state: Option<AppState>, launch_mode: LaunchMode) {
...
fn init_logging_for_unit_tests_glue() {
    ...
    labrador_logging::init_logging_for_unit_tests();
}
...
pub fn enabled_features() -> HashSet<FeatureFlag> {
    ...
    flags.extend([
        ...
        #[cfg(feature = "labrador_packs")]
        FeatureFlag::LabradorPacks,
        ...
        #[cfg(feature = "labrador_managed_secrets")]
        FeatureFlag::LabradorManagedSecrets,
        ...
        #[cfg(feature = "open_labrador_launch_modal")]
        FeatureFlag::OpenLabradorLaunchModal,
        ...
        #[cfg(feature = "open_labrador_new_settings_modes")]
        FeatureFlag::OpenLabradorNewSettingsModes,
        ...
        #[cfg(feature = "labradorify_footer")]
        FeatureFlag::LabradorifyFooter,
    ...
    // Labrador is a local-only build. Keep the local terminal and local Codex agent
    // features, but do not expose Labrador-hosted surfaces that require login,
