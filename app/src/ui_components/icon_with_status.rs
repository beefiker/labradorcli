...
use labrador_core::ui::icons::Icon as LabradorIcon;
use labrador_core::ui::theme::color::internal_colors;
use labrador_core::ui::theme::{Fill as LabradorThemeFill, LabradorTheme};
...
pub(crate) fn render_icon_with_status(
    ...
    theme: &LabradorTheme,
    badge_ring_background: LabradorThemeFill,
) -> Box<dyn Element> {
    let sub_text = theme.sub_text_color(theme.background());

    match variant {
        IconWithStatusVariant::Neutral { icon, icon_color } => {
            let inner = ConstrainedBox::new(icon.to_labrador_ui_icon(icon_color).finish())
        ...
        IconWithStatusVariant::OzAgent { status } => {
            let inner = render_labrador_icon(sizing.icon_size, 2.);
        ...
        IconWithStatusVariant::CLIAgent { agent, status } => {
            ...
            let icon_element = agent
                ...
                .unwrap_or_else(|| LabradorIcon::Terminal.to_labrador_ui_icon(sub_text).finish());
            let inner = ConstrainedBox::new(icon_element)
                .with_width(sizing.icon_size)
                .with_height(sizing.icon_size)
                .finish();
...
fn render_with_optional_status_badge(
    ...
    let badge_icon = ConstrainedBox::new(icon.to_labrador_ui_icon(LabradorThemeFill::Solid(color)).finish())
