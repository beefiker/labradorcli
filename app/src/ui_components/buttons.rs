...
use crate::themes::theme::LabradorTheme;
use labrador_ui::elements::Radius;
use labrador_ui::elements::{CornerRadius, MouseStateHandle};
use labrador_ui::ui_components::button::Button;
...
fn all_icon_button_styles(labrador_theme: &LabradorTheme, mode: ButtonMode) -> AllButtonStyles {
    AllButtonStyles {
        default_styles: icon_button_styles(labrador_theme, mode, ButtonState::Default),
        hovered_styles: Some(icon_button_styles(labrador_theme, mode, ButtonState::Hover)),
        clicked_styles: Some(icon_button_styles(labrador_theme, mode, ButtonState::Pressed)),
        disabled_styles: Some(icon_button_styles(labrador_theme, mode, ButtonState::Disabled)),
    }
}
...
fn icon_button_styles(
    ...
    let icon_color = icon_color(labrador_theme, mode);
    ...
    let (background_color, border_color): (Option<Fill>, Option<Fill>) = match (mode, state) {
        ...
        (ButtonMode::Base, ButtonState::Hover) => {
            (Some(labrador_theme.surface_2()), Some(labrador_theme.surface_3()))
        }
        (ButtonMode::Base, ButtonState::Pressed) | (ButtonMode::Base, ButtonState::Disabled) => {
            (Some(labrador_theme.background()), Some(labrador_theme.surface_3()))
        }
        (ButtonMode::Accent, ButtonState::Default) => (None, None),
        (ButtonMode::Accent, ButtonState::Hover) => (
            Some(labrador_theme.surface_3()),
            Some(blended_colors::accent(labrador_theme)),
        ),
        (ButtonMode::Accent, ButtonState::Pressed)
        | (ButtonMode::Accent, ButtonState::Disabled) => (
            Some(labrador_theme.background()),
            Some(blended_colors::accent_pressed(labrador_theme)),
        ),
    };
...
fn combo_inner_button_styles(labrador_theme: &LabradorTheme, state: ButtonState) -> UiComponentStyles {
    let background = match state {
        ButtonState::Default => None,
        ButtonState::Hover => Some(blended_colors::neutral_2(labrador_theme)),
        ButtonState::Pressed => Some(blended_colors::neutral_4(labrador_theme)),
        ButtonState::Disabled => Some(labrador_theme.background().into()),
    };
    ...
    UiComponentStyles {
        ...
        border_radius: None,
        font_color: Some(labrador_theme.foreground().into()),
        border_color: None,
...
pub fn combo_inner_button(
    ...
    .with_icon_label(icon.to_labrador_ui_icon(theme.foreground()));
...
fn icon_color(labrador_theme: &LabradorTheme, mode: ButtonMode) -> Fill {
    match mode {
        ButtonMode::Base => labrador_theme.foreground(),
        ButtonMode::Accent => blended_colors::accent(labrador_theme),
    }
}
...
fn icon_button_internal(
    ...
    .with_icon_label(icon.to_labrador_ui_icon(color.unwrap_or(icon_color(theme, mode))));
