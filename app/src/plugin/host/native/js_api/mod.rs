...
cfg_if::cfg_if! {
    if #[cfg(feature = "completions_v2")] {
        ...
        use labrador_completer::signatures::CommandSignature;
        use labrador_js::FromLabradorJs;
    }
}

/// Returns a JS object representing the Labrador Plugin API exposed to external JavaScript plugins.
///
...
/// Returns a JS object representing the Completions namespace for the Labrador Plugin API.
///
/// API methods:
///
        ...
        Function::new(
            ...
            MutFn::from(move |val: Value<'js>| {
                if val.is_array() {
                    ...
                    match Vec::<CommandSignature>::from_labrador_js(
                        ctx,
                        val,
                    ...
                    let mut plugin = plugin.get_mut();
                    match CommandSignature::from_labrador_js(
                        ctx,
                        val,
                        plugin.js_function_registry_mut(),
                    ) {
