...
pub fn run() -> Result<()> {
    labrador_ui::r#async::block_on(async move {
        let executor = Arc::new(Background::default());

        // Initialize a client connection to the Labrador app process.
        ...
        // Initialize logging, which internally transmits logs from this process to the Labrador app
        ...
        // Send the connection address for the plugin host server back to the Labrador app.
...
///
/// This assumes that all plugins are located in ~/.labrador/plugins.
fn plugin_paths() -> Vec<PathBuf> {
    const PLUGIN_PATH_SUFFIX: &str = ".labrador/plugins";
...
/// Returns `true` if the directory at the given path is directory containing JS source for a Labrador plugin.
