...
use labrador_ui::{
    accessibility::{AccessibilityContent, LabradorA11yRole},
    elements::{
...
use crate::themes::theme::{RespectSystemTheme, ThemeKind, LabradorTheme};
...
use crate::{
    ...
    user_config::{load_theme_configs, themes_dir, LabradorConfig, LabradorConfigUpdateEvent},
...
use crate::{
    themes::theme::LabradorThemeConfig,
...
fn theme_chooser_items(theme_config: &LabradorThemeConfig) -> Vec<ThemeChooserItem> {
    let mut theme_items: Vec<ThemeChooserItem> = theme_config
        .theme_items()
        .filter(|(key, _)| {
            // Referral-reward themes are gated on a Labrador-hosted referral status
            // that no longer exists, so always hide them.
            !matches!(
                key,
...
impl ThemeChooser {
    pub fn new(
        ...
        let labrador_config_handle = LabradorConfig::handle(ctx);
        ctx.subscribe_to_model(&labrador_config_handle, |me, _, event, ctx| {
            if let LabradorConfigUpdateEvent::Themes = event {
        ...
        let themes = theme_chooser_items(LabradorConfig::as_ref(ctx).theme_config());
        ...
        ctx.spawn(
            ...
            move |theme_chooser, loaded_themes, ctx| {
                ctx.update_model(&LabradorConfig::handle(ctx), move |labrador_config, ctx| {
                    labrador_config.update_theme_config(loaded_themes, ctx);
                });
    ...
    fn update_themes(&mut self, ctx: &mut ViewContext<Self>) {
        *self.themes = theme_chooser_items(LabradorConfig::as_ref(ctx).theme_config());
    }
    ...
    fn render_list(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let element = if list_len == 0 {
            ...
            Scrollable::vertical(
                ...
                SCROLLBAR_WIDTH,
                labrador_theme
                    .disabled_text_color(labrador_theme.surface_2())
                    .into(),
                labrador_theme.main_text_color(labrador_theme.surface_2()).into(),
                Fill::None,
            )
            .finish()
        };
...
impl ThemeChooserItem {
    pub fn new(kind: ThemeKind, labrador_theme: LabradorTheme) -> Self {
        Self {
            kind,
            labrador_theme,
    ...
    fn render_thumbnail(&self, font_family: FamilyId) -> Box<dyn Element> {
        theme::render_preview(&self.labrador_theme, font_family, None)
    }
