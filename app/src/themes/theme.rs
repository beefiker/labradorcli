...
use labrador_core::{channel::ChannelState, ui::color::pick_foreground_color};
...
pub use labrador_core::ui::color::blend::Blend;
pub use labrador_core::ui::theme::*;
...
impl InMemoryThemeOptions {
    ...
    pub fn theme(&self) -> LabradorTheme {
...
#[derive(Debug, Clone)]
pub struct LabradorThemeConfig {
    theme_map: HashMap<ThemeKind, LabradorTheme>,
}

impl LabradorThemeConfig {
    pub fn new() -> Self {
        // preload with built-in themes
        let theme_map: HashMap<ThemeKind, LabradorTheme> = HashMap::from_iter([
        ...
        LabradorThemeConfig { theme_map }
    }

    pub fn add_new_theme(&mut self, theme_name: ThemeKind, theme: LabradorTheme) {
    ...
    pub fn theme_items(&self) -> impl Iterator<Item = (&ThemeKind, &LabradorTheme)> {
        self.theme_map.iter()
    }

    pub fn theme(&self, name: &ThemeKind) -> LabradorTheme {
        self.theme_map.get(name).cloned().unwrap_or_else(dark_theme)
    }
}

impl Default for LabradorThemeConfig {
    fn default() -> Self {
        Self::new()
    }
}
...
impl From<LabradorTheme> for PromptColors {
    fn from(theme: LabradorTheme) -> Self {
        PromptColors {
...
pub fn render_preview(
    ...
    if let Some(background_image) = theme.background_image() {
        thumbnail.add_child(
            Shrinkable::new(
                1.,
                labrador_ui::elements::Image::new(
                    background_image.source(),
                    labrador_ui::elements::CacheOption::BySize,
                )
                .cover()
                .finish(),
            )
