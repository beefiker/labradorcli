...
pub fn is_labrador_default_terminal() -> bool {
    unsafe {
        ...
        let Some(labrador_bundle_id) = get_labrador_bundle_id() else {
            return false;
        };
        ...
        current_handler == labrador_bundle_id
    }
}
...
pub fn set_labrador_as_default_terminal() -> Result<(), String> {
    ...
    let bundle_id = get_labrador_bundle_id().ok_or("No bundle ID".to_string())?;
...
/// Gets Labrador's bundle identifier. This may be `None` if not running as a bundle, i.e. through
/// `cargo run` without `cargo bundle`.
fn get_labrador_bundle_id() -> Option<String> {
    unsafe {
        let bundle_class = class!(NSBundle);
        let main_bundle: id = msg_send![bundle_class, mainBundle];
        let bundle_id: id = msg_send![main_bundle, bundleIdentifier];
