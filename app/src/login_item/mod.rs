//! Login-item registration — makes Labrador start automatically when the user
...
/// Reconciles whether Labrador is registered to launch at login with the user's
/// current preference.
///
/// Respects the existing `app_added_as_login_item` bookkeeping so a user who
/// removed Labrador from their OS's startup UI isn't silently re-added — the
/// platform backends only run the registration flow when the setting was
/// explicitly re-toggled.
///
/// Skipped entirely when the `LABRADOR_INTEGRATION` env var is set, so integration
...
/// auto-launch `target/debug/labrador` at sign-in.
#[cfg(any(target_os = "macos", target_os = "windows"))]
pub fn maybe_register_app_as_login_item(ctx: &mut AppContext) {
    if std::env::var_os("LABRADOR_INTEGRATION").is_some() {
