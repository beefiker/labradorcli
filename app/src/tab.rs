...
use labrador_core::ui::theme::AnsiColors;
...
use labrador_ui::{AppContext, SingletonEntity, ViewHandle};
...
const LABRADOR_2_TAB_COLOR_OPACITY: Opacity = 25;
const LABRADOR_2_HOVERED_TAB_COLOR_OPACITY: Opacity = 50;
    ...
    fn render_indicator(&self) -> Option<Box<dyn Element>> {
        let icon = match &self.indicator {
            ...
            Indicator::Synced => Some(
                Icon::LinkHorizontal
                    .to_labrador_ui_icon(self.styles.synced_input_indicator_color.into())
                    .finish(),
            ),
            Indicator::Error => Some(
                Icon::AlertTriangle
                    .to_labrador_ui_icon(self.styles.error_color.into())
                    .finish(),
            ),
            Indicator::Shared => Some(
                Icon::Sharing
                    .to_labrador_ui_icon(self.styles.sharing_color.into())
                    .finish(),
            ),
            ...
            Indicator::Shell(shell_indicator_type) => Some(
                shell_indicator_type
                    .to_icon()
                    .to_labrador_ui_icon(internal_colors::neutral_5(self.appearance.theme()).into())
                    .finish(),
            ),
            Indicator::Agent {
                conversation_status,
                ...
                if let Some(status) = conversation_status {
                    ...
                    Some(Icon::Oz.to_labrador_ui_icon(icon_color).finish())
                }
            }
            Indicator::AmbientAgent => {
                ...
                Some(
                    Hoverable::new(mouse_state, move |state| {
                        let mut stack = Stack::new()
                            .with_child(Icon::OzCloud.to_labrador_ui_icon(icon_color.into()).finish());
    ...
    fn render_tab_container_internal(
        ...
        let (background_color, border_fill) = if FeatureFlag::NewTabStyling.is_enabled() {
            ...
            let tab_opacity = if is_active || is_hovered {
                LABRADOR_2_HOVERED_TAB_COLOR_OPACITY
            } else {
                LABRADOR_2_TAB_COLOR_OPACITY
            };
        ...
        let full_tab_content = {
            let mut flex_row = Flex::row()
                ...
                .with_cross_axis_alignment(labrador_ui::elements::CrossAxisAlignment::Center);
                    ...
                    let tooltip_content: Box<dyn Element> = if has_extra_info {
                        ...
                        if let Some(directory) = &tooltip_directory_clone {
                            let folder_icon = Icon::Folder
                                .to_labrador_ui_icon(ThemeFill::Solid(font_color))
                                .finish();
