...
mod macos_app_icon {
    ...
    pub use labrador_core::channel::{Channel, ChannelState};
    pub use labrador_ui::platform::mac::{make_nsstring, AutoreleasePoolGuard};
...
use crate::{
    ...
    themes::theme::{ThemeKind, LabradorTheme},
    ASSETS,
};

use anyhow::anyhow;

pub use labrador_core::ui::appearance::{Appearance, AppearanceEvent};
...
pub struct AppearanceManager {
    ...
    transient_theme: Option<LabradorTheme>,
...
impl AppearanceManager {
    ...
    pub fn set_app_icon(&self, app: &AppContext) {
        ...
        unsafe {
            ...
            let plugin_name = make_nsstring("LabradorDockTilePlugin.docktileplugin");
            let plugin_path: id =
...
fn load_default_monospace_font_family(ctx: &mut AppContext) -> anyhow::Result<FamilyId> {
    labrador_ui::fonts::Cache::handle(ctx).update(ctx, |font_cache, _| {
...
fn get_or_load_font_family(font_name: &str, ctx: &mut AppContext) -> Option<FamilyId> {
    labrador_ui::fonts::Cache::handle(ctx).update(ctx, |font_cache, _| {
        match font_cache.get_or_load_system_font(font_name) {
            Ok(family) => {
                let font_id = font_cache.select_font(family, labrador_ui::fonts::Properties::default());
...
#[cfg(target_family = "wasm")]
fn emit_theme_background_event(theme: &LabradorTheme) {
    let bg = theme.background().into_solid();
    let color = format!("#{:02x}{:02x}{:02x}", bg.r, bg.g, bg.b);
    crate::platform::wasm::emit_event(crate::platform::wasm::LabradorEvent::ThemeBackgroundChanged {
        color,
    });
}
