...
use super::model::ansi::{FinishUpdateValue, LabradorificationUnavailableReason};
    ...
    fn handle_terminal_model_event(&mut self, event: Event, ctx: &mut ModelContext<Self>) {
        let event_to_emit = match event {
            ...
            Event::Handler(HandlerEvent::TmuxControlModeReady {
                ...
                {
                    if let Some(TmuxControlModeContext::LabradorInitiatedForSsh(control_mode)) =
                        context
                    {
            ...
            Event::RemoteLabradorificationIsUnavailable(reason) => {
                ModelEvent::RemoteLabradorificationIsUnavailable(reason)
            }
...
pub enum ModelEvent {
    ...
    /// useful for detecting when an ssh session becomes ready for labradorification.
    DetectedEndOfSshLogin(SshLoginStatus),
    RemoteLabradorificationIsUnavailable(LabradorificationUnavailableReason),
    ...
    PromptUpdated,
    /// Emitted when the honor_ps1 state of the shell is out-of-sync with Labrador's settings.
    /// This can happen in cases such as when the user changes between PS1 and Labrador prompt inside
