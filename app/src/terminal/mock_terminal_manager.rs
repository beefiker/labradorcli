...
use labrador_ui::{AppContext, ModelHandle, ViewHandle, WindowId};
...
#[cfg(test)]
mod testing {
    use labrador_ui::{platform::WindowStyle, App, Element, SingletonEntity};
    ...
    impl labrador_ui::Entity for TerminalRootView {
        type Event = ();
    }

    impl labrador_ui::View for TerminalRootView {
        fn ui_name() -> &'static str {
            "TerminalRootView"
        }

        fn render(&self, _app: &labrador_ui::AppContext) -> Box<dyn labrador_ui::Element> {
            labrador_ui::elements::ChildView::new(&self.terminal_view).finish()
        }
    }

    impl labrador_ui::TypedActionView for TerminalRootView {
        type Action = ();
    }
