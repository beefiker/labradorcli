...
use labrador_ui::assets::asset_cache::{AssetCache, AssetSource, AssetState};
use labrador_ui::color::ColorU;
use labrador_ui::elements::{Border, CornerRadius, Fill, Radius, DEFAULT_UI_LINE_HEIGHT_RATIO};
use labrador_ui::fonts::{FamilyId, FontId, Properties, Style, Weight};
use labrador_ui::geometry::rect::RectF;
use labrador_ui::geometry::vector::{vec2f, Vector2F};
use labrador_ui::image_cache::{AnimatedImageBehavior, CacheOption, FitType, Image, ImageCache};
use labrador_ui::platform::LineStyle;
use labrador_ui::text_layout::{Line, StyleAndFont, TextStyle, DEFAULT_TOP_BOTTOM_RATIO};
use labrador_ui::units::{IntoLines as _, Lines, Pixels};
use labrador_ui::{AppContext, Element, EntityId, PaintContext, Scene, SingletonEntity};
...
fn render_grid_without_ligatures<'a>(
    ...
    if FeatureFlag::ITermImages.is_enabled() {
        ...
        if !foreground_image_ids.is_empty() {
            ctx.scene.start_layer(labrador_ui::ClipBounds::ActiveLayer);
    ...
    for (offset, row_idx) in visible_rows.enumerate() {
        ...
        for col in 0..grid.columns() {
            ...
            // When Labrador draws the cursor (SHOW_CURSOR on), we keep the cell
