...
///
/// The field names need to be kept the same as the proxy server's
/// `WindowSizeChange` struct.
...
impl EventLoop {
    ...
    /// order to dogfood Labrador on Web over the remote tty.
    async fn write_env_vars(
        sink: &mut impl Sink,
        is_honor_ps1_enabled: bool,
    ) -> anyhow::Result<()> {
        let honor_ps1_env_var = format!(r#"LABRADOR_HONOR_PS1="{}";"#, is_honor_ps1_enabled as u8);
