...
//! Labrador's SSH extension on the remote host the shell just connected to,
//! or continue without installing (falling back to the existing
//! ControlMaster labradorification path).
//!
...
use labrador_core::{channel::ChannelState, ui::theme::color::internal_colors};
use labrador_ui::{
    elements::{
...
use crate::{
    ...
    terminal::labradorify::settings::{SshExtensionInstallMode, LabradorifySettings},
        ...
        let buttons = ctx.add_typed_action_view(|_| {
            KeyboardNavigableButtons::new(vec![
                ...
                rich_navigation_button(
                    ...
                    Some(
                        "You'll still get a Labradorified experience just without the coding \
                         features."
                            .to_string(),
                    ),
                    /* recommended */ false,
    ...
    fn render_footer(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let manage_settings_link = appearance
            ...
            .link(
                ...
                None,
                Some(Box::new(|ctx| {
                    ctx.dispatch_typed_action(SshRemoteServerChoiceViewAction::OpenLabradorifySettings);
                })),
        ...
        match action {
            SshRemoteServerChoiceViewAction::Install => {
                if self.do_not_ask_again {
                    ...
                    LabradorifySettings::handle(ctx).update(ctx, |settings, ctx| {
            ...
            SshRemoteServerChoiceViewAction::OpenLabradorifySettings => {
                ctx.emit(SshRemoteServerChoiceViewEvent::OpenLabradorifySettings);
            }
        }
    }
}
