...
use labrador_ui::prelude::{vec2f, ChildView, Container, Hoverable};
...
use labrador_ui::{AppContext, Element, ModelHandle, SingletonEntity, TypedActionView, ViewContext};
...
impl TerminalView {
    ...
    fn maybe_render_header_back_button(&self, app: &AppContext) -> Box<dyn Element> {
        if !FeatureFlag::AgentView.is_enabled() || labrador_ui::platform::is_mobile_device() {
        ...
        let pane_indicator = if should_render_ambient_agent_indicator {
            ...
            }) = shared_session.kind().as_viewer()
            {
                ...
                Some(
                    ConstrainedBox::new(
                        icons::Icon::Sharing
                            .to_labrador_ui_icon(shared_session_indicator_color(appearance).into())
                            .finish(),
                    )
...
impl TerminalView {
    ...
    fn render_agent_indicator(
        ...
        let Some(conversation) =
            ...
            return labrador_ui::elements::Empty::new().finish();
        };
        ...
        if FeatureFlag::AgentView.is_enabled()
            ...
            && !is_long_running
        {
            render_labrador_icon(appearance.ui_font_size(), 2.)
            ...
            return Some(
                ConstrainedBox::new(
                    icons::Icon::AlertTriangle
                        .to_labrador_ui_icon(appearance.theme().ui_error_color().into())
                        .finish(),
                )
    ...
    fn render_ambient_agent_indicator(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        Hoverable::new(
            self.mouse_states
                ...
                .clone(),
            move |state| {
                let mut stack = Stack::new().with_child(render_labrador_icon(font_size * 1.5, 4.));
                if state.is_hovered() {
