...
use crate::{
    ...
    terminal::{
        ssh::labradorify::labradorify_description,
        view::{RememberForLabradorification, TerminalAction},
    },
    themes::theme::Fill,
...
#[derive(Clone)]
pub enum LabradorificationMode {
    Ssh {
        command: String,
        host: Option<String>,
...
pub struct LabradorifyBannerState {
    pub mode: LabradorificationMode,
    pub height: f32,
    ...
    /// This keybinding gets rendered in the Labradorification banner, but we can't look it up
    ...
    pub initialize_labradorify_keybinding: Option<Keystroke>,
    pub hover_state: MouseStateHandle,
}

impl LabradorifyBannerState {
    pub fn new(
        mode: LabradorificationMode,
        initialize_labradorify_keybinding: Option<Keystroke>,
    ) -> Self {
        Self {
            mode,
            height: 0.0,
            initialize_labradorify_keybinding,
    ...
    pub fn action(&self) -> TerminalAction {
        match &self.mode {
            LabradorificationMode::Ssh { .. } => TerminalAction::LabradorifySSHSession,
            LabradorificationMode::Subshell { .. } => TerminalAction::TriggerSubshellBootstrap,
        }
    }

    fn remember_for_labradorification(
        &self,
        should_remember: bool,
    ) -> RememberForLabradorification {
        match &self.mode {
            LabradorificationMode::Ssh { command, host, .. } => {
                let Some(host) = host else {
                    if should_remember {
                        return RememberForLabradorification::RememberSubshellCommand(
                            command.to_owned(),
                        );
                    }
                    return RememberForLabradorification::DoNotRememberSSHHost;
                };
                if should_remember {
                    RememberForLabradorification::RememberSSHHost(host.to_owned())
                } else {
                    RememberForLabradorification::DoNotRememberSSHHost
                }
            }
            LabradorificationMode::Subshell { command } => {
                if should_remember {
                    RememberForLabradorification::RememberSubshellCommand(command.to_owned())
                } else {
                    RememberForLabradorification::DoNotRememberSubshellCommand
                }
            }
        }
    }
...
pub fn render_labradorification_banner(
    state: &LabradorifyBannerState,
    appearance: &Appearance,
    app: &AppContext,
) -> Box<dyn Element> {
    let yes_button = render_yes_button(
        state,
        &state.initialize_labradorify_keybinding,
        &state.accept_button_mouse_state,
        appearance,
    );

    let remember = state.remember_for_labradorification(true);
    let dont_ask_button = Container::new(
        appearance
            ...
            .on_click(move |ctx, _, _| {
                ctx.dispatch_typed_action(TerminalAction::DismissLabradorifyBanner(
                    remember.to_owned(),
                ));
            })
            .finish(),
    ...
    let do_not_remember = state.remember_for_labradorification(false);
    ...
    render_block_banner(
        |hover_state| {
            if let LabradorificationMode::Ssh {
                hyperlink_index, ..
            } = &state.mode
            {
                let description = Container::new(labradorify_description(app, hyperlink_index))
...
fn render_yes_button(
    state: &LabradorifyBannerState,
    initialize_labradorification_keybinding: &Option<Keystroke>,
    mouse_state: &MouseStateHandle,
    appearance: &Appearance,
) -> Box<dyn Element> {
    let yes_button = match initialize_labradorification_keybinding {
