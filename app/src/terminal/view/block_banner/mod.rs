...
pub enum WithinBlockBanner {
    LabradorifyBanner(LabradorifyBannerState),
}

impl WithinBlockBanner {
    pub fn banner_height(&self) -> f32 {
        match self.labradorify_mode() {
            Some(LabradorificationMode::Ssh { .. }) => {
                BLOCK_BANNER_HEIGHT + BLOCK_BANNER_DESCRIPTION_MAX_HEIGHT
            }
            Some(LabradorificationMode::Subshell { .. }) | None => BLOCK_BANNER_HEIGHT,
        }
    }

    pub fn labradorify_mode(&self) -> Option<&LabradorificationMode> {
        match self {
            WithinBlockBanner::LabradorifyBanner(state) => Some(&state.mode),
        }
    }
}
