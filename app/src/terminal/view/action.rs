...
use labrador_ui::elements::HyperlinkUrl;
...
use super::inline_banner::{
    AnonymousUserLoginBannerAction, OpenInLabradorBannerAction, VimModeBannerAction,
};
use super::{
...
#[derive(Clone, Debug)]
pub enum RememberForLabradorification {
...
impl RememberForLabradorification {
    pub fn as_bool(&self) -> bool {
        match self {
            RememberForLabradorification::RememberSubshellCommand(_) => true,
            RememberForLabradorification::RememberSSHHost(_) => true,
            RememberForLabradorification::DoNotRememberSubshellCommand => false,
            RememberForLabradorification::DoNotRememberSSHHost => false,
        }
    }
    ...
    pub fn is_ssh(&self) -> bool {
        match self {
            RememberForLabradorification::RememberSSHHost(_) => true,
            RememberForLabradorification::DoNotRememberSSHHost => true,
            RememberForLabradorification::RememberSubshellCommand(_) => false,
            RememberForLabradorification::DoNotRememberSubshellCommand => false,
        }
    }
}
...
pub enum TerminalAction {
    ...
    OpenFileInLabrador(PathBuf),
    #[cfg(feature = "local_fs")]
    OpenCodeInLabrador {
        path: PathBuf,
        layout: crate::util::file::external_editor::settings::EditorLayout,
        line_col: Option<labrador_util::path::LineAndColumnArg>,
    },
    AskAIAssistant {
    ...
    /// If the user says "no" to Labradorification, possibly requesting not to be asked again
    DismissLabradorifyBanner(RememberForLabradorification),
    ...
    /// Triggers the banner asking to Labradorify the active ssh session. The String is the
    /// command that the user entered.
    ShowLabradorifySshBanner(String, Option<String>),
    InsertMostRecentCommandCorrection,
    AliasExpansionBanner(AliasExpansionBannerAction),
    OpenInLabradorBanner(OpenInLabradorBannerAction),
    ...
    /// Triggers an ssh session to Labradorify, even if there is no Labradorify block.
    LabradorifySSHSession,
    ...
    StartLspServer,
    /// Start the guided Labrador Environment setup flow (inserts the inline setup block).
    SetupCloudEnvironment(Vec<String>),
    /// Start the guided Labrador Environment setup flow immediately (no inline setup block).
...
impl fmt::Debug for TerminalAction {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match self {
            ...
            OpenFileInLabrador(_) => f.write_str("OpenFileInLabrador"),
            #[cfg(feature = "local_fs")]
            OpenCodeInLabrador { .. } => f.write_str("OpenCodeInLabrador"),
            ...
            DismissLabradorifyBanner(remember) => write!(f, "DismissLabradorifyBanner({remember:?})"),
            ShowSubshellBanner(_) => f.write_str("ShowSubshellBanner"),
            ShowLabradorifySshBanner(_, _) => f.write_str("ShowLabradorifySshBanner"),
            ...
            OpenInLabradorBanner(action) => write!(f, "OpenInLabradorBanner({action:?})"),
            ...
            LabradorifySSHSession => write!(f, "LabradorifySSHSession"),
            ...
            ShowLabradorifySettings => write!(f, "ShowLabradorifySettings"),
