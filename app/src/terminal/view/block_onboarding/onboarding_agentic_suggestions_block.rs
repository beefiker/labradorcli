...
use labrador_ui::elements::CornerRadius;
use labrador_ui::elements::CrossAxisAlignment;
use labrador_ui::elements::FormattedTextElement;
...
use labrador_ui::WeakViewHandle;
use labrador_ui::{
    elements::{
        ...
        let mut content = Flex::row().with_child(
            ConstrainedBox::new(
                self.agent_suggestions[index]
                    .0
                    .icon
                    .to_labrador_ui_icon(main_text_color)
                    .finish(),
            )
            ...
            .with_child({
                let button = Hoverable::new(mouse_state_handle.clone(), |state| {
                    ...
                    let background_color = if is_disabled {
                        ...
                        labrador_core::ui::theme::Fill::Solid(internal_colors::neutral_1(theme))
                    };
                ...
                Flex::row()
                    ...
                    .with_child(
                        Container::new(
                            ConstrainedBox::new(
                                UIIcon::Icon::Loading
                                    .to_labrador_ui_icon(ai_brand_color(appearance.theme()).into())
                                    .finish(),
                            )
