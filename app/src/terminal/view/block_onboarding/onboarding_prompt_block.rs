...
pub struct OnboardingPromptBlock {
    ...
    mouse_state_handle_labrador_prompt: MouseStateHandle,
...
impl OnboardingPromptBlock {
    pub fn new(ps1_grid_info: Option<(BlockGrid, SizeInfo)>) -> Self {
        Self {
            ...
            mouse_state_handle_labrador_prompt: Default::default(),
    ...
    fn render_text(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        const LINK_DESTINATION: &str =
            "https://docs.labrador.dev/terminal/appearance/prompt#custom-prompt-compatibility-table";
    ...
    fn render_confirm_skip_buttons(&self, appearance: &Appearance) -> Box<dyn Element> {
        let mut confirm_button = appearance
            .ui_builder()
            .button(
                labrador_ui::ui_components::button::ButtonVariant::Accent,
        ...
        let col = Flex::column()
            .with_child(
                Hoverable::new(mouse_state_handle.clone(), |state| {
                    ...
                    ConstrainedBox::new(
                        Container::new(if prompt_type == OnboardingPromptType::LabradorDefault {
                            self.render_labrador_prompt_button_interior(appearance)
                        } else {
    ...
    fn render_existing_prompt_button_interior(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        const LINK_DESTINATION: &str = "https://github.com/beefiker/labrador/issues/new?assignees=&labels=Bug&projects=&template=01_bug_report.yml";
    ...
    fn render_labrador_prompt_button_interior(&self, appearance: &Appearance) -> Box<dyn Element> {
        // Pixel values pulled from Figma mocks
        // https://www.figma.com/file/y888viqzWBoMpFTxQqkQEN/Activation?node-id=568:1595&mode=dev
        const HEADER_MARGIN_LEFT: f32 = 4.;
            ...
            .with_child(
                Wrap::row()
                    ...
                    .with_child(self.render_prompt_button(
                        appearance,
                        self.mouse_state_handle_labrador_prompt.clone(),
                        OnboardingPromptType::LabradorDefault,
                    ))
                    .with_child(self.render_prompt_button(
                        appearance,
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            OnboardingPromptBlockAction::PromptSelected(prompt) => {
                ...
                match prompt {
                    OnboardingPromptType::LabradorDefault => {
                        self.selected_prompt = Some(OnboardingPromptType::LabradorDefault);
