...
use base64::Engine;
use labrador_ui::clipboard::{ClipboardContent, ImageData};
mod labradorify_footer;

pub use crate::terminal::CLIAgent;
use labradorify_footer::{LabradorifyFooterView, LabradorifyFooterViewEvent};

use std::sync::{Arc, LazyLock};
use std::time::Duration;

use labrador_ui::r#async::Timer;
...
use labrador_terminal::model::escape_sequences::{BRACKETED_PASTE_END, BRACKETED_PASTE_START};
...
impl TerminalView {
    ...
    fn handle_use_agent_footer_event(
        ...
        match event {
            ...
            UseAgentToolbarEvent::Labradorify { mode } => {
                self.hide_use_agent_footer_in_blocklist(ctx);
                match mode {
                    LabradorificationMode::Ssh { .. } => {
                        self.handle_action(&TerminalAction::LabradorifySSHSession, ctx);
                    }
    ...
    pub(super) fn should_render_use_agent_footer(
        ...
        // If a Labradorify mode is set, ssh or subshell is detected and we should show the footer.
        if self
            .use_agent_footer
            .as_ref(app)
            .labradorify_mode(app)
            .is_some()
        {
            return true;
        }
        ...
        if cli_agent.is_some() {
            ...
            // disable Labrador AI still get the footer for third-party coding agents.
    ...
    pub(super) fn tag_in_agent_for_user_long_running_command(
        ...
        if !self.model.lock().is_alt_screen_active() {
            self.use_agent_footer.update(ctx, |footer, ctx| {
                footer.clear_labradorify_mode(ctx);
            });
            self.hide_use_agent_footer_in_blocklist(ctx);
        }
...
pub struct UseAgentToolbar {
    ...
    // Labradorify footer UI (shown when a subshell/SSH command is detected).
    labradorify_footer_view: ViewHandle<LabradorifyFooterView>,
...
impl UseAgentToolbar {
    pub(crate) fn new(
        ...
        let labradorify_footer_view =
            ctx.add_typed_action_view(|ctx| LabradorifyFooterView::new(terminal_model.clone(), ctx));

        ctx.subscribe_to_view(&labradorify_footer_view, |me, _, event, ctx| {
            me.handle_labradorify_footer_event(event, ctx);
        });
    ...
    fn handle_labradorify_footer_event(
        &mut self,
        event: &LabradorifyFooterViewEvent,
        ctx: &mut ViewContext<Self>,
    ) {
        match event {
            LabradorifyFooterViewEvent::Labradorify { mode } => {
                ctx.emit(UseAgentToolbarEvent::Labradorify { mode: mode.clone() });
            }
            LabradorifyFooterViewEvent::UseAgent => {
                ctx.emit(UseAgentToolbarEvent::UseAgent);
            }
            LabradorifyFooterViewEvent::Dismiss => {
    ...
    pub(in crate::terminal) fn notify_and_notify_children(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        self.labradorify_footer_view.update(ctx, |_, ctx| ctx.notify());
    ...
    /// Sets the current Labradorification mode. When set, the footer shows the
    /// Labradorify view instead of the CLI agent or regular "Use agent" views.
    pub(in crate::terminal) fn set_labradorify_mode(
        &mut self,
        mode: LabradorificationMode,
        ctx: &mut ViewContext<Self>,
    ) {
        self.labradorify_footer_view.update(ctx, |view, ctx| {
            view.set_mode(mode, ctx);
        });
        ctx.notify();
    }

    /// Clears the Labradorification mode so the footer reverts to its default behavior.
    pub(in crate::terminal) fn clear_labradorify_mode(&mut self, ctx: &mut ViewContext<Self>) {
        self.labradorify_footer_view.update(ctx, |view, ctx| {
            view.clear_mode(ctx);
        });
        ctx.notify();
    ...
    pub(in crate::terminal) fn labradorify_mode(
        &self,
        app: &AppContext,
    ) -> Option<LabradorificationMode> {
        self.labradorify_footer_view.as_ref(app).mode().cloned()
    }
...
pub enum UseAgentToolbarEvent {
    ...
    HideRichInput,
    /// User chose to Labradorify the subshell/SSH session.
...
impl View for UseAgentToolbar {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        // If a Labradorify mode is set, delegate rendering to the Labradorify footer view.
        if self.labradorify_footer_view.as_ref(app).mode().is_some() {
            return ChildView::new(&self.labradorify_footer_view).finish();
        }
