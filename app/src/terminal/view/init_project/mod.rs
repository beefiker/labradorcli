...
// Native Labrador rules file format.
...
impl InitStepBlock {
    ...
    fn render_success_completion(text: &str, app: &AppContext) -> Box<dyn Element> {
        RenderableAction::new(text, app)
            .with_icon(Icon::Check.to_labrador_ui_icon(Fill::success()).finish())
            .with_content_item_spacing()
            .render(app)
            .finish()
    }
    ...
    fn render_skipped_completion(text: &str, app: &AppContext) -> Box<dyn Element> {
        RenderableAction::new(text, app)
            .with_icon(Icon::X.to_labrador_ui_icon(Fill::error()).finish())
            .with_content_item_spacing()
            .render(app)
            .finish()
    }
    ...
    fn render_create_environment(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        match &step.status {
            ...
            InitStepStatus::Ready(_) => {
                ...
                Self::render_ready_with_buttons(
                    action_view,
                    "Would you like to create a local environment for this project? The agent will guide you through choosing repos, configuring a Docker image, and specifying startup commands.",
                    app,
                )
            }
            InitStepStatus::Running => {
