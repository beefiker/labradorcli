...
//! We fall back to the existing Labradorification behavior and display this banner so the user knows why advanced features are unavailable.
...
const BANNER_BODY: &str =
    ...
    disabled, the rest of your Labradorified experience is fully available.";
...
impl View for SshRemoteServerFailedBanner {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let icon = Container::new(
            ConstrainedBox::new(Icon::AlertCircle.to_labrador_ui_icon(fg_color.into()).finish())
                .with_width(16.)
                .with_height(16.)
                .finish(),
        )
        ...
        let close = Hoverable::new(self.close_mouse_state.clone(), move |_| {
            ConstrainedBox::new(Icon::X.to_labrador_ui_icon(close_icon_color.into()).finish())
                .with_width(16.)
                .with_height(16.)
                .finish()
        })
