...
use labrador_completer::completer::TopLevelCommandCaseSensitivity;
use labrador_util::path::EscapeChar;

use super::{check_openable_in_labrador, OpenablePath};
...
fn test_single_markdown_file() {
    let result = async_io::block_on(check_openable_in_labrador(
...
fn test_any_text_file_type_supported() {
    ...
    // .svg is a non-markdown file, and we allow opening it in the Labrador Editor (as a "code" file).
    assert_eq!(
        result,
        Some(OpenablePath {
