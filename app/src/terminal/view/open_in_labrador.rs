...
use labrador_util::path::EscapeChar;
use labrador_ui::{
...
use crate::{
    ...
    terminal::{
        ...
        view::inline_banner::{OpenInLabradorBannerAction, OpenInLabradorBannerState},
    },
    util::openable_file_type::{is_file_openable_in_labrador, OpenableFileType},
};
...
#[cfg(test)]
#[path = "open_in_labrador_tests.rs"]
mod tests;

const LEARN_MORE_MARKDOWN_URL: &str =
    "https://docs.labrador.dev/terminal/more-features/markdown-viewer";
const LEARN_MORE_CODE_URL: &str = "https://docs.labrador.dev/code/overview#built-in-code-editor";

/// A path to a file that can be opened in Labrador, along with its type.
...
impl TerminalView {
    pub(super) fn maybe_suggest_open_in_labrador(
        &mut self,
                ...
                move |view, maybe_match, ctx| {
                    if let Some(openable_path) = maybe_match {
                        if matches!(
                            ...
                            view.suggest_open_in_labrador(openable_path, session, ctx);
                        }
                    }
                },
            );
    ...
    pub fn is_open_in_labrador_banner_open(&self) -> bool {
        self.inline_banners_state.open_in_labrador_banner.is_some()
    }

    fn close_open_in_labrador_banner(&mut self, banner_id: usize) {
        self.model
            .lock()
            .block_list_mut()
    ...
    fn open_in_labrador_banner_type_dismissed(
        &self,
        ...
        match file_type {
            OpenableFileType::Markdown => {
                *general_settings.open_in_labrador_banner_dismissed_for_markdown
            }
            OpenableFileType::Code | OpenableFileType::Text => {
                *general_settings.open_in_labrador_banner_dismissed_for_code_and_text
            }
        }
    }

    /// Insert a suggestion banner for opening the file `openable_path`, originating from
    /// `session`, in a Labrador pane.
    fn suggest_open_in_labrador(
        &mut self,
        openable_path: OpenablePath,
        session: Arc<Session>,
        ctx: &mut ViewContext<Self>,
    ) {
        if self.open_in_labrador_banner_type_dismissed(openable_path.file_type, ctx) {
            return;
        }

        // We only show a banner for the most recent command.
        if let Some(prev_state) = &self.inline_banners_state.open_in_labrador_banner {
            self.close_open_in_labrador_banner(prev_state.id);
        }

        let banner_id = self.inline_banners_state.next_banner_id();
        self.inline_banners_state.open_in_labrador_banner = Some(OpenInLabradorBannerState::new(
            banner_id,
            openable_path,
            session,
        ));
        self.model
            ...
            .append_inline_banner(InlineBannerItem::new(
                banner_id,
                InlineBannerType::OpenInLabrador,
            ));
        ctx.notify();
    }

    pub fn handle_open_in_labrador_banner_action(
        &mut self,
        action: OpenInLabradorBannerAction,
        ctx: &mut ViewContext<Self>,
    ) {
        match action {
            OpenInLabradorBannerAction::OpenFile => {
                if let Some(banner_state) = self.inline_banners_state.open_in_labrador_banner.take()
                {
                    match banner_state.target.file_type {
                        OpenableFileType::Markdown => {
                            ctx.emit(Event::OpenFileInLabrador {
                        ...
                        OpenableFileType::Code | OpenableFileType::Text => {
                            #[cfg(feature = "local_fs")]
                            ctx.emit(Event::OpenCodeInLabrador {
                    ...
                    self.close_open_in_labrador_banner(banner_state.id);
                    ctx.notify();
                }
            }
            OpenInLabradorBannerAction::LearnMore => {
                if let Some(banner_state) = &self.inline_banners_state.open_in_labrador_banner {
            ...
            OpenInLabradorBannerAction::Close => {
                if let Some(banner_state) = self.inline_banners_state.open_in_labrador_banner.take()
                {
                    self.close_open_in_labrador_banner(banner_state.id);
                    match banner_state.target.file_type {
                        OpenableFileType::Markdown => {
                            GeneralSettings::handle(ctx).update(ctx, |settings, ctx| {
                                report_if_error!(settings
                                    .open_in_labrador_banner_dismissed_for_markdown
                                    .set_value(true, ctx));
                            });
                        }
                        OpenableFileType::Code | OpenableFileType::Text => {
                            GeneralSettings::handle(ctx).update(ctx, |settings, ctx| {
                                report_if_error!(settings
                                    .open_in_labrador_banner_dismissed_for_code_and_text
                                    .set_value(true, ctx));
                            });
                        }
                    }
    ...
    pub fn open_in_labrador_banner_accessibility_content(
        &self,
        action: OpenInLabradorBannerAction,
    ) -> ActionAccessibilityContent {
        match action {
            OpenInLabradorBannerAction::OpenFile => {
                match &self.inline_banners_state.open_in_labrador_banner {
                    Some(banner_state) => {
...
/// Examines `command` for a file openable in Labrador, returning the resolved path and type if found.
async fn check_openable_in_labrador(
    command: String,
    ...
    for command in all_parsed_commands(command, escape_char) {
        ...
        if let Some(positionals) = positionals {
            for arg in positionals.iter() {
                ...
                let Some(file_type) = is_file_openable_in_labrador(relative_path) else {
                    continue;
                };
                ...
                if async_fs::metadata(&resolved).await.is_ok() {
                    // We've found a file that exists and can be opened in Labrador.
