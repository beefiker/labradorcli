...
/// If appropriate, returns a GridTooltipLink for opening the file in Labrador.
...
fn open_in_labrador_tooltip(
    ...
    use crate::{
        ...
        util::tooltips::should_show_open_in_labrador_link,
    };
    use settings::Setting as _;
    use labrador_ui::SingletonEntity;

    if !should_show_open_in_labrador_link(&path, app) {
        return None;
    }
    ...
    Some(GridTooltipLink {
        ...
        action: TerminalAction::OpenCodeInLabrador {
            path,
            ...
            #[cfg(feature = "local_fs")]
            {
                if let GridHighlightedLink::File(file_link) = link {
                    if let Some(path) = file_link.get_inner().absolute_path() {
                        open_in_labrador = open_in_labrador_tooltip(
                            path.clone(),
                            file_link.get_inner().line_and_column_num,
                            &mut detail,
                            self.mouse_states.open_in_labrador_tooltip.clone(),
                            app,
                        );
