...
use crate::ai::agent::api::labrador_documentation_search_metadata;
use crate::ai::agent::{
...
#[test]
fn test_should_show_task_in_blocklist_hides_labrador_docs_subagent_task() {
    let conversation = create_restored_conversation(labrador_documentation_search_metadata());

    let subtask = conversation
        .all_tasks()
        .find(|task| task.is_labrador_documentation_search_subagent())
        .expect("labrador docs subagent task should exist");
...
#[test]
fn test_exchanges_for_blocklist_excludes_labrador_docs_subagent_exchanges() {
    let conversation = create_restored_conversation(labrador_documentation_search_metadata());
