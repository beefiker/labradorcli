...
impl TerminalManager {
    ...
    pub fn create_model(
        ...
        // This is purely for measuring throughput on Labrador dev builds.
        ...
        // Send Labrador prompt updates.
        ctx.observe_model(&current_prompt, move |current_prompt, ctx| {
            // If for some reason ctx.notify() was called on the Labrador prompt but we're using ps1, do nothing.
            ...
            if let Some(network) = session_sharer_clone.borrow().as_ref() {
                ...
                network.update(ctx, |network, _| {
                    network.send_labrador_active_prompt_update_if_changed(serialized_prompt)
                });
            }
        });
        ...
        ctx.subscribe_to_model(&SessionSettings::handle(ctx), move |_, event, ctx| {
            if let SessionSettingsChangedEvent::HonorPS1 { .. } = event {
                if !*SessionSettings::as_ref(ctx).honor_ps1 {
                    // We don't need to send an active prompt message here when turning off PS1 because this will be sent
                    // as part of observing the Labrador prompt and sending messages on updates.
                    return;
                }
                if let Some(network) = session_sharer_clone.borrow().as_ref() {
                    network.update(ctx, |network, _| {
    ...
    /// Sends bindkey to notify shell process to switch to Labrador prompt logic for prompt
    /// with the combined prompt/command grid (we unset the PS1, but save the value for potential
    /// future restoration).
    pub fn send_switch_to_labrador_prompt_bindkey(&self, app_ctx: &mut AppContext) {
        self.pty_controller.update(app_ctx, |pty_controller, ctx| {
            pty_controller.send_switch_to_labrador_prompt_bindkey(ctx);
        });
    }
    ...
    fn enqueue_init_script(&self, shell_starter: &ShellStarter) -> Result<(), SendError<Message>> {
        ...
        if shell_type == crate::terminal::shell::ShellType::Zsh
            // Git Bash needs the init script enqueued explicitly.
    ...
    fn start_sharing_session(
        ...
        let active_prompt = if *SessionSettings::as_ref(ctx).honor_ps1 {
            ...
            Network::labrador_active_prompt(serialized_prompt)
        };
...
pub fn get_shell_starter(
    ...
    shell_starter_or_wsl_name
        .and_then(|starter| {
            labrador_ui::r#async::block_on(async { starter.to_shell_starter_source().await })
        })
        .map(|starter_source| {
