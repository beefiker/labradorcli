...
fn build_host_shell_command(
    ...
    if let Some(version) = ChannelState::app_version() {
        ...
        // We also insert this app-specific version so that
        // plugins can do app-specific version checks without worrying
        // that the version env var might be coming from a different terminal
        // (for ex., in the ssh case).
        builder.env("LABRADOR_CLIENT_VERSION", version);
    } else {
        ...
        builder.env("LABRADOR_CLIENT_VERSION", "local");
    }
    ...
    if enable_ssh_wrapper {
        builder.env("LABRADOR_USE_SSH_WRAPPER", "1");
    } else {
        builder.env("LABRADOR_USE_SSH_WRAPPER", "0");
    }
    ...
    builder.env("LABRADOR_IS_LOCAL_SHELL_SESSION", "1");

    builder.env(
        "LABRADOR_CLI_AGENT_PROTOCOL_VERSION",
        current_protocol_version().to_string(),
    );

    if shell_debug_mode {
        builder.env("LABRADOR_SHELL_DEBUG_MODE", "1");
    }
    if honor_ps1 {
        builder.env("LABRADOR_HONOR_PS1", "1");
    } else {
        builder.env("LABRADOR_HONOR_PS1", "0");
    }
    ...
    let path_append = extra_path_entries()
        ...
        .join(":");
    builder.env("LABRADOR_PATH_APPEND", path_append);
    ...
    if matches!(shell_starter.shell_type(), ShellType::Bash) {
        ...
        builder.env("LABRADOR_INITIAL_HISTFILESIZE", sentinel_value);
    }
    ...
    if let Some(start_dir) = start_dir {
        builder.env("LABRADOR_INITIAL_WORKING_DIR", start_dir);
    }
...
fn spawn_command_in_pty(
    ...
    #[cfg(target_os = "linux")]
    let is_isolated = labrador_isolation_platform::detect().is_some();
    ...
    unsafe {
        ...
        command.pre_exec(move || {
            ...
            if is_isolated {
                ...
                // in case of OOM. If the Labrador process is killed while hosting an ambient
...
fn build_docker_sandbox_command(
    ...
    // split. Many of these (e.g. `LABRADOR_USE_SSH_WRAPPER`,
    // `SSH_SOCKET_DIR`, `HISTFILESIZE`, `LABRADOR_IS_LOCAL_SHELL_SESSION`)
    ...
    builder.env("LABRADOR_HONOR_PS1", if honor_ps1 { "1" } else { "0" });
    let path_append = extra_path_entries()
        .map(|p| p.to_string_lossy().into_owned())
        .join(":");
    builder.env("LABRADOR_PATH_APPEND", path_append);
    ...
    builder.env("HISTFILESIZE", sentinel_value);
    builder.env("LABRADOR_INITIAL_HISTFILESIZE", sentinel_value);
    // Intentionally do NOT set `LABRADOR_INITIAL_WORKING_DIR` for sandboxes:
...
/// Labrador panes/sandboxes don't race on or share the same host directories.
///
/// The actual sandbox creation + attachment happens via
/// `sbx run --name labrador-sandbox-<id> shell WORKSPACE ... -- -c "cd /home/agent && exec bash --rcfile ..."`
...
/// under the Labrador cache dir, and (2) the stopped `labrador-sandbox-<id>`
...
///   - `sbx rm --force labrador-sandbox-<id>` to drop the container,
...
fn prepare_docker_sandbox(starter: &DockerSandboxShellStarter) -> Result<()> {
    ...
    // the per-user Labrador cache dir rather than `/tmp`) prevents the init
    // script from being read or symlink-attacked by anyone other than the
    // Labrador user. The file itself is left at the default mode so the
