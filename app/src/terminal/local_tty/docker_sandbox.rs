...
//!
//! This module owns everything specific to running a Labrador shell inside a
...
use labrador_ui::{AppContext, SingletonEntity as _};
...
/// Prefix for generated container names: `labrador-sandbox-<id>`.
const DOCKER_SANDBOX_NAME_PREFIX: &str = "labrador-sandbox";

/// Root directory on the host under which Docker-sandbox scratch files
/// (bash init scripts, empty workspace mount points) live.
///
/// Lives under the Labrador per-user cache directory rather than `/tmp` so:
...
fn docker_sandbox_host_root() -> PathBuf {
    labrador_core::paths::cache_dir().join("docker-sandbox")
}

/// Resolves the absolute path to the `sbx` CLI binary using the Labrador
/// process's `PATH`.
///
/// Labrador's process `PATH` is minimal and often misses user-shell-installed
/// tools (e.g. homebrew on Apple Silicon when Labrador is launched from Finder,
...
///
/// Each instance carries a unique `sandbox_id` so multiple Labrador panes can run
...
impl DockerSandboxShellStarter {
    ...
    /// Host directory where Labrador writes this sandbox's bash init script.
