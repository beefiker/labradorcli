...
use labrador_util::path::{canonicalize_git_bash_path, is_msys2_path, labrador_shell_path};
...
/// * On macOS, this includes `$APP_PATH/Contents/Resources/bin`, in which we put a wrapper around the Labrador CLI.
...
pub fn extra_path_entries() -> impl Iterator<Item = PathBuf> {
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            ...
            if let Some(resources_path) = labrador_core::paths::bundled_resources_dir() {
...
/// executable binary for one of Labrador's supported shell types (bash, fish, zsh).
...
impl ShellStarter {
    ...
    /// used to spawn a shell process for a new top-level Labrador session. If a WSL Distribution is
    ...
    pub fn init(preferred_shell: AvailableShell) -> Option<ShellStarterSourceOrWslName> {
        ...
        if let Some(labrador_shell_env_var) = labrador_shell_path() {
            let (labrador_shell_path, shell_type) =
                supported_shell_path_and_type(&labrador_shell_env_var).unwrap_or_else(|| {
                    panic!(
                        "Cannot spawn shell; $LABRADOR_SHELL_PATH or its legacy fallback is invalid: {labrador_shell_env_var}"
                    )
                });
            return Some(
                ShellStarterSource::Environment(DirectShellStarter {
                    args: arguments_for_session_spawning_command(
                        labrador_shell_path.as_path().to_string_lossy().as_ref(),
                        shell_type,
                    ),
                    shell_path: labrador_shell_path,
                    shell_type,
                })
                .into(),
            );
...
pub struct DirectShellStarter {
    ...
    /// Arguments to be passed to the shell binary at [`shell_path`] when spawning a new Labrador
    /// session.
    args: Vec<OsString>,
}
...
pub enum ShellStarterSource {
    ...
    /// The user chose the path to the shell by setting `LABRADOR_SHELL_PATH`
    /// or its legacy fallback environment variable.
    Environment(DirectShellStarter),
    /// The default shell for the user (as indicated by the user's passwd entry on UNIX).
    /// On Windows, this an ordered list of shells hardcoded _by Labrador_.
...
impl WslShellStarter {
    ...
    pub(super) fn home_directory(&self) -> Option<PathBuf> {
        ...
        labrador_util::path::convert_wsl_to_windows_host_path(
...
fn arguments_for_session_spawning_command(
    ...
    match shell_type {
        ShellType::Zsh => {
            ...
            // begins with a space. We use this to hide Labrador bootstrap commands from the history.
            vec![
                "-c".to_owned().into(),
        ...
        ShellType::Bash => {
             ...
             * 5. Send an InitShell message to Labrador through escape sequences.
             * The Labrador message send function is inlined here.
        ...
        ShellType::Fish => {
            ...
            vec![
                ...
                format!(
                    ...
                    // Fish's implementation of this breaks Labrador by emitting `OSC 133 A` but not
