...
use labrador_ui::{r#async::SpawnedFutureHandle, Entity, ModelContext, ModelHandle};
...
/// This prevents Labrador sending an escape sequence to an arbitrary running program.
///
...
pub struct LineEditorStatus {
    ...
    /// not its a Labradorified session, so to in order properly signal downstream that the line editor
    /// (for Labradorified sessions) is active, we must check if there was a corresponding precmd
...
impl LineEditorStatus {
    ...
    fn mark_line_editor_active_after_delay(
        ...
        let abort_handle = ctx.spawn_abortable(
            async move { labrador_ui::r#async::Timer::after(delay).await },
            |me, _, ctx| {
