...
pub fn test_long_running_block_bottom_padding() {
    labrador_ui::r#async::block_on(async {
        let mut block = TestBlockBuilder::new().build();
        ...
        labrador_ui::r#async::Timer::after(Duration::from_millis(duration)).await;
...
pub fn test_block_emits_block_completed_event_for_in_band_command() {
    ...
    let block_completed_event =
        labrador_ui::r#async::block_on(pin!(events_rx).find(|event| match event {
...
/// Regression test (CORE-1947): checks Labrador prompt case for is_command_empty. Specifically,
...
/// be against (0, 0) in the combined grid, for the Labrador prompt case, rather than checking against the
/// prompt grid (which we do in the PS1 active case).
#[test]
fn test_command_is_empty_labrador_prompt() {
    let block_index = BlockIndex::zero();
    ...
    // Note that we are indicating Labrador prompt, not PS1 here!
    let mut block = create_test_block_with_grids(
        block_index,
        prompt_and_command_grid,
        rprompt_grid,
