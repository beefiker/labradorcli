...
use labrador_terminal::model::grid::Dimensions as _;
use labrador_util::path::user_friendly_path;
use labrador_ui::units::{IntoLines, Lines};
use labrador_ui::{r#async::executor::Background, record_trace_event};
...
impl From<&Block> for BlockType {
    fn from(block: &Block) -> Self {
        ...
        match block.bootstrap_stage() {
            ...
            BootstrapStage::LabradorInput | BootstrapStage::Bootstrapped => {
...
pub struct BlockSize {
    ...
    pub labrador_prompt_height_lines: f32,
}
...
pub enum BlockState {
    ...
    /// This block holds static content and is programmatically added to the blocklist by Labrador. An
...
impl Block {
    ...
    pub fn new(
        ...
        Block {
            ...
            event_proxy,
            bootstrap_stage,
            show_bootstrap_block: show_labrador_bootstrap_input,
    ...
    pub fn should_hide_block(&self, agent_view_state: &AgentViewState) -> bool {
        ...
        let is_bootstrap_block = self.bootstrap_stage == BootstrapStage::LabradorInput;
    ...
    /// line prompt, we render on the same line for PS1, but not for Labrador prompt!
    ...
    /// when the shell echoes the command bytes that Labrador wrote to the PTY.
    ...
    pub fn prompt_and_command_grid_offset(&self) -> Lines {
        if self.header_grid.honor_ps1() {
            ...
            // Grid is drawn below custom Labrador prompt in finished blocks.
            self.block_banner_height()
                + self.padding_top()
                + self.prompt_height()
                + self.command_padding_top()
    ...
    /// In the case of combined grid: for Labrador prompt, this includes the height of both the Labrador prompt
    ...
    pub fn prompt_and_command_height(&self) -> Lines {
        if !self.ready_to_render() {
            ...
            // Handle the case of Labrador built-in prompt with combined grid.
    ...
    pub fn top_level_command(&self, sessions: &Sessions) -> Option<String> {
        ...
        let command = labrador_completer::parsers::simple::top_level_command(
        ...
        session
            ...
            .and_then(|s| labrador_completer::parsers::simple::top_level_command(s, escape_char))
    ...
    fn wakeup_after_delay(&self) {
        ...
        self.background_executor
            .spawn(async move {
                labrador_ui::r#async::Timer::after(std::time::Duration::from_millis(delay_ms)).await;
...
impl ansi::Handler for Block {
    ...
    fn linefeed(&mut self) -> ScrollDelta {
        ...
        // prompt, but this is not needed in Labrador due to us visually separating
        // blocks.
        match self.header_grid.receiving_chars_for_prompt {
