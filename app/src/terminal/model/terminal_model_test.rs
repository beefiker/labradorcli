...
use labrador_terminal::model::ansi::ClearMode;
use labrador_ui::text::str_to_byte_vec;
use labrador_ui::text::SelectionType;

#[test]
fn tmux_installation_state_accepts_labrador_tokens() {
    assert_eq!(
        TMUX_INSTALLED_BY_LABRADOR_ROOT_IN_THIS_SESSION
            .parse::<TmuxInstallationState>()
            .unwrap(),
    ...
    assert_eq!(
        TMUX_LABRADOR_INSTALLED_IN_PRIOR_SESSION
            .parse::<TmuxInstallationState>()
            .unwrap(),
        TmuxInstallationState::InstalledByLabradorInPriorSession
    );
}
...
pub fn test_restored_empty_command_block() {
    ...
    // The mocked terminal model comes with a LabradorInput block and the active block.
...
fn test_restored_blocks_on_different_host() {
    let restored_blocks = [
        SerializedBlock {
            ...
            stylized_output: str_to_byte_vec("TestTerminal"),
            pwd: Some("/".to_owned()),
            git_head: None,
            git_branch_name: None,
            virtual_env: None,
    ...
    assert_eq!(blocks[0].output_to_string(), "TestTerminal",);
