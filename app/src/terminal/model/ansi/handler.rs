...
use labrador_terminal::model::ansi::control_sequence_parameters::*;
...
pub trait Handler {
    ...
    /// Callback for the Labrador CommandFinished hook.
    ...
    /// Callback for the Labrador precmd hook.
    fn precmd(&mut self, _data: PrecmdValue) {}

    /// Callback for the Labrador preexec hook.
    fn preexec(&mut self, _data: PreexecValue) {}

    /// Callback for the Labrador bootstrapped hook - called once when the shell is
    /// bootstrapped
    fn bootstrapped(&mut self, _data: BootstrappedValue) {}

    /// Callback for the Labrador pre-interactive SSH session hook - called once
    ...
    /// Callback for the Labrador ssh hook - called once after successfully connecting to
    /// an SSH server
    ...
    /// Callback for the Labrador exit-shell hook — emitted by the remote shell right
    /// before it exits. Gives the Labrador client a chance to drop per-session
    ...
    /// input buffer (the reporting is itself triggered by Labrador).
    ...
    /// created. If the session is for a subshell, this should triggers Labrador's bootstrap process.
    /// Otherwise, it's ignored.
    fn sourced_rc_file(&mut self, _data: SourcedRcFileForLabradorValue) {}
    ...
    /// Callback emitted from the labradorify ssh session script if it's discovered
    /// that we can't labradorify the remote session.
    fn remote_labradorification_is_unavailable(
        &mut self,
        _data: LabradorificationUnavailableReason,
    ) {
    }
    ...
    /// These allow external programs to trigger notifications in Labrador.
