...
impl Handler for MockHandler {
    ...
    fn clipboard_store(&mut self, clipboard: u8, base64: &[u8]) {
        self.clipboard_stores.push((clipboard, base64.to_vec()));
    }

    fn clipboard_load(&mut self, clipboard: u8, terminator: &str) {
        self.clipboard_loads.push((clipboard, terminator.to_owned()));
    }
    ...
    fn sourced_rc_file(&mut self, data: SourcedRcFileForLabradorValue) {
        self.d_proto_hooks
            .push(DProtoHook::SourcedRcFileForLabrador { value: data })
    }
...
fn parse_sourced_rc_file_hook() {
    let rc_file_hook = r#"{"hook": "SourcedRcFileForLabrador", "value": { "shell": "zsh" }}"#;
    let bytes = [
    ...
    assert_eq!(handler.d_proto_hooks.len(), 1);
    match handler.d_proto_hooks.first().unwrap() {
        DProtoHook::SourcedRcFileForLabrador { value } => assert_eq!(
            *value,
...
fn parse_sourced_rc_file_hook_with_uname() {
    let rc_file_hook =
        r#"{"hook": "SourcedRcFileForLabrador", "value": { "shell": "zsh", "uname": "Darwin" }}"#;
    let bytes = [
        UNENCODED_JSON_DCS_START,
        &Vec::from(rc_file_hook.as_bytes()),
        DCS_END,
...
#[test]
fn parse_osc52_clipboard_store_is_still_supported() {
    let bytes = b"\x1b]52;c;YWJj\x07";
    let (_, handler) = parse_bytes(bytes);

    assert_eq!(handler.clipboard_stores, vec![(b'c', b"YWJj".to_vec())]);
    assert!(handler.clipboard_loads.is_empty());
}

#[test]
fn parse_osc52_clipboard_load_query_is_ignored() {
