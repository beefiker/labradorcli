...
/// Indicates that the following JSON-encoded message is hex-encoded for Labrador's lifecycle hooks.
...
/// Indicates that the following JSON-encoded message is unencoded for Labrador's lifecycle hooks.
/// In DCS, it is used as the final char in the DCS start sequence.
/// In OSC, it is used as the first parameter.
pub(super) const UNENCODED_JSON_MARKER: char = 'f';

/// Indicates that the following message is a ANSI-C quoted message for receiving Labrador's lifecycle
/// hooks via key-value pairs.
/// In OSC< it is used as the first parameter.
pub(super) const UNENCODED_KV_MARKER: char = 'k';

/// Enum representing all possible JSON payloads for Labrador's DCS's.
...
pub(super) enum DProtoHook {
    ...
    SourcedRcFileForLabrador {
        value: SourcedRcFileForLabradorValue,
    },
    InitSsh {
        value: InitSshValue,
    },
    ...
    RemoteLabradorificationIsUnavailable {
        // If a value is provided, it's suggesting a way to install TMUX on the remote.
        value: LabradorificationUnavailableReason,
    },
    SshTmuxInstaller {
        value: String,
    },
...
impl DProtoHook {
    pub fn name(&self) -> &'static str {
        match self {
            ...
            DProtoHook::SourcedRcFileForLabrador { .. } => "SourcedRcFileForLabrador",
            DProtoHook::InitSsh { .. } => "InitSsh",
            DProtoHook::FinishUpdate { .. } => "FinishUpdate",
            DProtoHook::RemoteLabradorificationIsUnavailable { .. } => {
                "RemoteLabradorificationIsUnavailable"
            }
    ...
    pub fn default_from_name(hook: &str) -> Option<Self> {
        match hook {
            ...
            "SourcedRcFileForLabrador" => Some(DProtoHook::SourcedRcFileForLabrador {
                value: Default::default(),
            }),
...
/// The reason that labradorification was not available when the user tried
/// to labradorify.
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all(serialize = "snake_case"))]
pub enum LabradorificationUnavailableReason {
    TmuxFailed,
...
impl PrecmdValue {
    ...
    ///
    /// This relies on the assumption that the labrador_precmd shell function (responsible for writing
...
/// Received from the pty after the shell has finished executing Labrador's
/// bootstrap script.
...
/// Received from the pty when Labrador's SSH wrapper is executed, prior to
...
/// created; if the session is for a subshell, this triggers Labrador's bootstrap process.
/// Otherwise, it's ignored.
#[derive(Debug, Default, Deserialize, Serialize, PartialEq, Eq)]
pub struct SourcedRcFileForLabradorValue {
    pub shell: String,
    pub uname: Option<String>,
...
/// The binding is triggered when Labrador writes the `ESC-i` escape sequence to the pty.
/// Labrador usually does this after a block completes, to collect any typeahead
...
/// Received from the pty when labrador_finish_update is called at the end of an
...
/// `logout`, Ctrl-D on an empty prompt, etc.). Lets the Labrador client drop
