...
//! executing one of Labrador's DCS hooks, etc. [`Handler`] should be implemented by
...
pub use labrador_terminal::model::ansi::control_sequence_parameters::*;
...
/// Marks an OSC as one that is sent by Labrador logic registered in the shell.
///
/// Legacy app-local HTTP port.
const LABRADOR_IN_BAND_GENERATOR_OSC_MARKER: &[u8] = b"9277";
const LABRADOR_IN_BAND_GENERATOR_START_BYTE: &[u8] = b"A";
const LABRADOR_IN_BAND_GENERATOR_END_BYTE: &[u8] = b"B";

/// Marks an OSC that is used for messages containing shell hooks.
const LABRADOR_OSC_MARKER: &[u8] = b"9278";
/// Marks an OSC that is used for resetting ConPTY's grid. This is useful for performing a series
/// of checks ensuring that Labrador's grids and ConPTY's grid are in sync.
const LABRADOR_RESET_GRID_OSC_MARKER: &[u8] = b"9279";
...
const LABRADOR_COMPLETIONS_OSC_MARKER: &[u8] = b"9280";
const LABRADOR_COMPLETIONS_START_BYTE: &[u8] = b"A";
const LABRADOR_COMPLETIONS_END_BYTE: &[u8] = b"B";
const LABRADOR_COMPLETIONS_MATCH_RESULT_BYTE: &[u8] = b"C";
...
const LABRADOR_COMPLETIONS_MATCH_UPDATE_METADATA: &[u8] = b"D?";

/// Marks an OSC that tells the terminal that the shell is ready to receive
/// the the string to run completions for.
const LABRADOR_COMPLETIONS_PROMPT_BYTE: &[u8] = b"P";

const LABRADOR_KV_START_BYTE: &[u8] = b"A";
const LABRADOR_KV_ENTRY_BYTE: &[u8] = b"B";
const LABRADOR_KV_END_BYTE: &[u8] = b"C";
...
/// between each update.
///
/// There are two mechanisms to prevent Labrador from falling too behind:
...
impl<'a, H: Handler + 'a, W: io::Write> Performer<'a, H, W> {
    ...
    fn handle_decoded_hook(&mut self, hook: Result<DProtoHook, serde_json::Error>) {
        match hook {
            ...
            Ok(DProtoHook::SourcedRcFileForLabrador { .. }) => {
                // The SourcedRcFileForLabrador hook should only be emitted by the
                // shell without hex encoding. The RC file snippet given to
                // users is not hex-encoded for the sake of transparency and
                // debugability.
                log::error!("Received hex-encoded sourced rc file escape sequence.");
            }
            Ok(DProtoHook::FinishUpdate { value }) => self.handler.finish_update(value),
            Ok(DProtoHook::RemoteLabradorificationIsUnavailable { value }) => {
                self.handler.remote_labradorification_is_unavailable(value)
            }
    ...
    fn handle_unencoded_hook(&mut self, hook: Result<DProtoHook, serde_json::Error>) {
        // Currently, only the `SourcedRcFileForLabrador`, `InitShell`, `InitSubshell`, and `InitSsh`
        ...
        match hook {
            ...
            Ok(DProtoHook::SourcedRcFileForLabrador { value }) => {
            ...
            Ok(_) => {
                log::error!("Received non hex-encoded hook that is not a sourced rc file hook");
            }
            Err(err) => {
                log::warn!("Received malformed sourced rc file hook {err:#}");
            }
        }
    }
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
            ...
            b"52" => {
                ...
                match params[2] {
                    // Ignore clipboard read requests from untrusted terminal output to
                    // prevent clipboard exfiltration via OSC 52 query sequences.
                    b"?" => {}
            ...
            // Received a Labrador OSC used for in-band generators.
            LABRADOR_IN_BAND_GENERATOR_OSC_MARKER => match params.get(1) {
                Some(&LABRADOR_IN_BAND_GENERATOR_START_BYTE) => {
                    log::info!(
                        "Received a Labrador OSC marker for starting in-band command output."
                    );
                    self.handler.start_in_band_command_output();
                }
                Some(&LABRADOR_IN_BAND_GENERATOR_END_BYTE) => {
                    self.handler.end_in_band_command_output(true);
                }
                _ => {
                    log::warn!("Received a Labrador OSC marker missing required param.");
                }
            },

            // Received a Labrador OSC used for shell hooks.
            LABRADOR_OSC_MARKER => {
                ...
                match json_marker_char {
                    HEX_ENCODED_JSON_MARKER => {
                        ...
                        let Some(data_str) = params
                            ...
                            log::error!("Labrador OSC marker did not contain payload");
                            return;
                        };
                        safe_debug!(
                            safe: ("Received Labrador OSC string for shell hook"),
                            full: ("Received Labrador OSC string for shell hook with JSON payload: {:?}", data_str)
                        );
            ...
            LABRADOR_RESET_GRID_OSC_MARKER => {
                log::debug!("Received Labrador OSC string for reset grid");
                self.handler.on_reset_grid();
            }

            // Received a Labrador OSC used for completions.
            LABRADOR_COMPLETIONS_OSC_MARKER => match params.get(1) {
                Some(&LABRADOR_COMPLETIONS_START_BYTE) => {
                    let Some(format) = params
                        ...
                        log::warn!(
                            "Labrador start completions OSC marker contained invalid format."
                        );
                        return;
                    };
                    self.handler.start_completions_output(format);
                }
                Some(&LABRADOR_COMPLETIONS_END_BYTE) => {
                    self.handler.end_completions_output();
                }
                Some(&LABRADOR_COMPLETIONS_MATCH_RESULT_BYTE) => {
                    // The payload for the OSC is contained in the third parameter.
                    let Some(data_str) = params
                        .get(2)
                        .map(|osc_data| String::from_utf8_lossy(osc_data))
                        ...
                        log::warn!(
                            "Labrador completions match result OSC marker did not contain payload"
                        );
                        return;
                    };
                ...
                Some(bytes) if bytes.starts_with(LABRADOR_COMPLETIONS_MATCH_UPDATE_METADATA) => {
                    ...
                    let Some(data_str) = params
                        ...
                        log::warn!(
                            "Labrador completions match metadata OSC marker did not contain payload"
                        );
                        return;
                    };
                    ...
                    match &parameter[2..] {
                        ...
                        _ => {
                            log::warn!("Invalid Labrador OSC marker parameter for completions match metadata: {parameter}");
                        }
                    }
                }
                Some(&LABRADOR_COMPLETIONS_PROMPT_BYTE) => {
                    self.handler.send_completions_prompt();
                }
                _ => {
                    log::warn!(
                        "Received a Labrador OSC completions marker missing required param."
                    );
                }
            },
