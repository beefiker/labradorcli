...
use labrador_completer::completer::CommandOutput;
use labrador_ui::ModelContext;
...
fn new_command_executor_for_local_tty_session(
    ...
    if FeatureFlag::SSHTmuxWrapper.is_enabled()
        ...
        // We don't allow nested tmux labradorification, so if our parent session is already labradorified using
    ...
    // `sbx exec labrador-sandbox-<id> -- sh -c "<cmd>"` (analogous to
    ...
    match &session_info.session_type {
        ...
        _ => {
            if *should_force_disable_in_band_generators {
                ...
                // codepath is only accessible via a user defaults command that a Labrador
