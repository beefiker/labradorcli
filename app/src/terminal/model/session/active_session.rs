use std::sync::Arc;
use labrador_ui::{AppContext, Entity, ModelContext, ModelHandle};
...
impl ActiveSession {
    ...
    /// Returns the `LabradorAiExecutionContext` for the active session.
    pub fn ai_execution_environment(&self, app: &AppContext) -> Option<LabradorAiExecutionContext> {
        self.session(app).as_ref().map(LabradorAiExecutionContext::new)
    }
}
