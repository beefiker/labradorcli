...
use labrador_terminal::model::Point;
...
use labrador_util::on_cancel::OnCancelFutureExt;
            ...
            }) = running_command.as_ref()
            {
                ...
                let in_band_command = match shell.shell_type() {
                    ShellType::PowerShell => {
                        format!("Labrador-Run-GeneratorCommand {id} '{escaped_command}' -ErrorAction Ignore")
                    }
                    ShellType::Fish => {
                        ...
                        // fish.
                        format!(" labrador_run_generator_command {id} '{escaped_command}'")
                    }
                    _ => {
                        format!("labrador_run_generator_command {id} '{escaped_command}'")
                    }
...
impl CommandExecutor for InBandCommandExecutor {
    ...
    /// `labrador_run_generator_command`/`Labrador-Run-GeneratorCommand` shell script API that is declared as
    /// part of Labrador's bootstrap script.
    ///
    ...
    /// shell side in `labrador_preexec`). This merely clears the running and pending command IDs from
...
pub fn is_in_band_command(command: &str) -> bool {
    ...
    trimmed.starts_with("Labrador-Run-GeneratorCommand ")
        || trimmed.starts_with("labrador_run_generator_command ")
}
