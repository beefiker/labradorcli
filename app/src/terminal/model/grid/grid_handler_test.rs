...
use labrador_terminal::model::char_or_str::CharOrStr;
...
fn nested_regex() {
    ...
    let dfas = RegexDFAs::new("Te.*st").unwrap();
    ...
    let dfas = RegexDFAs::new("Te[^y]*st").unwrap();
    let start = Point::new(0, 0);
...
fn test_secrets_serialization() {
    let mut blockgrid = mock_blockgrid("foo zach@test.dev bar");
...
fn test_find_url_with_delimiter() {
    ...
    let blockgrid = mock_blockgrid("https://google.com/search?q=test");
    assert_eq!(
        blockgrid
            .grid_handler
            .url_at_point(Point { row: 0, col: 0 }),
...
fn test_find_url_omits_trailing_periods() {
    ...
    let blockgrid = mock_blockgrid("Visit https://github.com/exampleorg/Test/issues.");
    assert_eq!(
        blockgrid
            .grid_handler
            .url_at_point(Point { row: 0, col: 10 }),
    ...
    let blockgrid = mock_blockgrid("Visit https://github.com/test.dev/Test/issues.");
    assert_eq!(
        blockgrid
            .grid_handler
            .url_at_point(Point { row: 0, col: 10 }),
