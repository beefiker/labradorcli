...
use labrador_terminal::model::ansi::CharsetIndex;
use labrador_terminal::model::grid::cell;
use labrador_terminal::model::{KeyboardModes, KeyboardModesApplyBehavior};
use labrador_ui::image_cache::{resize_dimensions, FitType};
...
pub(super) struct State {
    ...
    /// correctly. Notably, Zsh does NOT support this well in bracketed paste mode (which we use for all Labrador
...
impl ansi::Handler for GridHandler {
    ...
    fn identify_terminal<W: std::io::Write>(&mut self, writer: &mut W, intermediate: Option<char>) {
        match intermediate {
            ...
            Some('>') => {
                ...
                // Labrador, rather than passing the higher `Pv` value, we decided to use one of the
    ...
    fn report_xtversion<W: std::io::Write>(&mut self, writer: &mut W) {
        ...
        let _ = writer.write_all(format!("\x1bP>|Labrador({version})\x1b\\").as_bytes());
    }
    ...
    fn set_color(&mut self, _: usize, _: labrador_ui::color::ColorU) {
