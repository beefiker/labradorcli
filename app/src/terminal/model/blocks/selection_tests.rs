    ...
    App::test((), |app| async move {
        app.read(|ctx| {
            ...
            let block_index = insert_block(
                &mut block_list,
                "echo https://labrador.dev/about hello/world.js\n",
                "https://labrador.dev/about hello/world.js\n",
            );
            ...
            assert_eq!(
                ...
                Some("https://labrador.dev/about".to_string())
            );
            block_list.clear_selection();

            // Start a selection in "labrador", which has wrapped to the third
            // line of the command grid.
            block_list.start_selection(
            ...
            block_list.start_selection(
                BlockListPoint::new(command_grid_offset + 3.0, 5),
                SelectionType::Semantic,
                Side::Left,
            );
            block_list.update_selection(
                BlockListPoint::new(command_grid_offset + 4.0, 5),
                Side::Right,
            );

            assert_eq!(
                block_list.selection_to_string(&semantic_selection, false, ctx),
                Some("https://labrador.dev/about hello".to_string())
            );
            block_list.clear_selection();
...
fn test_smart_selection_in_multiple_blocks() {
    App::test((), |app| async move {
        app.read(|ctx| {
            ...
            assert_eq!(
                block_list.selection_to_string(&semantic_selection, false, ctx),
                Some("https://labrador.dev/about hello/world.js\nhttps".to_string())
            );
            block_list.clear_selection();

            // Start a selection at "e" in "hello" in the 1st command (which has wrapped
            // to the fifth line of the command grid) to the "6" in the "168" in
            // the 2nd output.
            block_list.start_selection(
                BlockListPoint::new(first_command_grid_offset + 4.0, 5),
                SelectionType::Semantic,
                Side::Left,
            );
            block_list.update_selection(
        ...
        Some(
            ...
            block_list.update_selection(
                BlockListPoint::new(first_output_grid_offset + 2., 4),
                Side::Right,
            );
...
fn test_smart_selection_override() {
    App::test((), |app| async move {
        app.read(|ctx| {
            ...
            let block_index = insert_block(
                &mut block_list,
                "echo https://labrador.dev/about hello/world",
                "https://labrador.dev/about hello/world",
            );

            // the override wraps "https://labrador.dev/about hello/world"
            block_list.set_smart_select_override(WithinBlock::new(
                Point::new(0, 5)..=Point::new(6, 0),
                block_index,
            ...
            // Start a selection inside the selected URL span.
            ...
            assert_eq!(
                block_list.selection_to_string(&semantic_selection, false, ctx),
                Some("https://labrador.dev/about hello/world".to_string())
            );
        })
    })
}
