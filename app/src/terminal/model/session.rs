...
// SessionId is defined in labrador_core and re-exported here for backward compatibility.
pub use labrador_core::SessionId;
...
impl Sessions {
    ...
    pub fn initialize_bootstrapped_session(
        ...
        // For labradorified-remote sessions, pick up the current host_id from
        ...
        if FeatureFlag::SshRemoteServer.is_enabled()
            && matches!(
                ...
                BootstrapSessionType::LabradorifiedRemote
            )
        {
        ...
        let _labrador_attributed_bootstrap_duration_seconds =
        ...
        crate::History::handle(ctx).update(ctx, |history, ctx| {
...
impl From<SessionType> for command_corrections::SessionType {
    fn from(session_type: SessionType) -> Self {
        match session_type {
            SessionType::LabradorifiedRemote { .. } => command_corrections::SessionType::Remote,
...
impl SessionInfo {
    ...
    pub fn create_pending(
        ...
        is_labradorified_ssh_session: bool,
        ...
        let session_type = Self::determine_session_type(
            &init_shell_value,
            is_labradorified_ssh_session
                || matches!(&is_legacy_ssh_session, IsLegacySSHSession::Yes { .. }),
        );

        let spawning_session_id =
            if matches!(session_type, BootstrapSessionType::LabradorifiedRemote)
    ...
    fn determine_session_type(
        ...
        _is_labradorified_ssh_session: bool,
    ) -> BootstrapSessionType {
...
pub enum BootstrapSessionType {
    /// The session host is the same host where Labrador is running.
    Local,

    /// The session host is a different host from where Labrador is running.
    LabradorifiedRemote,
}
...
pub enum SessionType {
    ...
    /// The session host is a different host from where Labrador is running.
    /// Note that we only know this for sure when we Labradorify a block.
    ///
    ...
    /// established yet.
    LabradorifiedRemote { host_id: Option<labrador_core::HostId> },
}
...
impl From<BootstrapSessionType> for SessionType {
    fn from(bst: BootstrapSessionType) -> Self {
        match bst {
            ...
            BootstrapSessionType::LabradorifiedRemote => {
                SessionType::LabradorifiedRemote { host_id: None }
            }
        }
    }
}
...
impl Session {
    ...
    /// Updates the `host_id` on a `LabradorifiedRemote` session type after the
    /// remote server handshake completes (or clears it on disconnect).
    pub fn set_remote_host_id(&self, host_id: Option<labrador_core::HostId>) {
        let mut st = self.session_type.lock();
        if let SessionType::LabradorifiedRemote { host_id: ref mut h } = *st {
            *h = host_id;
        }
    }
    ...
    pub fn home_dir(&self) -> Option<&str> {
        if cfg!(test) {
            return labrador_util::path::TEST_SESSION_HOME_DIR.as_deref();
        }
    ...
    pub fn is_subshell_or_ssh(&self) -> bool {
        matches!(self.session_type(), SessionType::LabradorifiedRemote { .. })
    ...
    pub fn windows_path_converter(&self) -> Option<fn(&str) -> String> {
        if self.is_wsl() {
            Some(labrador_util::path::convert_windows_path_to_wsl)
        } else if self.is_msys2() {
            Some(labrador_util::path::convert_windows_path_to_msys2)
        } else {
            None
        }
    }
    ...
    pub fn convert_directory_to_typed_path_buf(&self, pwd: String) -> TypedPathBuf {
        ...
        // or unix file paths. This needs to be resilient to labradorified ssh. Some examples:
        // - bash on mac ---> unix
        // - powershell on linux ---> unix
        // - powershell on windows ---> windows
        // - wsl on windows ---> unix
        // - labradorified zsh --> unix

        // If the host architecture is unix, we can infer unix file paths. This would break
        // if we supported labradorifying a powershell-on-windows SSH session.
        if cfg!(unix) {
        ...
        match self.shell_family() {
            // Cases: WSL, MSYS2, labradorified bash
...
/// Returns the hostname for the local machine where Labrador is running.
...
pub mod testing {
    ...
    impl SessionInfo {
        ...
        pub fn with_ssh_socket_path(mut self, socket_path: PathBuf) -> Self {
            if let BootstrapSessionType::Local = self.session_type {
                self.session_type = BootstrapSessionType::LabradorifiedRemote;
            }
    ...
    impl Session {
        ...
        pub fn test_remote() -> Self {
            let info = SessionInfo::new_for_test()
                .with_session_type(BootstrapSessionType::LabradorifiedRemote)
