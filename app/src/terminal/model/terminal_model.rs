...
use super::ansi::{
    FinishUpdateValue, InputBufferValue, LabradorificationUnavailableReason, Mode, PendingHook,
    TmuxInstallFailedInfo,
};
use super::block::{
...
use labrador_ui::util::save_as_file;
...
pub use labrador_terminal::model::BlockIndex;
use labrador_terminal::model::{KeyboardModes, KeyboardModesApplyBehavior};
use labrador_ui::assets::asset_cache::Asset;
...
/// Information needed to render a labradorify "success" block upon successful subshell bootstrap.
#[derive(Debug, Clone)]
...
pub enum TmuxInstallationState {
    /// This means tmux was installed by Labrador in this session, successfully or unsuccessfully.
    /// It also means we had root access and used a package manager to install tmux and all
    /// dependencies.
    InstalledByLabradorRootInThisSession,
    /// This means tmux was installed by Labrador in this session, successfully or unsuccessfully.
    InstalledByLabradorInThisSession,
    InstalledByLabradorInPriorSession,
    /// This means that Labrador did not install it locally. It was either installed by the user
    /// or it was installed by Labrador in a prior session using the package manager.
    InstalledByUser,
...
const TMUX_INSTALLED_BY_LABRADOR_ROOT_IN_THIS_SESSION: &str =
    "installed_by_labrador_root_in_this_session";
const TMUX_INSTALLED_BY_LABRADOR_IN_THIS_SESSION: &str = "installed_by_labrador_in_this_session";
const TMUX_LABRADOR_INSTALLED_IN_PRIOR_SESSION: &str = "labrador";
const TMUX_INSTALLED_BY_LABRADOR_IN_PRIOR_SESSION: &str = "installed_by_labrador_in_prior_session";
...
impl FromStr for TmuxInstallationState {
    ...
    fn from_str(input: &str) -> Result<Self, Self::Err> {
        match input {
            TMUX_INSTALLED_BY_LABRADOR_ROOT_IN_THIS_SESSION => {
                Ok(TmuxInstallationState::InstalledByLabradorRootInThisSession)
            }
            TMUX_INSTALLED_BY_LABRADOR_IN_THIS_SESSION => {
                Ok(TmuxInstallationState::InstalledByLabradorInThisSession)
            }
            TMUX_LABRADOR_INSTALLED_IN_PRIOR_SESSION
            | TMUX_INSTALLED_BY_LABRADOR_IN_PRIOR_SESSION => {
                Ok(TmuxInstallationState::InstalledByLabradorInPriorSession)
            }
...
pub struct LabradorInitiatedTmuxControlMode {
    pub start_time: Instant,
    pub tmux_installation: Option<TmuxInstallationState>,
}

impl LabradorInitiatedTmuxControlMode {
...
pub enum TmuxControlModeContext {
    UserInitiated,
    LabradorInitiatedForSsh(LabradorInitiatedTmuxControlMode),
}
...
impl TmuxControlModeContext {
    pub fn tmux_installation(&self) -> Option<TmuxInstallationState> {
        match self {
            ...
            TmuxControlModeContext::LabradorInitiatedForSsh(labrador_initiated) => {
                labrador_initiated.tmux_installation
            }
        }
    }
}
...
pub struct TerminalModel {
    ...
    /// This variable allows us to differentiate between Labrador-initiated and user-initiated invocations of
    /// control mode. Whenever we attempt to labradorify an ssh session, we track the context of when Labrador initiated
    ...
    pending_labrador_initiated_control_mode: Option<LabradorInitiatedTmuxControlMode>,
...
pub enum TerminalInputState {
    ...
    AltScreen,
    /// Labrador Input View is visible.
    InputEditor,
...
impl TerminalModel {
    ...
    fn new_internal(
        ...
        Self {
            ...
            pending_labrador_initiated_control_mode: None,
    ...
    /// Previously sent an Agent ResponseEvent to viewers if this session was shared.
    /// Shared sessions have been removed; this is now a no-op kept for call-site compatibility.
    pub fn send_agent_response_for_shared_session(
        &mut self,
        _response: &labrador_multi_agent_api::ResponseEvent,
        _response_initiator: Option<ParticipantId>,
    ...
    pub fn set_pending_labrador_initiated_control_mode(&mut self) {
        let tmux_installation = self
            .tmux_control_mode_context
            .and_then(|context| context.tmux_installation());
        self.pending_labrador_initiated_control_mode =
            Some(LabradorInitiatedTmuxControlMode::new(tmux_installation));
    }

    pub fn set_pending_labrador_initiated_control_mode_with_install_tmux(
        &mut self,
        with_root: bool,
    ) {
        self.pending_labrador_initiated_control_mode =
            Some(LabradorInitiatedTmuxControlMode::new(Some(if with_root {
                TmuxInstallationState::InstalledByLabradorRootInThisSession
            } else {
                TmuxInstallationState::InstalledByLabradorInThisSession
            })));
    }

    pub fn clear_pending_labrador_initiated_control_mode(&mut self) {
        self.pending_labrador_initiated_control_mode = None;
    }
    ...
    /// before indicating we're ready for labradorification.
            ...
            SshLoginState::LastLogin | SshLoginState::PromptDetected => {
                self.event_proxy
                    .send_terminal_event(Event::DetectedEndOfSshLogin(
                        SshLoginStatus::ReadyToLabradorify,
                    ));
            ...
            SshLoginState::NonSshOutput => {
                ...
                if is_initial_check {
                    if ssh_login_state.notification_state == SshLoginNotificationState::Monitoring {
                        self.event_proxy
                            .send_terminal_event(Event::DetectedEndOfSshLogin(
                                SshLoginStatus::RecheckBeforeLabradorifying,
                            ));
    ...
    pub fn is_pending_labrador_initiated_control_mode(&self) -> bool {
        self.pending_labrador_initiated_control_mode.is_some()
    }

    pub fn is_labradorified_ssh(&self) -> bool {
        matches!(
            self.tmux_control_mode_context,
            Some(TmuxControlModeContext::LabradorInitiatedForSsh { .. })
        )
    }
}
...
impl ansi::Handler for TerminalModel {
    ...
    fn set_color(&mut self, index: usize, color: labrador_ui::color::ColorU) {
    ...
    fn init_subshell(&mut self, data: InitSubshellValue) {
        let is_tmux_ssh = self.pending_labrador_initiated_control_mode.is_some();
        ...
        if let Some(shell_type) = shell_type {
            ...
            if is_tmux_ssh {
                self.event_proxy
                    .send_terminal_event(Event::RemoteLabradorificationIsUnavailable(
    ...
    fn sourced_rc_file(&mut self, data: SourcedRcFileForLabradorValue) {
        ...
        if self.block_list.is_bootstrapped() {
            ...
            match shell_type {
                ...
                None => {
                    log::error!(
                        "Received invalid shell name in sourced rc file hook: {}",
                        data.shell
                    );
                }
            }
    ...
    fn remote_labradorification_is_unavailable(
        &mut self,
        data: LabradorificationUnavailableReason,
    ) {
        self.event_proxy
            .send_terminal_event(Event::RemoteLabradorificationIsUnavailable(data));
    }

    fn notify_ssh_tmux_is_installed(&mut self, tmux_installation: TmuxInstallationState) {
        if let Some(ref mut labrador_initiated_for_ssh) =
            self.pending_labrador_initiated_control_mode
        {
            labrador_initiated_for_ssh.tmux_installation = Some(tmux_installation);
        }
        self.event_proxy
    ...
    fn tmux_control_mode_event(&mut self, event: tmux::ControlModeEvent) {
        match event {
            ...
            tmux::ControlModeEvent::Starting => {
                if let Some(labrador_initiated_for_ssh) =
                    self.pending_labrador_initiated_control_mode
                {
                    self.tmux_control_mode_context = Some(
                        TmuxControlModeContext::LabradorInitiatedForSsh(labrador_initiated_for_ssh),
                    );
                } else {
...
/// Validates and decodes in-band command output sent via `labrador_send_generator_output_osc_message`.
