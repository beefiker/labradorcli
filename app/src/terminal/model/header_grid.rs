...
//! Labrador. This struct is abstracted away from Block, for the purposes of enabling same-line prompt,
...
use labrador_ui::units::{IntoLines as _, Lines};
...
pub struct HeaderGrid {
    ...
    /// The height of the Labrador prompt in lines (non-PS1).
...
impl HeaderGrid {
    ...
    pub fn set_honor_ps1(&mut self, honor_ps1: bool) {
        ...
        if !self.honor_ps1 {
            // If we are switching to Labrador prompt (from PS1), we need to clear the cached prompt end point
    ...
    fn is_command_finished_and_empty(&self) -> bool {
        if !self.honor_ps1 {
            // If we are using Labrador prompt, we expect the combined grid cursor to be at the start, if
    ...
    pub fn prompt_height(&self) -> Lines {
        ...
        if self.is_command_empty() {
            ...
            self.labrador_prompt_height_lines.into_lines()
        }
    }
    ...
    fn precmd(&mut self, data: PrecmdValue) {
        if let Some(honor_ps1) = data.honor_ps1 {
            if honor_ps1 != self.honor_ps1 {
                ...
                // switch the prompt mode via the $LABRADOR_HONOR_PS1 environment variable.
                self.event_proxy
                ...
                // to be emitted. This may be de-synced from Labrador settings in particular niche cases (which are
