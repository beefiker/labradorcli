...
use crate::{
    ...
    ai_assistant::execution_context::LabradorAiExecutionContext,
...
fn cloud_mode_v2_hint_text() -> String {
    format!("Kick off a {} agent", ChannelState::app_name_display())
}
...
pub enum CommandExecutionSource {
    /// A non-shared command execution request from Labrador AI++.
...
impl Input {
    ...
    fn handle_prompt_alert(
        ...
        match prompt_alert {
            ...
            PromptAlertEvent::OpenBillingAndUsagePage => {
                ctx.emit(Event::OpenSettings(SettingsSection::LabradorAgent));
            }
    ...
    /// Predicts the next action using an AI model and past context on blocks within Labrador.
    ...
    fn maybe_predict_next_action_ai(
        ...
        let context = LabradorAiExecutionContext::new(&session);
        ...
        match event {
                            ...
                            #[cfg(feature = "local_fs")]
                            {
                                ...
                                let processed_path = self
                                    ...
                                    .and_then(|pwd| {
                                        ...
                                        let relative_path = labrador_util::path::to_relative_path(
                                            is_wsl,
                                            &absolute_path,
                                            Path::new(pwd),
                                        );
            ...
            EditorEvent::DroppedImageFiles(image_filepaths) => {
                ...
                if num_attached < image_filepaths.len() {
                    ...
                    let paths_str =
                        labrador_ui::clipboard_utils::escaped_paths_str(&transformed, shell_family);
    ...
    fn process_paste_event(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if labrador_ui::clipboard::should_insert_text_on_paste(&content) {
        ...
        let should_insert_text = if content.has_image_data() {
            ...
            let image_filepaths = labrador_ui::clipboard_utils::get_image_filepaths_from_paths(
    ...
    fn open_completion_suggestions(
        ...
        // However, completions are disabled on labradorified remote hosts because
        // in-band generators don't work in this context (with CLI agent).
        let is_cli_agent_shell_mode = self.is_locked_in_shell_mode(ctx)
            && CLIAgentSessionsModel::as_ref(ctx).is_input_open(self.terminal_view_id)
            && !self.active_session(ctx).is_some_and(|s| {
                matches!(s.session_type(), SessionType::LabradorifiedRemote { .. })
            });
    ...
    /// `is_for_same_conversation: true` keeps the conversation status `InProgress` so the agent
    /// status indicator stays visible.
    ...
    fn render_attached_chip(
        ...
        .with_icon(icon.to_labrador_ui_icon(
