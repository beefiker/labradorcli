...
pub enum Event {
    ...
    /// External programs can use this to trigger notifications in Labrador.
    ///
    /// References:
...
pub enum BlockType {
    ...
    /// This is a block containing static/hardcoded content (e.g. the subshell Labradorification
    /// welcome block).
    Static,
}
...
impl Debug for Event {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Event::RemoteLabradorificationIsUnavailable(_) => {
                write!(f, "RemoteLabradorificationIsUnavailable")
            }
