...
use labrador_ui::{AppContext, AssetProvider, SingletonEntity};
...
use super::{
    labradorify::settings::{PIPENV_SUBSHELL_COMMAND_REGEX, POETRY_SUBSHELL_COMMAND_REGEX},
...
/// Returns `true` if Labrador should use an RC-file based bootstrap (e.g. dump the bootstrap script to
...
pub fn should_use_rc_file_bootstrap_method(
    ...
    match session_type {
        ...
        BootstrapSessionType::LabradorifiedRemote => false,
    }
}
...
pub fn init_subshell_command(
    ...
    match shell_type {
        Some(shell_type) => {
            ...
            format!(r#" [ -z "$LABRADOR_BOOTSTRAPPED" ] && eval '{subshell_script}'"#)
        }
