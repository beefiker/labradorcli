...
/// For some [`SshLabradorifyCommand`]s, we do not support parsing
...
impl SshLabradorifyCommand {
    pub fn matches(command: &str) -> Option<SshLabradorifyCommand> {
        let command = if let Some(suffix) = command.strip_prefix("command ") {
            suffix
        } else {
            command
        ...
        if INTERACTIVE_SSH.is_match(command) {
            Some(SshLabradorifyCommand::Ssh)
        } else if GCLOUD_REGEX.is_match(command) {
            Some(SshLabradorifyCommand::SshLike(SshLikeCommand::Gcloud))
        } else if ELASTIC_BEANSTALK_REGEX.is_match(command) {
            Some(SshLabradorifyCommand::SshLike(SshLikeCommand::ElasticBeanstalk))
        } else if DIGITAL_OCEAN_DROPLET_REGEX.is_match(command) {
            Some(SshLabradorifyCommand::SshLike(
...
pub fn parse_interactive_ssh_command(command: &str) -> Option<InteractiveSshCommand> {
    match SshLabradorifyCommand::matches(command) {
        Some(SshLabradorifyCommand::Ssh) => InteractiveSshCommand::parse_ssh_command(command),
        Some(SshLabradorifyCommand::SshLike(SshLikeCommand::Gcloud)) => {
        ...
        Some(SshLabradorifyCommand::SshLike(SshLikeCommand::DigitalOceanDroplet)) => {
            Some(InteractiveSshCommand::default())
        }
        None => None,
    }
...
/// Creates an sftp command that copies a given local file into the pwd in the labradorified ssh session.
