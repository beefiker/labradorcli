...
use crate::terminal::labradorify::render;
...
use labrador_ui::ui_components::toggle_menu::ToggleMenuStateHandle;
use labrador_ui::{
    elements::{Border, Container, CrossAxisAlignment, Flex, ParentElement},
    AppContext, Element, Entity, SingletonEntity, TypedActionView, View, ViewContext,
};
use labrador_ui::{BlurContext, FocusContext};

pub const WHY_INSTALL_TMUX_URL: &str =
    "https://docs.labrador.dev/terminal/labradorify/ssh#why-do-i-need-tmux-on-the-remote-machine";
...
pub enum SshInstallTmuxBlockEvent {
    InstallTmuxAndLabradorify(TmuxInstallMethod),
...
pub struct SshInstallTmuxBlock {
    ...
    /// The script to install tmux locally, in a ~/.labrador directory
...
impl SshInstallTmuxBlock {
    ...
    pub fn emit_install_tmux(
        ...
        ctx.emit(SshInstallTmuxBlockEvent::InstallTmuxAndLabradorify(
            install_method,
        ));
        ctx.notify()
    }
...
impl SshInstallTmuxBlock {
    ...
    fn render_system_install_ui(
        ...
        Container::new(requested_script::render_requested_scripts(
            ...
            TitledScript {
                title: "Install to ~/.labrador".to_string(),
    ...
    fn render_title_ui(
        ...
        let right_hand_size = is_awaiting_action
            .then(|| {
                render::render_never_labradorify_ssh_link(
                    &self.ssh_host,
                    app,
                    appearance,
...
impl View for SshInstallTmuxBlock {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        content
            .add_child(render::apply_spacing_styles(Container::new(labradorify_description)).finish());
...
pub fn install_tmux_script(system: &SystemDetails, app: &AppContext) -> Option<String> {
    ...
    let asset_source = match (
        ...
        ("Linux", _, "bash" | "zsh") => {
            bundled_asset!("ssh/bash_zsh/install_tmux_and_labradorify_linux.sh")
        }
        ("Linux", _, "fish") => {
            bundled_asset!("ssh/fish/install_tmux_and_labradorify_linux.sh")
        }
        ("Darwin", "homebrew", "bash" | "zsh") => {
            bundled_asset!("ssh/bash_zsh/install_tmux_and_labradorify_brew.sh")
        }
        ("Darwin", "homebrew", "fish") => {
            bundled_asset!("ssh/fish/install_tmux_and_labradorify_brew.sh")
        }
        _ => return None,
    };
...
pub fn install_root_tmux_script(
    ...
    let asset_source = match (
        ...
        ("Linux", "apt", "bash" | "zsh") => {
            bundled_asset!("ssh/bash_zsh/root/install_tmux_and_labradorify_apt.sh")
        }
        ("Linux", "dnf", "bash" | "zsh") => {
            bundled_asset!("ssh/bash_zsh/root/install_tmux_and_labradorify_dnf.sh")
        }
        ("Linux", "pacman", "bash" | "zsh") => {
            bundled_asset!("ssh/bash_zsh/root/install_tmux_and_labradorify_pacman.sh")
        }
        ("Linux", "yum", "bash" | "zsh") => {
            bundled_asset!("ssh/bash_zsh/root/install_tmux_and_labradorify_yum.sh")
        }
        ("Linux", "zypper", "bash" | "zsh") => {
            bundled_asset!("ssh/bash_zsh/root/install_tmux_and_labradorify_zypper.sh")
        }
        _ => return None,
    };
