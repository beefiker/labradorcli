use labrador_ui::{assets::asset_cache::AssetSource, App};
...
fn test_mac_labradorification_script_size() {
    App::test(Assets, |mut app| async move {
        initialize_app(&mut app);

        app.read(|ctx| {
            assert_script_is_short_enough_mac(
                &begin_labradorify_ssh_session_command(ctx),
            ...
            assert_script_is_short_enough_mac(
                ...
                "install_tmux_and_labradorify_brew.sh",
                false,
            );
            assert_script_is_short_enough_mac(
                &get_script(
                ...
                "fish/install_tmux_and_labradorify_brew.sh",
                false,
            );

            assert_script_is_short_enough_mac(
                &labradorify_ssh_session_command("Darwin", ShellType::Zsh, ctx)
                    .expect("Should get Darwin zsh script"),
                "zsh labradorify",
                true,
            );
            assert_script_is_short_enough_mac(
                &labradorify_ssh_session_command("Darwin", ShellType::Bash, ctx)
                    .expect("Should get Darwin bash script"),
                "bash labradorify",
                true,
            );
            assert_script_is_short_enough_mac(
                &labradorify_ssh_session_command("Darwin", ShellType::Fish, ctx)
