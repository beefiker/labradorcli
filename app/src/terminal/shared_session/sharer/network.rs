...
use labrador_ui::{Entity, ModelContext, ModelHandle, RequestState, RetryOption, SingletonEntity};
...
impl Network {
    ...
    pub fn labrador_active_prompt(serialized_prompt: String) -> ActivePrompt {
        ActivePrompt::WarpPrompt(serialized_prompt)
    }

    pub fn send_labrador_active_prompt_update_if_changed(&mut self, serialized_prompt: String) {
        self.send_active_prompt_update_if_changed(Self::labrador_active_prompt(serialized_prompt));
    }
            ...
            move |network, conn, ctx| match conn {
                Ok(((sink, stream), user_id)) => {
                    ...
                    let message = UpstreamMessage::Initialize(InitPayload {
                        ...
                        telemetry_context: Some(TelemetryContext(serde_json::json!({}))),
    ...
    /// TODO(roland): non OrderedTerminalEvents (like Labrador prompt) can be dropped if we're not connected. For non OrderedTerminalEvents,
...
pub fn failed_to_add_guests_user_error(reason: &FailedToAddGuestsReason) -> String {
    if is_not_labrador_user_reason(reason) {
        return format!(
...
fn is_not_labrador_user_reason(reason: &FailedToAddGuestsReason) -> bool {
    matches!(reason, FailedToAddGuestsReason::NotWarpUsers)
}
