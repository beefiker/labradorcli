...
fn test_ignore_duplicate_prompt_updates() {
    App::test((), |mut app| async move {
        ...
        network.update(&mut app, |network, _ctx| {
            network
                .send_labrador_active_prompt_update_if_changed("test Labrador prompt".to_owned());
        });
        assert_eq!(ws_proxy_rx.len(), 1);
        ...
        network.update(&mut app, |network, _ctx| {
            network.send_labrador_active_prompt_update_if_changed(
                "different Labrador prompt".to_owned(),
            );
        });
...
fn test_events_are_saved_on_send_and_removed_on_ack() {
    App::test((), |mut app| async move {
        ...
        network.update(
            &mut app,
            |network, ctx: &mut labrador_ui::ModelContext<'_, Network>| {
