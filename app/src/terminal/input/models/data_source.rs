...
use labrador_ui::fonts::{Properties, Style, Weight};
...
impl SearchItem for ModelSearchItem {
    ...
    fn render_icon(
        ...
        let icon = self
            .provider_icon
            .unwrap_or(Icon::Oz)
            .to_labrador_ui_icon(icon_color)
            .finish();
    ...
    fn render_item(
        ...
        use labrador_ui::elements::{Flex, ParentElement as _};
        use labrador_ui::prelude::CrossAxisAlignment;
        ...
        if self.setup_kind.is_none() && is_using_api_key_for_provider(&self.provider, app) {
            let key_icon =
                ConstrainedBox::new(Icon::Key.to_labrador_ui_icon(secondary_text_color).finish())
        ...
        if self.disable_reason.as_ref() == Some(&DisableReason::RequiresUpgrade) {
            ...
            .register_default_click_handlers_with_action_support(|hyperlink_lens, event, ctx| {
                match hyperlink_lens {
                    ...
                    labrador_ui::elements::HyperlinkLens::Action(action_ref) => {
...
/// Discounts only apply when the user is billing through Labrador credits,
