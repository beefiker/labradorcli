...
use labrador_ui::elements::{ConstrainedBox, Container, Highlight, ParentElement, Shrinkable, Text};
use labrador_ui::fonts::{Properties, Weight};
use labrador_ui::prelude::{Align, CrossAxisAlignment, Flex, MainAxisAlignment, MainAxisSize};
use labrador_ui::scene::{CornerRadius, Radius};
use labrador_ui::text_layout::ClipConfig;
use labrador_ui::{AppContext, Element, SingletonEntity};
    ...
    fn render_icon(
        ...
        let icon = match &self.item_type {
            ...
            HistoryItemType::Command { .. } => {
                ...
                Container::new(
                    ConstrainedBox::new(Icon::Terminal.to_labrador_ui_icon(icon_color).finish())
            ...
            HistoryItemType::AIPrompt { .. } => {
                let icon_color = inline_styles::icon_color(appearance);
                Container::new(
                    ConstrainedBox::new(Icon::Prompt.to_labrador_ui_icon(icon_color).finish())
                        .with_width(icon_size)
                        .with_height(icon_size)
                        .finish(),
                )
