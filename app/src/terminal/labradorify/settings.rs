...
maybe_define_setting!(AddedSubshellCommands, group: LabradorifySettings, {
    type: Vec<String>,
    default: Vec::new(),
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "labradorify.subshells.added_subshell_commands",
    description: "Additional regex patterns for commands that should be recognized as subshells.",
});

maybe_define_setting!(SubshellCommandsDenylist, group: LabradorifySettings, {
    type: Vec<String>,
    default: Vec::new(),
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "labradorify.subshells.subshell_commands_denylist",
    description: "Commands that should not trigger the subshell bootstrapping prompt.",
});

maybe_define_setting!(SshHostsDenylist, group: LabradorifySettings, {
    type: Vec<String>,
    default: Vec::new(),
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "labradorify.ssh.ssh_hosts_denylist",
    description: "SSH hosts that should not trigger the shell bootstrapping prompt.",
});

maybe_define_setting!(EnableSshLabradorification, group: LabradorifySettings, {
    type: bool,
    default: true,
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "labradorify.ssh.enable_ssh_labradorification",
    description: format!("Whether to enable {} features in SSH sessions.", ChannelState::app_name_display()),
});

maybe_define_setting!(UseSshTmuxWrapper, group: LabradorifySettings, {
    type: bool,
    default: false,
...
pub enum SshExtensionInstallMode {
    ...
    AlwaysInstall,
    /// Never install; fall back to legacy Labradorification.
    NeverInstall,
}

maybe_define_setting!(SshExtensionInstallModeSetting, group: LabradorifySettings, {
    type: SshExtensionInstallMode,
    default: SshExtensionInstallMode::default(),
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "labradorify.ssh.ssh_extension_install_mode",
    description: "Controls SSH extension installation behavior.",
});
...
pub struct LabradorifySettings {
    ...
    /// A list of commands that we shouldn't attempt to Labradorify. These can be added either b/c the
    /// "don't ask again" button was clicked in the trigger banner, or it was added explicitly on
    /// the Labradorify settings page. This represents the raw, serialized value.
    ...
    /// A list of hosts that we shouldn't attempt to Labradorify. This supports regex.
    /// These can be added either b/c the "don't ask again" button was clicked in the trigger banner,
    /// or it was added explicitly on the Labradorify settings page.
    /// While this could live in the `SshSettings` group, the custom processing shared with the other
    /// subshell logic better justifies it living in the `LabradorifySettings` group.
    ...
    /// This setting controls whether we should ever Labradorify ssh sessions.
    pub enable_ssh_labradorification: EnableSshLabradorification,

    /// This setting controls whether we should prompt the user to Labradorify an ssh session using the
...
lazy_static! {
    ...
    static ref SUBSHELL_COMMAND_REGEXES: Vec<Regex> = vec![
        ...
        // Commands that spawn an aws-vault subshell.
...
/// define_settings_group!(LabradorifySettings). Since we didn't use that macro we define it manually
/// here. It's the event emitted by the setter methods when a setting value changes.
pub enum LabradorifySettingsChangedEvent {
    AddedSubshellCommands {
...
impl Entity for LabradorifySettings {
    type Event = LabradorifySettingsChangedEvent;
}

impl SingletonEntity for LabradorifySettings {}
...
impl LabradorifySettings {
    ...
    pub fn is_compatible_subshell_command(&self, command: &str, shell_family: ShellFamily) -> bool {
        ...
        if !self.use_ssh_tmux_wrapper.value()
            && SshLabradorifyCommand::matches(command)
        ...
        // While in-band generators are our best option for Labradorifying ssh sessions from
        // powershell, hard-code the Labradorify subshell banner to show up.
    ...
    pub fn add_subshell_command(&mut self, command_to_add: &str, ctx: &mut ModelContext<Self>) {
        ...
        // care of emitting the LabradorifySettingsChangedEvent::AddedSubshellCommands event to keep
    ...
    /// Check if the user has asked us to remember a command and avoid asking to Labradorify a subshell.
