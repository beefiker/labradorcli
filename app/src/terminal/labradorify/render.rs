...
use labrador_ui::ui_components::components::UiComponent as _;
use labrador_ui::ui_components::components::UiComponentStyles;
use labrador_ui::{AppContext, Element, EventContext, PaintContext, SingletonEntity as _};
...
const LABRADOR_DRIVE_ENV_VAR_COLLECTION_ICON_COLOR: u32 = 0xC464FFFF;
const ICON_MARGIN: f32 = 4.;
const TERMINAL_ICON: &str = "bundled/svg/terminal.svg";
pub const HORIZONTAL_TEXT_MARGIN: f32 = 20.;
pub const SSH_DOCS_URL: &str = "https://docs.labrador.dev/terminal/labradorify/ssh";
pub const SUBSHELL_DOCS_URL: &str = "https://docs.labrador.dev/terminal/labradorify/subshells";
...
/// UI helper to render the ssh command that caused the Labradorification prompt.
...
pub fn description_row(text: &str, theme: &LabradorTheme, appearance: &Appearance) -> Box<dyn Element> {
...
pub fn render_never_labradorify_ssh_link(
    ssh_host: &Option<String>,
    app: &AppContext,
    appearance: &Appearance,
    mouse_state_handle: MouseStateHandle,
    on_never_labradorify: fn(&mut EventContext<'_>, ssh_host: String),
) -> Option<Box<dyn Element>> {
    let Some(ssh_host) = ssh_host else {
        return None;
    };

    let settings = LabradorifySettings::handle(app);
    if settings.as_ref(app).is_ssh_host_denylisted(ssh_host) {
        // Should only happen if user manually attempts to Labradorify a denylisted host.
        return None;
    }
    ...
    let link = appearance
        ...
        .link(
            ...
            Some(Box::new({
                ...
                move |ctx| on_never_labradorify(ctx, ssh_host.to_owned())
            })),
            mouse_state_handle,
        )
        .soft_wrap(false)
...
fn get_subshell_flag_info(subshell_source: &SubshellSource, theme: &LabradorTheme) -> (String, Fill) {
    match subshell_source {
        SubshellSource::EnvVarCollection(environment_name) => (
            environment_name.to_string(),
            Fill::Solid(ColorU::from_u32(
                LABRADOR_DRIVE_ENV_VAR_COLLECTION_ICON_COLOR,
            )),
        ),
...
pub fn render_subshell_flag_pole(
    max_height: f32,
    fill: impl Into<labrador_ui::elements::Fill>,
) -> Box<dyn Element> {
