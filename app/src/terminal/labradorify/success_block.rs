...
use super::settings::LabradorifySettings;
use super::{render, subshell_bootstrap_success_block_bytes, LabradorificationSource};

const VERTICAL_TEXT_MARGIN: f32 = 16.;

#[derive(Debug, Clone)]
pub enum LabradorifySuccessBlockEvent {
    OpenLabradorifySettings,
}

#[derive(Debug, Clone, Eq, PartialEq)]
pub enum LabradorifySuccessBlockAction {
    ClearAutoLabradorifySnippet,
    OpenLabradorifySettings,
    OpenUrl(String),
}

struct AutoLabradorifySnippet {
    /// On subshell initialization, this will contain the output grid to display,
    /// containing info like how to auto-labradorify the subshell.
...
pub struct LabradorifySuccessBlock {
    source: LabradorificationSource,
    spawning_command: String,
    learn_more_link_mouse_states: MouseStateHandle,
    auto_labradorify_snippet: Option<AutoLabradorifySnippet>,
}

impl LabradorifySuccessBlock {
    ...
    pub fn new(
        ...
        ctx.subscribe_to_model(&LabradorifySettings::handle(ctx), move |_, _, _, ctx| {
            ctx.notify();
        });
        ...
        let is_auto_labradorify_configured = subshell_info
            .as_ref()
            .map(|info| info.was_triggered_by_rc_file_snippet)
            .unwrap_or_default();

        let auto_labradorify_snippet = if is_auto_labradorify_configured {
            None
        } else {
            subshell_info.and_then(|subshell_info| {
                // If Labradorification wasn't triggered automatically, show a snippet about
                // how to automatically Labradorify.
        ...
        let auto_labradorify_snippet = auto_labradorify_snippet.map(|(output_grid, can_write_to_rc)| {
            AutoLabradorifySnippet {
        ...
        Self {
            ...
            spawning_command,
            auto_labradorify_snippet,
        }
    }

    pub fn selected_text(&self) -> Option<String> {
        self.auto_labradorify_snippet
            .as_ref()
    ...
    pub fn render_title_ui(&self, theme: &LabradorTheme, appearance: &Appearance) -> Box<dyn Element> {
        let header_contents = render::build_header_row(
            format!("Session {}", ChannelState::app_name_verbed()),
            Icon::new(UiIcon::Labrador.into(), theme.active_ui_detail()),
            theme,
            appearance,
        )
        .with_margin_right(8.)
    ...
    fn render_learn_more_link(&self, appearance: &Appearance) -> Box<dyn Element> {
        let url = match self.source {
            LabradorificationSource::Ssh => SSH_DOCS_URL,
            LabradorificationSource::Subshell => SUBSHELL_DOCS_URL,
        };
        ...
        appearance
            ...
            .link(
                ...
                Some(Box::new({
                    move |ctx| {
                        ctx.dispatch_typed_action(LabradorifySuccessBlockAction::OpenUrl(
                            url.to_owned(),
                        ));
                    }
                })),
    ...
    /// Fired when a block ends and we are not in a Labradorified session.
    pub fn on_labradorified_session_complete(&mut self, ctx: &mut ViewContext<Self>) {
        self.clear_auto_labradorify_snippet(ctx);
    }

    pub fn clear_auto_labradorify_snippet(&mut self, ctx: &mut ViewContext<Self>) {
        self.auto_labradorify_snippet = None;
        ctx.notify();
    }
    ...
    pub fn render_output_grid(
        ...
        let auto_labradorify_snippet = self.auto_labradorify_snippet.as_ref()?;

        if auto_labradorify_snippet.output_grid.is_empty() {
            return None;
        }

        let shell_language = ProgrammingLanguage::Shell(auto_labradorify_snippet.shell_type);
        let runnable_command = render_runnable_code_snippet(
            &auto_labradorify_snippet.output_grid,
            if auto_labradorify_snippet.can_write_to_rc {
                Some(&shell_language)
            } else {
                None
            },
            Some(Box::new({
                move |code_snippet, ctx| {
                    ...
                    ctx.dispatch_typed_action(
                        LabradorifySuccessBlockAction::ClearAutoLabradorifySnippet,
                    );
                }
            })),
            Some(Box::new({
            ...
            Some(auto_labradorify_snippet.code_snippet_handles.clone()),
            app,
        );

        let semantic_selection = SemanticSelection::as_ref(app);
        let selected_text = auto_labradorify_snippet.selected_text.clone();

        // TODO(Simon): Implement full selection and copying functionality for the LabradorifySuccessBlock.
        ...
        let output_grid = SelectableArea::new(
            auto_labradorify_snippet.selection_handle.clone(),
        ...
        let output_grid = Flex::column()
            .with_child(
                Container::new(
                    Text::new(
                        auto_labradorify_snippet.description.clone(),
...
impl Entity for LabradorifySuccessBlock {
    type Event = LabradorifySuccessBlockEvent;
}

pub const LABRADORIFY_SUCCESS_BLOCK_VISIBLE_KEY: &str = "LabradorifySuccessBlockVisible";

impl View for LabradorifySuccessBlock {
    fn ui_name() -> &'static str {
        "LabradorifySuccessBlock"
    }
...
impl TypedActionView for LabradorifySuccessBlock {
    type Action = LabradorifySuccessBlockAction;

    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            LabradorifySuccessBlockAction::OpenLabradorifySettings => {
                ctx.emit(LabradorifySuccessBlockEvent::OpenLabradorifySettings);
            }
            LabradorifySuccessBlockAction::OpenUrl(url) => {
                ctx.open_url(url);
            }
            LabradorifySuccessBlockAction::ClearAutoLabradorifySnippet => {
                self.clear_auto_labradorify_snippet(ctx);
