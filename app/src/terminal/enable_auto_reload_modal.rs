...
use labrador_graphql::billing::AddonCreditsOption;
...
use labrador_ui::ui_components::components::{Coords, UiComponent as _, UiComponentStyles};
use labrador_ui::{AppContext, Element, Entity, SingletonEntity as _, View, ViewContext, ViewHandle};
                ...
                if discount_percent > 0 {
                    MenuItemFields::new_with_custom_label(
                        Arc::new(enclose!((primary_text) move |is_selected, is_hovered, appearance, _| {
                            ...
                            let discount_badge = labrador_ui::elements::Empty::new().finish();
    ...
    fn render_content(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let explanation_fragments = vec![
            ...
            FormattedTextFragment::hyperlink(
                "Learn more",
                "https://docs.labrador.dev/support-and-community/plans-and-billing/add-on-credits#id-2.-enable-auto-reload",
            ),
        ];
        let explanation_text = labrador_ui::elements::FormattedTextElement::new(
        ...
        .register_default_click_handlers_with_action_support(|hyperlink_lens, _event, ctx| {
            match hyperlink_lens {
                labrador_ui::elements::HyperlinkLens::Url(url) => {
                    ctx.open_url(url);
                }
                labrador_ui::elements::HyperlinkLens::Action(_action_ref) => {}
            }
        })
        .finish();
...
impl labrador_ui::TypedActionView for EnableAutoReloadModalBody {
    type Action = Action;
...
impl labrador_ui::TypedActionView for EnableAutoReloadModal {
    type Action = ();
}
