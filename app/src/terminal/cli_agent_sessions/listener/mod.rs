use labrador_ui::{EntityId, ModelContext, ModelHandle, SingletonEntity};
...
fn create_handler(agent: &CLIAgent) -> Option<Box<dyn CLIAgentSessionHandler>> {
    match agent {
        // Auggie is supported via a community-maintained integration, which
        // emits the same structured OSC 777 events as the first-party
        // Claude/OpenCode/Gemini plugins. We don't ship an install flow for it;
        // we just listen.
...
impl labrador_ui::Entity for CLIAgentSessionListener {
    type Event = ();
}
