...
pub const CLI_AGENT_NOTIFICATION_SENTINEL: &str = "labrador://cli-agent";

pub fn is_cli_agent_notification_title(title: Option<&str>) -> bool {
    title.is_some_and(|title| title == CLI_AGENT_NOTIFICATION_SENTINEL)
}
...
/// The current CLI agent protocol version this build of Labrador supports.
/// Exported as the `LABRADOR_CLI_AGENT_PROTOCOL_VERSION` env var on the PTY
...
pub fn parse_event(title: Option<&str>, body: &str) -> Option<CLIAgentEvent> {
    if !is_cli_agent_notification_title(title) {
        return None;
    }
        ...
        None => {
            log::error!(
                "Received CLI agent event with unsupported schema version \
                 {version}. The CLI agent plugin or Labrador may need to be updated."
            );
            None
        }
    }
