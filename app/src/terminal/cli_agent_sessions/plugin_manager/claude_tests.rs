...
use super::{
    check_installed, installed_version, ClaudeCodePluginManager, CliAgentPluginManager, PLUGIN_KEY,
};

fn plugins_json(plugin_value: serde_json::Value) -> serde_json::Value {
    let mut plugins = serde_json::Map::new();
    plugins.insert(PLUGIN_KEY.to_owned(), plugin_value);
...
fn installed_when_plugin_present() {
    ...
    let json = plugins_json(serde_json::json!([{"version": "1.0.0"}]));
    fs::write(
...
fn not_installed_when_plugin_array_empty() {
    ...
    let json = plugins_json(serde_json::json!([]));
    fs::write(
        plugins_dir.join("installed_plugins.json"),
        serde_json::to_string(&json).unwrap(),
    )
...
fn installed_version_returns_version_when_present() {
    ...
    let json = plugins_json(serde_json::json!([{"version": "1.5.0"}]));
    fs::write(
        plugins_dir.join("installed_plugins.json"),
        serde_json::to_string(&json).unwrap(),
    )
...
fn installed_version_returns_none_when_no_version_field() {
    ...
    let json = plugins_json(serde_json::json!([{"scope": "user"}]));
    fs::write(
        plugins_dir.join("installed_plugins.json"),
        serde_json::to_string(&json).unwrap(),
    )
