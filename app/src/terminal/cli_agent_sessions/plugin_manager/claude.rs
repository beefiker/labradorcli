...
const MARKETPLACE_ADD_COMMAND: &str = "claude plugin marketplace add warpdotdev/claude-code-warp";
const MARKETPLACE_REMOVE_COMMAND: &str = "claude plugin marketplace remove claude-code-warp";
const PLUGIN_INSTALL_COMMAND: &str = "claude plugin install warp@claude-code-warp";
...
// Keep in sync with the plugin version documented by the extension repo.
...
static UPDATE_INSTRUCTIONS: LazyLock<PluginInstructions> = LazyLock::new(|| PluginInstructions {
    ...
    steps: &[
        PluginInstructionStep {
            ...
            command: MARKETPLACE_REMOVE_COMMAND,
            executable: true,
            link: None,
        },
        PluginInstructionStep {
...
/// Reads the installed version string for the Labrador plugin, if present.
