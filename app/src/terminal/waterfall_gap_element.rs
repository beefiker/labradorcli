...
use labrador_ui::elements::ZIndex;
use labrador_ui::event::ModifiersState;
use labrador_ui::units::{IntoLines, IntoPixels, Pixels};
...
pub struct WaterfallGapElement {
    ...
    origin: Option<labrador_ui::elements::Point>,
...
impl Element for WaterfallGapElement {
    fn layout(
        ...
        ctx: &mut labrador_ui::LayoutContext,
        app: &AppContext,
    ) -> Vector2F {
    ...
    fn after_layout(&mut self, ctx: &mut labrador_ui::AfterLayoutContext, app: &AppContext) {
        self.input_element.after_layout(ctx, app);
        self.block_list_element.after_layout(ctx, app);
    }

    fn paint(&mut self, origin: Vector2F, ctx: &mut labrador_ui::PaintContext, app: &AppContext) {
        self.origin = Some(labrador_ui::elements::Point::from_vec2f(
            origin,
            ctx.scene.z_index(),
        ));
    ...
    fn origin(&self) -> Option<labrador_ui::elements::Point> {
        self.origin
    }

    fn dispatch_event(
        &mut self,
        event: &labrador_ui::event::DispatchedEvent,
        ctx: &mut labrador_ui::EventContext,
        app: &AppContext,
    ) -> bool {
        ...
        if !handled {
            ...
            handled |= match event_at_z_index {
                Some(labrador_ui::Event::LeftMouseDown { position, .. }) => {
                    self.mouse_down(*position, ctx)
                }
                Some(labrador_ui::Event::RightMouseDown { position, .. }) => {
                ...
                Some(labrador_ui::Event::ScrollWheel {
                    position,
                    delta,
                    precise,
...
impl ScrollableElement for WaterfallGapElement {
    fn scroll_data(&self, app: &AppContext) -> Option<labrador_ui::elements::ScrollData> {
