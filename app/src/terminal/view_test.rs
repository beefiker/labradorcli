...
use crate::settings::{AISettings, AppEditorSettings, LabradorPromptSeparator};
    ...
    App::test((), |mut app| async move {
        ...
        app.update(enclose!((presenter) move |ctx| {
            ctx.simulate_window_event(
                labrador_ui::Event::LeftMouseDown {
        ...
        app.update(enclose!((presenter) move |ctx| {
            ctx.simulate_window_event(
                labrador_ui::Event::LeftMouseDragged {
...
// Regression test for a find bar selection crash.
#[test]
...
fn test_bash_vim_banner_already_shown() {
    App::test((), |mut app| async move {
        ...
        // Ensure Labrador's vim keybindings are off.
...
fn test_bash_vim_banner_off() {
    App::test((), |mut app| async move {
        ...
        // Ensure Labrador's vim keybindings are on.
        AppEditorSettings::handle(&app).update(&mut app, |editor_settings, ctx| {
...
fn test_link_at_range_trims_zero_width_spaces() {
    App::test((), |mut app| async move {
        ...
        let input_url = "(\u{200b}https://labrador.dev\u{200b})";
        // NOTE: the final character in this string is a zero-width space
        let non_escaped_url = "https://labrador.dev\u{200b}";
        let escaped_url = "https://labrador.dev";

        terminal.update(&mut app, |view, _ctx| {
            view.model.lock().simulate_block(
                r"printf '(%bhttps://labrador.dev%b)\n' '\U200b' '\U200b'",
                input_url,
            );
        });
...
fn linear_deeplink_does_not_auto_submit_when_already_in_agent_view() {
    ...
    App::test((), |mut app| async move {
        ...
        // agent view when the `labrador://linear/work?prompt=...` URI is dispatched.
        let original_conversation_id = terminal.update(&mut app, |view, ctx| {
            view.agent_view_controller().update(ctx, |controller, ctx| {
                controller
                    .try_enter_agent_view(
