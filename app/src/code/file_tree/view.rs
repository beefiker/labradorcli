...
use labrador_ui::{BlurContext, ModelHandle};
...
use crate::util::openable_file_type::{
    resolve_file_target_to_open_in_labrador, resolve_file_target_with_editor_choice,
};
use crate::{
...
use labrador_core::ui::theme::{color::internal_colors, Fill};
...
impl FileTreeView {
    ...
    pub fn set_root_directories(&mut self, paths: Vec<PathBuf>, ctx: &mut ViewContext<Self>) {
        ...
        // via `labrador_util::path::group_roots_by_common_ancestor`.
        let grouping = labrador_util::path::group_roots_by_common_ancestor(&local_inputs);
    ...
    fn render_item_with_hover(
        ...
        let expand_icon = match expand_icon {
            Some(icon) => {
                ...
                icon.to_labrador_ui_icon(chevron_icon_color.into()).finish()
            }
        ...
        let icon = match render_state.icon {
            ImageOrIcon::Icon(icon) => icon.to_labrador_ui_icon(icon_color.into()).finish(),
    ...
    fn open_file(
        ...
        let target = if editor_layout.is_some() {
            resolve_file_target_to_open_in_labrador(path, settings, editor_layout)
        } else {
        ...
        let main_column = Flex::column()
            ...
            .with_child(
                Container::new(
                    ConstrainedBox::new(
                        Icon::AlertTriangle
                            .to_labrador_ui_icon(Fill::Solid(internal_colors::neutral_6(theme)))
                            .finish(),
                    )
                    .with_width(24.)
                    .with_height(24.)
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        if self.displayed_directories.is_empty() {
            if let CodingPanelEnablementState::RemoteSession { has_remote_server } = self.enablement
            {
                ...
                // Labradorification / mode 1), show a loading state — the server
