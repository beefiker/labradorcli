...
use std::path::Path;
use vec1::vec1;
use labrador_editor::content::buffer::{InitialBufferState, SelectionOffsets};
use labrador_editor::multiline::MultilineString;
...
fn test_move_by_word() {
    App::test((), |mut app| async move {
        ...
        editor.update(&mut app, |editor, ctx| {
            ...
            editor.forward_word_with_unit(true, TextUnit::Word(labrador_ui::text::word_boundaries::WordBoundariesPolicy::Default), ctx);
        });
        ...
        editor.update(&mut app, |editor, ctx| {
            editor.backward_word_with_unit(false, TextUnit::Word(labrador_ui::text::word_boundaries::WordBoundariesPolicy::Default), ctx);
        });
