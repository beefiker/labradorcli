...
use labrador_editor::search::{SearchEvent, Searcher};
use labrador_ui::elements::MainAxisAlignment;
use labrador_ui::elements::{ChildAnchor, OffsetPositioning, Radius, SavePosition, Shrinkable};
use labrador_ui::keymap::EditableBinding;
...
use labrador_ui::{presenter::ChildView, ViewContext, ViewHandle};
use labrador_ui::{FocusContext, ModelHandle};
...
impl CodeEditorFind {
    ...
    pub fn emit_result_a11y_content(&mut self, ctx: &mut ViewContext<Self>) {
        let content = if let Some(match_index) = self.searcher.as_ref(ctx).selected_match() {
            ...
            AccessibilityContent::new_without_help("No results.", LabradorA11yRole::UserAction)
        };
    ...
    fn render_next_match_button(
        ...
        Container::new(
            ConstrainedBox::new(match_icon.to_labrador_ui_icon(icon_color).finish())
                .with_height(height)
                .with_width(height)
                .finish(),
        )
    ...
    fn render_search_button(
        ...
        Container::new(
            ConstrainedBox::new(Icon::Search.to_labrador_ui_icon(icon_color).finish())
                .with_height(height)
                .with_width(height)
                .finish(),
        )
...
impl View for CodeEditorFind {
    ...
    fn on_blur(&mut self, _blur_ctx: &labrador_ui::BlurContext, ctx: &mut ViewContext<Self>) {
