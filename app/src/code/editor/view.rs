...
use crate::code::{
    editor::EditorReviewComment, DiffResult, NoopFindReferencesCardProvider,
...
use labrador_editor::{
    ...
    render::{
        ...
        model::{
            AutoScrollMode, Decoration, ExpansionType, LineCount, RichTextStyles,
            CODE_EDITOR_HIDDEN_SECTION_EXPANSION_LINES,
        },
    },
    search::{SearchEvent, Searcher, MATCH_FILL, SELECTED_MATCH_FILL},
};
use labrador_util::content_version::ContentVersion;
use labrador_ui::{
    elements::{
        new_scrollable::{
            AxisConfiguration, DualAxisConfig, NewScrollableElement, ScrollableAppearance,
        },
        ChildView, Dismiss, Fill, Flex, MouseStateHandle, NewScrollable, OffsetPositioning,
...
impl CodeEditorView {
        ...
        Self {
            ...
            display_options: CodeEditorViewDisplayOptions {
                ...
                horizontal_scrollbar_appearance: ScrollableAppearance::new(
                    labrador_ui::elements::ScrollbarWidth::Auto,
                    false,
                ),
    ...
    fn expand_hidden_section(
        ...
        let lines_to_unhide = match expansion_type {
            ExpansionType::Both => {
                labrador_editor::content::text::LineCount::from(hidden_section_start)
                    ..labrador_editor::content::text::LineCount::from(hidden_section_end)
            }
            ExpansionType::ExpandDown => {
                let end = hidden_section_end
                    .min(hidden_section_start + CODE_EDITOR_HIDDEN_SECTION_EXPANSION_LINES);
                labrador_editor::content::text::LineCount::from(hidden_section_start)
                    ..labrador_editor::content::text::LineCount::from(end)
            }
            ExpansionType::ExpandUp => {
                ...
                labrador_editor::content::text::LineCount::from(start)
                    ..labrador_editor::content::text::LineCount::from(hidden_section_end)
            }
        };
        self.model.update(ctx, |model, ctx| {
            ...
            FindViewEvent::SelectAll => {
                ...
                if let Some(results) = self.searcher.as_ref(ctx).results() {
                    if !results.matches.is_empty() {
                        ...
                        let selection_offsets: Vec<labrador_editor::content::buffer::SelectionOffsets> =
                            results
                                .matches
                                .iter()
                                .map(|match_result| {
                                    labrador_editor::content::buffer::SelectionOffsets {
                            ...
                            self.model.update(ctx, |model, ctx| {
                                model.selection().update(ctx, |selection_model, ctx| {
                                    selection_model.update_selection(
                                        labrador_editor::content::buffer::BufferSelectAction::SetSelectionOffsets { selections },
                                        labrador_editor::content::buffer::AutoScrollBehavior::Selection,
                                        ctx,
                                    );
                                });
                            });
                    ...
                    self.model.update(ctx, |model, ctx| {
                        model.update_content( |mut content_model, ctx| {
                            content_model.apply_edit(
                                labrador_editor::content::buffer::BufferEditAction::InsertAtCharOffsetRanges { edits: &edits },
                                ...
                                self.model.update(ctx, |model, ctx| {
                                    model.selection_model().update(ctx, |selection, ctx| {
                                        selection.update_selection(
                                            labrador_editor::content::buffer::BufferSelectAction::MoveRight,
                                            labrador_editor::content::buffer::AutoScrollBehavior::Selection,
                                            ctx,
                                        );
                                    });
...
/// In the Labrador fork we only need enough of the bundle for the code editor's
...
pub fn code_text_styles(
    ...
    use labrador_editor::render::model::{
    ...
    use labrador_ui::elements::{Border, DEFAULT_UI_LINE_HEIGHT_RATIO};
