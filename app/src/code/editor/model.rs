...
use std::sync::Arc;
use std::{cmp, mem};
use labrador_core::platform::SessionPlatform;
use labrador_core::ui::theme::Fill;
use labrador_editor::content::anchor::Anchor;
use labrador_editor::content::edit::EditDelta;
use labrador_editor::content::find::{SearchConfig, SearchResults};
use labrador_editor::content::selection_model::BufferSelectionModel;
use labrador_editor::content::version::BufferVersion;
use labrador_editor::multiline::{AnyMultilineString, MultilineString, LF};
use labrador_editor::render::model::{AutoScrollMode, LineCount, StyleUpdateAction};
use labrador_editor::selection::TextDirection;
use labrador_ui::units::{IntoPixels, Pixels};
...
use labrador_editor::content::buffer::{ShouldAutoscroll, VimInsertPoint};
use labrador_editor::{
    content::{
        buffer::{
...
use labrador_ui::text::{point::Point, TextBuffer};
use labrador_ui::{AppContext, Entity, ModelContext, ModelHandle, SingletonEntity};
...
impl CodeEditorModel {
    ...
    pub fn set_hidden_lines(
        &mut self,
        ranges: RangeSet<labrador_editor::content::text::LineCount>,
        ctx: &mut ModelContext<Self>,
    ) {
    ...
    pub fn set_visible_line_range(
        &mut self,
        range: Range<labrador_editor::content::text::LineCount>,
        ctx: &mut ModelContext<Self>,
    ) {
    ...
    fn calculate_hidden_lines(&mut self, ctx: &mut ModelContext<Self>) {
        if let Some(context_line) = self.hide_lines_outside_of_active_diff {
            ...
            let mut visible_ranges: RangeSet<labrador_editor::content::text::LineCount> =
                RangeSet::new();
            ...
            let all_lines: Range<labrador_editor::content::text::LineCount> =
                labrador_editor::content::text::LineCount::from(0)
                    ..labrador_editor::content::text::LineCount::from(line_count);

            // Find gaps in the visible ranges
            let hidden_ranges = visible_ranges
                .gaps(&all_lines)
                .collect::<RangeSet<labrador_editor::content::text::LineCount>>();
    ...
    pub fn forward_word(&mut self, select: bool, ctx: &mut ModelContext<Self>) {
        self.forward_word_with_unit(select, TextUnit::Word(labrador_ui::text::word_boundaries::WordBoundariesPolicy::Default), ctx)
    }

    // TODO(CLD-1593)
    pub fn backward_word(&mut self, select: bool, ctx: &mut ModelContext<Self>) {
        self.backward_word_with_unit(select, TextUnit::Word(labrador_ui::text::word_boundaries::WordBoundariesPolicy::Default), ctx)
    }
    ...
    pub fn autocomplete_symbol(&mut self, open: char, close: char, ctx: &mut ModelContext<Self>) {
        ...
        if all_cursors {
            ...
            if let Ok(texts) = vec1::Vec1::try_from_vec(texts) {
                self.update_content(
                    |mut content, ctx| {
                        content.apply_edit(
                        labrador_editor::content::buffer::BufferEditAction::InsertForEachSelection {
                            texts: &texts,
                        },
                        labrador_editor::content::buffer::EditOrigin::UserTyped,
                        selection_model,
                        ctx,
                    );
                    },
        ...
        if let Ok(edits) = vec1::Vec1::try_from_vec(edits) {
            ...
            self.update_content(
                |mut content, ctx| {
                    content.apply_edit(
                        labrador_editor::content::buffer::BufferEditAction::InsertAtCharOffsetRanges {
                            edits: &edits,
                        },
                        labrador_editor::content::buffer::EditOrigin::UserTyped,
                        selection_model,
...
impl CoreEditorModel for CodeEditorModel {
    ...
    fn validate(&self, _ctx: &impl labrador_ui::ModelAsRef) {}

    // Since this is a plain text editor, there is no text styles.
    fn active_text_style(&self) -> labrador_editor::content::text::TextStyles {
        Default::default()
    }
