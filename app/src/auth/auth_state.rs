...
use labrador_ui::{AppContext, Entity, SingletonEntity};
...
impl AuthState {
    ...
    pub fn initialize(ctx: &AppContext, api_key: Option<String>) -> Self {
        ...
        // Try compile-time user secret environment variables.
        if let Some(persisted) = option_env!("LABRADOR_USER_SECRET")
    ...
    /// Returns whether the user considered onboarded to Labrador.
    ...
    /// Anonymous users are real Labrador users, but have no providers linked in Firebase.
    ...
    /// originated from viewing Labrador on web.
...
// Adapter for the [`labrador_managed_secrets`] crate, which needs to access the current user.
impl labrador_managed_secrets::ActorProvider for AuthState {
