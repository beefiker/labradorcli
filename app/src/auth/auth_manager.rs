...
use labrador_ui::{Entity, ModelContext, SingletonEntity, UpdateModel};
...
impl AuthManager {
    ...
    ///
    /// This is only used by the Labrador CLI if running on a device that does not have the Labrador app installed.
    ...
    /// Callback for handling a successful fetch of a user from the server and Firebase.
    ...
    fn on_user_fetched(
        ...
        match fetch_user_result {
            Ok(fetch_user_result) => {
                ...
                // Once the user is authenticated, attempt to report the sandbox that Labrador is running in, if any.
                ctx.spawn(
                    async { labrador_isolation_platform::detect() },
                    |_, platform, _ctx| {
