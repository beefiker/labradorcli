    ...
    A: Action + Clone,
{
    ...
    let text = "You are currently offline. An internet connection is required to use Dwarf for the first time.";
    ...
    A: Action + Clone,
{
    ...
    let paragraph_1 = "Dwarf’s non-cloud features work offline.";
    let paragraph_2 = "However, we require users to be online when using Dwarf for the first time in order to enable Dwarf's AI and cloud features.";
    let paragraph_3 = "We offer cloud features to all users, and so we need an internet connection to meter AI usage, prevent abuse, and associate cloud objects with users. If you opt to use Dwarf logged-out, a unique ID will be attached to an anonymous user account in order to support these features.";
...
pub fn render_privacy_settings_toggles<A: Action + Clone + 'static>(
    ...
    let telemetry_toggle = Flex::row()
        ...
        .with_child(
            Shrinkable::new(
                1.,
                render_privacy_settings_section_header("Help improve Dwarf", appearance).finish(),
            )
            .finish(),
        )
        .with_child(
    ...
    let telemetry_description = render_description(
        appearance,
        "High-level feature usage data helps Dwarf's product team prioritize the roadmap.".into(),
    );
