...
use labrador_ui::geometry::vector::Vector2F;
use labrador_ui::platform::Cursor;
use labrador_ui::{EntityId, WeakViewHandle, WindowId};
...
pub enum WorkspaceAction {
    ...
    DropTab,
    FinalizeDropTab,
    /// Toggles the left panel. In Code Mode V1 this toggles Labrador Drive.
    /// In Code Mode V2 this toggles the left panel which contains both the project explorer and
    /// Labrador Drive. This happens as explicit action from the user.
    ToggleLeftPanel,
    ...
    OpenLink(String),
    /// On WASM, opens a given URL in the desktop Labrador app (if installed) or redirects to download page.
    ...
    TerminateApp,
    CloseWindow,
    /// Help the user call the Labrador executable with the [`crate::args::DEBUG_DUMP_FLAG`].
    DumpDebugInfo,
    ...
    /// Install the Labrador CLI command to /usr/local/bin
    #[cfg(target_os = "macos")]
    InstallCLI,
    /// Uninstall the Labrador CLI command from /usr/local/bin
    #[cfg(target_os = "macos")]
    UninstallCLI,
    ...
    /// Open the OpenLabrador Launch Modal (for debugging)
    #[cfg(debug_assertions)]
    OpenOpenLabradorLaunchModal,
    /// Reset the OpenLabrador launch modal dismissed state (for debugging)
    #[cfg(debug_assertions)]
    ResetOpenLabradorLaunchModalState,
    /// Install the OpenCode plugin from GitHub into the global opencode config.
    #[cfg(debug_assertions)]
    InstallOpenCodeLabradorPlugin,
    /// Use a local checkout of the OpenCode plugin (for testing/development).
    #[cfg(debug_assertions)]
...
impl WorkspaceAction {
    pub fn blocked_for_anonymous_user(&self) -> bool {
        use WorkspaceAction::*;
        matches!(self, CreateTeamAIPrompt | OpenShareSessionModal(_))
    }
    ...
    pub fn should_save_app_state_on_action(&self) -> bool {
        ...
        match self {
            ...
            | ResetOpenLabradorLaunchModalState
            | InstallOpenCodeLabradorPlugin
            | UseLocalOpenCodeLabradorPlugin => false,
            #[cfg(not(target_family = "wasm"))]
            ViewLogs => false,
            #[cfg(target_os = "macos")]
            SampleProcess => false,
