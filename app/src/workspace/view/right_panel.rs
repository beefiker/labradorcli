...
use labrador_ui::elements::{ChildAnchor, Empty, PositionedElementAnchor};
...
pub enum ReviewDestination {
    ...
    None,
    /// A Labrador agent terminal is available (input box visible, not executing).
    Labrador,
    ...
    pub fn new(ctx: &mut ViewContext<RightPanelView>) -> Self {
        CodeReviewState {
            dropdown: ctx.add_typed_action_view(|ctx| {
                ...
                dropdown.set_top_bar_height(labrador_core::ui::icons::ICON_DIMENSIONS, ctx);
...
impl RightPanelView {
    ...
    fn render_maximize_pane_button(&self) -> Box<dyn Element> {
        ConstrainedBox::new(ChildView::new(&self.maximize_button).finish())
            .with_height(labrador_core::ui::icons::ICON_DIMENSIONS)
            .with_width(labrador_core::ui::icons::ICON_DIMENSIONS)
            .finish()
    }
    ...
    /// considered available (non-CLI Labrador terminals require AI to be on).
            ...
            .map(|tv| {
                tv.read(ctx, |t, ctx| {
                    t.active_cli_agent(ctx)
                        ...
                        .unwrap_or(ReviewDestination::Labrador)
                })
            })
