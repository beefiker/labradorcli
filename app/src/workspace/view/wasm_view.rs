...
use labrador_ui::elements::{ChildView, Element};
use labrador_ui::{AppContext, SingletonEntity, ViewContext, ViewHandle};
...
use crate::workspace::view::{NotebookSource, OpenLabradorDriveObjectSettings, Workspace};
...
impl Workspace {
    ...
    pub(super) fn build_open_in_labrador_button(
        ...
        ctx.add_typed_action_view(|_ctx| {
            ...
            .on_click(move |ctx| {
                ...
                if let Some(url) = parse_current_url() {
                    ...
                    log::warn!(
                        "Could not get URL for Open in {} button",
        ...
        ctx.subscribe_to_view(&panel, |me, _, event, ctx| match event {
            ...
            ConversationDetailsPanelEvent::OpenPlanNotebook { notebook_uid } => {
                me.open_notebook(
                    ...
                    &OpenLabradorDriveObjectSettings::default(),
                    ctx,
                    true,
                );
            }
