...
use labrador_core::ui::color::blend::Blend;
use labrador_core::ui::color::coloru_with_opacity;
use labrador_core::ui::theme::color::internal_colors;
use labrador_core::ui::theme::{AnsiColorIdentifier, Fill as LabradorThemeFill, LabradorTheme};
use labrador_core::ui::Icon as LabradorIcon;
use labrador_ui::elements::DispatchEventResult;
...
use labrador_ui::fonts::{Properties, Weight};
use labrador_ui::platform::Cursor;
use labrador_ui::prelude::{Align, MainAxisAlignment};
use labrador_ui::text_layout::ClipConfig;
use labrador_ui::ui_components::components::{UiComponent, UiComponentStyles};
use labrador_ui::ui_components::text_input::TextInput;
use labrador_ui::{AppContext, EntityId, SingletonEntity, ViewHandle, WindowId};
...
fn render_vertical_tab_hover_indicator(theme: &LabradorTheme) -> Box<dyn Element> {
    ConstrainedBox::new(
...
fn render_control_bar(
    ...
    let search_icon = ConstrainedBox::new(LabradorIcon::Search.to_labrador_ui_icon(sub_text).finish())
        ...
        TypedPane::Terminal(terminal_pane) => {
            ...
            if let Some(icon) = cli_agent_session.and_then(|session| session.agent.icon()) {
                let color = cli_agent_session
                    ...
                    .map(LabradorThemeFill::Solid)
                    .unwrap_or_else(|| theme.accent());
                return icon.to_labrador_ui_icon(color).finish();
            }
            ...
            if terminal_view.is_ambient_agent_session(app)
                || terminal_view
                    ...
                    .is_some()
            {
                return render_labrador_icon(BADGE_ICON_SIZE, 2.);
            }

            LabradorIcon::Terminal.to_labrador_ui_icon(disabled_text).finish()
        }
        TypedPane::Code(_) => icon_from_file_path(&props.title, appearance)
            .unwrap_or_else(|| LabradorIcon::Code2.to_labrador_ui_icon(sub_text).finish()),
        typed => typed.icon().to_labrador_ui_icon(sub_text).finish(),
    }
}
    ...
    let button = Hoverable::new(
        ...
        move |hover_state| {
            let icon = ConstrainedBox::new(
                LabradorIcon::Settings
                    .to_labrador_ui_icon(if is_popup_open { main_text } else { sub_text })
                    .finish(),
            )
            .with_width(16.)
            .with_height(16.)
...
fn render_group_action_buttons(
    ...
    let kebab_button = Hoverable::new(kebab_mouse_state, move |button_state| {
        let mut container = Container::new(
            ConstrainedBox::new(LabradorIcon::DotsVertical.to_labrador_ui_icon(meta_color).finish())
    ...
    let close_button = Hoverable::new(close_mouse_state, move |button_state| {
        let mut container = Container::new(
            ConstrainedBox::new(LabradorIcon::X.to_labrador_ui_icon(meta_color).finish())
                .with_width(GROUP_ACTION_BUTTON_ICON_SIZE)
                .with_height(GROUP_ACTION_BUTTON_ICON_SIZE)
                .finish(),
        )
...
fn resolve_icon_with_status_variant(
    ...
    match typed {
        ...
        // AI Fact (rules) uses sub-text color now that drive coloring is gone.
...
fn render_title_indicator(theme: &LabradorTheme) -> Box<dyn Element> {
    ConstrainedBox::new(
        LabradorIcon::CircleFilled
            .to_labrador_ui_icon(theme.accent())
            .finish(),
    )
...
impl TypedPane<'_> {
    ...
    fn icon(&self) -> LabradorIcon {
        match self {
            TypedPane::Terminal(_) => LabradorIcon::Terminal,
            TypedPane::Code(_) => LabradorIcon::Code2,
            TypedPane::CodeDiff => LabradorIcon::Diff,
            TypedPane::Settings => LabradorIcon::Gear,
            TypedPane::AIFact => LabradorIcon::BookOpen,
            TypedPane::ExecutionProfileEditor => LabradorIcon::Lightning,
            TypedPane::Other => LabradorIcon::File,
        }
    }
}
...
fn pane_display_title_and_subtitle(
    ...
    if matches!(typed, TypedPane::Code(_)) && !title.is_empty() {
        ...
        let parent = labrador_util::path::user_friendly_path(&parent_raw, home_str).to_string();
        (filename, parent)
    } else {
        (
            if title.is_empty() {
...
fn render_git_branch_text(
    ...
    Flex::row()
        ...
        .with_spacing(2.)
        .with_child(
            ConstrainedBox::new(UiIcon::GitBranch.to_labrador_ui_icon(text_color).finish())
...
fn render_summary_pane_kind_icon_circle(
    ...
    let (icon_element, background): (Box<dyn Element>, ElementFill) = match kind {
        SummaryPaneKind::OzAgent { .. } => (
            render_labrador_icon(icon_size, 2.),
        ...
        SummaryPaneKind::CLIAgent { agent } => {
            ...
            let icon_element = agent
                .icon()
                .map(|icon| {
                    icon.to_labrador_ui_icon(LabradorThemeFill::Solid(icon_color))
                        .finish()
                })
                .unwrap_or_else(|| {
                    LabradorIcon::Terminal
                        .to_labrador_ui_icon(theme.sub_text_color(theme.background()))
                        .finish()
                });
            (
                icon_element,
        ...
        | SummaryPaneKind::Other => {
            ...
            (
                icon.to_labrador_ui_icon(icon_color).finish(),
                internal_colors::fg_overlay_2(theme).into(),
            )
        }
    };
...
fn summary_pane_kind_icon(
    kind: SummaryPaneKind,
    appearance: &Appearance,
) -> (LabradorIcon, LabradorThemeFill) {
    let theme = appearance.theme();
    let main_text = theme.main_text_color(theme.background());
    let sub_text = theme.sub_text_color(theme.background());

    match kind {
        SummaryPaneKind::Terminal => (LabradorIcon::Terminal, main_text),
        SummaryPaneKind::OzAgent { is_ambient } => (
            if is_ambient {
                LabradorIcon::OzCloud
            } else {
                LabradorIcon::Oz
            },
            main_text,
        ...
        SummaryPaneKind::CLIAgent { agent } => (
            agent.icon().unwrap_or(LabradorIcon::Terminal),
            LabradorThemeFill::Solid(agent.brand_icon_color()),
        ),
        SummaryPaneKind::Code { .. } => (LabradorIcon::Code2, sub_text),
        SummaryPaneKind::CodeDiff => (LabradorIcon::Diff, sub_text),
        SummaryPaneKind::Settings => (LabradorIcon::Gear, main_text),
        SummaryPaneKind::AIFact => (LabradorIcon::BookOpen, sub_text),
        SummaryPaneKind::ExecutionProfileEditor => (LabradorIcon::Lightning, sub_text),
        SummaryPaneKind::Other => (LabradorIcon::File, sub_text),
    }
}
            ...
            .with_child(
                ConstrainedBox::new(
                    UiIcon::AlertTriangle
                        .to_labrador_ui_icon(error_color.into())
                        .finish(),
                )
...
fn render_pull_request_badge_content(label: &str, appearance: &Appearance) -> Box<dyn Element> {
    ...
    Flex::row()
        ...
        .with_spacing(4.)
        .with_child(
            ConstrainedBox::new(UiIcon::Github.to_labrador_ui_icon(main_text_color).finish())
                .with_width(BADGE_ICON_SIZE)
                .with_height(BADGE_ICON_SIZE)
                .finish(),
        )
    ...
    let per_pane: HashMap<PaneId, Option<AnsiColorIdentifier>> = visible_pane_ids
        ...
        .map(|&pane_id| {
            let color = if let Some(tv) = pane_group.terminal_view_from_pane_id(pane_id, app) {
                ...
                // matching against configured directories, so e.g. labrador-internal/code.rs
                // inherits the color assigned to labrador-internal.
                code_view
                    .as_ref(app)
                    .local_path(app)
                    .as_deref()
...
pub(super) fn render_settings_popup(
    ...
    let make_divider = |theme: &LabradorTheme| {
        Container::new(
            ConstrainedBox::new(
                Container::new(Empty::new().finish())
                    .with_background(internal_colors::fg_overlay_2(theme))
...
fn render_compact_subtitle_option(
    ...
    Hoverable::new(mouse_state, move |hover_state| {
        let check_icon: Box<dyn Element> = if is_selected {
            ConstrainedBox::new(LabradorIcon::Check.to_labrador_ui_icon(main_text).finish())
                .with_width(ICON_SIZE)
                .with_height(ICON_SIZE)
                .finish()
        } else {
    ...
    Hoverable::new(mouse_state, move |hover_state| {
            ...
            (info_mouse_state.clone(), info_tooltip_text.clone())
        {
            ...
            let info_icon = Hoverable::new(info_ms, move |info_hover| {
                let icon = ConstrainedBox::new(UiIcon::Info.to_labrador_ui_icon(info_color).finish())
...
fn render_popup_segment(
    ...
    Hoverable::new(mouse_state, move |hover_state| {
        ...
        Container::new(
            Align::new(
                ConstrainedBox::new(icon.to_labrador_ui_icon(icon_color).finish())
...
struct DetailSidecarTextColors {
    main: LabradorThemeFill,
    sub: LabradorThemeFill,
    disabled: LabradorThemeFill,
}

fn detail_sidecar_background(theme: &LabradorTheme) -> ColorU {
    theme
        .background()
...
fn detail_sidecar_border_fill(theme: &LabradorTheme) -> ThemeFill {
    theme
        .background()
        .blend(&internal_colors::fg_overlay_4(theme))
}

fn detail_sidecar_text_colors(theme: &LabradorTheme) -> DetailSidecarTextColors {
...
fn render_detail_status_pill(
    ...
    Container::new(
        Flex::row()
            ...
            .with_spacing(4.)
            .with_child(
                ConstrainedBox::new(icon.to_labrador_ui_icon(LabradorThemeFill::Solid(color)).finish())
                    .with_width(12.)
                    .with_height(12.)
                    .finish(),
            )
            .with_child(
                Text::new_inline(status.to_string(), appearance.ui_font_family(), 10.)
                    .with_color(LabradorThemeFill::Solid(color).into())
                    .finish(),
            )
            .finish(),
    )
