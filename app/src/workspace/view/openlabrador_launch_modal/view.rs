...
use labrador_core::ui::theme::{phenomenon::PhenomenonStyle, Fill};
...
const HERO_IMAGE_PATH: &str = "async/png/onboarding/openlabrador_launch_banner.png";
const REPO_URL: &str = "https://github.com/beefiker/labrador";
const CONTRIBUTING_URL: &str = "https://github.com/beefiker/labrador/blob/main/CONTRIBUTING.md";
...
pub fn init(app: &mut AppContext) {
    ...
    app.register_fixed_bindings([FixedBinding::new(
        "escape",
        OpenLabradorLaunchModalAction::Close,
        id!(OpenLabradorLaunchModal::ui_name()),
    )]);
}

#[derive(Clone, Debug)]
pub enum OpenLabradorLaunchModalAction {
    Close,
    VisitRepo,
}

#[derive(Clone, Debug)]
pub enum OpenLabradorLaunchModalEvent {
    Close,
}
...
pub struct OpenLabradorLaunchModal {
    close_button: ViewHandle<ActionButton>,
    cta_button: ViewHandle<ActionButton>,
}

impl OpenLabradorLaunchModal {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let close_button = ctx.add_view(|_ctx| {
            ActionButton::new("", CloseButtonTheme)
                .with_icon(Icon::X)
                .with_size(ButtonSize::Small)
                .on_click(|ctx| ctx.dispatch_typed_action(OpenLabradorLaunchModalAction::Close))
        });

        let cta_button = ctx.add_view(|_ctx| {
            ActionButton::new("Visit the repo", CtaButtonTheme)
                .with_full_width(true)
                .on_click(|ctx| ctx.dispatch_typed_action(OpenLabradorLaunchModalAction::VisitRepo))
        });
    ...
    fn render_feature_row(item: &FeatureItem, appearance: &Appearance) -> Box<dyn Element> {
        let icon_el = ConstrainedBox::new(
            item.icon
                .to_labrador_ui_icon(Fill::Solid(
                    PhenomenonStyle::modal_feature_description_text(),
                ))
                .finish(),
        )
...
impl Entity for OpenLabradorLaunchModal {
    type Event = OpenLabradorLaunchModalEvent;
}

impl View for OpenLabradorLaunchModal {
    fn ui_name() -> &'static str {
        "OpenLabradorLaunchModal"
    }

    fn on_focus(&mut self, _focus_ctx: &labrador_ui::FocusContext, ctx: &mut ViewContext<Self>) {
        ctx.focus_self();
    }
...
impl TypedActionView for OpenLabradorLaunchModal {
    type Action = OpenLabradorLaunchModalAction;

    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            OpenLabradorLaunchModalAction::Close => {
                ctx.emit(OpenLabradorLaunchModalEvent::Close);
            }
            OpenLabradorLaunchModalAction::VisitRepo => {
