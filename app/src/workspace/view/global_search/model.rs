...
use labrador_ui::{Entity, ModelContext, ModelSpawner};
...
impl GlobalSearch {
    ...
    pub fn run_search(
        ...
        let handle = ctx.spawn(
            ...
            move |_, result, ctx| match result {
                ...
                Err(err) => {
                    log::error!("GlobalSearch: Labrador ripgrep CLI search failed or aborted: {err}");
    ...
    async fn run_labrador_ripgrep_cli(
        search_id: u32,
        pattern: String,
        roots: Vec<PathBuf>,
        ignore_case: bool,
        ...
        log::info!(
            "GlobalSearch: starting Labrador ripgrep CLI search with pattern={pattern}, roots={:?}",
            roots_display
        );

        let stream =
            labrador_ripgrep::search::search_streaming(&[pattern], &roots, ignore_case, multiline)?;
