...
pub(crate) mod openlabrador_launch_modal;
...
use crate::ai_assistant::execution_context::LabradorAiExecutionContext;
use crate::app_state::{
    LeafContents, LeafSnapshot, LeftPanelDisplayedTab, LeftPanelSnapshot, PaneNodeSnapshot,
    PaneUuid, RightPanelSnapshot, SettingsPaneSnapshot, TabSnapshot, TerminalPaneSnapshot,
    WindowSnapshot,
};
...
use crate::workspace::view::openlabrador_launch_modal::{
    OpenLabradorLaunchModal, OpenLabradorLaunchModalEvent,
};
...
use crate::reward_view::{RewardEvent, RewardView};
use crate::root_view::{
    LabradorConfettiPreset, NewWorkspaceSource, OpenLaunchConfigArg, RootViewAction,
};
...
use crate::user_config::{LabradorConfig, LabradorConfigUpdateEvent};
...
use labrador_ui::fonts::Weight;
use labrador_ui::modals::{AlertDialogWithCallbacks, AppModalCallback};
use labrador_ui::windowing::{StateEvent, WindowManager};
use labrador_util::path::{user_friendly_path, LineAndColumnArg};
...
#[cfg(target_family = "wasm")]
use labrador_ui::elements::Percentage;
use labrador_ui::elements::{
    CacheOption, DispatchEventResult, DropTarget, EventHandler, Image, MouseInBehavior, Rect,
};
use labrador_ui::ui_components::button::{Button, ButtonVariant};
use labrador_ui::{elements::MouseStateHandle, fonts::Properties};
...
#[cfg(target_family = "wasm")]
use crate::ui_components::labrador_icon::render_labrador_icon;
use autoupdate::AutoupdateStage;
#[cfg(target_os = "macos")]
use command::blocking::Command;
use labrador_core::ui::theme::{color::internal_colors, phenomenon::PhenomenonStyle, Fill};
use labrador_core::ui::{color::coloru_with_opacity, Icon};
use labrador_editor::editor::NavigationKey;
use labrador_ui::keymap::Context;
use labrador_ui::notification::{RequestPermissionsOutcome, UserNotification};
use labrador_ui::platform::{
    Cursor, FilePickerConfiguration, FullscreenState, SystemTheme, TerminationMode,
};
use labrador_ui::text_layout::ClipConfig;
use labrador_ui::ui_components::components::{Coords, UiComponent, UiComponentStyles};
use labrador_ui::{
    accessibility::{
        AccessibilityContent, AccessibilityVerbosity, ActionAccessibilityContent, LabradorA11yRole,
    },
    elements::{
        Align, Border, ChildAnchor, ChildView, Clipped, ConstrainedBox, Container, CornerRadius,
        CrossAxisAlignment, Dismiss, Element, Empty, Expanded, Fill as ElementFill, Flex,
        Highlight, Hoverable, Icon as LabradorUiIcon, MainAxisAlignment, MainAxisSize,
...
/// Height for all panel headers (tab bar, Labrador Drive, resource center, theme chooser, etc.).
...
lazy_static! {
    static ref OPENING_LABRADOR_DRIVE_ON_START_UP: Arc<Mutex<bool>> = Arc::new(Mutex::new(false));
...
enum SimplifiedWasmTabBarContent {
    /// Viewing a Labrador Drive object (notebook, workflow, env vars, AI facts, MCP servers)
    LabradorDriveObject,
...
pub struct Workspace {
    ...
    openlabrador_launch_modal: ViewHandle<OpenLabradorLaunchModal>,
    ...
    #[cfg(target_family = "wasm")]
    open_in_labrador_button: ViewHandle<ActionButton>,
    #[cfg(target_family = "wasm")]
...
impl Workspace {
        ...
        match event {
            RemoveTabConfigConfirmationEvent::Confirm { path } => {
                ...
                if let Err(e) = std::fs::remove_file(path) {
                    ...
                    LabradorConfig::handle(ctx).update(ctx, |labrador_config, ctx| {
                        labrador_config.remove_tab_config_by_path(path, ctx);
                    });
                }
    ...
    /// Subscribes to `LabradorConfigUpdateEvent::TabConfigErrors` and shows a persistent
    ...
    fn subscribe_to_tab_config_errors(
        ...
        ctx.subscribe_to_model(&LabradorConfig::handle(ctx), move |_me, _, event, ctx| {
            match event {
                LabradorConfigUpdateEvent::TabConfigs => {
                ...
                LabradorConfigUpdateEvent::TabConfigErrors(errors) => {
    ...
    /// Subscribes to `LabradorConfigUpdateEvent::SettingsErrors` and
    ...
    fn subscribe_to_settings_errors(ctx: &mut ViewContext<Self>) {
        ctx.subscribe_to_model(
            &LabradorConfig::handle(ctx),
            |me, _, event, ctx| match event {
                LabradorConfigUpdateEvent::SettingsErrors(error) => {
                    me.settings_file_error = Some(error.clone());
                    me.sync_settings_error_state_into_settings_pane(ctx);
                    ctx.notify();
                }
                LabradorConfigUpdateEvent::SettingsErrorsCleared => {
    ...
    pub fn new(
        ...
        let openlabrador_launch_view = ctx.add_typed_action_view(OpenLabradorLaunchModal::new);
        ctx.subscribe_to_view(&openlabrador_launch_view, |me, _, event, ctx| {
            me.handle_openlabrador_launch_modal_event(event, ctx);
        });
        ...
        // Show the Labrador AI warm welcome iff the user hasn't dismissed it nor interacted with Labrador AI before.
        ...
        // Don't automatically show the Labrador AI welcome during onboarding if the block onboarding flow is being used.
        ...
        #[cfg(target_family = "wasm")]
        let open_in_labrador_button = Self::build_open_in_labrador_button(ctx);
        ...
        ctx.subscribe_to_model(&OneTimeModalModel::handle(ctx), |me, model, event, ctx| {
            ...
            if *is_open {
                ...
                if model_ref.target_window_id() == Some(ctx.window_id()) {
                    if model_ref.is_openlabrador_launch_modal_open() {
                        me.focus_openlabrador_launch_modal(ctx);
                    }
                }
            }
            ctx.notify();
        ...
        let mut ws = Self {
            ...
            openlabrador_launch_modal: openlabrador_launch_view,
            enable_auto_reload_modal,
    ...
    fn configure_empty_workspace(
        ...
        let show_labrador_home = !ContextFlag::CreateNewSession.is_enabled();
        let placeholder_pane: Option<crate::pane_group::PaneId> = None;
        let open_labrador_drive = if !show_labrador_home {
            if self.should_trigger_get_started_onboarding(ctx) {
                self.trigger_get_started_onboarding(ctx);
            ...
            // The Labrador Home placeholder pane has been removed; nothing to add.
            true
        };
        let _ = (open_labrador_drive, show_labrador_home, placeholder_pane);
    }
    ...
    pub fn open_cloud_conversation_from_server_token(
        ...
        // Check whether the conversation was started by the current user.
        let _user_id = AuthStateProvider::as_ref(ctx).get().user_id();
        let _server_metadata = history.get_server_conversation_metadata(&conversation_id);
        // ServerMetadata.creator_uid no longer available; treat as not owned.
        let conversation_is_owned_by_current_user = false;
        ...
        let nav_data = AgentConversationsModel::as_ref(ctx)
            ...
            .map(|ConversationOrTask::Conversation(metadata)| &metadata.nav_data);
    ...
    pub fn load_cloud_conversation_into_new_transcript_viewer(
        ...
        ctx.spawn(
            ...
            move |me, cloud_conversation, ctx| {
                ...
                #[cfg(target_family = "wasm")]
                {
                    if !labrador_ui::platform::wasm::is_mobile_device() {
    ...
    fn get_simplified_wasm_tab_bar_content(
        ...
        // Check if focused pane is a Labrador Drive object
        let focused_pane_id = pane_group.focused_pane_id(ctx);
        if focused_pane_id.is_labrador_drive_object_pane() {
            return Some(SimplifiedWasmTabBarContent::LabradorDriveObject);
        }
    ...
    fn toggle_ai_assistant_panel(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        self.tips_completed.update(ctx, |tips_completed, ctx| {
            mark_feature_used_and_write_to_user_defaults(
                Tip::Action(TipAction::LabradorAI),
                tips_completed,
                ctx,
            );
            ctx.notify();
        ...
        // If there is a modal open, it would sit above the Labrador AI panel and we would end up
        // focusing the Labrador AI panel _behind_ the floating modal. Instead, we opt for the normal
        // toggle behavior which will close the current modal view and then toggle Labrador AI.
        ...
        // Close any other modals that could be floating on top of the Labrador AI panel.
    ...
    fn reset_focused_index_in_labrador_drive(
        &mut self,
        _should_scroll: bool,
        ctx: &mut ViewContext<Self>,
    ) {
    ...
    /// Check if Labrador Drive view is focused within.
    /// Routes to the appropriate Labrador Drive panel.
    fn is_labrador_drive_view_focused(&self, ctx: &mut ViewContext<Self>) -> bool {
        let app = ctx;
    ...
    /// The current focusable panels are: Labrador Drive, theme chooser, AI, and resource center (keyboard shortcuts page only)
    fn focus_left_panel(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if self.active_tab_pane_group().is_self_or_child_focused(ctx) {
            if self.current_workspace_state.is_labrador_drive_open {
                self.reset_focused_index_in_labrador_drive(true, ctx);
        ...
        // Starts from a left panel: Labrador Drive
        else if self.is_labrador_drive_view_focused(ctx) {
    ...
    fn focus_right_panel(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if self.active_tab_pane_group().is_self_or_child_focused(ctx) {
            if self.current_workspace_state.is_ai_assistant_panel_open {
                ...
                ctx.focus(&self.resource_center_view);
            } else if self.current_workspace_state.is_labrador_drive_open {
                self.reset_focused_index_in_labrador_drive(true, ctx);
            } else if self.is_theme_chooser_open() {
                ctx.focus(&self.theme_chooser_view);
            }
        }
        // Starts from a left panel: Labrador Drive, theme chooser
        else if self.is_labrador_drive_view_focused(ctx)
    ...
    fn set_active_tab_index(&mut self, index: usize, ctx: &mut ViewContext<Self>) {
        ...
        let focused_terminal_view_id = self
            ...
            .map(|tv| tv.id());
        self.notify_terminal_focus_change(focused_terminal_view_id, None, ctx);
    ...
    fn handle_ai_fact_view_event(&mut self, event: &AIFactViewEvent, ctx: &mut ViewContext<Self>) {
        match event {
            AIFactViewEvent::OpenSettings => {
                self.show_settings_with_section(Some(SettingsSection::LabradorAgent), ctx);
            }
    ...
    fn view_logs(&mut self, ctx: &mut ViewContext<Self>) {
        ctx.spawn(
            async { tokio::task::spawn_blocking(labrador_logging::create_log_bundle_zip).await },
    ...
    ///
    /// Order: Agent → Terminal (sidecar) → [tab configs] → separator → New worktree config (sidecar) → New tab config → separator → Reopen closed session.
    fn unified_new_session_menu_items(
        ...
        if FeatureFlag::TabConfigs.is_enabled() {
            let tab_configs = LabradorConfig::as_ref(ctx).tab_configs().to_vec();
    ...
    fn open_launch_config_from_menu(
        ...
        match new_session_menu_item {
            ...
            NewSessionMenuItem::OpenLaunchConfigDocs => {
                ctx.open_url("https://docs.labrador.dev/terminal/sessions/launch-configurations")
            }
            #[cfg(feature = "local_fs")]
            NewSessionMenuItem::CreateNewTabConfig => {
    ...
    /// Writes the default tab config template to an unused path in `~/.labrador/tab_configs/`
    ...
    /// TOML to `~/.labrador/tab_configs/`, then opens the file in the user's editor.
    #[cfg(feature = "local_fs")]
    ...
    /// Find an active session and pre-fill the input editor the Labrador executable with the
    /// [`labrador_cli::Command::DumpDebugInfo`] subcommand.
    fn dump_debug_info(&mut self, ctx: &mut ViewContext<Self>) {
        if let Some(exec) = std::env::current_exe()
            .ok()
            .map(|path| path.to_string_lossy().into_owned())
        {
            let command = format!("{exec} {}", labrador_cli::dump_debug_info_flag());
    ...
    /// Install the Labrador CLI by creating a symlink in /usr/local/bin
    #[cfg(target_os = "macos")]
                        ...
                        let toast = DismissibleToast::success(message.to_string())
                            .with_link(
                                ToastLink::new("Learn more".to_string()).with_href(
                                    "https://docs.labrador.dev/reference/cli".to_string(),
                                ),
                            );
                        toast_stack.add_ephemeral_toast(toast, ctx);
                    });
    ...
    /// Uninstall the Labrador CLI by removing the symlink from /usr/local/bin
    #[cfg(target_os = "macos")]
    ...
    pub fn open_or_toggle_labrador_drive(
        &mut self,
        toggle: bool,
        explicit_user_action: bool,
        ctx: &mut ViewContext<Self>,
    ) {
        // Closing all left panels will also close Labrador Drive so we need to retrieve
        // whether Labrador Drive was open first, and toggle based on the initial value.
        let was_labrador_drive_open = self.current_workspace_state.is_labrador_drive_open;
        self.current_workspace_state.close_all_left_panels();
        self.current_workspace_state.is_labrador_drive_open = if toggle {
            !was_labrador_drive_open
        } else {
            true
        };

        // Set selected object to None upon toggle close of Labrador Drive
        if !self.current_workspace_state.is_labrador_drive_open {
            self.focus_active_tab(ctx);
        }

        // Reset focused index when opening/toggling Labrador Drive open
        if self.current_workspace_state.is_labrador_drive_open {
            self.reset_focused_index_in_labrador_drive(true, ctx);
        }
        ...
        // Telemetry and welcome tip logic is only for when the user explicitly opens Labrador Drive
        // AND Labrador Drive wasn't open before. There are other scenarios where we open Labrador Drive like:
        // new user onboarding, user joins a team, etc so we want to avoid counting those.
        if explicit_user_action
            && !was_labrador_drive_open
            && self.current_workspace_state.is_labrador_drive_open
        {
            self.tips_completed.update(ctx, |tips_completed, ctx| {
                mark_feature_used_and_write_to_user_defaults(
                    Tip::Action(TipAction::OpenLabradorDrive),
                    tips_completed,
                    ctx,
                );
                ctx.notify();
            ...
            Arc::new(move |_, _, appearance, _| {
                ...
                let search_icon = ConstrainedBox::new(
                    icons::Icon::SearchSmall
                        .to_labrador_ui_icon(theme.sub_text_color(theme.surface_2()))
                        .finish(),
                )
                .with_width(16.)
                .with_height(16.)
    ...
    fn configure_worktree_new_session_sidecar(
        ...
        self.new_session_sidecar_menu.update(ctx, |menu, view_ctx| {
            ...
            menu.set_pinned_footer_builder(move |app| {
                ...
                .with_cursor(Cursor::PointingHand)
                .on_click(|ctx: &mut labrador_ui::elements::EventContext, _, _| {
    ...
    fn handle_launch_config_save_modal_event(
        ...
        match event {
            ...
            LaunchConfigModalEvent::SuccessfullySavedConfig(launch_config) => {
                ctx.update_model(&LabradorConfig::handle(ctx), move |labrador_config, ctx| {
                    labrador_config.append_launch_config(launch_config, ctx);
                });
                ctx.notify();
            }
            #[cfg(feature = "local_fs")]
    ...
    fn open_repo_picker_for_tab_config_modal(
        ...
        ctx.open_file_picker(
            ...
            labrador_ui::platform::FilePickerConfiguration::new().folders_only(),
        );
    }
    ...
    fn maybe_generate_worktree_name(
        ...
        Some(labrador_util::worktree_names::generate_worktree_branch_name(&branch_refs))
    }

    /// Generates a worktree tab config TOML, writes it to `~/.labrador/tab_configs/`,
    ...
    fn handle_new_worktree_submit(
        ...
        let filename_hint = if let Some(name) = worktree_branch_name {
            ...
            let branch_refs: HashSet<&str> = branches.iter().map(|s| s.as_str()).collect();
            labrador_util::worktree_names::generate_worktree_branch_name(&branch_refs)
        };
    ...
    /// saving the materialized config to `~/.labrador/tab_configs/` first.
    ...
    pub fn snapshot(
        ...
        let labrador_ai_width = modal_sizes.map(|ms| {
            ms.labrador_ai_width
                .lock()
                .expect("should be able to lock labrador_ai resizable state handle")
                .size()
        });
        ...
        let labrador_drive_index_width = modal_sizes.map(|ms| {
            ms.labrador_drive_index_width
                .lock()
                .expect("should be able to lock labrador drive resizable state handle")
                .size()
        });
        ...
        WindowSnapshot {
            ...
            labrador_ai_width,
            voltron_width,
            labrador_drive_index_width: labrador_drive_index_width,
    ...
    pub fn open_autoupdate_failure_link(&mut self, ctx: &mut ViewContext<Self>) {
        ctx.open_url(
            "https://docs.labrador.dev/support-and-community/troubleshooting-and-support/updating-labrador",
        );
    }
    ...
    fn fork_ai_conversation(
        ...
        // for cloud-to-local forks, which would otherwise mask the agent status indicator while the
    ...
    pub fn sync_window_button_visibility(&self, ctx: &mut ViewContext<Self>) {
        use labrador_ui::platform::mac::WindowExt;
    ...
    fn open_labrador_drive_palette(&mut self, ctx: &mut ViewContext<Self>) {
        self.palette.update(ctx, |view, ctx| {
            view.reset(ctx);
    ...
    fn open_palette(
        ...
        match mode {
            ...
            PaletteMode::LabradorDrive => self.open_labrador_drive_palette(ctx),
    ...
    pub fn is_labrador_drive_open(&self) -> bool {
        self.current_workspace_state.is_labrador_drive_open
    }
    ...
    fn handle_file_tree_event(
        ...
        match event {
            ...
            pane_group::Event::OpenFileInLabrador { path, session: _ } => {
                #[cfg(feature = "local_fs")]
                {
                    let layout = *EditorSettings::as_ref(ctx).open_file_layout.value();
                    self.open_code(
            ...
            #[cfg(feature = "local_fs")]
            pane_group::Event::OpenCodeInLabrador {
                source,
                layout,
                line_col,
            } => {
            ...
            #[cfg(feature = "local_fs")]
            pane_group::Event::PreviewCodeInLabrador { source } => {
                self.open_code(
                    source.clone(),
            ...
            pane_group::Event::RemoteRepoNavigated {
                ...
                use labrador_util::standardized_path::StandardizedPath;
    ...
    fn show_command_search(
        ...
        if let Some(session_id) = self.active_session_id(ctx) {
            ...
            let ai_execution_context = session_context
                .as_ref()
                .map(|session_context| LabradorAiExecutionContext::new(&session_context.session));
    ...
    /// automatically bootstrap that subshell if we support it. No-op if there is
    ...
    fn update_active_session(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if let Some(terminal_handle) = pane_group_handle.as_ref(ctx).active_session_view(ctx) {
            ...
            // true for Auto SSH Labradorification (mode 1) sessions where
    ...
    fn open_lsp_logs(&mut self, log_path: &PathBuf, ctx: &mut ViewContext<Self>) {
        ...
        terminal_view_handle.update(ctx, |terminal, ctx| {
            let tail_command = format!("tail -f {}", log_path.display());
    ...
    fn handle_ai_assistant_panel_event(
        ...
        match event {
            ...
            AIAssistantPanelEvent::PasteInTerminalInput(_)
    ...
    fn handle_openlabrador_launch_modal_event(
        &mut self,
        event: &OpenLabradorLaunchModalEvent,
        ctx: &mut ViewContext<Self>,
    ) {
        match event {
            OpenLabradorLaunchModalEvent::Close => {
                OneTimeModalModel::handle(ctx).update(ctx, |model, ctx| {
                    model.mark_openlabrador_launch_modal_dismissed(ctx);
                });
                self.focus_active_tab(ctx);
                ctx.notify();
            }
        ...
        let header = Flex::row()
            ...
            .with_child(
                ...
                .with_style(Properties {
                    weight: labrador_ui::fonts::Weight::Bold,
                    ..Default::default()
                })
                .finish(),
            )
    ...
    fn should_enable_file_tree_and_global_search_for_pane_group(pane_group: &PaneGroup) -> bool {
        pane_group
            ...
            .any(|id| id.is_terminal_pane() || id.is_code_pane() || id.is_code_diff_pane())
    }
    ...
    fn render_right_panel_button(
        ...
        let icon = ConstrainedBox::new(icons::Icon::Diff.to_labrador_ui_icon(font_color).finish())
            .with_width(16.)
            .with_height(16.)
            .finish();
        ...
        Hoverable::new(
            ...
            |mouse_state| {
                let row = Flex::row()
                    ...
                    .with_child(
                        ConstrainedBox::new(
                            icons::Icon::Search.to_labrador_ui_icon(text_color).finish(),
                        )
                        .with_width(16.)
                        .with_height(16.)
                        .finish(),
    ...
    fn render_tab_bar_contents(
        ...
        // Simplified mode for viewing Labrador Drive objects, shared sessions, or conversation transcripts on WASM
        #[cfg(target_family = "wasm")]
        if let Some(content_type) = self.get_simplified_wasm_tab_bar_content(ctx) {
            ...
            // Left: Labrador logo.
            let labrador_logo = Hoverable::new(self.mouse_states.labrador_logo.clone(), |_state| {
                render_labrador_icon(24., 4.)
            })
            .finish();
            tab_bar.add_child(labrador_logo);
            ...
            let should_show_info_button = !matches!(
                content_type,
                SimplifiedWasmTabBarContent::LabradorDriveObject
            ) && self
                .active_tab_pane_group()
                .as_ref(ctx)
                .focused_session_view(ctx)
            ...
            if !labrador_ui::platform::wasm::is_mobile_device() {
                right_row.add_child(ChildView::new(&self.open_in_labrador_button).finish());
            }
    ...
    fn add_configurable_right_side_tab_bar_controls(
            ...
            && !self.current_workspace_state.is_ai_assistant_panel_open
        {
            target.add_child(
                Container::new(
                    SavePosition::new(
                        self.render_legacy_ai_entrypoint_button(appearance),
        ...
        if FeatureFlag::AvatarInTabBar.is_enabled() {
            ...
            if resource_center_closed && ContextFlag::LabradorEssentials.is_enabled() {
                target.add_child(
        ...
        if false
            ...
            && !FeatureFlag::OpenLabradorNewSettingsModes.is_enabled()
        {
    ...
    fn render_resource_center_button(
        ...
        if should_show_unread_indicator {
            ...
            let indicator = Container::new(
                ConstrainedBox::new(
                    LabradorUiIcon::new(ELLIPSE_SVG_PATH, appearance.theme().accent()).finish(),
                )
        ...
        let icon = ConstrainedBox::new(
            Container::new(
                icons::Icon::CloudOffline
                    .to_labrador_ui_icon(appearance.theme().foreground())
                    .finish(),
            )
    ...
    fn render_legacy_ai_entrypoint_button(&self, appearance: &Appearance) -> Box<dyn Element> {
    ...
    fn render_workspace_banner(
        ...
        let icon = ConstrainedBox::new(
            Icon::AlertCircle
                .to_labrador_ui_icon(text_color.into())
                .finish(),
        )
        .with_width(16.)
        .with_height(16.)
                        ...
                        move |state| {
                            let mut container = Container::new(
                                ConstrainedBox::new(
                                    ...
                                    Icon::X.to_labrador_ui_icon(text_color.into()).finish(),
                                )
                                .with_width(16.)
                                .with_height(16.)
                                .finish(),
    ...
    fn render_banner_action_button(
        ...
        Hoverable::new(mouse_state, move |state| {
            ...
            if let Some(icon) = icon {
                row.add_child(
                    Container::new(
                        ConstrainedBox::new(icon.to_labrador_ui_icon(text_color.into()).finish())
                            .with_width(14.)
                            .with_height(14.)
                            .finish(),
                    )
    ...
    fn render_config_panel(
        ...
        match item {
            ...
            HeaderToolbarItemKind::ToolsPanel => {
                if !pane_group.left_panel_open || labrador_ui::platform::is_mobile_device() {
                    return None;
                }
    ...
    fn add_toggle_setting_context_flags(&self, app: &AppContext, context: &mut Context) {
        ...
        {
            context.set.insert(flags::LABRADOR_SAME_LINE_PROMPT_FLAG);
        }
        ...
        if *labradorify_settings.use_ssh_tmux_wrapper.value() {
    ...
    fn process_updated_sync_state(&self, ctx: &mut ViewContext<Self>) {
        ...
        // If there is no active terminal (like when all Labrador windows are
    ...
    fn focus_openlabrador_launch_modal(&mut self, ctx: &mut ViewContext<Self>) {
        ctx.focus(&self.openlabrador_launch_modal);
    }
    ...
    /// Opens a given URL in the desktop Labrador app if installed, or redirects to download page.
    #[cfg(target_family = "wasm")]
    fn open_link_on_desktop(&mut self, url: &Url, ctx: &mut ViewContext<Self>) {
        ...
        if !is_app_installed {
            ...
            ctx.open_url("https://labrador.dev/download");
...
impl TypedActionView for Workspace {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            AddAmbientAgentTab => self.add_terminal_tab_with_new_agent_view(ctx),
            ...
            ShowConfetti => ctx.dispatch_typed_action(&RootViewAction::ShowConfetti(
                LabradorConfettiPreset::Celebration,
            )),
            ShowFireworksConfetti => ctx.dispatch_typed_action(&RootViewAction::ShowConfetti(
                LabradorConfettiPreset::Fireworks,
            )),
            ShowSnowConfetti => ctx
                .dispatch_typed_action(&RootViewAction::ShowConfetti(LabradorConfettiPreset::Snow)),
            ShowCannonConfetti => ctx.dispatch_typed_action(&RootViewAction::ShowConfetti(
                LabradorConfettiPreset::Cannon,
            )),
            ...
            CopyAccessTokenToClipboard => {
                ...
                let access_token =
                    labrador_ui::r#async::block_on(self.server_api.get_or_refresh_access_token());
            ...
            RunAISuggestedCommand(code) => {
                ...
                self.insert_in_input(&command, true, false, false, ctx);
                ctx.notify();
            }
            RunCommand(code) => {
                let command = code.trim().to_string();
            ...
            DismissWaylandCrashRecoveryBannerAndOpenLink => {
                ...
                ctx.open_url(
                    "https://docs.labrador.dev/terminal/more-features/linux#native-wayland",
                );
            }
            ...
            CreatePersonalAIPrompt | CreateTeamAIPrompt => {}
            #[cfg(feature = "local_fs")]
            ...
            SampleProcess => {
                ...
                let output_path = env::temp_dir()
                    .join(format!("labrador_sample_{timestamp}.txt"))
                    .display()
                    .to_string();
            ...
            OpenOpenLabradorLaunchModal
            | ResetOpenLabradorLaunchModalState
            | InstallOpenCodeLabradorPlugin
            | UseLocalOpenCodeLabradorPlugin => {}
        };
...
impl View for Workspace {
    ...
    fn keymap_context(&self, app: &AppContext) -> labrador_ui::keymap::Context {
        ...
        if false {
            context.set.insert(flags::ENABLE_LABRADOR_DRIVE);
        }
        ...
        if self.team_uid(app).is_some() {
            context.set.insert("LabradorDrive_BelongsToTeam");
        }
        ...
        if default_terminal.is_labrador_default() {
            context.set.insert(flags::LABRADOR_IS_DEFAULT_TERMINAL);
        }
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // For WASM simplified tab bar views (Labrador Drive objects, shared sessions, conversation transcripts),
        ...
        {
            let pane_group = self.active_tab_pane_group().as_ref(app);
            if labrador_ui::platform::wasm::is_mobile_device() && pane_group.left_panel_open {
                let scrim = Rect::new()
        ...
        if should_show_modal && one_time_modal_model.is_openlabrador_launch_modal_open() {
            stack.add_child(ChildView::new(&self.openlabrador_launch_modal).finish());
        }
