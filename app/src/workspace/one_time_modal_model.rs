...
use labrador_ui::{Entity, ModelContext, SingletonEntity, WindowId};
...
pub struct OneTimeModalModel {
    ...
    /// Whether the Labrador launch modal is currently being shown.
    is_oz_launch_modal_open: bool,
    /// Whether the OpenLabrador launch modal is currently being shown.
    is_openlabrador_launch_modal_open: bool,
...
impl OneTimeModalModel {
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
        ...
        ctx.subscribe_to_model(&AuthManager::handle(ctx), |me, event, ctx| {
            ...
            if is_existing_user {
                // Cloud preferences sync was removed from this fork; trigger modals immediately.
                ...
                GeneralSettings::handle(ctx).update(ctx, |settings, ctx| {
                    if let Err(e) = settings
                        .did_check_to_trigger_openlabrador_launch_modal
                        .set_value(true, ctx)
                    {
                        log::warn!("Failed to mark OpenLabrador launch modal as dismissed: {e}");
                    }
                });
            }
        });
        ...
        Self {
            ...
            is_openlabrador_launch_modal_open: false,
    ...
    /// Returns whether the OpenLabrador launch modal is currently open.
    pub fn is_openlabrador_launch_modal_open(&self) -> bool {
        self.is_openlabrador_launch_modal_open && self.target_window_id.is_some()
    }

    pub fn mark_openlabrador_launch_modal_dismissed(&mut self, ctx: &mut ModelContext<Self>) {
        self.set_openlabrador_launch_modal_open(false, ctx);
    }

    /// Returns true if any one-time modal is currently open.
    pub fn is_any_modal_open(&self) -> bool {
        (self.is_oz_launch_modal_open
            || self.is_openlabrador_launch_modal_open
    ...
    #[cfg(debug_assertions)]
    pub fn force_open_openlabrador_launch_modal(&mut self, ctx: &mut ModelContext<Self>) {
        self.set_openlabrador_launch_modal_open(true, ctx);
    }
    ...
    fn set_openlabrador_launch_modal_open(
        &mut self,
        is_open: bool,
        ctx: &mut ModelContext<Self>,
    ) -> bool {
        if self.is_openlabrador_launch_modal_open != is_open {
            self.is_openlabrador_launch_modal_open = is_open;
            ctx.emit(OneTimeModalEvent::VisibilityChanged { is_open });
            return true;
        }
        false
    ...
    fn check_and_trigger_all_modals(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        // The OpenLabrador launch modal takes priority over the Oz launch modal
        // when both are enabled.
        if self.check_and_trigger_openlabrador_launch_modal(ctx) {
            return;
        }
    ...
    fn check_and_trigger_openlabrador_launch_modal(
        &mut self,
        ctx: &mut ModelContext<Self>,
    ) -> bool {
        // Only show if the feature flag is enabled.
        if !FeatureFlag::OpenLabradorLaunchModal.is_enabled() {
            return false;
        }

        let general_settings = GeneralSettings::as_ref(ctx);
        let openlabrador_modal_shown = *general_settings
            .did_check_to_trigger_openlabrador_launch_modal
            .value();

        if openlabrador_modal_shown {
            return false;
        }
        ...
        let should_show_openlabrador_modal =
            !matches!(ChannelState::channel(), Channel::Integration | Channel::Oss);
        self.set_openlabrador_launch_modal_open(should_show_openlabrador_modal, ctx);
        should_show_openlabrador_modal
    }
