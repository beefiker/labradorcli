...
pub const DISMISSED_AI_ASSISTANT_WELCOME_KEY: &str = "DismissedLabradorAIWarmWelcome";
...
pub struct QuakeModeSettings {
    ...
    /// user focuses on another Labrador window or another app.
...
impl Settings {
    pub fn theme_for_theme_kind(theme_kind: &ThemeKind, ctx: &mut AppContext) -> LabradorTheme {
        match theme_kind {
            ThemeKind::InMemory(in_memory_theme) => in_memory_theme.theme(),
            _ => LabradorConfig::as_ref(ctx).theme_config().theme(theme_kind),
        }
    }
}
...
pub fn user_preferences_file_path() -> PathBuf {
    labrador_core::paths::config_local_dir().join("user_preferences.json")
}

/// Returns the path to the TOML settings file.
pub fn user_preferences_toml_file_path() -> PathBuf {
    labrador_core::paths::config_local_dir().join("settings.toml")
}
