...
use labrador_core::ui::{
    ...
    theme::{AnsiColor, AnsiColors, TerminalColors, LabradorTheme},
};
use labrador_ui::fonts::FontInfo;
...
impl AlacrittyTheme {
    fn parse(self) -> Result<ThemeType, ThemeError> {
            ...
            || background == DEFAULT_ALACRITTY_BACKGROUND.into()
        {
            ...
            Ok(ThemeType::Single(LabradorTheme::new(
                background.into(),
                foreground.into(),
                accent.into(),
...
impl AlacrittyColors {
    /// Returns terminal colors with Labrador's default colors substituted in for any
