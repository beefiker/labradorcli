...
use labrador_core::{channel::ChannelState, settings::Setting, ui::appearance::Appearance};
...
use crate::{
    ...
    user_config::{self, LabradorConfig},
...
impl SettingsImportView {
    ...
    fn render_reset_button(&self, appearance: &Appearance) -> Box<dyn labrador_ui::Element> {
        appearance
            .ui_builder()
                            ...
                            .with_child(
                                Flex::row()
                                    ...
                                    .with_cross_axis_alignment(
                                        labrador_ui::elements::CrossAxisAlignment::Center,
                                    )
                                    .with_main_axis_size(MainAxisSize::Max)
    ...
    fn set_theme(
        ...
        match theme_type {
            ThemeType::LightAndDark { light, dark } => {
                ...
                LabradorConfig::handle(ctx).update(ctx, |config, ctx| {
    ...
    pub(crate) fn interrupt_block(&mut self, ctx: &mut labrador_ui::ViewContext<Self>) {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            SettingsImportAction::ImportButtonClicked => {
                ...
                // set_preferences should not fail because it is writing directly to Labrador's preferences.
