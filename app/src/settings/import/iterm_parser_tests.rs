...
use labrador_ui::{fonts::FontInfo, keymap::Keystroke};
...
#[test]
fn test_into_labrador_theme_valid() {
    let theme: LabradorTheme = solarized_dark_theme()
        .into_labrador_theme("", &default_dark_theme())
        .expect("Should be able to convert into a theme");
    assert_eq!(
        theme.accent(),
        Fill::Solid(ColorU {
            r: 229,
...
#[test]
fn test_into_labrador_theme_invalid() {
    default_dark_theme()
        .into_labrador_theme("", &default_dark_theme())
...
<key>PrefsCustomFolder</key><string>{}</string>
        ...
        let ThemeType::Single(ref labrador_theme) =
...
fn test_parse_font_with_default_size() {
    let labrador_default_profile = ITermProfile {
        font_name: Some("CourierNewPSMT".to_string()),
