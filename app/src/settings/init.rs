...
use labrador_ui::{rendering::GPUPowerPreference, AppContext, SingletonEntity};
...
use crate::{
    ...
    terminal::{
        ...
        labradorify::settings::LabradorifySettings,
        BlockListSettings,
    },
...
use super::{
    ...
    VimBannerSettings, LabradorDrivePrivacySettings,
};
...
pub fn register_all_settings(ctx: &mut AppContext) {
    ...
    LabradorDrivePrivacySettings::register(ctx);
    ...
    LabradorifySettings::register(ctx);
...
pub fn init(
    ...
    // Set up hot-reload for the settings file. When the LabradorConfig watcher
    // detects a change to settings.toml, reload preferences from disk and
    // push changed values into setting models.
    #[cfg(feature = "local_fs")]
    {
        let prefs = <settings::PublicPreferences as labrador_ui::SingletonEntity>::as_ref(ctx);
        if prefs.is_settings_file() {
            ctx.subscribe_to_model(
                &crate::user_config::LabradorConfig::handle(ctx),
                handle_labrador_config_change,
            );
        }
    }
...
/// Handles a `LabradorConfig` change event, reloading settings from disk when
/// the settings file is modified, created, or deleted.
#[cfg(feature = "local_fs")]
fn handle_labrador_config_change(
    _: labrador_ui::ModelHandle<crate::user_config::LabradorConfig>,
    event: &crate::user_config::LabradorConfigUpdateEvent,
    ctx: &mut AppContext,
) {
    use crate::user_config::{LabradorConfig, LabradorConfigUpdateEvent};

    if !matches!(event, LabradorConfigUpdateEvent::Settings) {
        return;
    }
    let prefs = <settings::PublicPreferences as labrador_ui::SingletonEntity>::as_ref(ctx);
    if let Err(err) = prefs.reload_from_disk() {
        log::warn!("Settings file reload failed: {err}");
        LabradorConfig::handle(ctx).update(ctx, |_, ctx| {
            ctx.emit(LabradorConfigUpdateEvent::SettingsErrors(
    ...
    LabradorConfig::handle(ctx).update(ctx, |_, ctx| {
        if failed_keys.is_empty() {
            ctx.emit(LabradorConfigUpdateEvent::SettingsErrorsCleared);
        } else {
            ctx.emit(LabradorConfigUpdateEvent::SettingsErrors(
...
fn init_platform_native_preferences() -> user_preferences::Model {
    cfg_if::cfg_if! {
        if #[cfg(test)] {
            ...
            let app_id = labrador_core::channel::ChannelState::app_id();
            ...
            Box::new(user_preferences::user_defaults::UserDefaultsPreferencesStorage::new(
                labrador_core::channel::ChannelState::data_domain_if_not_default()
            ))
...
pub fn init_public_user_preferences() -> (user_preferences::Model, Option<user_preferences::Error>)
{
    cfg_if::cfg_if! {
        if #[cfg(test)] {
            ...
            if labrador_core::features::FeatureFlag::SettingsFile.is_enabled() {
