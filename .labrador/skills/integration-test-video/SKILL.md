...
- `crates/labrador_ui_core/src/integration/driver.rs`
- `crates/labrador_ui_core/src/integration/step.rs`
- `crates/labrador_ui_core/src/integration/video_recorder.rs`
- `crates/labrador_ui_core/src/integration/artifacts.rs`
- `crates/labrador_ui_core/src/integration/overlay.rs`
...
```bash
LABRADOR_UI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS=1 \
cargo run -p integration --bin integration -- test_video_recording
```

That is the command shown by the sample test in `crates/integration/src/test/video_recording.rs`.

If you want the driver to auto-record a test or set of tests, add `LABRADOR_INTEGRATION_TEST_VIDEO`:

```bash
LABRADOR_UI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS=1 \
LABRADOR_INTEGRATION_TEST_VIDEO=test_video_recording \
cargo run -p integration --bin integration -- test_video_recording
```
...
```bash
LABRADOR_UI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS=1 \
LABRADOR_INTEGRATION_TEST_VIDEO=test_foo,test_bar \
cargo nextest run --no-fail-fast --workspace test_foo
```

## Environment variables

### `LABRADOR_UI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS`
...
### `LABRADOR_INTEGRATION_TEST_VIDEO`
This is the main env var that controls driver-managed video recording in `crates/labrador_ui_core/src/integration/driver.rs`.

The old compatibility env var name is still accepted as a fallback.
...
LABRADOR_INTEGRATION_TEST_VIDEO=all
```
...
Important nuance:
- You do not need `LABRADOR_INTEGRATION_TEST_VIDEO` if the test itself explicitly calls `with_start_recording()` and `with_stop_recording()`.
- Use the env var when you want whole-test recording without changing the test code.

### `LABRADOR_INTEGRATION_TEST_ARTIFACTS_DIR`
This controls the root artifact directory used by `TestArtifacts` in `crates/labrador_ui_core/src/integration/artifacts.rs`.
...
```text
$TMPDIR/labrador_integration_test_artifacts
```
...
### `LABRADOR_INTEGRATION_TEST_VIDEO_DIR`
This env var exists in `crates/labrador_ui_core/src/integration/video_recorder.rs` as the lower-level recorder output root helper, defaulting to:

The old compatibility env var name is still accepted as a fallback.

```text
$TMPDIR/labrador_integration_video_captures
```

On this branch, the normal integration driver flow writes the finalized video into the test artifacts directory instead, so `LABRADOR_INTEGRATION_TEST_ARTIFACTS_DIR` is the one you usually care about when reviewing results.
...
Set `LABRADOR_INTEGRATION_TEST_VIDEO` to:
- `all`
- `1`
...
Overlay annotations are produced from the input events the test dispatches while recording is active. The overlay pipeline is implemented in `crates/labrador_ui_core/src/integration/overlay.rs`, and the event capture hooks live in `crates/labrador_ui_core/src/integration/step.rs`.
...
```text
${LABRADOR_INTEGRATION_TEST_ARTIFACTS_DIR:-$TMPDIR/labrador_integration_test_artifacts}/<test_name>/<timestamp>/
```
...
It is:
- registered in `crates/integration/src/bin/integration.rs`
- listed in `crates/integration/tests/integration/ui_tests.rs`
- implemented in `crates/integration/src/test/video_recording.rs`
...
2. Decide whether recording should be explicit in the test or enabled via `LABRADOR_INTEGRATION_TEST_VIDEO`.
3. Ensure the run uses `LABRADOR_UI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS=1`.
