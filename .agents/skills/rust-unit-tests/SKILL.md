...
description: Write, improve, and run Rust unit tests in the Labrador Rust codebase.
---

# Rust Unit Tests in Labrador
...
- Prefer `labrador_ui::App::test` for deterministic unit tests around views/models.
- Initialize app models once, then mutate via `update` and assert via `read`.

```rust
use labrador_ui::App;
// In app crate tests prefer `crate::test_util::...`; from other crates use `labrador::test_util::...`.
...
```rust
use labrador::features::FeatureFlag; // or `use crate::features::FeatureFlag;` inside the app crate
