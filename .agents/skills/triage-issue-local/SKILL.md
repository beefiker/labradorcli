...
description: Repo-specific triage guidance for Labrador. Only the categories declared overridable by the core triage-issue skill may be specialized here.
---

# Repo-specific triage guidance for Labrador
...
- Labrador is the public-facing Labrador desktop client repository. Treat public issue reports as potentially incomplete and avoid asking for secrets, tokens, private workspace names, private repository names, or account identifiers in the public issue thread.
- Distinguish the user's observed Labrador behavior from their guesses about Rust modules, UI components, server behavior, feature flags, or product intent.
- For issue reports that mention another terminal, editor, shell, or CLI tool, identify whether the problem is Labrador-specific or generally reproducible outside Labrador before assigning Labrador ownership.
...
- Use the configured Drive area label for Labrador Drive objects, sync, sharing, workflows, notebooks, tab configs, and persisted artifacts.
...
- Labrador channel and version/build number, especially whether the report is for Dev, Canary, Preview, Beta, or Stable.
...
- Clear reproduction steps, expected behavior, actual behavior, frequency, regression timing, and whether the user can reproduce outside Labrador.
...
- Performance issue: ask for approximate scale, how long Labrador has been running, what action triggers the spike or hang, and whether logs or a profile are available.
