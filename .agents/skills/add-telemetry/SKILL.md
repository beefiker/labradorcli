---
name: add-telemetry
description: Add telemetry events to track user behavior or system events in the Labrador codebase. Use when instrumenting new features, debugging issues, or measuring product metrics.
---
...
Labrador uses a trait-based telemetry system where feature-specific enums implement the `TelemetryEvent` trait. This approach keeps telemetry events organized by domain rather than in one giant enum.
...
```rust
labrador_core::register_telemetry_event!(YourFeatureTelemetryEvent);
```
...
```rust
use labrador_core::send_telemetry_from_ctx;
...
Run Labrador with the `log_named_telemetry_events` feature flag to see telemetry events logged to the console:
