---
name: create-pr
description: Create a pull request in the Labrador repository for the current branch. Use when the user mentions opening a PR, creating a pull request, submitting changes for review, or preparing code for merge.
---
...
This guide covers best practices for creating pull requests in the Labrador repository, including merging master, running presubmit checks, linking Linear tasks, ensuring appropriate test coverage, and structuring your PR for effective review.
...
- `labrador-integration-test` - Add or update integration coverage for user-visible flows, regressions, and P0 use cases
...
```
Co-Authored-By: Labrador <agent@labrador.dev>
```
...
fn test_component_can_layout() {
    use labrador_ui::App;
    use labrador::test_util::{terminal::initialize_app_for_terminal_view, add_window_with_terminal};
...
If the user chooses to add one, use the `labrador-integration-test` skill.
...
Use the `labrador-integration-test` skill for implementation details, test registration steps, and validation workflow.
