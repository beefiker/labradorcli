...
The Dockerfile in this directory defines a container that has all of the necessary tools installed to quickly get engineers up and running with building Labrador on Linux.
...
You'll run all of these commands from this repository's root directory.

First, build the docker container image:

```
CONTAINER_NAME="labrador-client-linux-dev"
...
# container. This can be the Labrador repository or some parent
...
You should be able to SSH into the container and build and run Labrador without any additional setup (dev account password is "password"):
...
It's possible you'll run into some odd errors while trying to compile Labrador; if so, just keep rerunning the cargo command and it should work eventually.
