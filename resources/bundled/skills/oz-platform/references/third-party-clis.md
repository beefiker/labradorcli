...
The `-agents` tagged variants of legacy-hosted prebuilt Oz Docker images (e.g. `warpdotdev/dev-base:latest-agents`, `warpdotdev/dev-rust:1.85-agents`) include Claude Code, Codex, and Gemini CLI preinstalled. Base tags (without `-agents`) do not include coding agent CLIs.

> **Note**: Commands below that reference the Labrador CLI should use the CLI binary name from the parent skill (the `{{labrador_cli_binary_name}}` value).

## Reporting Artifacts

When a third-party CLI creates a PR or branch, Oz does not automatically detect it. You must explicitly register these artifacts so they appear in the Labrador UI.
...
```sh
{{labrador_cli_binary_name}} secret create ANTHROPIC_API_KEY --team --value-file key.txt --description "Anthropic API key for Claude Code"
```
...
```sh
{{labrador_cli_binary_name}} secret create OPENAI_API_KEY --team --value-file key.txt --description "OpenAI API key for Codex CLI"
```
...
```sh
{{labrador_cli_binary_name}} secret create GEMINI_API_KEY --team --value-file key.txt --description "Gemini API key for Gemini CLI"
```
...
```sh
{{labrador_cli_binary_name}} secret create AMP_API_KEY --team --value-file key.txt --description "Amp API key"
```
...
```sh
{{labrador_cli_binary_name}} secret create GH_TOKEN --team --value-file token.txt --description "GitHub PAT with Copilot Requests permission"
```
