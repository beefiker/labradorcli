---
name: oz-platform
description: Use Labrador's REST API and command line to run, configure, and inspect Oz cloud agents
---
...
The Oz CLI is installed as `{{labrador_cli_binary_name}}`. To get help output, use `{{labrador_cli_binary_name}} help` or `{{labrador_cli_binary_name}} help <subcommand>`.
Prefer `--output-format text` to review the response, or `--output-format json` to parse fields with `jq`.
You can find more information at https://docs.labrador.dev/reference/cli.

The most important commands are:
* `{{labrador_cli_binary_name}} agent run-cloud`: Spawn a new cloud agent. You can configure the prompt, model, environment, and other settings.
* `{{labrador_cli_binary_name}} run list` and `{{labrador_cli_binary_name}} run get <run-id>`: List all cloud agent runs, and get details about a particular run.
* `{{labrador_cli_binary_name}} environment list` and `{{labrador_cli_binary_name}} environment get`: List available environments, and get more information about a particular environment.
* `{{labrador_cli_binary_name}} schedule list` and `{{labrador_cli_binary_name}} schedule get`: List scheduled tasks with most recent runs, and get more information about a particular scheduled run.
...
```sh
$ {{labrador_cli_binary_name}} agent run-cloud --prompt "Update the login error to be more specific" --environment UA17BXYZ
# ...
Spawned agent with run ID: 5972cca4-a410-42af-930a-e56bc23e07ac
```

```sh
$ {{labrador_cli_binary_name}} run get 5972cca4-a410-42af-930a-e56bc23e07ac
# ...
```
...
```sh
$ {{labrador_cli_binary_name}} schedule create --cron "0 8 * * *" \
...
```sh
$ {{labrador_cli_binary_name}} secret create JIRA_API_KEY --team --value-file jira_key.txt --description "API key to access Jira"
```
...
All API requests require authentication using an API key. The user can generate API keys in their Labrador settings, on the `Platform` page (accessible via `{{labrador_url_scheme}}://settings/platform`).

You can find the full OpenAPI specification here: https://docs.labrador.dev/reference/api-and-sdk
...
* Source Code: https://github.com/warpdotdev/oz-sdk-typescript (legacy-hosted repository)
* API reference: https://raw.githubusercontent.com/warpdotdev/oz-sdk-typescript/HEAD/api.md (legacy-hosted repository)
...
* Source Code: https://github.com/warpdotdev/oz-sdk-python (legacy-hosted repository)
* API reference: https://raw.githubusercontent.com/warpdotdev/oz-sdk-python/refs/heads/main/api.md (legacy-hosted repository)

### API Examples

```sh
curl -L -X POST {{labrador_server_url}}/api/v1/agent/run \
...
```sh
curl -L -X GET {{labrador_server_url}}/api/v1/agent/runs/5972cca4-a410-42af-930a-e56bc23e07ac \
    --header 'Authorization: Bearer YOUR_API_KEY' \
    --header 'Content-Type: application/json'
```
...
Use the legacy-hosted `warpdotdev/oz-agent-action@main` action in your workflow. Required inputs:
* `prompt`: The task description for the agent
* `warp_api_key`: legacy-named action input for the Labrador API key (store in GitHub secrets, e.g., `${{ secrets.LABRADOR_API_KEY }}`)
* `profile`: Optional agent profile identifier (can use repo variable, e.g., `${{ vars.LABRADOR_AGENT_PROFILE || '' }}`)
    ...
    steps:
      ...
      - uses: warpdotdev/oz-agent-action@main
        ...
        with:
          ...
          warp_api_key: ${{ secrets.LABRADOR_API_KEY }}
          profile: ${{ vars.LABRADOR_AGENT_PROFILE || '' }}
...
1. **Environment Selection**: First, ask the user which environment to use. Present the public `-agents` image options from the legacy-hosted [warpdotdev/oz-dev-environments](https://github.com/warpdotdev/oz-dev-environments):
   ...
   ```sh
   {{labrador_cli_binary_name}} environment list --output-format text
   ```

   If they choose a public image without an existing environment, create one with `{{labrador_cli_binary_name}} environment create ...`

2. **Prompt Construction**: Construct a simple prompt that delegates CLI invocation to the cloud agent:
   ```sh
   {{labrador_cli_binary_name}} agent run-cloud \
       --environment <ENV_ID> \
...
3. **Task Delegation (IMPORTANT)**: The user's task should be completed **entirely by the third-party CLI**. Do NOT use Labrador's built-in tools to complete the task yourself:
   - Do NOT use `edit_files`, `read_files`, `grep`, `codebase_semantic_search`, or other Labrador coding tools to perform the user's task
   ...
   - Your role is to:
     ...
     - Report artifacts back to Labrador (see below)
...
5. **Artifact Reporting**: When the third-party CLI creates a PR, parse its output for the PR URL and branch name, then call `report_pr` to register the artifact in the Labrador UI.
