...
description: Create new Labrador tab config TOML files from natural-language requests. Use when the user wants a new tab config, a new tab layout, or asks for a slash command to generate a tab config.
---

# create-tab-config

Create a new Labrador tab config based on what the user wants.
...
4. Determine the correct tab config directory for the user's Labrador build.
   Tab configs live under `~/.labrador/tab_configs/` for standard builds. Non-stable builds use a channel-specific variant (e.g. `~/.labrador-<channel>/tab_configs/`).
   To find the correct directory, run `ls -d ~/.labrador*/` to list the available Labrador data directories and pick the one that corresponds to the running build. When in doubt, ask the user which build they are using.
