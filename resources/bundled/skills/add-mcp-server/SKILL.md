---
name: add-mcp-server
description: Use this skill when helping users add MCP servers to their Labrador configuration.
---

# Adding MCP Servers to Labrador

Labrador supports MCP servers via native config files. Follow these steps when helping a user add an MCP server.
...
Config file paths:
- **Global (user-scoped):** `~/.labrador/.mcp.json`
- **Project-scoped:** `{repo_root}/.labrador/.mcp.json`
...
By default, Labrador spawns stdio servers from the directory the config was discovered in:
- Project-scoped configs (`{repo_root}/.labrador/.mcp.json`) run from the repo root.
- Global configs (`~/.labrador/.mcp.json`, `~/.claude.json`, etc.) run from the home directory.
...
For environment variables containing secrets, use `${VAR_NAME}` syntax — Labrador will substitute the value from the user's environment at runtime.

## Notes

- Labrador auto-detects changes to `.mcp.json` files on save — no restart required.
- Configured servers appear in Labrador's Settings under MCP, labeled **"Detected from Labrador"**.
