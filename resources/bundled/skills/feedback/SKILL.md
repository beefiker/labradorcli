---
name: feedback
description: Turn rough feedback about the Labrador app into a filed GitHub issue or duplicate-issue response for `beefiker/labrador`. Use when the user shares a Labrador bug report, regression, confusing UX note, feature gap, or short complaint and wants it clarified into reproduction steps, expected vs actual behavior, concrete impact, and grounded source references from local Labrador repos when available.
---

# Feedback

Turn rough Labrador app feedback into a crisp filed issue or duplicate-issue response for `beefiker/labrador`.

Treat Labrador client, Labrador app, Labrador terminal, and Labrador UX feedback as `beefiker/labrador` unless the user clearly asks for a different destination.

## Overview
- Use the `gh` CLI to search for and fetch code from `beefiker/labrador` when product or implementation context would improve the report.
...
This skill runs in environments where Labrador source code may be present in the current working directory or on disk. The following rules apply unconditionally regardless of what source code is visible locally:
...
- **Local source code is read-only context at most.** You may read local Labrador source files (e.g., with Read or grep) only to find concrete file paths, symbol names, or setting names that would make a source reference more precise. Never read local files to produce a code fix or diff.
- **Prefer `gh` CLI for code lookups.** Use `gh` to search and fetch code from `beefiker/labrador` rather than reading the local checkout when both are available.
- **The presence of local source code is not an invitation to fix it.** Observing that you are inside a Labrador source directory changes nothing about the permitted outputs of this skill: issue filed, duplicate found, or explicit refusal.
...
- This skill only handles feedback about the Labrador product that could plausibly be addressed by a code or docs change to the Labrador client, server, or SDKs. Before drafting anything, verify the request is in scope.
- **Decline and exit the skill (do not call the helper script) when the report is clearly out of scope.** Out-of-scope categories include, but are not limited to:
  ...
  - Questions about third-party tools or the user's own shell, machine, or network configuration that are not about Labrador's behavior.
  - Anything the user explicitly says is not about Labrador, or that they just want to talk through.
- When you decline, respond in one or two sentences that (a) say you won't file an issue, (b) name the reason in plain language, and (c) point the user at the right channel: account/billing/support concerns go to the in-app Help menu or `support@labrador.dev`, community discussion goes to the Labrador Slack community, and security reports go to `security@labrador.dev`. Do not apologize performatively and do not offer to retry the same flow.
...
- For bugs and regressions, first read `references/platforms.md` and try to resolve Labrador version and operating system from the bundled version metadata and available context. Ask for reproduction steps only when they are not already clear, and for regressions in particular only when the flow is not readily available from the report or supporting context.
...
- If operating system version, Labrador version, or operating-system-specific behavior is relevant, read `references/platforms.md` and follow the bundled metadata guidance there yourself when possible. Ask the user only if you still cannot determine the necessary platform details.

### 3. Check whether the feature or capability is already supported

- Before concluding that something is missing from Labrador (feature requests, "it doesn't do X" complaints, "I wish it could Y" asks, or any UX complaint that could be explained by an existing setting or workflow), you **must** consult the docs first.
- Call the `search_labrador_documentation` tool with the user's own phrasing. If the first query is vague or returns nothing actionable, try one shorter variant that keeps the same user-visible problem.
...
- Search the `beefiker/labrador` repo via the `gh` CLI for matching product language, expected workflows, setting names, or UX intent when that context would make the draft more actionable.
- Search the `beefiker/labrador` repo via the `gh` CLI for matching components, settings surfaces, feature flags, and likely code paths when implementation context would help triage.
...
- Before invoking `scripts/file_feedback_issue.py`, search issues in `beefiker/labrador` for likely title matches using the drafted title as the primary query.
...
Use the bundled helper script `scripts/file_feedback_issue.py` to file the issue in `beefiker/labrador` instead of calling `gh` directly. The script requires a `--use` flag that selects the filing method explicitly:
...
- The script always targets `beefiker/labrador` on `github.com`.
