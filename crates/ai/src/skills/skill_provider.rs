...
//!
//! This module defines the supported skill providers (i.e. Labrador, Agents, Claude, Codex) and their
...
use labrador_core::ui::color::CLAUDE_ORANGE;
use labrador_core::ui::icons::Icon;
use labrador_core::ui::theme::Fill;

/// Represents a skill provider/origin (Labrador, Agents, Claude, Codex, etc.).
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum SkillScope {
    ...
    Project,
    /// Bundled skills distributed with Labrador.
    Bundled,
}
...
impl SkillProvider {
    ...
    pub fn icon(&self) -> Icon {
        match self {
            ...
            SkillProvider::Labrador
            | SkillProvider::Agents
            | SkillProvider::Cursor
            | SkillProvider::Copilot
            | SkillProvider::Github => Icon::LabradorLogoLight,
        }
    }
...
pub static SKILL_PROVIDER_DEFINITIONS: LazyLock<Vec<SkillProviderDefinition>> =
    LazyLock::new(|| {
        vec![
            ...
            SkillProviderDefinition {
                provider: SkillProvider::Labrador,
                skills_path: PathBuf::from(".labrador").join("skills"),
            },
            SkillProviderDefinition {
...
pub fn home_skills_path(provider: SkillProvider) -> Option<PathBuf> {
    if provider == SkillProvider::Labrador {
        return labrador_core::paths::labrador_home_skills_dir();
    }
...
mod tests {
    ...
    #[test]
    fn labrador_home_skills_path_uses_labrador_home_path() {
        assert_eq!(
            home_skills_path(SkillProvider::Labrador),
            labrador_core::paths::labrador_home_skills_dir()
        );
    }

    #[test]
    fn labrador_home_skill_path_is_home_labrador_skill() {
        let Some(labrador_home_skills_dir) = labrador_core::paths::labrador_home_skills_dir() else {
            eprintln!("Skipping test: home directory not available");
            return;
        };
        let path = labrador_home_skills_dir.join("my-skill").join("SKILL.md");

        assert_eq!(get_provider_for_path(&path), Some(SkillProvider::Labrador));
