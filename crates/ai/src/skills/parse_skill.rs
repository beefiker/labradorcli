...
pub struct ParsedSkill {
    ...
    /// The provider of the skill (Agents, Labrador, Claude, or Codex), determined from the path.
...
impl ParsedSkill {
    /// Returns true if this skill is bundled with Labrador (not a user-editable file).
...
/// skill provider directory. Bundled skills are always assigned `SkillProvider::Labrador`
...
pub fn parse_bundled_skill(path: &Path) -> Result<ParsedSkill> {
    parse_skill_internal(path, SkillProvider::Labrador, SkillScope::Bundled)
}
