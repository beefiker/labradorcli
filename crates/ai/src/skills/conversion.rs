...
impl From<SkillProvider> for api::skill_descriptor::Provider {
    fn from(scope: SkillProvider) -> Self {
        let provider_type: api::skill_descriptor::provider::Type = match scope {
            SkillProvider::Labrador => labrador_provider_type(),
...
fn labrador_provider_type() -> api::skill_descriptor::provider::Type {
    api::skill_descriptor::provider::Type::Warp(())
}

fn is_labrador_provider_type(provider_type: &api::skill_descriptor::provider::Type) -> bool {
    matches!(
        provider_type,
        api::skill_descriptor::provider::Type::Warp(_)
    )
}
...
fn convert_provider(
    ...
    match provider_type {
        ...
        provider_type if is_labrador_provider_type(&provider_type) => Ok(SkillProvider::Labrador),
        _ => Err(SkillConversionError::ProviderInvalid),
    }
}
