...
cfg_if::cfg_if! {
    if #[cfg(feature = "local_fs")] {
        ...
        const RULES_FILE_PATTERN: [&str; 2] = ["LABRADOR.md", "AGENTS.md"];
...
struct RuleAtPath {
    parent_path: PathBuf,
    labrador_md: Option<ProjectRule>,
    agents_md: Option<ProjectRule>,
}

impl RuleAtPath {
    fn respected_rule(&self) -> Option<&ProjectRule> {
        self.labrador_md.as_ref().or(self.agents_md.as_ref())
    }
}
...
impl ProjectRules {
    ...
    fn remove_rule(&mut self, path: &Path) -> Option<ProjectRule> {
        ...
        if file_name.to_lowercase() == "labrador.md" {
            rule.labrador_md.take()
...
/// Currently supports LABRADOR.md files, but designed to be extensible
#[cfg_attr(not(feature = "local_fs"), allow(dead_code))]
#[derive(Debug, Default)]
...
impl ProjectContextModel {
                    ...
                    Ok(rule_files) => {
                        ...
                        let delta = RulesDelta {
                            discovered_rules: rule_files
                                ...
                                .iter()
                                .filter_map(|rule| {
                                    rule.labrador_md.as_ref().map(|rule| ProjectRulePath {
    ...
    /// Scan a directory for rule files (currently LABRADOR.md, extensible for future file types)
