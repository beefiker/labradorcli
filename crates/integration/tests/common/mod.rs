...
use labrador_ui::integration::RERUN_EXIT_CODE;
use std::env;
...
///
/// This runs the `integration` binary from the `labrador` crate, passing it the
    ...
    while keep_going {
        let inherited_envs = env::vars_os().filter(|(k, _v)| {
            ...
            k == "PATH"
                ...
                || k.starts_with("RUST_")
                // Propagate any Labrador-specific variables.
                || k.starts_with("LABRADOR_")
                || k.starts_with("LABRADOR_UI_")
