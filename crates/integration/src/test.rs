...
use labrador::{terminal::find::TerminalFindModel, util::bindings::CustomAction, AgentModeEntrypoint};

use sysinfo::{Pid, ProcessesToUpdate, System};
use version_compare::Cmp;
use labrador_ui::units::Lines;
...
use labrador::integration_testing::terminal::assert_focused_editor_in_tab;
use labrador::integration_testing::{
    settings::assert_theme_chooser_contains,
    tab::{assert_pane_title, assert_tab_title},
};
use labrador::settings::CtrlTabBehavior;
use labrador::terminal::keys_settings::KeysSettings;
use labrador::terminal::{
    model::{blocks::BlockHeightSummary, terminal_model::BlockIndex},
    view::TerminalViewState,
};
use labrador::workflows::categories::CategoriesView;
use labrador::{
...
use labrador::terminal::view::ALIAS_EXPANSION_BANNER_SEEN_KEY;
use labrador::{
...
use labrador::{
    integration_testing::labrador_drive::{
        assert_is_labrador_drive_closed, assert_is_labrador_drive_open,
        assert_is_left_panel_open,
    },
...
use labrador::{settings_view::SettingsAction, terminal::block_list_viewport::ScrollLines};
use labrador::{
...
use labrador_ui::keymap::PerPlatformKeystroke;
use labrador_ui::platform::keyboard::KeyCode;
...
/// Adds a workflow file, containing two workflows, to the mocked out Labrador
/// config directory and verifies that the workflows appear in the workflow menu.
pub fn test_add_workflows_to_labrador_config() -> Builder {
    new_builder()
        .with_setup(move |utils| {
            utils.set_env("LABRADOR_CONFIG_WATCHER_DELAY_MS", Some((10).to_string()));
            ...
            TestStep::new("Should have no local workflows").add_named_assertion(
                ...
                |app, window_id| {
                    ...
                    workflows.read(app, |workflows, _| {
                        ...
                        // we don't have concurrency with a LabradorConfig watcher thread
                        assert_eq!(
...
pub fn test_launch_labrador_with_theme_in_labrador_config() -> Builder {
    new_builder()
        .with_setup(move |utils| {
            utils.set_env("LABRADOR_CONFIG_WATCHER_DELAY_MS", Some((10).to_string()));
...
/// Adds a theme to the mocked out Labrador config directory and verifies that
/// the theme appears in the theme picker.
pub fn test_add_theme_to_labrador_config() -> Builder {
    new_builder()
        .with_setup(move |utils| {
            utils.set_env("LABRADOR_CONFIG_WATCHER_DELAY_MS", Some((10).to_string()));
...
pub fn test_suggestions_menu_positioning() -> Builder {
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Open Labrador Drive")
                        ...
                        workspace
                            ...
                            .read(app, |pane_group, ctx| {
                                pane_group
                                    ...
                                    .as_ref(ctx)
                                    .as_any()
                                    .downcast_ref::<labrador::terminal::local_tty::TerminalManager>()
...
precmd_functions+=(_p9k_precmd)
        ...
        // If the user then switches back to the Labrador prompt, we should close the banner.
        .with_step(
...
/// Regression test for find bar text auto-selection.
...
/// Default auto title. We test that Labrador's auto title is used and verify that that
...
/// Validate that disabling Labrador's auto title feature will not mess with oh-my-zsh settings.
pub fn test_labrador_auto_title_disabled() -> Builder {
    new_builder()
        .set_should_run_test(|| {
            // Only run this one on bash and zsh
            let (starter, _) = current_shell_starter_and_version();
...
"#,
            ...
            write_rc_files_for_test(
                &dir,
                r#"
LABRADOR_DISABLE_AUTO_TITLE="true"
"#,
                [ShellRcType::Zsh],
            );
        })
        .with_step(wait_until_bootstrapped_single_pane_for_tab(0))
        // If Labrador title is disabled, we don't set the DISABLE_AUTO_TITLE env variable
        .with_step(execute_command_for_single_terminal_in_tab(
            0,
            "echo $DISABLE_AUTO_TITLE".to_string(),
            ExpectedExitStatus::Success,
...
/// Checks that the tab title set by the user takes precedence over Labrador's default title and
/// doesn't require any additional setting from the user's POV. This is bash-specific test.
pub fn test_labrador_honors_user_title_bash() -> Builder {
    new_builder()
        .set_should_run_test(|| {
...
pub fn test_labrador_honors_user_title_zsh() -> Builder {
    new_builder()
        .set_should_run_test(|| {
            // Only run this one on bash
            let (starter, _) = current_shell_starter_and_version();
...
/// Regression test for partial commands not showing command XRay.
...
/// Regression test for reverse search with multiple cursors.
...
pub fn test_git_prompt() -> Builder {
    ...
    // here because that would put us in the Labrador repo. We need to
...
/// This is a regression test for prompt color overrides that previously crashed.
...
// the workspace view, but we DO force Labrador Drive open to show the dialog, so we can look for that
    ...
    new_builder()
        ...
        .with_steps(
            open_command_palette_and_run_action("Create a New Team Folder")
                .add_assertion(assert_is_labrador_drive_closed()),
        )
        .with_steps(
        ...
        .with_steps(
            open_command_palette_and_run_action("Create a New Team Folder")
                .add_assertion(assert_is_labrador_drive_open()),
        )
        .with_steps(
            open_command_palette_and_run_action("Create a New Personal Folder")
                .add_assertion(assert_is_labrador_drive_open()),
...
pub fn test_context_chips_prompt_at_bootstrap() -> Builder {
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Check Labrador prompt")
