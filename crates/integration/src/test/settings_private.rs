...
use labrador_ui::{async_assert, async_assert_eq, integration::TestStep, SingletonEntity};
...
fn read_toml_file() -> String {
    let path = labrador::settings::user_preferences_toml_file_path();
...
fn read_json_prefs_file() -> String {
    let path = labrador::settings::user_preferences_file_path();
    std::fs::read_to_string(path).unwrap_or_default()
}
        ...
        .with_step(
            new_step_with_default_assertions("Verify second pair routing")
                .add_named_assertion("CodeAsDefaultEditor in TOML", |_, _| {
                    let toml = read_toml_file();
                    async_assert!(
                        toml.contains("use_labrador_as_default_editor"),
