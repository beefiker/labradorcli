...
use labrador_ui::event::{Event, ModifiersState};
use labrador_ui::integration::{artifacts_root_dir, TestStep};

use crate::Builder;
use labrador::integration_testing::step::new_step_with_default_assertions;
use labrador::integration_testing::terminal::util::ExpectedExitStatus;
    ...
    Builder::new()
        ...
        .with_on_finish(
            move |_app, _window_id, _persisted_data| -> Pin<Box<dyn Future<Output = ()> + Send>> {
                Box::pin(async move {
                    let artifacts_root = artifacts_root_dir();
