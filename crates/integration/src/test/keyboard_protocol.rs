...
use labrador::features::FeatureFlag;
...
use labrador_ui::event::{KeyEventDetails, KeyState};
use labrador_ui::keymap::Keystroke;
use labrador_ui::platform::keyboard::KeyCode;
use labrador_ui::{async_assert, integration::TestStep, Event};
...
) -> impl FnMut(&mut labrador_ui::App, labrador_ui::WindowId) -> labrador_ui::integration::AssertionOutcome {
    move |app, window_id| {
        let terminal_view = single_terminal_view_for_tab(app, window_id, 0);
        terminal_view.read(app, |view, _ctx| {
            let model = view.model.lock();
