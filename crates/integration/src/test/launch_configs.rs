...
use labrador::search::command_palette::launch_config;
use labrador::workspace::NEW_TAB_BUTTON_POSITION_ID;
use labrador::{features::FeatureFlag, integration_testing::settings::set_window_custom_size};
use labrador::{
...
/// Adds a launch config to the mocked out Labrador config directory and verifies that
/// the launch config appears in the launch config palette.
pub fn test_add_launch_config_to_labrador_config() -> Builder {
    new_builder()
            ...
            new_step_with_default_assertions("Opening a configuration template").with_action(
                move |app, _, _| {
                    app.dispatch_global_action(
                        ...
                        labrador::root_view::OpenLaunchConfigArg {
                            launch_config:
                                labrador::launch_configs::launch_config::make_mock_single_window_launch_config(),
...
pub fn test_launch_config_single_child_branch() -> Builder {
    use labrador::launch_configs::launch_config::{
