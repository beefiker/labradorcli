...
use labrador_ui::async_assert;

use super::new_builder;

/// Checks if the Ask Labrador AI keybinding works correctly when a block is selected.
/// This is a regression test for the Ask Labrador AI keybinding.
pub fn test_ask_labrador_ai_keybinding_for_selected_block() -> Builder {
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("select block")
                ...
                .add_named_assertion("ask Labrador AI from selected block", |app, window_id| {
                    let ai_assistant_panel = ai_assistant_panel_view(app, window_id);
                    ai_assistant_panel.read(app, |view, ctx| {
                        let expected_code_block = "```labrador\nfoo\n```";
