...
use labrador_ui::integration::{self, PersistedDataMap, TestStep};
use labrador_ui::integration::{TestDriver, TestSetupUtils};
use labrador_ui::{App, WindowId};
use labrador_ui_extras::user_preferences::file_backed::FileBackedUserPreferences;
...
impl Builder {
    pub fn new() -> Self {
        ...
        if std::env::var("LABRADOR_UI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS").is_ok() {
    ...
    pub fn build(self, test_name: &str, create_temp_dir_for_test: bool) -> TestDriver {
        ...
        let inner = inner.with_setup(move |utils| {
            ...
            // filesystem
            utils.set_env(labrador::keyboard::DISABLE_SAVE_ENV_VAR, Some("true"));
        ...
        let file_path = labrador::settings::user_preferences_file_path();
