...
use labrador_cli::WorkerCommand;
use labrador_core::channel::{Channel, ChannelConfig, ChannelState, OzConfig, LabradorServerConfig};
use labrador_core::AppId;

/// The Labrador integration test runner.
#[derive(Debug, Default, Parser, Clone)]
#[command(name = "labrador-integration-test")]
...
pub fn main() -> Result<()> {
    ChannelState::set(ChannelState::new(
        ...
        ChannelConfig {
            ...
            logfile_name: "labrador_integration.log".into(),
            server_config: LabradorServerConfig {
    ...
    if let Some(command) = &args.command {
        match command {
            ...
            WorkerCommand::TerminalServer(args) => {
                ...
                labrador::terminal::local_tty::server::run_terminal_server(args);
                return Ok(());
            }
    ...
    labrador::run_integration_test(driver)
}
...
fn register_tests() -> HashMap<&'static str, BoxedBuilderFn> {
    ...
    register_test!(test_labrador_auto_title_disabled);
    register_test!(test_labrador_honors_user_title_bash);
    register_test!(test_labrador_honors_user_title_zsh);
    ...
    register_test!(test_add_theme_to_labrador_config);
    register_test!(test_palette_opens_when_theme_chooser_is_open);
    #[cfg(target_os = "macos")]
    register_test!(test_preview_config_dir_migration);
    register_test!(test_launch_labrador_with_theme_in_labrador_config);
    register_test!(test_add_launch_config_to_labrador_config);
    register_test!(test_add_workflows_to_labrador_config);
    ...
    register_test!(test_ask_labrador_ai_keybinding_for_selected_block);
    ...
    register_test!(test_open_in_labrador_banner);
    ...
    // File tree tests
    register_test!(test_file_tree_opens_files_in_labrador);
