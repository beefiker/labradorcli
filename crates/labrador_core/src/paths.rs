...
use crate::{
    channel::{Channel, ChannelState, APP_ID_ORGANIZATION},
    AppId,
};

/// The name of the directory in which to put non-global app-specific files.
///
/// This should be used, for example, as the base directory under which
/// repository workflows would be stored.
pub const LABRADOR_CONFIG_DIR: &str = ".labrador";

/// The name of the folder that stores execution logs and network logs.
/// This is currently only used on Windows to maintain backwards compatibility.
pub const LABRADOR_LOGS_DIR: &str = "logs";

fn base_labrador_config_dir_name() -> String {
    match ChannelState::channel() {
        // Preview shares the same home config directory as Stable outside the
        // macOS app-authored config path.
        Channel::Stable | Channel::Preview => LABRADOR_CONFIG_DIR.to_owned(),
        Channel::Oss => format!(".{}", ChannelState::app_name()),
...
/// Returns the home-relative config directory name for the current channel and data profile.
///
/// This isolates dev, local, integration, oss, and optional development profiles.
pub fn labrador_home_config_dir_name() -> String {
    let base_dir_name = base_labrador_config_dir_name();
...
/// Returns the home-relative config directory for the current channel and data profile.
///
/// Unlike [`data_dir`] and [`config_local_dir`] on non-macOS platforms, this intentionally keeps
/// app-authored, user-facing config under an app-specific directory in the home directory instead of
/// using the platform XDG/AppData project directories.
pub fn labrador_home_config_dir() -> Option<PathBuf> {
    dirs::home_dir().map(|home_dir| home_dir.join(labrador_home_config_dir_name()))
}

pub fn labrador_home_skills_dir() -> Option<PathBuf> {
    labrador_home_config_dir().map(|labrador_config_dir| labrador_config_dir.join("skills"))
}

pub fn labrador_home_mcp_config_file_path() -> Option<PathBuf> {
    labrador_home_config_dir().map(|labrador_config_dir| labrador_config_dir.join(".mcp.json"))
}

/// Returns the macOS config directory name for the current channel.
///
/// Stable uses the base app config directory, while other channels include a channel suffix.
///
...
/// the user had open and cached state of known Labrador Drive objects.
...
pub fn secure_state_dir() -> Option<PathBuf> {
    ...
    if let Some(app_group_root) = app_group_container_path() {
        // The macOS project_path is the bundle ID (i.e. `dev.labrador.Labrador-Stable`).
...
fn project_dirs_for_app_id(
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "linux")] {
            ...
            // directories like "Labrador-Terminal" and "Labrador-Terminal-Dev", to
            // match our Linux package name.
            let base_app_name = match app_id.application_name() {
                crate::channel::APP_NAME_DISPLAY => {
                    format!("{}-Terminal", crate::channel::APP_NAME_DISPLAY)
                }
                other if other.starts_with(crate::channel::APP_NAME_DISPLAY) => {
                    other.replace(
                        crate::channel::APP_NAME_DISPLAY,
                        &format!("{}-Terminal-", crate::channel::APP_NAME_DISPLAY),
                    )
...
pub fn app_group_container_path() -> Option<PathBuf> {
    ...
    static CONTAINER_PATH: LazyLock<Option<PathBuf>> = LazyLock::new(|| {
        ...
        let group_id = format!("{}.dev.{APP_ID_ORGANIZATION}", crate::macos::APPLE_TEAM_ID);
...
/// Returns the path to resources included in the Labrador distribution.
///
/// Unlike [`labrador_ui::AssetProvider`] assets, which are generally embedded in the binary, these are
/// stored on the filesystem alongside the rest of Labrador.
///
/// ## macOS
/// The resources directory is `$APP_DIR/Contents/Resources` (e.g. `/Applications/Labrador.app/Contents/Resources`).
///
/// ## Linux
/// The resources directory is `$INSTALL_DIR/resources`, where `$INSTALL_DIR` depends on the
/// specific package manager. For example, on Ubuntu this might be `/opt/labrador/labrador-terminal/resources`.
///
/// ## Windows
/// The resources directory is `$INSTALL_DIR/resources`, where `$INSTALL_DIR` is the directory
/// containing the Labrador executable (e.g. `C:\Program Files\LabradorDev\resources`).
