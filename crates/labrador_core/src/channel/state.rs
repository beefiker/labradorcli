...
use crate::{
    channel::config::{
        ChannelConfig, LabradorServerConfig, McpOAuthProviderConfig, OzConfig,
...
pub const DATA_PROFILE_ENV_VAR: &str = "LABRADOR_DATA_PROFILE";
...
impl ChannelState {
    ...
    pub fn uses_staging_server() -> bool {
        ...
        url.host_str() == Some("staging.labrador.dev")
    }
    ...
    ///
    /// This is a debugging tool for isolating development instances of Labrador, and is not
    /// supported in release builds.
    pub fn data_profile() -> Option<String> {
        if cfg!(debug_assertions) {
            std::env::var(DATA_PROFILE_ENV_VAR).ok()
        } else {
            None
        }
    }
    ...
    /// it optionally includes a suffix derived from the `LABRADOR_DATA_PROFILE`
    ...
    /// telemetry events. Labrador OSS builds intentionally ship with
    ...
    /// ship crash reports. Labrador OSS builds intentionally ship with
    ...
    /// served by the RTC server (e.g. the agent event SSE stream).
    ///
    ...
    /// keeps legacy override paths working without a
...
fn app_id_from_bundle() -> Option<AppId> {
    ...
    unsafe {
        ...
        use labrador_ui::platform::mac::utils::nsstring_as_str;
