...
use super::theme::{Fill, LabradorTheme};
...
use labrador_ui::geometry::vector::{vec2f, Vector2F};
use labrador_ui::platform::Cursor;
use labrador_ui::ui_components::components::UiComponent;
use labrador_ui::ui_components::keyboard_shortcut::KeyboardShortcut;
use labrador_ui::ui_components::link::{LinkStyles, OnClickFn};
use labrador_ui::ui_components::list::{List, ListStyle};
use labrador_ui::ui_components::radio_buttons::{
    RadioButtonItem, RadioButtonLayout, RadioButtonStateHandle, RadioButtons,
};
use labrador_ui::ui_components::slider::{Slider, SliderStateHandle};
use labrador_ui::ui_components::switch::{Switch, SwitchStateHandle, TRACK_COLOR};
use labrador_ui::ui_components::text::WrappableText;
use labrador_ui::ui_components::toggle_menu::{
    ToggleMenu, ToggleMenuCallback, ToggleMenuItem, ToggleMenuStateHandle,
};
use labrador_ui::ui_components::tool_tip::{Tooltip, TooltipWithSublabel};
use labrador_ui::View;
use labrador_ui::{
...
impl UiBuilder {
    ...
    fn default_text_input_styles(&self) -> UiComponentStyles {
        UiComponentStyles {
            ...
            border_width: Some(1.),
            border_radius: Some(CornerRadius::with_all(Radius::Pixels(4.))),
            background: Some(self.labrador_theme.background().into()),
            border_color: Some(self.labrador_theme.foreground().with_opacity(20).into()),
            font_color: None,
            ..Default::default()
        }
    }
    ...
    fn default_progress_bar_styles(&self) -> UiComponentStyles {
        UiComponentStyles {
            background: Some(self.labrador_theme.background().into()),
            foreground: Some(self.labrador_theme.accent().into()),
            width: Some(70.),
            height: Some(2.),
            ..Default::default()
        }
    ...
    pub fn default_tool_tip_styles(&self) -> UiComponentStyles {
        UiComponentStyles {
            background: Some(self.labrador_theme().tooltip_background().into()),
            font_color: Some(self.labrador_theme.background().into_solid()),
            padding: Some(Coords {
                top: 4.,
                bottom: 4.,
                left: 8.,
            ...
            border_radius: Some(CornerRadius::with_all(Radius::Pixels(4.))),
            border_width: Some(1.),
            border_color: Some(self.labrador_theme.outline().into()),
            font_family_id: Some(self.ui_font_family),
            font_size: Some(10.),
            ..Default::default()
        }
    ...
    fn default_button_styles(&self, variant: ButtonVariant) -> UiComponentStyles {
        let details = self.labrador_theme.details();
        let (background, border) = match variant {
            ButtonVariant::Accent => (
                self.labrador_theme.accent().blend(
                    &self
                        .labrador_theme
                        .foreground()
                        .with_opacity(*details.accent_button_opacity()),
                ),
                self.labrador_theme.foreground().with_opacity(0),
            ),
            ButtonVariant::Basic => (
                self.labrador_theme.background().blend(
                    &self
                        .labrador_theme
                        .foreground()
            ...
            ButtonVariant::Secondary => {
                return self.base_styles(
                    None,
                    self.labrador_theme.outline(),
                    self.labrador_theme
                        .main_text_color(self.labrador_theme.background()),
                )
            }
            ButtonVariant::Warn => (Fill::warn(), self.labrador_theme.foreground().with_opacity(0)),
            ButtonVariant::Error => (Fill::error(), self.labrador_theme.foreground().with_opacity(0)),
            ButtonVariant::Outlined => {
                return self.base_styles(
                    None,
                    self.labrador_theme.foreground().with_opacity(20),
                    self.labrador_theme.sub_text_color(self.labrador_theme.background()),
                );
            }
            ButtonVariant::Text => {
            ...
            ButtonVariant::Link => {
                return self.text_button_styles(self.labrador_theme.accent());
            }
        };
        let font_color = match variant {
            ButtonVariant::Outlined => self.labrador_theme.sub_text_color(background),
            ButtonVariant::Text => unreachable!(),
            _ => self.labrador_theme.main_text_color(background),
        };
    ...
    fn disabled_button_styles(&self, variant: ButtonVariant) -> UiComponentStyles {
        let background: Fill = self.labrador_theme().surface_3();
        let font_color = self.labrador_theme().disabled_text_color(background);
        match variant {
    ...
    fn hovered_button_styles(&self, variant: ButtonVariant) -> UiComponentStyles {
        ...
        let (background, border) = match variant {
            ButtonVariant::Accent => (
                self.labrador_theme
                    .accent()
                    .blend(&self.labrador_theme.foreground().with_opacity(
            ...
            ButtonVariant::Secondary => (self.labrador_theme.surface_3(), self.labrador_theme.outline()),
            ButtonVariant::Warn => (
    ...
    fn clicked_button_styles(&self, variant: ButtonVariant) -> UiComponentStyles {
        ...
        self.base_styles(
            ...
            self.labrador_theme.main_text_color(background),
        )
    }
    ...
    pub fn default_keyboard_shortcut_styles(&self) -> UiComponentStyles {
        UiComponentStyles {
            ...
            font_color: Some(
                self.labrador_theme
                    .hint_text_color(self.labrador_theme.background())
                    .into_solid(),
            ),
            border_radius: Some(CornerRadius::with_all(Radius::Pixels(4.))),
            background: Some(internal_colors::fg_overlay_1(self.labrador_theme()).into()),
    ...
    pub fn link_styles(&self) -> LinkStyles {
        let font_color: ColorU = self
            .labrador_theme
            .accent()
            .on_background(self.labrador_theme.surface_2(), MinimumAllowedContrast::Text)
            .into();
        ...
        let hover_link_style = UiComponentStyles {
            border_color: Some(self.labrador_theme.accent().into()),
            ..base_link_style
        };
        LinkStyles {
    ...
    pub fn tooltip_link_styles(&self) -> LinkStyles {
        let tooltip_background = self.labrador_theme().tooltip_background();
        let font_color: ColorU = self
            .labrador_theme
            .accent()
            .on_background(
    ...
    pub fn switch(&self, mouse_state: SwitchStateHandle) -> Switch {
        ...
        Switch::new(
            ...
            switch_margin_styles.merge(self.base_styles(
                ...
                Fill::white(),
                self.labrador_theme.main_text_color(self.labrador_theme.surface_2()),
            )),
            None,
            Some(switch_margin_styles.merge(self.base_styles(
                Some(self.labrador_theme.accent()),
                Fill::white(),
                self.labrador_theme.main_text_color(self.labrador_theme.surface_2()),
            ))),
            Some(switch_margin_styles.merge(self.base_styles(
                Some(self.labrador_theme.disabled_ui_text_color()),
                Fill::white(),
                self.labrador_theme.main_text_color(self.labrador_theme.surface_2()),
            ))),
        )
    ...
    pub fn checkbox(&self, mouse_state: MouseStateHandle, size: Option<f32>) -> Checkbox {
        ...
        Checkbox::new(
            ...
            self.base_styles(
                None,
                self.labrador_theme.outline(),
                self.labrador_theme.nonactive_ui_text_color(),
            )
            ...
            Some(
                self.base_styles(
                    Some(self.labrador_theme.accent()),
                    self.labrador_theme.outline(),
                    self.labrador_theme.font_color(self.labrador_theme.accent()),
                )
    ...
    pub fn radio_buttons<'a>(
        ...
        RadioButtons::new(
            ...
            self.base_styles(
                None,
                self.labrador_theme.nonactive_ui_text_color(),
                self.labrador_theme.active_ui_text_color(),
            )
            ...
            self.base_styles(
                None,
                self.labrador_theme.disabled_ui_text_color(),
                self.labrador_theme.disabled_ui_text_color(),
            )
            .set_font_size(font_size),
            layout,
    ...
    pub fn toggle_menu(
        ...
        ToggleMenu::new(
            ...
            self.toggle_menu_styles(
                default_background.unwrap_or(self.labrador_theme.surface_1()),
                self.labrador_theme.active_ui_text_color(),
            )
            .set_font_size(font_size),
            self.toggle_menu_styles(
                selected_background.unwrap_or(self.labrador_theme.surface_3()),
                self.labrador_theme.active_ui_text_color(),
            ),
            self.toggle_menu_styles(
                hover_background.unwrap_or(self.labrador_theme.surface_2()),
                self.labrador_theme.active_ui_text_color(),
            ),
            on_toggle_change,
        )
    ...
    pub fn span(&self, text: impl Into<Cow<'static, str>>) -> Span {
        Span::new(
            text,
            self.base_styles(
                Some(self.labrador_theme.surface_2()),
                self.labrador_theme.outline(),
                self.labrador_theme.main_text_color(self.labrador_theme.surface_2()),
            ),
        )
    ...
    pub fn animated_button(
        ...
        let color = options.color.unwrap_or_else(|| {
            self.labrador_theme
                .foreground()
                .on_background(self.labrador_theme.surface_2(), MinimumAllowedContrast::NonText)
        });
        ...
        let hovered_styles = base_styles.merge(UiComponentStyles {
            background: Some(internal_colors::fg_overlay_2(&self.labrador_theme).into()),
            ..Default::default()
        });
        ...
        let disabled_button_styles = base_styles.merge(UiComponentStyles {
            font_color: Some(
                self.labrador_theme()
                    .disabled_text_color(internal_colors::neutral_3(&self.labrador_theme).into())
                    .into(),
            ),
            ..Default::default()
        });
    ...
    pub fn copy_button(&self, size: f32, mouse_state: MouseStateHandle) -> Button {
        let color = self.labrador_theme.main_text_color(self.labrador_theme.surface_2());
    ...
    fn icon_with_tooltip(
        ...
        let icon = Container::new(
            ConstrainedBox::new(
                Icon::new(icon_path, self.labrador_theme.active_ui_text_color()).finish(),
            )
    ...
    pub fn keyboard_shortcut_button(
        ...
        let default_styles = UiComponentStyles {
            border_radius: Some(CornerRadius::with_all(Radius::Pixels(4.))),
            border_color: Some(self.labrador_theme.surface_3().into()),
            border_width: Some(1.),
            padding: Some(Coords {
                top: 8.,
                bottom: 8.,
        ...
        let hovered_styles = UiComponentStyles {
            background: Some(self.labrador_theme().surface_3().into()),
            border_color: Some(self.labrador_theme().accent().into()),
            ..default_styles
        };

        let text = Text::new_inline(text, self.ui_font_family(), self.ui_font_size() + 2.)
            .with_color(text_main(&self.labrador_theme, self.labrador_theme.background()))
        ...
        let keystroke_styles = self
            ...
            .set_font_color(text_main(&self.labrador_theme, self.labrador_theme.background()));
    ...
    pub fn labrador_theme(&self) -> &LabradorTheme {
        &self.labrador_theme
    }
