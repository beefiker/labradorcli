...
#[test]
fn test_labrador_home_config_dir_path() {
    let home_dir = home_dir().expect("Should be able to compute home directory");
...
#[test]
fn test_labrador_home_skills_and_mcp_paths() {
    let Some(config_dir) = labrador_home_config_dir() else {
        panic!("Should be able to compute Labrador home config directory");
    };

    assert_eq!(labrador_home_skills_dir(), Some(config_dir.join("skills")));
    assert_eq!(
