...
/// in the Labrador ecosystem. It enables events to be defined in any crate while maintaining
...
/// table in Labrador's documentation.
///
...
/// No-op: was previously an asynchronous Rudderstack `track` send from a
/// `ViewContext`/`ModelContext`. labrador does not phone home to any analytics
/// backend, so the arguments are parsed by rustc but never evaluated.
#[macro_export]
macro_rules! send_telemetry_from_ctx {
    ($event:expr, $ctx:expr) => {{}};
}

/// No-op: was previously an asynchronous Rudderstack `track` send from an
/// `AppContext`. See [`send_telemetry_from_ctx`].
#[macro_export]
