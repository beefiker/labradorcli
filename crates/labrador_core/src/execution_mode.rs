...
/// Execution mode that Labrador is running under.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ExecutionMode {
    /// Labrador is running as a normal desktop app.
    App,
    /// Labrador is running as a CLI.
    Sdk,
}
...
impl ExecutionMode {
    ...
    pub fn client_id(&self) -> &'static str {
        match self {
            ExecutionMode::App => "labrador-app",
            ExecutionMode::Sdk => "labrador-cli",
        }
    }
}

/// Model tracking the mode that Labrador is running in.
///
/// This gates functionality that's disabled when Labrador is running in SDK mode.
#[derive(Clone, Debug)]
...
impl AppExecutionMode {
    ...
    /// If true, Labrador is running in a sandbox like a Docker container or VM, rather than directly
    /// on a user machine.
...
/// Returns the current global client ID string ("labrador-app" or "labrador-cli").
