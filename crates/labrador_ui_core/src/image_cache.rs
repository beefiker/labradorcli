...
const CUSTOM_IMAGE_HEADER_PREFIX: &[u8] = b"labrador-img:";
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::MissingHeaderIdentifier => {
                write!(f, "Image did not contain the 'labrador-img:' prefix.")
            }
        }
    }
}
...
impl CustomImageHeader {
    ...
    fn try_from_bytes(data: &[u8]) -> Result<(CustomImageHeader, &[u8]), CustomHeaderParsingError> {
        let data = match data.strip_prefix(CUSTOM_IMAGE_HEADER_PREFIX) {
            Some(data) => data,
...
impl Asset for ImageType {
    fn try_from_bytes(data: &[u8]) -> anyhow::Result<ImageType> {
        ...
        if data.starts_with(CUSTOM_IMAGE_HEADER_PREFIX) {
            let (custom_labrador_header, data) = match CustomImageHeader::try_from_bytes(data) {
                Ok((custom_labrador_header, data)) => (custom_labrador_header, data),
                Err(err) => return Err(anyhow!(err.to_string())),
            };

            let data = data.into();
            let Some(img) = (match custom_labrador_header.image_format {
                CustomImageFormat::Rgb => {
                    let dynamic_image = ImageBuffer::from_raw(
                        custom_labrador_header.width,
                        custom_labrador_header.height,
                        data,
                    )
                ...
                return Err(anyhow!(
                    "Could not convert custom app image into approprate dynamic image."
                ));
            };
