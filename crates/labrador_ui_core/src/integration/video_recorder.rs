...
pub const VIDEO_ENABLED_ENV_VAR: &str = "LABRADOR_INTEGRATION_TEST_VIDEO";

/// Environment variable that sets the output directory for video recordings
/// and screenshots. Defaults to `$TMPDIR/labrador_integration_video_captures` when
/// unset.
pub const VIDEO_DIR_ENV_VAR: &str = "LABRADOR_INTEGRATION_TEST_VIDEO_DIR";

pub(super) fn video_enabled_env_value() -> Option<String> {
    super::env_var(VIDEO_ENABLED_ENV_VAR)
}
...
pub fn output_dir() -> PathBuf {
    let dir = super::env_var(VIDEO_DIR_ENV_VAR)
        .map(PathBuf::from)
        .unwrap_or_else(|| std::env::temp_dir().join("labrador_integration_video_captures"));
