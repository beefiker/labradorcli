...
///
/// Checks the `LABRADOR_INTEGRATION_TEST_VIDEO` environment variable, falling
/// back to the legacy video-recording env var name:
...
/// ```sh
/// # Record all tests
/// LABRADOR_INTEGRATION_TEST_VIDEO=1 cargo nextest run ...
///
/// # Record only specific tests
/// LABRADOR_INTEGRATION_TEST_VIDEO=test_foo,test_bar cargo nextest run ...
/// ```
fn video_recording_enabled_for_test(test_name: &str) -> bool {
    let Some(value) = video_recorder::video_enabled_env_value() else {
        return false;
    };
