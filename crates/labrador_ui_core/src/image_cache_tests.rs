    ...
    fn get(&self, path: &str) -> Result<Cow<'_, [u8]>> {
        match path {
            ...
            "numbers-1000ms.gif" => Ok(Cow::Borrowed(include_bytes!(
                "../../labrador_ui/examples/assets/numbers-1000ms.gif"
            ))),
...
#[test]
fn custom_image_header_uses_labrador_prefix() {
    let data = vec![0, 0, 0, 255];
    let data = CustomImageHeader::prepend_custom_header(data, 1, 1, CustomImageFormat::Rgba)
        .expect("valid raw image data");

    assert!(data.starts_with(CUSTOM_IMAGE_HEADER_PREFIX));
}
...
fn test_svg_text_rasterizes_with_loaded_system_fonts() {
  ...
  <text x="10" y="24" font-size="20" fill="#000000">Labrador</text>
</svg>
"##,
    )
  ...
  <text x=\"10\" y=\"24\" font-family=\"{font_family}\" font-size=\"20\" fill=\"#000000\">Labrador</text>\
</svg>"
    );
