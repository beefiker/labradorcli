...
//!
//! ## How does a11y work in Labrador?
//! Because Labrador uses its own rust UI framework (labrador_ui), we don’t benefit from the built-in
...
//!
//! Labrador UI framework right now provides 3 ways of announcing what’s happening in the app:
...
pub struct AccessibilityContent {
    ...
    /// Frame support is a work-in-progress in Labrador and right now this field is omitted and not set.
    pub frame: Option<RectF>,
    /// The role a given element has. Note that we use our own, Labrador UI-defined roles (vs those that
...
impl AccessibilityContent {
    ...
    pub fn new_without_help<T>(value: T, role: LabradorA11yRole) -> Self
    where
        T: Into<String>,
    {
    ...
    pub fn new<V, H>(value: V, help: H, role: LabradorA11yRole) -> Self
    where
        V: Into<String>,
        H: Into<String>,
    {
    ...
    fn new_internal<V, H>(value: V, help: Option<H>, role: LabradorA11yRole) -> Self
    where
        V: Into<String>,
        H: Into<String>,
    {
...
impl std::fmt::Display for LabradorA11yRole {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        use LabradorA11yRole::*;
        let word = match self {
            ButtonRole => "Button",
...
impl ActionAccessibilityContent {
    pub fn from_debug() -> Self {
        Self::CustomFn(|action| {
            AccessibilityContent::new_without_help(format!("{action:?}."), LabradorA11yRole::UserAction)
        })
    }
}
