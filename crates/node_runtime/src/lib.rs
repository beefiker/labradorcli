//! Node.js and npm runtime management for Labrador.
//!
...
pub fn node_installation_dir() -> Result<PathBuf> {
    ...
    Ok(labrador_core::paths::data_dir().join("node").join(folder_name))
}
...
/// 3. Extract it to the Labrador data directory
///
/// # Returns
...
pub async fn install_npm(client: &http_client::Client) -> Result<PathBuf> {
    ...
    let node_containing_dir = labrador_core::paths::data_dir().join("node");
...
/// 1. First, our custom Node.js installation in the Labrador data directory
...
pub async fn detect_system_node(path_env_var: impl AsRef<OsStr>) -> Result<()> {
    ...
    // via Labrador's inherited env rather than the captured interactive PATH.
    #[cfg(windows)]
