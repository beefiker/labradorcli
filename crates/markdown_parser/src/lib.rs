...
/// This purposefully shadows the `Action` trait from `labrador_ui`.
///
/// Since `labrador_ui` depends on this crate, we can't depend on the `labrador_ui::Action` trait directly.
/// Instead, we create a new trait with a blanket implementation that implicitly results
/// in any `labrador_ui::Action` implementing this `Action`.
...
impl FormattedTable {
    /// Parse from the internal tab-separated format used in `labrador-markdown-table` code blocks.
    ...
    /// Serialize to the internal tab-separated format used in `labrador-markdown-table` code blocks.
