...
use crate::{
    compute_formatted_text_delta, CustomWeight, FormattedTable, FormattedTextStyles, LineCount,
};
...
fn test_parse_link_in_tag() {
    ...
    let source = "a [https://google.com](https://labrador.dev) link";
    assert_eq!(
        test_parse_markdown(source),
        vec![FormattedTextLine::Line(vec![
            FormattedTextFragment::plain_text("a "),
            FormattedTextFragment::hyperlink("https://google.com", "https://labrador.dev"),
            FormattedTextFragment::plain_text(" link")
        ])]
    );
}
...
fn test_parse_escape_in_style() {
    let source = "Some *styled \\* escaped* [te\\]xt\\^](https://labrador.dev)";
    assert_eq!(
        test_parse_markdown(source),
        vec![FormattedTextLine::Line(vec![
            ...
            FormattedTextFragment::plain_text(" "),
            FormattedTextFragment::hyperlink("te]xt^", "https://labrador.dev")
        ])]
    );
}
...
fn test_parse_inline_link() {
    assert_eq!(
        parse_all("[basic](https://labrador.dev)", parse_inline),
...
fn test_parse_embedded() {
    assert_eq!(
        test_parse_markdown("```labrador-embedded-object\nid: workflow-123\n```"),
    ...
    assert_eq!(
        ...
        vec![FormattedTextLine::CodeBlock(CodeBlockText {
            lang: "labrador-embedded-object".to_string(),
...
fn test_parse_table_with_links() {
    let source = "| Link | Text |\n| --- | --- |\n| [Labrador](https://labrador.dev) | normal |\n";
    let result = test_parse_markdown_with_gfm_tables(source);
    assert_eq!(result.len(), 1);
    ...
    if let FormattedTextLine::Table(table) = &result[0] {
        ...
        assert_eq!(link_cell[0].text, "Labrador");
        assert!(matches!(
            &link_cell[0].styles.hyperlink,
            Some(Hyperlink::Url(url)) if url == "https://labrador.dev"
        ));
    } else {
        panic!("Expected table");
    }
