...
pub const LABRADOR_EMBED_ATTRIBUTE_NAME: &str = "data-labrador-embedded-item";
        ...
        match &node.data {
            ...
            NodeData::Element { name, attrs, .. } => {
                ...
                result.push_back(match node_name.as_str() {
                    ...
                    "pre" => {
                        let embedded_item_id = {
                            let attrs = attrs.borrow();
                            get_attribute(&attrs, LABRADOR_EMBED_ATTRIBUTE_NAME).map(str::to_string)
                        };

                        if let Some(val) = embedded_item_id {
        ...
        match &node.data {
            ...
            NodeData::Element { name, attrs, .. } => {
                ...
                match node_name.as_ref() {
                    ...
                    // TODO: Add more supported phrasing styles here, e.g. links.
                    _ => (),
                };
