...
#[test]
fn test_embedded_item_attribute() {
    let expected = vec![FormattedTextLine::Embedded(Mapping::from_iter([(
        Value::String("id".to_string()),
        Value::String("embed-123".to_string()),
    )]))];

    assert_eq!(
        test_parse_html(r#"<pre data-labrador-embedded-item="embed-123"></pre>"#),
        expected
    );
}
