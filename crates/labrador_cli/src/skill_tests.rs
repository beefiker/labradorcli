...
fn test_parse_repo_qualified() {
    let spec: SkillSpec = "labrador-internal:code-review".parse().unwrap();
    assert_eq!(spec.org, None);
    assert_eq!(spec.repo, Some("labrador-internal".to_string()));
    assert_eq!(spec.skill_identifier, "code-review");
    assert!(!spec.is_full_path());
}
...
fn test_parse_org_repo_qualified() {
    let spec: SkillSpec = "beefiker/labrador-internal:code-review".parse().unwrap();
    assert_eq!(spec.org, Some("beefiker".to_string()));
    assert_eq!(spec.repo, Some("labrador-internal".to_string()));
    assert_eq!(spec.skill_identifier, "code-review");
    assert!(!spec.is_full_path());
}
...
fn test_parse_full_path_with_org_repo() {
    let spec: SkillSpec = "beefiker/labrador-internal:.claude/skills/deploy/SKILL.md"
        .parse()
        .unwrap();
    assert_eq!(spec.org, Some("beefiker".to_string()));
    assert_eq!(spec.repo, Some("labrador-internal".to_string()));
...
fn test_parse_full_path_with_repo() {
    let spec: SkillSpec = "labrador-server:.agents/skills/test/SKILL.md".parse().unwrap();
    assert_eq!(spec.org, None);
    assert_eq!(spec.repo, Some("labrador-server".to_string()));
...
fn test_display_repo_qualified() {
    let spec = SkillSpec::with_repo("labrador-internal".to_string(), "code-review".to_string());
    assert_eq!(spec.to_string(), "labrador-internal:code-review");
}
...
fn test_display_org_repo_qualified() {
    let spec = SkillSpec::with_org_and_repo(
        "beefiker".to_string(),
        "labrador-internal".to_string(),
        "code-review".to_string(),
    );
    assert_eq!(spec.to_string(), "beefiker/labrador-internal:code-review");
}
...
fn test_display_full_path() {
    ...
    assert_eq!(
        spec.to_string(),
        "beefiker/labrador-internal:.claude/skills/deploy/SKILL.md"
    );
}
...
fn test_parse_empty_path_fails() {
    let result: Result<SkillSpec, _> = "labrador-internal:".parse();
    assert!(result.is_err());
}
...
fn test_skill_name_repo_qualified_name() {
    let spec: SkillSpec = "beefiker/feedback-triage-bot:feedback-triage-bot"
        .parse()
        .unwrap();
    assert_eq!(spec.skill_name(), "feedback-triage-bot");
}
...
fn test_skill_name_repo_qualified_path() {
    let spec: SkillSpec = "beefiker/feedback-triage-bot:.agents/skills/slack-triage/SKILL.md"
        .parse()
        .unwrap();
