...
fn agent_run_accepts_model() {
    let args = Args::try_parse_from([
        "labrador", "agent", "run", "--prompt", "hello", "--model", "gpt-4o",
    ])
    .unwrap();

    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador agent run` command");
    };
...
fn model_list_parses() {
    let args = Args::try_parse_from(["labrador", "model", "list"]).unwrap();

    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador model list` command");
    };
    let CliCommand::Model(model_cmd) = boxed_cmd.as_ref() else {
        panic!("Expected `labrador model` command");
    };
...
#[test]
fn login_parses() {
    let args = Args::try_parse_from(["labrador", "login"]).unwrap();

    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador login` command");
    };
...
#[test]
fn logout_parses() {
    let args = Args::try_parse_from(["labrador", "logout"]).unwrap();

    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador logout` command");
    };
...
#[test]
#[serial_test::serial]
fn api_key_prefers_labrador_env() {
    let previous_api_key = set_env_var(API_KEY_ENV, "new-key");

    let args = Args::try_parse_from(["labrador", "whoami"]).unwrap();

    restore_env_var(API_KEY_ENV, previous_api_key);

    assert_eq!(args.api_key().map(String::as_str), Some("new-key"));
}

#[test]
#[serial_test::serial]
fn api_key_arg_overrides_env() {
    let previous_api_key = set_env_var(API_KEY_ENV, "env-key");

    let args = Args::try_parse_from(["labrador", "whoami", "--api-key", "arg-key"]).unwrap();

    restore_env_var(API_KEY_ENV, previous_api_key);

    assert_eq!(args.api_key().map(String::as_str), Some("arg-key"));
}

#[test]
#[serial_test::serial]
fn output_format_prefers_labrador_env() {
    let previous_output_format = set_env_var(OUTPUT_FORMAT_ENV, "json");

    let args = Args::try_parse_from(["labrador", "whoami"]).unwrap();

    restore_env_var(OUTPUT_FORMAT_ENV, previous_output_format);
...
#[test]
#[serial_test::serial]
fn config_file_prefers_labrador_env() {
    let previous_file = set_env_var(
        crate::config_file::AGENT_CONFIG_FILE_ENV,
        "labrador-config.yaml",
    );

    let args = Args::try_parse_from(["labrador", "agent", "run", "--prompt", "hello"]).unwrap();

    restore_env_var(crate::config_file::AGENT_CONFIG_FILE_ENV, previous_file);
    ...
    assert_eq!(
        run_args.config_file.file().and_then(|p| p.to_str()),
        Some("labrador-config.yaml")
    );
}
...
fn agent_run_cloud_accepts_file_short_flag() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador agent run-cloud` command");
    };
...
fn agent_run_cloud_accepts_run_ambient_alias() {
    ...
    let args =
        Args::try_parse_from(["labrador", "agent", "run-ambient", "--prompt", "hello"]).unwrap();

    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador agent run-ambient` (alias) command");
    };
    let CliCommand::Agent(AgentCommand::RunCloud(_)) = boxed_cmd.as_ref() else {
        panic!("Expected `labrador agent run-ambient` to parse as RunCloud");
    };
}
...
fn agent_run_rejects_without_prompt_or_task_id() {
    let result = Args::try_parse_from(["labrador", "agent", "run", "--model", "gpt-4o"]);
    assert!(result.is_err());
...
fn agent_run_accepts_saved_prompt_only() {
    let args =
        Args::try_parse_from(["labrador", "agent", "run", "--saved-prompt", "sp-123"]).unwrap();
...
fn agent_run_accepts_skill_only() {
    let args = Args::try_parse_from(["labrador", "agent", "run", "--skill", "my-skill"]).unwrap();
...
fn agent_run_accepts_task_id_only() {
    let args = Args::try_parse_from(["labrador", "agent", "run", "--task-id", "tid-456"]).unwrap();
...
fn agent_run_accepts_prompt_and_skill() {
    let args = Args::try_parse_from([
        "labrador", "agent", "run", "--prompt", "do stuff", "--skill", "my-skill",
    ])
    .unwrap();
...
fn schedule_create_accepts_file() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador schedule create` command");
    };
    ...
    let Some(ScheduleSubcommand::Create(create_args)) = schedule_cmd.subcommand() else {
        panic!("Expected `labrador schedule create` subcommand");
    };
...
fn schedule_resume_alias_parses_as_unpause() {
    let args = Args::try_parse_from(["labrador", "schedule", "resume", "schedule-id"]).unwrap();

    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador schedule resume` command");
    };
    let CliCommand::Schedule(schedule_cmd) = boxed_cmd.as_ref() else {
        panic!("Expected `labrador schedule resume` command");
    };
    ...
    let Some(ScheduleSubcommand::Unpause(unpause_args)) = schedule_cmd.subcommand() else {
        panic!("Expected `labrador schedule resume` to parse as `unpause`");
    };
...
fn artifact_upload_accepts_run_id() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador artifact upload` command");
    };
...
fn artifact_help_hides_upload_but_keeps_download_visible() {
    labrador_core::features::mark_initialized();
...
fn artifact_upload_accepts_missing_association_target_for_env_fallback() {
    let args =
        Args::try_parse_from(["labrador", "artifact", "upload", "path/to/file.json"]).unwrap();
...
fn artifact_download_parses_artifact_id_and_out() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador artifact download` command");
    };
...
fn artifact_get_parses_artifact_uid() {
    let args = Args::try_parse_from(["labrador", "artifact", "get", "artifact-123"]).unwrap();

    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador artifact get` command");
    };
...
fn integration_create_accepts_file() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador integration create` command");
    };
...
fn integration_update_accepts_file() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador integration update` command");
    };
...
fn integration_create_accepts_mcp_json() {
    ...
    let args = Args::try_parse_from(["labrador", "integration", "create", "slack", "--mcp", json])
        .unwrap();
...
fn schedule_update_accepts_file() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador schedule update` command");
    };
    let CliCommand::Schedule(schedule_cmd) = boxed_cmd.as_ref() else {
        panic!("Expected `labrador schedule update` command");
    };
    ...
    let Some(ScheduleSubcommand::Update(update_args)) = schedule_cmd.subcommand() else {
        panic!("Expected `labrador schedule update` subcommand");
    };
...
fn environment_image_list_parses() {
    let args = Args::try_parse_from(["labrador", "environment", "image", "list"]).unwrap();

    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador environment image list` command");
    };
    let CliCommand::Environment(EnvironmentCommand::Image(image_cmd)) = boxed_cmd.as_ref() else {
        panic!("Expected `labrador environment image` command");
    };
...
fn environment_create_accepts_description() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador environment create` command");
    };
...
fn environment_update_accepts_description() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador environment update` command");
    };
...
fn agent_run_cloud_defaults_to_no_computer_use_override() {
    let args =
        Args::try_parse_from(["labrador", "agent", "run-cloud", "--prompt", "hello"]).unwrap();
...
fn run_message_send_parses() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador run message send` command");
    };
...
fn run_message_list_parses_filters() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador run message list` command");
    };
...
fn run_message_list_rejects_non_positive_limit() {
    assert!(
        Args::try_parse_from([
            "labrador", "run", "message", "list", "run-123", "--limit", "0",
        ])
        .is_err()
    );
}
...
fn run_message_watch_parses() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador run message watch` command");
    };
...
fn run_message_read_parses() {
    let args = Args::try_parse_from(["labrador", "run", "message", "read", "message-123"]).unwrap();

    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador run message read` command");
    };
...
fn run_message_mark_delivered_parses() {
    ...
    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador run message mark-delivered` command");
    };
...
fn run_message_delivered_alias_parses() {
    let args =
        Args::try_parse_from(["labrador", "run", "message", "delivered", "message-456"]).unwrap();

    let Some(Command::CommandLine(boxed_cmd)) = args.command else {
        panic!("Expected `labrador run message delivered` command");
    };
    let CliCommand::Run(TaskCommand::Message(MessageCommand::MarkDelivered(delivered_args))) =
        boxed_cmd.as_ref()
    else {
