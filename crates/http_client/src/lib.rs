...
pub mod headers {
    /// Custom Labrador server header indicating the version of the Labrador app.
    pub const CLIENT_RELEASE_VERSION_HEADER_KEY: &str = "X-Labrador-Client-Version";

    /// Custom Labrador server header indicating the OS category the request was sent from.
    pub(crate) const CLIENT_OS_CATEGORY_HEADER_KEY: &str = "X-Labrador-OS-Category";
    /// Custom Labrador server header indicating the OS name the request was sent from. On Linux this is the
    /// name of the distribution. On all other platforms it should be equivalent to
    /// `CLIENT_OS_CATEGORY_HEADER_KEY`.
    pub(crate) const CLIENT_OS_NAME_HEADER_KEY: &str = "X-Labrador-OS-Name";
    /// Custom Labrador server header indicating the version of the operating system. On Linux this is the
    /// version of the distribution, not the Linux kernel version.
    pub(crate) const CLIENT_OS_VERSION_HEADER_KEY: &str = "X-Labrador-OS-Version";

    /// Custom Labrador server header indicating the linux kernel version. This is only sent from Linux.
    pub(crate) const CLIENT_OS_LINUX_KERNEL_VERSION_HEADER_KEY: &str =
        "X-Labrador-OS-Linux-Kernel-Version";

    /// Custom Labrador server header indicating the client role. We don't use the User-Agent header
    /// because it can't be set from WASM.
    pub(crate) const CLIENT_ID_HEADER_KEY: &str = "X-Labrador-Client-ID";
}
...
const EXTRA_HTTP_HEADERS_ENV_VAR: &str = "LABRADOR_EXTRA_HTTP_HEADERS";
...
cfg_if::cfg_if! {
    if #[cfg(target_family = "wasm")] {
        ...
        // single-threaded (and we don't leverage WebWorkers for async execution in Labrador Web).
...
impl Client {
    ...
    fn builder(
        ...
        if include_labrador_headers {
            builder = Self::add_labrador_http_headers(builder);
        }
    ...
    pub fn get<U: IntoUrl + Clone>(&self, url: U) -> RequestBuilder<'_> {
        self.builder(
            ...
            Self::include_labrador_http_headers(url),
        )
    }
    ...
    /// Helper method to determine if the request should include Labrador-specific headers. The only case
    /// where we should include custom headers is if the request is same-origin and is targetted to our server.
    /// For example, app.labrador.dev --> app.labrador.dev.
    #[cfg(target_family = "wasm")]
    fn include_labrador_http_headers<U: IntoUrl + Clone>(url: U) -> bool {
        url.into_url().is_ok_and(|url| {
            url.host_str().is_some_and(|dest_host| {
                ...
                // If the request is going to our server, the destination host should be "app.labrador.dev" or
                // "staging.labrador.dev". The window hostname should also return the same.
    ...
    #[cfg(not(target_family = "wasm"))]
    fn include_labrador_http_headers<U: IntoUrl + Clone>(_url: U) -> bool {
        true
    }

    fn add_labrador_http_headers(mut builder: RequestBuilder) -> RequestBuilder {
        // Include the client ID header.
        if let Some(client_id) = execution_mode::current_client_id() {
            builder = builder.header(
                headers::CLIENT_ID_HEADER_KEY,
        ...
        if ChannelState::channel() == Channel::Integration
            && let Some(raw) = extra_http_headers_from_env()
        {
            for line in raw.lines() {
                ...
                match (
                    ...
                    _ => {
                        log::warn!("Ignoring invalid entry in extra HTTP headers env: {line}");
                    }
                }
            }
        }
        ...
        if let Ok(os_system_info) = OperatingSystemInfo::get() {
            ...
            if let Ok(category) = HeaderValue::from_str(&category) {
                builder = builder.header(headers::CLIENT_OS_CATEGORY_HEADER_KEY, category);
            }

            // Operating system name.
            builder = builder.header(
                headers::CLIENT_OS_NAME_HEADER_KEY,
            ...
            {
                builder = builder.header(headers::CLIENT_OS_VERSION_HEADER_KEY, version);
            }
            ...
            {
                builder = builder.header(
                    headers::CLIENT_OS_LINUX_KERNEL_VERSION_HEADER_KEY,
                    linux_kernel_version,
                );
            }
        }
...
fn extra_http_headers_from_env() -> Option<String> {
    std::env::var(EXTRA_HTTP_HEADERS_ENV_VAR).ok()
}
...
/// This allows callers to inspect server-provided error-code headers when handling errors.
#[derive(Debug)]
...
mod tests {
    ...
    #[test]
    fn extra_http_headers_prefers_labrador_env() {
        let _guard = ENV_LOCK.lock().unwrap();
        let previous_headers = set_env_var(EXTRA_HTTP_HEADERS_ENV_VAR, "X-New: 1");

        let headers = extra_http_headers_from_env();

        restore_env_var(EXTRA_HTTP_HEADERS_ENV_VAR, previous_headers);

        assert_eq!(headers.as_deref(), Some("X-New: 1"));
    }

    #[test]
    fn extra_http_headers_returns_none_without_labrador_env() {
        let _guard = ENV_LOCK.lock().unwrap();
        let previous_headers = remove_env_var(EXTRA_HTTP_HEADERS_ENV_VAR);
    ...
    #[test]
    fn labrador_header_constants_are_labrador_named() {
        assert_eq!(headers::CLIENT_ID_HEADER_KEY, "X-Labrador-Client-ID");
        assert_eq!(
            headers::CLIENT_RELEASE_VERSION_HEADER_KEY,
            "X-Labrador-Client-Version"
        );
    }
}
...
impl<'c> oauth2::AsyncHttpClient<'c> for Client {
    ...
    fn call(&'c self, request: oauth2::HttpRequest) -> Self::Future {
        Box::pin(async move {
            let include_labrador_headers =
                Self::include_labrador_http_headers(request.uri().to_string());
            ...
            let response = self
                .builder(builder, include_labrador_headers)
                .send()
                .await
                .map_err(Box::new)?;
