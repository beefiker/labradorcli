...
/// 1) Renaming the log file from the main process (which just panicked) to `labrador.log.old.temp`.
/// 2) Moving the crash recovery process log (which is located at `labrador.log.recovery`) to the usual
///    path Labrador logs are located (log_directory/labrador.log).
///    The temp log file (`labrador.log.old.temp`) will ultimately be rotated to `labrador.log.old.0` the next
...
pub async fn rotate_files(channel_file_name: &str, max_rotation: usize) -> Result<()> {
    ...
    // Rename `labrador.log.old.temp` (the temporary file) to `labrador.log.old.0`.
...
/// location at `log_directory/labrador.log`.
...
/// Collects a list of the paths to both the current Labrador instance's log file,
...
fn current_and_rotated_log_paths() -> Result<Vec<PathBuf>> {
    ...
    if files.is_empty() {
        return Err(anyhow::anyhow!(
            "No Labrador logs were found for {}",
...
fn sentry_log_filter(md: &log::Metadata) -> sentry_log::LogFilter {
    if labrador_core::errors::should_ignore_log_for_sentry(md) {
    ...
    match md.target() {
        ...
        "labrador_ui::core::redraw_frame" => sentry_log::LogFilter::Ignore,
        ...
        t if t.starts_with("labrador::crash_reporting::") => sentry_log::LogFilter::Ignore,
...
fn init_internal(
    ...
    /// Returns an empty file named `labrador.log` to log the current execution, and
    ...
    fn setup_log_files_for_current_execution(
        ...
        fs::create_dir_all(log_directory)?;
            ...
            let main_log_path = if is_from_crash_recovery_process {
                ...
                // not rename the old main log file to `labrador.log.temp` like we do below because this
                ...
                // Rename the old main log file to `labrador.log.temp`.
    ...
    let integration_test = std::env::var_os("LABRADOR_INTEGRATION").is_some();
...
fn init_log_directory() -> Result<std::path::PathBuf> {
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            ...
            Ok(labrador_core::paths::state_dir())
        } else if #[cfg(windows)] {
            Ok(labrador_core::paths::state_dir().join(labrador_core::paths::LABRADOR_LOGS_DIR))
        } else {
