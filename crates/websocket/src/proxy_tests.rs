...
fn no_proxy_suffix_with_dot() {
    ...
    clear_proxy_env();
    env::set_var("HTTPS_PROXY", "http://proxy:3128");
    env::set_var("NO_PROXY", ".labrador.dev");

    assert!(resolved_proxy_tls("sessions.app.labrador.dev").is_none());

    assert!(resolved_proxy_tls("labrador.dev").is_some()); // Exact "labrador.dev" != ".labrador.dev"
    assert!(resolved_proxy_tls("other.com").is_some());
    clear_proxy_env();
}
...
fn no_proxy_suffix_without_dot() {
    ...
    // "sessions.app.labrador.dev" ends with ".labrador.dev" → matches
    assert!(resolved_proxy_tls("sessions.app.labrador.dev").is_none());
    // Exact match too
    assert!(resolved_proxy_tls("labrador.dev").is_none());
    assert!(resolved_proxy_tls("notlabrador.dev").is_some());
    clear_proxy_env();
}
