...
use vim::vim::VimMode;
use labrador_core::ui::theme::Fill as ThemeFill;
use labrador_ui::{
...
/// and the parent view can overlay UI controls on top of them using a [`labrador_ui::elements::Stack`].
///
...
impl<V: EditorView> Element for RichTextElement<V> {
    ...
    fn paint(&mut self, origin: Vector2F, ctx: &mut PaintContext, app: &AppContext) {
        ...
        ctx.scene
            .start_layer(labrador_ui::ClipBounds::BoundedBy(clip_bounds));
...
impl<V: EditorView> NewScrollableElement for RichTextElement<V> {
    ...
    fn scroll(&mut self, delta: labrador_ui::units::Pixels, axis: Axis, ctx: &mut EventContext) {
...
impl<V: EditorView> ScrollableElement for RichTextElement<V> {
    ...
    fn scroll(&mut self, delta: labrador_ui::units::Pixels, ctx: &mut EventContext) {
