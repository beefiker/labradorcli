...
use vec1::{vec1, Vec1};
use labrador_ui::{App, AppContext, ModelContext, ModelHandle, ReadModel};
...
use labrador_ui::elements::ListIndentLevel;
use labrador_ui::text::point::Point;
...
fn test_inline_markdown_roundtrips() {
    ...
    let inputs = &[
        ...
        "[link1](https://labrador.dev)[**link2**](https://example.com)",
...
fn test_import_markdown() {
    App::test((), |mut app| async move {
        ...
        let markdown_string = "```labrador-runnable-command\ntest\nblock\n```\n";
        let (buffer, _selection) = Buffer::mock_from_markdown(
            markdown_string,
            None,
            Box::new(|_, _| IndentBehavior::Ignore),
...
```"#,
        ...
        buffer.read(&app, |buffer, _| {
            assert_eq!(buffer.debug(), "<code:Shell>default code<code:Shell>sh code<code:Rust>rust code<code:Shell>labrador code<text>");
        });
        buffer.read(&app, |buffer, _| {
            selection.read(&app, |selection, _| {
                buffer.validate(&selection.anchors);
...
fn test_enter_after_empty_block() {
    // Regression test for preserving enter behavior after an empty block.
    App::test((), |mut app| async move {
        let buffer = app.add_model(|_| Buffer::new(Box::new(|_, _| IndentBehavior::Ignore)));
        let selection = app.add_model(|_| BufferSelectionModel::new(buffer.clone()));
    ...
    App::test((), |mut app| async move {
        ...
        buffer.update(&mut app, |buffer, ctx| {
            ...
            assert_eq!(
                buffer.selected_text_as_html(selection.clone(), ctx),
                Some(
                    "<pre><code class=\"language-labrador-runnable-command\">Blo</code></pre>".to_string()
                )
            );
            ...
            assert_eq!(
                buffer.selected_text_as_html(selection.clone(), ctx),
                Some("<p><strong>ore</strong></p><pre><code class=\"language-labrador-runnable-command\">Blo</code></pre>".to_string())
            );
            ...
            assert_eq!(
                buffer.selected_text_as_html(selection.clone(), ctx),
                Some(
                    "<pre><code class=\"language-labrador-runnable-command\">ck</code></pre><p>Af</p>"
                        .to_string()
                )
            );
            ...
            assert_eq!(
                buffer.selected_text_as_html(selection.clone(), ctx),
                Some(
                    "<pre><code class=\"language-labrador-runnable-command\">ck</code></pre><h1>After</h1>"
                        .to_string()
                )
            );
...
fn test_link_style_exact() {
    App::test((), |mut app| async move {
        ...
        buffer.update(&mut app, |buffer, ctx| {
            ...
            assert_eq!(
                buffer.content.debug(),
                "<text><a_www.google.com>li<a><a_www.labrador.dev>ne\\nb<a>lock"
            );
                    ...
                    StyledBufferBlock::Text(StyledTextBlock {
                        block: vec![
                            ...
                            StyledBufferRun {
                                run: "ne\n".to_string(),
                                text_styles: TextStylesWithMetadata::default()
                                    .link("www.labrador.dev".to_string()),
                                block_style: BufferBlockStyle::PlainText
                            }
                        ],
                        style: BufferBlockStyle::PlainText,
...
fn test_link_style_different_tag() {
    App::test((), |mut app| async move {
        ...
        buffer.update(&mut app, |buffer, ctx| {
            ...
            assert_eq!(
                buffer.content.debug(),
                "<text><a_www.google.com>g<a>n<a_www.labrador.dev>normal long text<a>ock"
            );
...
fn test_link_style_surrounded() {
    App::test((), |mut app| async move {
        ...
        buffer.update(&mut app, |buffer, ctx| {
            ...
            assert_eq!(
                buffer.content.debug(),
                "<text><a_www.google.com>li<a><a_www.labrador.dev>ne<a>\\nblock"
            );
...
fn test_unstyle_link_overlapping() {
    App::test((), |mut app| async move {
        ...
        buffer.update(&mut app, |buffer, ctx| {
            ...
            assert_eq!(
                buffer.content.debug(),
                "<text><a_www.google.com>lin<a><a_www.labrador.dev>e\\nb<a>lock"
            );
            ...
            assert_eq!(
                buffer.content.debug(),
                "<text><a_www.google.com>l<a>ine<a_www.labrador.dev>\\nb<a>lock"
            );
...
fn test_multiselect_copy_blocks() {
    App::test((), |mut app| async move {
        ...
        buffer.update(&mut app, |buffer, ctx| {
            ...
            assert_eq!(buffer.selected_text_as_html(selection.clone(), ctx), Some("<ul><li>Hey</li><li>Yo</li></ul><p>ext</p><pre><code class=\"language-labrador-runnable-command\">This</code></pre>".to_string()));
        })
    });
}
