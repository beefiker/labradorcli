use super::{
    layout_mermaid_diagram_block, layout_table_block, layout_text_block, BlockLocation, LayOutArgs,
};
use crate::{
    content::{
        buffer::{StyledBufferRun, StyledTextBlock},
        edit::{highlight_urls, resolve_asset_source_relative_to_directory, ParsedUrl},
        mermaid_diagram::{mermaid_asset_source, mermaid_diagram_layout},
        text::{BufferBlockStyle, CodeBlockType, TextStylesWithMetadata},
    },
    render::{
        layout::{add_link_to_style_and_font, markdown_inline_to_text_and_style_runs, TextLayout},
        model::{test_utils::TEST_STYLES, BlockItem},
    },
};
use std::path::Path;
...
fn test_highlight_urls() {
    ...
    test_styled_buffer_runs.extend(vec![
        ...
        StyledBufferRun {
            run: "https://labrador.dev".to_string(),
            text_styles: TextStylesWithMetadata::default(),
            block_style: BufferBlockStyle::PlainText,
        },
    ]);
    ...
    assert_eq!(
        ...
        [
            ...
            ParsedUrl {
                url_range: 23..43,
                link: "https://labrador.dev".to_string()
            }
        ]
    );
}
...
fn test_highlight_urls_unicode() {
    let test_runs = vec![StyledBufferRun {
        run: "This (not https://example.com) is a 🔥 link about a 🇨🇦 🏡:\u{a0}https://labrador.dev"
            .to_string(),
...
fn test_links_not_auto_highlighted() {
    ...
    let runs = &[
        StyledBufferRun {
            run: "first link is https://labrador.dev ".to_string(),
            text_styles: Default::default(),
            block_style: BufferBlockStyle::PlainText,
        },
        StyledBufferRun {
            run: "http://example.com".to_string(),
            text_styles: TextStylesWithMetadata::default().link("https://labrador.dev".to_string()),
            block_style: BufferBlockStyle::PlainText,
        },
        StyledBufferRun {
...
fn test_table_inline_style_runs_preserve_markdown_cell_styles() {
    App::test((), |app| async move {
        ...
        app.read(|ctx| {
            ...
            let table = crate::content::text::table_from_internal_format_with_inline_markdown(
                "Header\tValue\nText\t**Bold** *Italic* [Link](https://labrador.dev) `code`\n",
                Vec::new(),
            );
            ...
            assert!(layout_input.style_runs[4]
                .1
                .style
                .foreground_color
                .is_some());
            ...
            assert!(layout_input.style_runs[6]
                .1
                .style
                .background_color
                .is_some());
