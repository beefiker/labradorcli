...
#[cfg(windows)]
use labrador_core::paths::base_config_dir;
...
impl Shell {
    ...
    ///
    /// These sequences are bound to Labrador shell functions during session bootstrap that print the
...
impl From<ShellType> for labrador_util::path::ShellFamily {
...
impl ShellType {
    ...
    /// If true, Labrador will bootstrap the shell if it's the login shell on the remote host.
                        ...
                        output_string
                            ...
                            .flat_map(|line| {
                                ...
                                for ext in PATHEXT {
                                    ...
                                    // Labrador about this command as-is and also sans-suffix, e.g.
                                    // "git" and "git.exe".
...
impl ShellLaunchData {
    ...
    fn to_native_path_encoding(&self, path_str: &str) -> Option<PathBuf> {
        match self {
            ...
            // The container is Unix; Labrador runs on the host, so paths are
