...
Let's say we are interested in parsing the input `labrador --disable-telemetry`. Command calls generally have the form `cmd <arg1> <arg2> <argN>` where `<arg>`s are positional parameters. Let's use the primary functions of the parser and inspect. The first step we do is calling the tokenizer (the function `lex` here):

```rust
let input = "labrador --disable-telemetry";
let start_offset = 0;
...
let word1 = Span::new(0,8);
let word2 = Span::new(8,9);
let word3 = Span::new(9,28);

assert_eq!(word1.slice(input), "labrador");
...
let example = Spanned { item: String::from("labrador"), span: Span::new(0,8) };
assert_eq!(example.item, "labrador".to_string());
assert_eq!(example.span, Span::new(0,8));

let example = String::from("labrador").spanned(Span::new(0,8));
assert_eq!(example.item, "labrador".to_string());
assert_eq!(example.span, Span::new(0,8));

let example = "labrador -p --disable-telemetry";

let full_span = Span::new(0, example.len());
let first_flag_span = Span::new(9,11);

assert_eq!(first_flag_span.slice(example), "-p");
assert_eq!(first_flag_span.until(full_span), Span::new(9,31));
assert_eq!(first_flag_span.until(full_span).slice(example), "-p --disable-telemetry");

```

This is useful because as soon as the `lite` parse step happens. We get as output everything we need with `spans` correctly calculated. Let's go ahead and do a lite parse (the function `parse_tokens`) to our original example using as input the tokens generated by the lexer when processing the input `labrador --disable-telemetry`, like so:
...
let expected_word1 = String::from("labrador").spanned(Span::new(0,8));
let expected_word2 = String::from("--disable-telemetry").spanned(Span::new(9,28));
...
For more involved inputs (say commands separated by `|` and/or having `;`) the lite parser will effectively create the necessary `LitePipeline`s to express it, let's explore what happens if we lite parse the input `labrador config-set --extension-path="/path/to/dir" ; echo $LABRADOR_VAR"` (*two pipelines here due to the ; character*), like so:

```rust
let input = "labrador config-set --extension-path=\"/path/to/dir\" ; echo $LABRADOR_VAR";
let start_offset = 0;
