...
fn test_find_command_from_a_top_level_signature() {
    let bundle = labrador_command_signatures::signature_by_name("bundle")
...
fn test_find_subcommand_signature_with_flags() {
    let kubectl = labrador_command_signatures::signature_by_name("kubectl")
...
fn test_alias_expansion_path_skips_flag_with_value_before_subcommand() {
    ...
    let result = labrador_ui::r#async::block_on(ctx.command_registry().signature_with_alias_expansion(
        &["kubectl", "-n", "default", "get"],
        true,
        &ctx,
    ));
