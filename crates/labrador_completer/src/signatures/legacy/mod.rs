...
use labrador_command_signatures::Signature;
...
impl CommandRegistry {
    ...
    fn new_with_embedded_signatures() -> Self {
        let registry = CommandRegistry::new(
            |command| {
                ...
                let signature = labrador_command_signatures::signature_by_name(command);
                log::debug!(
            ...
            labrador_command_signatures::dynamic_command_signature_data(),
        );

        Self::register_labrador_signatures(&registry);

        registry
    }

    /// Register signatures for Labrador CLI commands.
    ///
    /// Ideally this would be done outside of the `labrador_completer` crate, but it's not currently
    /// possible to configure the shared [`Self::global_instance`].
    fn register_labrador_signatures(registry: &Self) {
        ...
        for channel in channels {
            ...
            let mut clap_cmd = labrador_cli::Args::clap_command();
            let signature =
    ...
    /// labrador-command-signatures crate.
    #[cfg(feature = "test-util")]
    pub fn new_for_test(
        signatures: impl IntoIterator<Item = Signature>,
        generators: std::collections::HashMap<
            String,
            labrador_command_signatures::DynamicCompletionData,
        >,
    ) -> Self {
