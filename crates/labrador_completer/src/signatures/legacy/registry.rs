...
use labrador_command_signatures::{Argument, DynamicCompletionData, IsArgumentOptional, Signature};
...
fn find_option_by_name<'a>(
    options: &'a [labrador_command_signatures::Opt],
    token: &str,
) -> Option<&'a labrador_command_signatures::Opt> {
    options
        .iter()
