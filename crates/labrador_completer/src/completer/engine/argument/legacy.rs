...
//! command signature struct (`labrador_command_signatures::Signature`).
use std::{borrow::Cow, collections::HashMap};

use itertools::Itertools;
use labrador_command_signatures::{
...
async fn generate_suggestions_for_argument_type(
    ...
    match argument_type {
        ArgumentType::Suggestion(suggestion) => {
            let signature_suggestion: Suggestion = suggestion.clone().into();
            match matcher
                .get_match_type(parsed_token.as_str(), signature_suggestion.display.as_str())
            {
                Some(match_type) => vec![MatchedSuggestion::new(signature_suggestion, match_type)],
                None => vec![],
            }
        }
...
fn shell_command<'a>(
    ...
    let shell = if cfg!(windows) && FeatureFlag::RunGeneratorsWithCmdExe.is_enabled() {
        labrador_command_signatures::Shell::CmdExe
    } else {
        match shell_family {
            ShellFamily::Posix => labrador_command_signatures::Shell::Posix,
            ShellFamily::PowerShell => labrador_command_signatures::Shell::Powershell,
        }
    };
...
impl From<labrador_command_signatures::Suggestion> for Suggestion {
    /// Convert the `labrador_command_signatures::Suggestion`s (which are meant to map
    /// 1:1 to the `completer::Suggestion`s)
    fn from(suggestion: labrador_command_signatures::Suggestion) -> Self {
...
impl From<Suggestion> for labrador_command_signatures::Suggestion {
    fn from(suggestion: Suggestion) -> labrador_command_signatures::Suggestion {
        labrador_command_signatures::Suggestion {
