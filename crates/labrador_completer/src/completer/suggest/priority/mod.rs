...
impl From<labrador_command_signatures::Priority> for Priority {
    fn from(value: labrador_command_signatures::Priority) -> Self {
        match value {
            labrador_command_signatures::Priority::Global(importance)
            | labrador_command_signatures::Priority::Local(importance) => match importance {
                labrador_command_signatures::Importance::More(order) => Self::new(order.0 as i32),
                labrador_command_signatures::Importance::Less(order) => {
                    Self::new(-(101 - order.0 as i32))
                }
            },
            labrador_command_signatures::Priority::Default => Priority::default(),
        }
    }
}

impl From<Priority> for labrador_command_signatures::Priority {
    fn from(value: Priority) -> Self {
        match value.cmp(&Priority::default()) {
            Ordering::Less => labrador_command_signatures::Priority::Global(
                labrador_command_signatures::Importance::Less(labrador_command_signatures::Order(
                    101 - value.value().unsigned_abs(),
                )),
            ),
            Ordering::Equal => labrador_command_signatures::Priority::default(),
            Ordering::Greater => labrador_command_signatures::Priority::Global(
                labrador_command_signatures::Importance::More(labrador_command_signatures::Order(
