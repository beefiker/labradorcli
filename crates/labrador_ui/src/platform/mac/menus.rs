...
use labrador_ui_core::actions::StandardAction;
use labrador_ui_core::keymap::Keystroke;
use labrador_ui_core::platform::menu::{
...
#[no_mangle]
extern "C-unwind" fn labrador_menu_item_needs_update(item: id, ctx: *mut c_void) {
...
#[no_mangle]
extern "C-unwind" fn labrador_menu_item_triggered(_item: id, ctx: *mut c_void) {
    let func = &MenuItemData::read_context(ctx).triggered;
    callback_dispatcher().menu_item_triggered(func);
}

#[no_mangle]
extern "C-unwind" fn labrador_menu_item_deallocated(ctx: *mut c_void) {
...
extern "C" {
    ...
    fn make_labrador_custom_menu_item(ctx: *mut c_void) -> id;
...
fn resolve_standard_action(action: StandardAction) -> StandardMenuItemProperties {
    ...
    match action {
        ...
        StandardAction::Quit => make("Quit Labrador", "terminate:", cmd, "q"),
        StandardAction::Hide => make("Hide Labrador", "hide:", cmd, "h"),
...
unsafe fn apply_changes(changes: MenuItemPropertyChanges, item: id) {
    // Wrap in a local autorelease pool: AppKit invokes `labrador_menu_item_needs_update`
...
unsafe fn make_menu_item(menu_item: MenuItem) -> id {
    match menu_item {
        MenuItem::Custom(custom_menu_item) => {
            ...
            let nsmenu_item = make_labrador_custom_menu_item(MenuItemData::into_context(data));
