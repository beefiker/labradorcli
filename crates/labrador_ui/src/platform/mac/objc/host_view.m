...
void labrador_view_did_change_backing_properties(LabradorHostView *, BOOL);
void labrador_view_set_frame_size(LabradorHostView *, NSSize, BOOL);
void labrador_update_layer(LabradorHostView *);
BOOL labrador_handle_view_event(LabradorHostView *, NSEvent *, BOOL);
BOOL labrador_handle_first_mouse_event(LabradorHostView *, NSEvent *);
void labrador_handle_insert_text(LabradorHostView *, id);
void labrador_update_ime_state(LabradorHostView *, BOOL);
void labrador_handle_drag_and_drop(LabradorHostView *, NSArray *, NSPoint);
void labrador_handle_file_drag(LabradorHostView *, NSPoint);
void labrador_handle_file_drag_exit(LabradorHostView *);
NSRect labrador_ime_position(LabradorHostView *, NSRect *);
id labrador_get_accessibility_contents(LabradorHostView *);
void labrador_marked_text_updated(LabradorHostView *, NSString *, NSRange);
void labrador_marked_text_cleared(LabradorHostView *);

@implementation NSPasteboard (Labrador)
...
@implementation LabradorHostView {
...
- (void)viewDidChangeBackingProperties {
    if (self.readyForLabrador) labrador_view_did_change_backing_properties(self, asyncCallback);
...
- (void)setFrameSize:(NSSize)size {
    ...
    if (size.height >= self.window.minSize.height && size.width >= self.window.minSize.width) {
        ...
        if (self.readyForLabrador && changed) {
            labrador_view_set_frame_size(self, size, asyncCallback);
        }
    }
}

- (void)displayLayer:(CALayer *)layer {
    if (!testMode && self.readyForLabrador) {
        labrador_update_layer(self);
    }
}
...
- (BOOL)keyDownImpl:(NSEvent *)event {
    ...
    BOOL handled = NO;
    if (self.readyForLabrador) {
        handled = labrador_handle_view_event(self, event, wasComposing || [self hasMarkedText]);
    }
    ...
    if ([textToInsert length] > 0 && !handled) {
        labrador_handle_insert_text(self, (NSString *)textToInsert);
        [self unmarkText];
    }
...
- (BOOL)acceptsFirstMouse:(NSEvent *)event {
    ...
    // and we explicity fire the event here so that Labrador can handle it.
    if (self.readyForLabrador) labrador_handle_first_mouse_event(self, event);
...
- (void)mouseDown:(NSEvent *)event {
    if (self.readyForLabrador) {
        BOOL eventHandled = labrador_handle_view_event(self, event, NO);
        if (self->titlebarDragEnabled && !eventHandled && [self mouseInTitleBar:event]) {
            // If Labrador doesn't do anything with the event, indicated by returning `false`, and
...
- (void)mouseUp:(NSEvent *)event {
    ...
    BOOL labrador_handled = NO;
    if (self.readyForLabrador) {
        labrador_handled = labrador_handle_view_event(self, event, NO);
    }
    if (!labrador_handled) {
...
- (void)otherMouseDown:(NSEvent *)event {
    if (self.readyForLabrador) labrador_handle_view_event(self, event, NO);
}
...
- (NSDragOperation)draggingUpdated:(id<NSDraggingInfo>)sender {
    ...
    if (self.readyForLabrador) {
        ...
        if ([types containsObject:NSPasteboardTypeFileURL]) {
            labrador_handle_file_drag(self, localPoint);
            return YES;
        }
    }
    return NSDragOperationNone;
...
- (void)draggingExited:(id<NSDraggingInfo>)sender {
    if (self.readyForLabrador) {
        labrador_handle_file_drag_exit(self);
    }
}
...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender {
    ...
    if (self.readyForLabrador && (dragOperation & NSDragOperationCopy)) {
        NSArray *types = [pasteboard types];
        if ([types containsObject:NSPasteboardTypeFileURL]) {
            labrador_handle_drag_and_drop(self, [pasteboard getFilePaths], localPoint);
            return YES;
        }
    }
    return NO;
...
- (id)accessibilityValue {
    return labrador_get_accessibility_contents(self);
}
...
- (NSRect)firstRectForCharacterRange:(NSRange)range
    ...
    if (self.readyForLabrador) {
        ...
        NSRect rect = labrador_ime_position(self, &contentRect);
        return rect;
    } else {
        return NSZeroRect;
    }
...
- (void)insertText:(id)string replacementRange:(NSRange)replacementRange {
    if (self.readyForLabrador) {
        ...
        if (interpretingKeyEvents) {
            ...
            labrador_handle_insert_text(self, (NSString *)characters);
        }
...
- (void)setMarkedText:(id)string
    ...
    if (self.readyForLabrador) {
        labrador_marked_text_updated(self, markedText.string, selectedRange);
        if ([markedText length] > 0) {
            labrador_update_ime_state(self, YES);
        } else {
            labrador_update_ime_state(self, NO);
        }
    }
}
...
- (void)unmarkText {
    ...
    if (self.readyForLabrador) {
        labrador_update_ime_state(self, NO);
        labrador_marked_text_cleared(self);
    }
}
