...
// NSWindow.delegate is a weak reference, so the LabradorWindowDelegate we create in
// `create_labrador_nswindow` / `create_labrador_nspanel` would otherwise be leaked with a +1
...
static const void *kLabradorWindowDelegateAssocKey = &kLabradorWindowDelegateAssocKey;

NSWindowStyleMask labradorWindowMask = NSWindowStyleMaskClosable | NSWindowStyleMaskMiniaturizable |
...
@protocol LabradorWindowProtocol
...
@class LabradorWindow;
@class LabradorPanel;

// Declaration of functions implemented in Rust.
void labrador_dealloc_window(id self);
void labrador_dispatch_standard_action(id self, NSInteger tag);
void labrador_app_window_moved(id self, NSRect rect);
void labrador_open_panel_file_selected(id urls, void *callback);
void labrador_save_panel_file_selected(id url, void *callback);
...
@interface LabradorWindow : NSWindow <LabradorWindowProtocol>
@end

@interface LabradorWindowDelegate : NSObject <NSWindowDelegate>
@end

@implementation LabradorWindowDelegate {
    void *windowState;
...
- (void)windowDidMove:(NSNotification *)notification {
    if (windowState) {
        ...
        labrador_app_window_moved(self, window.frame);
    }
}

- (void)windowWillStartLiveResize:(NSNotification *)notification {
    LabradorWindow *labrador_window = notification.object;
    LabradorHostView *labrador_view = labrador_window.contentView;
    ...
    [labrador_view setAsyncCallback:NO];
}

- (void)windowDidEndLiveResize:(NSNotification *)notification {
    LabradorWindow *labrador_window = notification.object;
    LabradorHostView *labrador_view = labrador_window.contentView;
    [labrador_view setAsyncCallback:YES];
}
...
- (BOOL)windowShouldClose:(NSWindow *)window {
    ...
    BOOL okToClose = labrador_app_should_close_window(application, window);
...
- (void)windowWillClose:(NSNotification *)note {
    if (windowState) {
        labrador_app_window_will_close([NSApplication sharedApplication], self);
    }
}
...
- (void)windowWillEnterFullScreen:(NSNotification *)notification {
    NSWindow<LabradorWindowProtocol> *window = notification.object;
...
void init_labrador_nswindow(NSWindow<LabradorWindowProtocol> *window, bool testMode, bool hideTitleBar) {
...
- (BOOL)performKeyEquivalent:(NSEvent *)event {
    ...
    if ([event type] == NSEventTypeKeyDown) {
        ...
        BOOL keyBindingsDisabled = labrador_app_are_key_bindings_disabled_for_window(application, self);
        // If Labrador has assigned a binding for this keystroke.
        BOOL keystrokeIsAssigned = labrador_app_has_binding_for_keystroke(application, event);

        BOOL triggersCustomAction = labrador_app_has_custom_action_for_keystroke(application, event);
...
- (void)closeWindowAsync:(BOOL)forceTermination {
    dispatch_async(dispatch_get_main_queue(), ^{
      LabradorWindowDelegate *delegate = self.delegate;
      if (forceTermination) {
...
+ (LabradorWindow *)createWithContentRect:(NSRect)contentRect
    ...
    NSWindowStyleMask mask = labradorWindowMask;

    if (hideTitleBar) {
        mask |= NSWindowStyleMaskFullSizeContentView;
    }

    LabradorWindow *window_result = [[LabradorWindow alloc] initWithContentRect:contentRect
                                                              styleMask:mask
                                                                backing:NSBackingStoreBuffered
                                                                  defer:NO];
    init_labrador_nswindow(window_result, testMode, hideTitleBar);
...
@interface LabradorPanel : NSPanel <LabradorWindowProtocol>
@end

@implementation LabradorPanel {
    // The windowState is managed on the Rust side.
    void *windowState;
    // Height constraint for the titlebar view (also indicates if constraints are configured)
    NSLayoutConstraint *_titleBarHeightConstraint;
...
- (void)performClose:(id)sender {
    labrador_dispatch_standard_action(self, [sender tag]);
}
...
// Note this returns a retained object ("create" rule).
+ (LabradorPanel *)createWithContentRect:(NSRect)contentRect
                         metalDevice:(id)metalDevice
                      hidingTitleBar:(BOOL)hideTitleBar
          backgroundBlurRadiusPixels:(uint8)backgoundBlurRadiusPixels
                        withTestMode:(BOOL)testMode {
    NSWindowStyleMask mask = labradorWindowMask | NSWindowStyleMaskNonactivatingPanel;

    if (hideTitleBar) {
        mask |= NSWindowStyleMaskFullSizeContentView;
    }

    LabradorPanel *window_result = [[LabradorPanel alloc] initWithContentRect:contentRect
                                                            styleMask:mask
                                                              backing:NSBackingStoreBuffered
                                                                defer:NO];
    init_labrador_nswindow(window_result, testMode, hideTitleBar);
...
// Attaches a LabradorWindowDelegate to |window| and ties its lifetime to the window.
//
...
static void attach_labrador_window_delegate(NSWindow *window) {
    LabradorWindowDelegate *delegate = [[LabradorWindowDelegate alloc] init];
    [window setDelegate:delegate];
    objc_setAssociatedObject(window, kLabradorWindowDelegateAssocKey, delegate,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    [delegate release];
}

// \return a new, retained LabradorPanel with the given content rect.
id create_labrador_nspanel(NSRect contentRect, id metalDevice, BOOL hideTitleBar,
    ...
    LabradorPanel *window = [LabradorPanel createWithContentRect:contentRect
                                             metalDevice:metalDevice
                                          hidingTitleBar:hideTitleBar
                              backgroundBlurRadiusPixels:backgroundBlurRadiusPixels
                                            withTestMode:testMode];

    LabradorHostView *hostView = [[[LabradorHostView alloc] initWithFrame:contentRect
                                                      metalDevice:metalDevice
                                               enableTitlebarDrag:NO
                                                         testMode:testMode] autorelease];

    attach_labrador_window_delegate(window);
...
// \return a new, retained LabradorWindow with the given content rect.
id create_labrador_nswindow(NSRect contentRect, id metalDevice, BOOL hideTitleBar,
                        uint8 backgroundBlurRadiusPixels, BOOL testMode) {
    NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
    ...
    LabradorWindow *window = [LabradorWindow createWithContentRect:contentRect
                                               metalDevice:metalDevice
                                            hidingTitleBar:hideTitleBar
                                backgroundBlurRadiusPixels:backgroundBlurRadiusPixels
                                              withTestMode:testMode];
...
BOOL is_labrador_window(id window) {
    return [window isKindOfClass:[LabradorWindow class]] || [window isKindOfClass:[LabradorPanel class]];
}
...
// @param labradorRole - the role of the given element (we're using our own, internally defined roles,
//                    check labrador_ui::accessibility)
...
void set_accessibility_contents(id window, NSString *value, NSString *help, NSString *labradorRole,
...
void open_file_path_in_explorer(NSString *pathString) {
    ...
    // errors when the file viewer activation re-enters app state.
    dispatch_async(dispatch_get_main_queue(), ^{
...
void open_file_picker(void *callback, NSArray<NSString *> *fileTypes, BOOL allowFiles,
    ...
    [openPanel beginWithCompletionHandler:^(NSInteger result) {
      // labrador_open_panel_file_selected must be called unconditionally to avoid a memory leak
      if (result == NSModalResponseOK) {
          dispatch_async(dispatch_get_main_queue(), ^{
            labrador_open_panel_file_selected([openPanel URLs], callback);
          });
      } else {
          dispatch_async(dispatch_get_main_queue(), ^{
            labrador_open_panel_file_selected([NSArray array], callback);
          });
      }
    }];
}
...
void open_save_file_picker(void *callback, NSString *defaultFilename, NSString *defaultDirectory) {
    ...
    [savePanel beginWithCompletionHandler:^(NSInteger result) {
      // labrador_save_panel_file_selected must be called unconditionally to avoid a memory leak
      if (result == NSModalResponseOK) {
          dispatch_async(dispatch_get_main_queue(), ^{
            labrador_save_panel_file_selected([savePanel URL], callback);
          });
      } else {
          dispatch_async(dispatch_get_main_queue(), ^{
            labrador_save_panel_file_selected(nil, callback);
          });
      }
    }];
}
...
void show_window_and_focus_app(LabradorWindow<LabradorWindowProtocol> *window, bool bringToFront) {
    previouslyActiveAppPID = [PreviousStateHelper storePreviousState];
...
void hide_window(LabradorWindow<LabradorWindowProtocol> *window) {
    NSRunningApplication *runningApp = [[NSWorkspace sharedWorkspace] frontmostApplication];
...
void set_window_title(id window, NSString *title) {
    if ([window isKindOfClass:[LabradorPanel class]] && [window isVisible]) {
...
void set_titlebar_height(id window, CGFloat height) {
    if ([window conformsToProtocol:@protocol(LabradorWindowProtocol)]) {
        [(id<LabradorWindowProtocol>)window configureTitlebarHeight:height];
    }
}

void position_and_order_front(LabradorWindow<LabradorWindowProtocol> *window) {
...
void position_at_given_location(LabradorWindow<LabradorWindowProtocol> *window, NSPoint origin) {
...
void order_front_without_focus(LabradorWindow<LabradorWindowProtocol> *window, NSPoint origin) {
