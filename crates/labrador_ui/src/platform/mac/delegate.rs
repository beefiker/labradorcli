...
use std::path::Path;
use std::sync::Arc;
use labrador_ui_core::clipboard::InMemoryClipboard;
use labrador_ui_core::keymap::Keystroke;
use labrador_ui_core::modals::{AlertDialog, ModalId};
use labrador_ui_core::notification::{NotificationSendError, RequestPermissionsOutcome};
...
use labrador_ui_core::ApplicationBundleInfo;
...
impl platform::Delegate for AppDelegate {
    ...
    fn open_character_palette(&self) {
        ...
        dispatch::Queue::main().exec_async(move || unsafe {
            ...
            // first responder's cursor location. In our case, that will be the Labrador
    ...
    fn show_native_platform_modal(&self, id: ModalId, modal: AlertDialog) {
        ...
        unsafe {
            let _: () = msg_send![app::get_labrador_app(), showModal: alert modalId: id];
        }
    }
...
pub unsafe extern "C-unwind" fn labrador_on_request_notification_permissions_completed(
...
/// This function is marked unsafe because it retrieves the pointer to the callback
/// function that we sent down to the Objective-C code.
pub unsafe extern "C-unwind" fn labrador_on_notification_send_error(
