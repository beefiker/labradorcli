...
use labrador_ui_core::rendering::{GPUBackend, GPUDeviceInfo, GPUDeviceType};
use wgpu::{
...
lazy_static! {
    ...
    /// 22.1.2, in practice, Labrador windows don't render properly until 24.0.2.
...
/// Returns whether or not the provided adapter is an unsupported Nvidia driver version for labrador_ui
/// to render properly.
...
/// Returns whether this adapter is the integrated OpenGL driver for Windows
/// running in Parallels, which caused problems with theme background images.
...
/// labrador_ui to render properly. Currently, we limit this to "Intel UHD Graphics 620", but we do have
...
pub fn adapter_has_rendering_offset_bug(adapter_info: &wgpu::AdapterInfo) -> bool {
    ...
    // Known affected Intel integrated GPU models based on user reports.
    let affected_models = [
...
/// (Vulkan llvmpipe) adapter that may not work properly with labrador_ui.
...
fn adapter_stability_sort_func(
    ...
    // Same with Nvidia drivers, though this is only an issue with a Wayland window server.
        ...
        log::warn!(
            ...
            https://docs.labrador.dev/help/known-issues#linux-1",
...
enum AdapterSupport {
    ...
    /// The adapter is basically not viable. The UI will either crash or not render.
    Unsupported = 2,
}
