...
use labrador_ui::fonts::FamilyId;
use labrador_ui::keymap::FixedBinding;
use labrador_ui::platform::CapturedFrame;
use labrador_ui::presenter::ChildView;
use labrador_ui::SingletonEntity as _;
use labrador_ui::{
...
impl RootView {
    pub fn new(ctx: &mut ViewContext<Self>, capture_config: CaptureConfig) -> Self {
        ...
        let sub_view = ctx.add_typed_action_view(move |ctx| {
            let font_family = labrador_ui::fonts::Cache::handle(ctx)
    ...
    fn capture_current_demo(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if let Some(window) = ctx.windows().platform_window(self.window_id) {
            ...
            if auto_capture && !is_last_demo {
                ctx.spawn(
                    async {
                        labrador_ui::r#async::Timer::after(std::time::Duration::from_millis(350)).await;
                    },
                    |_, _, ctx| {
    ...
    fn render(&self, _app: &AppContext) -> Box<dyn Element> {
        ...
        Container::new(
            Flex::column()
                ...
                .with_cross_axis_alignment(labrador_ui::elements::CrossAxisAlignment::Stretch)
                .with_child(
                ...
                if self.capture_config.capture_screenshots {
                    ctx.spawn(
                        async {
                            ...
                            labrador_ui::r#async::Timer::after(std::time::Duration::from_millis(300))
                                .await;
                        },
                        |view, _, ctx| {
                            view.capture_current_demo(ctx);
