...
impl Dirs {
    ...
    pub fn git_repository_fixture(&self) -> PathBuf {
        Labrador::fixtures().join("git_repository")
    }
}
...
impl VirtualFS {
    pub fn test(tag: &str, test_callback: impl FnOnce(Dirs, VirtualFS)) {
        ...
        let labrador_box_dir = root.path().join(tag);

        if PathBuf::from(&labrador_box_dir).exists() {
            std::fs::remove_dir_all(PathBuf::from(&labrador_box_dir))
                .expect("can not remove directory");
        }

        std::fs::create_dir(PathBuf::from(&labrador_box_dir)).expect("can not create directory");

        let tests = dunce::canonicalize(&labrador_box_dir).unwrap_or_else(|e| {
            panic!(
                "Couldn't canonicalize test path {}: {:?}",
                labrador_box_dir.display(),
                e
            )
        });
        ...
        let labrador_box = VirtualFS {
            root,
            cwd: labrador_box_dir,
            tests: tag.to_string(),
        };

        test_callback(directories, labrador_box);
    }
