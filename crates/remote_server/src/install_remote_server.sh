#!/usr/bin/env bash
# Installs the Labrador remote server binary on a remote host.
#
# Placeholders (substituted at runtime by setup.rs):
#   {download_base_url}  — e.g. https://app.labrador.dev/download/cli
#   {channel}            — stable | preview | dev
#   {install_dir}        — e.g. ~/.labrador/remote-server
