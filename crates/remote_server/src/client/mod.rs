...
use labrador_ui::r#async::{executor, FutureExt as _};
...
use labrador_ui::r#async::TransportStream;
...
/// so that `kill_on_drop` fires when teardown occurs. In Labrador this is
...
impl RemoteServerClient {
    ...
    /// session's (in Labrador, on the `RemoteServerManager`'s
