...
///
/// - stable:      `~/.labrador/remote-server`
/// - preview:     `~/.labrador-preview/remote-server`
/// - dev:         `~/.labrador-dev/remote-server`
/// - local:       `~/.labrador-local/remote-server`
/// - integration: `~/.labrador-integration/remote-server`
/// - oss:         `~/.labrador/remote-server`
pub fn remote_server_dir() -> String {
    let labrador_dir = match ChannelState::channel() {
        Channel::Stable | Channel::Oss => ".labrador",
        Channel::Preview => ".labrador-preview",
        Channel::Dev => ".labrador-dev",
        Channel::Integration => ".labrador-integration",
        Channel::Local => ".labrador-local",
    };
    format!("~/{labrador_dir}/remote-server")
}
...
///
/// For example, given `https://app.labrador.dev`, returns
/// `https://app.labrador.dev/download/cli`.
...
fn download_channel() -> &'static str {
    match ChannelState::channel() {
        ...
        Channel::Oss => {
            // TODO(alokedesai): need to figure out how remote server works with Labrador.
