...
use labrador_graphql::{object_permissions::AccessLevel, scalars::time::ServerTimestamp};
use labrador_ui_core::{
    Element,
    elements::{
...
impl TryFrom<labrador_graphql::object::ObjectType> for ObjectIdType {
    type Error = anyhow::Error;
    fn try_from(object_type: labrador_graphql::object::ObjectType) -> Result<Self, Self::Error> {
        match object_type {
            labrador_graphql::object::ObjectType::AIConversation => Err(anyhow!(
                "AIConversation is not a supported object type for this operation"
            )),
            labrador_graphql::object::ObjectType::Notebook => Ok(ObjectIdType::Notebook),
            labrador_graphql::object::ObjectType::Workflow => Ok(ObjectIdType::Workflow),
            labrador_graphql::object::ObjectType::Folder => Ok(ObjectIdType::Folder),
            labrador_graphql::object::ObjectType::GenericStringObject => {
                Ok(ObjectIdType::GenericStringObject)
            }
            labrador_graphql::object::ObjectType::Unknown => {
...
impl From<ObjectType> for labrador_graphql::object::ObjectType {
    fn from(value: ObjectType) -> Self {
        match value {
            ObjectType::Notebook => labrador_graphql::object::ObjectType::Notebook,
            ObjectType::Workflow => labrador_graphql::object::ObjectType::Workflow,
            ObjectType::Folder => labrador_graphql::object::ObjectType::Folder,
            ObjectType::GenericStringObject(GenericStringObjectFormat::Json(
                JsonObjectType::EnvVarCollection,
            )) => labrador_graphql::object::ObjectType::GenericStringObject,
...
pub struct CloudObjectMetadata {
    ...
    /// access to Labrador Drive as part of onboarding.
    pub is_welcome_object: bool,
    pub last_editor_uid: Option<String>,
    pub creator_uid: Option<String>,
...
impl CloudObjectStatuses {
    ...
    pub fn render_icon(
        ...
        let icon_and_tooltip_text = if should_show_local_only_indicator {
            Some((
                Icon::Laptop.to_labrador_ui_icon(theme.main_text_color(theme.surface_1())),
                SYNC_STATUS_TOOLTIP_LOCAL_ONLY,
            ))
        } else if should_show_syncing_indicator {
            Some((
                Icon::Refresh.to_labrador_ui_icon(theme.sub_text_color(theme.surface_2())),
                SYNC_STATUS_TOOLTIP_INFLIGHT,
            ))
        } else if should_show_error_indicator {
            Some((
                Icon::AlertTriangle.to_labrador_ui_icon(Fill::Solid(theme.ui_error_color())),
...
impl From<GenericStringObjectFormat>
    for labrador_graphql::generic_string_object::GenericStringObjectFormat
{
    fn from(format: GenericStringObjectFormat) -> Self {
        use labrador_graphql::generic_string_object::GenericStringObjectFormat as GraphQLFormat;
        match format {
...
impl From<CloudObjectEventEntrypoint> for labrador_graphql::object::CloudObjectEventEntrypoint {
    fn from(entrypoint: CloudObjectEventEntrypoint) -> Self {
        use labrador_graphql::object::CloudObjectEventEntrypoint as GraphQLEntrypoint;
        match entrypoint {
...
impl TryFrom<labrador_graphql::object::ObjectMetadata> for ServerMetadata {
    type Error = anyhow::Error;

    fn try_from(value: labrador_graphql::object::ObjectMetadata) -> Result<Self, Self::Error> {
        let folder_id: Option<FolderId> = match value.parent {
            labrador_graphql::object::Container::FolderContainer(folder_container) => {
...
impl TryFrom<labrador_graphql::object_permissions::ObjectPermissions> for ServerPermissions {
    type Error = anyhow::Error;

    fn try_from(
        value: labrador_graphql::object_permissions::ObjectPermissions,
...
impl TryFrom<labrador_graphql::object_permissions::ObjectGuest> for ServerObjectGuest {
    type Error = anyhow::Error;

    fn try_from(value: labrador_graphql::object_permissions::ObjectGuest) -> Result<Self, Self::Error> {
...
impl TryFrom<labrador_graphql::object_permissions::GuestSubject> for ServerGuestSubject {
    type Error = anyhow::Error;

    fn try_from(
        value: labrador_graphql::object_permissions::GuestSubject,
    ) -> Result<Self, Self::Error> {
        match value {
            labrador_graphql::object_permissions::GuestSubject::UserGuest(user_guest) => {
                let guest_subject = ServerGuestSubject::User {
                    firebase_uid: user_guest.firebase_uid.into_inner(),
                };
                Ok(guest_subject)
            }
            labrador_graphql::object_permissions::GuestSubject::PendingUserGuest(guest) => {
                Ok(ServerGuestSubject::PendingUser { email: guest.email })
            }
            labrador_graphql::object_permissions::GuestSubject::TeamGuest(team_guest) => {
                Ok(ServerGuestSubject::Team {
                    team_uid: ServerId::from_string_lossy(team_guest.uid.inner()),
                })
            }
            labrador_graphql::object_permissions::GuestSubject::Unknown => {
...
impl TryFrom<labrador_graphql::object_permissions::LinkSharing> for ServerLinkSharing {
    type Error = anyhow::Error;

    fn try_from(value: labrador_graphql::object_permissions::LinkSharing) -> Result<Self, Self::Error> {
...
impl TryFrom<labrador_graphql::object::Container> for ServerObjectContainer {
    type Error = anyhow::Error;

    fn try_from(value: labrador_graphql::object::Container) -> Result<Self, Self::Error> {
        match value {
            labrador_graphql::object::Container::FolderContainer(folder) => {
                Ok(ServerObjectContainer::Folder {
                    folder_uid: ServerId::from_string_lossy(folder.folder_uid.inner()),
                })
            }
            labrador_graphql::object::Container::Space(space) => Ok(ServerObjectContainer::Drive {
                owner: space.try_into()?,
            }),
            labrador_graphql::object::Container::Unknown => {
...
impl TryFrom<labrador_graphql::object::Space> for Owner {
    type Error = anyhow::Error;

    fn try_from(value: labrador_graphql::object::Space) -> Result<Self, Self::Error> {
        let owner = match value.type_ {
            labrador_graphql::object::SpaceType::Team => Owner::Team {
                team_uid: ServerId::from_string_lossy(value.uid.inner()),
            },
            labrador_graphql::object::SpaceType::User => Owner::User {
...
impl From<Owner> for labrador_graphql::object_permissions::Owner {
    fn from(owner: Owner) -> Self {
        use labrador_graphql::object_permissions::Owner as GraphQLOwner;
        use labrador_graphql::object_permissions::OwnerType;
        match owner {
