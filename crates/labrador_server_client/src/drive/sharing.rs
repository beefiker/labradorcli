...
use labrador_graphql::object_permissions::AccessLevel;
...
/// A kind of user. In all cases, there is an underlying Labrador account, but it's represented
/// differently in certain cases.
#[derive(Debug, Clone)]
pub enum UserKind {
    /// A Labrador user account, tracked in the [`UserProfiles`] model.
    Account(UserUid),
