...
pub enum FeatureFlag {
    ...
    /// If `true`, fetch updated Labrador channel versions from the Labrador server endpoint instead of
    /// from GCP directly.
    FetchChannelVersionsFromLabradorServer,
    ...
    /// Used to gate an experiment we're doing on LabradorDev ONLY
    ...
    /// Whether the user is part of the Labrador Alpha Program (AI Trusted Testers).
    ...
    /// Enable multiselect in Notebooks and Labrador Text.
    RichTextMultiselect,
    ...
    /// Enables next action prediction within Labrador, powered by AI.
    AgentPredict,

    /// Enables receiving shared Labrador Drive objects.
    SharedWithMe,
    ...
    /// Playground for reducing Labrador UI clutter.
    MinimalistUI,
    ...
    /// Adds aliases for executing Labrador Drive workflows.
    WorkflowAliases,
    ...
    /// Enables support for Labrador Packs.
    LabradorPacks,
    ...
    /// Enables Labrador Drive objects (like workflows) as context in AI context menu
    DriveObjectsAsContext,

    /// Expands code diff edits to replace the current pane instead of opening in a new tab.
    ExpandEditToPane,
    /// Enables fallback model load output messaging in the agent status indicator.
    ...
    /// Enables the /create-environment slash command for setting up Labrador Environments
    ...
    /// Enables Labrador managed secrets functionality.
    LabradorManagedSecrets,
    ...
    /// Enables agent tips displayed below the agent status indicator in Agent Mode.
    AgentTips,
    ...
    /// Enables auto-syncing ambient plans to Labrador Drive.
    SyncAmbientPlans,

    /// Enables platform skills support (--skill flag) for agent runs.
    ///
    /// Skills are loaded from `.agents/skills/`, `.labrador/skills/`, `.claude/skills/`, and `.codex/skills/`
    ...
    /// Enables the OpenLabrador launch modal announcing Labrador going open-source.
    /// When enabled, the HOA onboarding flow is suppressed.
    OpenLabradorLaunchModal,
    ...
    /// Ctrl-G intercepts the keystroke and opens Labrador's input editor instead of $EDITOR.
    CLIAgentRichInput,
    ...
    /// Enables the new settings to control visibility of Labrador Drive, Code Review Panel,
    /// and Project Explorer & Global Search features.
    OpenLabradorNewSettingsModes,
    ...
    /// CLIs (e.g. `claude`) to execute prompts instead of Labrador's agent harness.
    AgentHarness,

    /// Enables the install/update chip for the OpenCode Labrador plugin.
    OpenCodeNotifications,

    /// Enables the install/update chip for the Codex Labrador notification plugin.
    CodexNotifications,

    /// Enables the install/update chip for the Gemini CLI Labrador extension.
    GeminiNotifications,
    ...
    /// Replaces the in-block Labradorification banner with a Labradorify footer.
    LabradorifyFooter,
...
pub const DOGFOOD_FLAGS: &[FeatureFlag] = &[
    ...
    // These are enabled via 100% experiment on prod Labrador server,
...
/// Features enabled for feature preview build users (e.g.: Friends of Labrador).
/// All PREVIEW_FLAGS are also automatically added to dogfood builds (LabradorDev).
...
/// Features enabled for all release builds (i.e.: everything but LabradorLocal).
...
pub const RELEASE_FLAGS: &[FeatureFlag] = &[
    ...
    // Promoted from PREVIEW_FLAGS for labrador: the in-pane commit / push / Create-PR
    // actions are labrador's primary surface for the local-LLM PR/commit-message
...
impl FeatureFlag {
    ...
    pub fn flag_description(&self) -> Option<&'static str> {
        ...
        match self {
            ...
            CloudEnvironments => Some("Enables creating and managing Labrador Environments via the CLI."),
            CreateEnvironmentSlashCommand => Some("Enables the /create environment slash command for setting up Labrador Environments with custom configurations."),
            ...
            SettingsFile => Some("Enables configuring Labrador via a user-editable `settings.toml` file, with hot reload and error reporting for invalid values."),
