...
impl<'a> Mouse<'a> {
    ...
    pub fn move_to(&mut self, target: Vector2I) -> Result<(), String> {
        // Use the X11 pointer reposition request instead of XTEST MotionNotify
        // so the server's pointer position is updated reliably and subsequent
        ...
        self.conn
            ...
            .map_err(|e| format!("Failed to move pointer: {e}"))?;
