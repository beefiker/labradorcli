...
impl SelectedSettings {
    pub fn is_ai_enabled(&self) -> bool {
        ...
        match self {
            ...
            SelectedSettings::Terminal { .. } => {
                // With old onboarding (no OpenLabradorNewSettingsModes), Terminal
    ...
    pub fn is_labrador_drive_enabled(&self) -> bool {
        match self {
            SelectedSettings::AgentDrivenDevelopment {
                ui_customization, ..
            } => ui_customization
                .as_ref()
                .map(|ui| ui.show_labrador_drive)
                .unwrap_or(true),
            SelectedSettings::Terminal {
                ui_customization, ..
            } => ui_customization
...
impl OnboardingStateModel {
    ...
    pub(crate) fn settings(&self) -> SelectedSettings {
        use labrador_core::features::FeatureFlag;
        let ui_customization = if FeatureFlag::OpenLabradorNewSettingsModes.is_enabled() {
    ...
    pub(crate) fn set_tools_panel_enabled(&mut self, enabled: bool, ctx: &mut ModelContext<Self>) {
        ...
        self.ui_customization.show_labrador_drive = enabled;
        ctx.notify();
    }
    ...
    pub(crate) fn set_show_labrador_drive(&mut self, value: bool, ctx: &mut ModelContext<Self>) {
        if self.ui_customization.show_labrador_drive == value {
            return;
        }
        self.ui_customization.show_labrador_drive = value;
        ctx.notify();
    }
    ...
    pub(crate) fn set_models(
        ...
        let is_past_agent_slide = if FeatureFlag::OpenLabradorNewSettingsModes.is_enabled() {
            matches!(
                self.step,
    ...
    pub(crate) fn back(&mut self, ctx: &mut ModelContext<Self>) {
        use labrador_core::features::FeatureFlag;
        let theme_picker_last = FeatureFlag::OpenLabradorNewSettingsModes.is_enabled();
