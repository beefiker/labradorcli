...
/// User-facing names of the Labrador Drive features enabled when the terminal
/// intention is selected with Labrador Drive turned on. Shared by the login slide's
/// skip-login confirmation dialog so the list stays in sync with any future
/// surfaces that need it.
pub fn labrador_drive_features() -> Vec<String> {
    vec![
        labrador_core::channel::ChannelState::app_name_with_suffix("Drive"),
...
pub fn init(app: &mut labrador_ui::AppContext) {
