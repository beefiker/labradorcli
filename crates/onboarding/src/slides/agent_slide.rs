...
use labrador_ui::{
    elements::{
        ...
        Icon as LabradorUiIcon, MainAxisAlignment, MainAxisSize, MouseStateHandle, OffsetPositioning,
...
impl button::Theme for UpgradeButtonTheme {
    fn background(
        ...
        appearance: &Appearance,
    ) -> Option<labrador_core::ui::theme::Fill> {
    ...
    fn text_color(
        &self,
        background: Option<labrador_core::ui::theme::Fill>,
        appearance: &Appearance,
    ) -> ColorU {
        let bg = background
...
pub struct AgentSlide {
    onboarding_state: labrador_ui::ModelHandle<OnboardingStateModel>,
...
impl AgentSlide {
            ...
            match event {
                ...
                OnboardingStateEvent::AuthStateChanged => {
                        ...
                        && me.last_auth_state != OnboardingAuthState::PayingUser
                    {
                        ...
                        let _ = ctx.spawn(
                            labrador_ui::r#async::Timer::after(PLAN_ACTIVATED_TOAST_DURATION),
                            |me: &mut Self, _, ctx| {
    ...
    fn render_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        let title = appearance
            ...
            .paragraph(format!(
                "Customize your {}",
                labrador_core::channel::ChannelState::app_name_with_suffix("Agent")
            ))
    ...
    fn render_collapsed_model_chip(
        ...
        let hoverable = Hoverable::new(mouse_state, move |_| {
            ...
            let title_row: Box<dyn Element> = if let Some(icon) = icon {
                ...
                let icon_el =
                    ConstrainedBox::new(Box::new(icon.to_labrador_ui_icon(title_color.into())))
            ...
            let chevron = ConstrainedBox::new(Box::new(LabradorUiIcon::new(
    ...
    /// (Recommended for the default row). Rows are always selectable in Labrador.
    ...
    fn render_model_row(
        ...
        let hoverable_body = Hoverable::new(mouse_state, move |_| {
            ...
            const ICON_SIZE: f32 = 14.;
            let icon_el = ConstrainedBox::new(Box::new(icon.to_labrador_ui_icon(title_color.into())))
                .with_width(ICON_SIZE)
                .with_height(ICON_SIZE)
                .finish();
    ...
    fn render_bottom_nav(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let step_index = 2;
        let step_count =
            if labrador_core::features::FeatureFlag::OpenLabradorNewSettingsModes.is_enabled() {
                5
            } else {
                4
            };
    ...
    fn render_auth_prompt_bar(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let icon = ConstrainedBox::new(Box::new(
            Icon::AlertCircle.to_labrador_ui_icon(Fill::Solid(text_color)),
        ))
        .with_width(ICON_SIZE)
        .with_height(ICON_SIZE)
        .finish();
    ...
    fn render_plan_activated_toast(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let check_icon = ConstrainedBox::new(Box::new(
            Icon::CheckSkinny.to_labrador_ui_icon(Fill::Solid(text_color)),
        ))
        .with_width(ICON_SIZE)
        .with_height(ICON_SIZE)
        .finish();
...
impl View for AgentSlide {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        stack.add_child(
            labrador_ui::elements::Align::new(bottom_overlay)
                .bottom_center()
                .finish(),
        );
        stack.finish()
