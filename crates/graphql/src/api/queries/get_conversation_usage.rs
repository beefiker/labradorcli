...
// The server still exposes legacy usage field names; keep the Rust-facing
// fragment fields Labrador-named and map them with `#[cynic(rename = ...)]`.
...
fn convert_token_usage(
    ...
    for usage in labrador_token_usage {
        ...
        entry.labrador_tokens += u32::try_from(usage.total_tokens).unwrap_or_default();
...
impl From<&ConversationUsageMetadata> for persistence::model::ConversationUsageMetadata {
    fn from(gql: &ConversationUsageMetadata) -> Self {
        Self {
            ...
            token_usage: convert_token_usage(&gql.labrador_token_usage, &gql.byok_token_usage),
