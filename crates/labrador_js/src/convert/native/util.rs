...
use super::{FromLabradorJs, JsFunctionRegistry};
    ...
    T: FromLabradorJs<'js>,
{
    ...
    if value.is_array() {
        Vec::<T>::from_labrador_js(ctx, value, js_function_registry)
    } else {
        Ok(vec![T::from_labrador_js(ctx, value, js_function_registry)?])
    }
}
    ...
    T: FromLabradorJs<'js>,
{
    ...
    T::from_labrador_js(ctx, value, js_function_registry)
}
