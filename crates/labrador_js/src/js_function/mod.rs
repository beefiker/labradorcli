...
/// types.
///
/// `I` is the type of the function's input, which must implement `IntoLabradorJs` and be
/// deserializable.
/// `O` is the type of the function's return value, which must implement `FromLabradorJs` and be
/// serializable.
