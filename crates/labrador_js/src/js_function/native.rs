...
use crate::{FromLabradorJs, IntoLabradorJs, JsFunctionId, SerializedJsValue, TypedJsFunctionRef};

impl<I, O> TypedJsFunctionRef<I, O>
where
    I: for<'a> IntoLabradorJs<'a> + DeserializeOwned + Clone + 'static,
    O: for<'a> FromLabradorJs<'a> + Serialize + Clone + 'static,
{
...
impl JsFunctionRegistry {
    ...
    ///
    /// `I` and `O` must have corresponding `IntoLabradorJs`/`FromLabradorJs` implementations so they
...
pub trait CallableJsFunction {
    ...
    /// `IntoLabradorJs`-implemmenting Rust type, and returns the serialized bytes representation of
    /// the function's output (which is of a Rust type that implemenets `FromLabradorJs`).
    fn call(
        &self,
    ...
    O: for<'a> FromLabradorJs<'a> + Serialize + Clone + 'static,
{
    fn call(
        ...
        let input_value = input.into_labrador_js(ctx)?;
        let func = self.js_function.clone().restore(ctx)?;
        let output = O::from_labrador_js(ctx, func.call((input_value,))?, function_registry)?;
...
///
/// `I` is the type of the function's input, which must implement `IntoLabradorJs`.
/// `O` is the type of the function's return value, which must implement `FromLabradorJs`.
#[derive(Clone)]
...
where
    I: for<'a> IntoLabradorJs<'a> + DeserializeOwned + 'static,
    O: for<'a> FromLabradorJs<'a> + Serialize + 'static,
{
