...
Assumptions:
- The fallback fonts in the prod and staging static asset buckets are identical.
...
4. Set `LABRADOR_STATIC_ASSETS_BUCKET` to override the default static assets bucket.
5. Run `python3 generate-mappings.py`.
'''
...
def download_fallback_fonts():
    ...
    for uri_prefix in candidate_fallback_font_prefixes():
        command = f"gcloud storage cp '{uri_prefix}**/*Regular*.ttf' '{FONT_DOWNLOAD_DIR}'"
    ...
    sys.exit("Failed to download fonts from Labrador or legacy static assets buckets")
