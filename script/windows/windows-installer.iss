...
#define MyAppURL "https://www.labrador.dev/"
#ifndef MyAppName
  #define MyAppName "LabradorDev"
#endif
#ifndef MyAppVersion
  #define MyAppVersion "0.1.0"
#endif
#ifndef MyAppExeName
  #define MyAppExeName "labrador-dev.exe"
#endif
...
#define ChannelPascalCase \
  ...
  "Unknown")))))
#define AppMutexName "Local\Labrador" + ChannelPascalCase + "_SingleInstance"
#define AppUrlScheme \
  (ReleaseChannel == "stable") ? "labrador" : \
  ((ReleaseChannel == "dev") ? "labradordev" : \
  ((ReleaseChannel == "preview") ? "labradorpreview" : \
  ((ReleaseChannel == "local") ? "labradorlocal" : \
  ((ReleaseChannel == "integration") ? "labradorintegration" : \
  ((ReleaseChannel == "oss") ? "labrador" : \
  "labrador")))))
...
AppId=labrador-terminal-{#ReleaseChannel}
AppName={#MyAppName}
...
WizardStyle=modern
WizardSmallImageFile="installer-images\labrador-logo.bmp"
WizardImageFile="installer-images\labrador-banner.bmp"
SetupIconFile="..\..\app\channels\{#ReleaseChannel}\icon\no-padding\icon.ico"
UninstallDisplayIcon="{app}\icon.ico"
; Force close previous Labrador if it hasn't shut down yet.
; In the update flow we already warn the user if they have something running and make them confirm
; before running this installer. Therefore, we are good to force close Labrador without fear of losing
; unsaved work.
...
; For manual installs: if Labrador is running, show a dialog prompting the user to close it
...
[Registry]
Root: HKCU; Subkey: "SOFTWARE\Labrador.dev\{#MyAppName}"; Flags: uninsdeletekey
; cleanup "Open Labrador Here" registry entries
...
; Add "Open Labrador in new tab" to directory context menu
...
; Add "Open Labrador in new tab" to directory background context menu
...
; Add "Open Labrador in new window" to directory context menu
...
; Add "Open Labrador in new window" to directory background context menu
...
[Tasks]
Name: addToPath; Description: "Add Labrador to PATH"

[UninstallDelete]
Type: filesandordirs; Name: "{userappdata}\labrador\{#MyAppName}"
Type: filesandordirs; Name: "{localappdata}\labrador\{#MyAppName}"
Type: filesandordirs; Name: "{app}\bin"

[Icons]
Name: "{autoprograms}\{#MyAppName}"; Filename: "{app}\{#MyAppExeName}"; IconFilename: "{app}\icon.ico"; AppUserModelID: "dev.labrador.{#MyAppName}"
Name: "{autodesktop}\{#MyAppName}"; Filename: "{app}\{#MyAppExeName}"; IconFilename: "{app}\icon.ico"; AppUserModelID: "dev.labrador.{#MyAppName}"; Tasks: desktopicon
...
{ Returns true when the installer was launched by Labrador's auto-update code.
  The auto-update path passes /update=1 on the command line and /NOCLOSEAPPLICATIONS
  so that the installer does not forcibly kill the running Labrador process. Instead we
  wait for Labrador to exit naturally by polling the app mutex below. }
...
{ For background updates, return an empty mutex name so that Inno Setup skips its
  ...
  installs, return the real mutex name so the user is prompted to close Labrador first. }
...
var
  ...
  ResultCode: Integer;
begin
  { Background update: the installer was launched while Labrador was still running.
    We passed /NOCLOSEAPPLICATIONS so Inno won't kill it. Wait here - before any
    files are touched - for Labrador to release its single-instance mutex, which
    happens as part of normal process exit. }
  if CurStep = ssInstall then
  begin
    if IsBackgroundUpdate() then
    begin
      Log('Background update: waiting for Labrador to exit (mutex: {#AppMutexName})...');
      WaitCounter := 0;
      ...
      begin
        Log('Labrador mutex still held after timeout; force-killing remaining processes.');
        { Kill by image name. {#MyAppExeName} (e.g. labrador.exe, labrador-dev.exe) is unique
          ...
          rely on Labrador's Job Object (JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE) to
          cascade-kill any child OpenConsole.exe processes when Labrador dies. }
        ...
        Log('Labrador has exited; proceeding with file installation.');
    end;
  end;

  { After a successful install, write a helper script for running the Labrador CLI. }
  { We use this to add a channel-specific wrapper (e.g. "oz-preview.cmd" vs. "labrador-preview.exe") }
  if CurStep = ssPostInstall then begin
    { Add Labrador to PATH if requested }
