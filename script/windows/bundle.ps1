...
Param (
    ...
    # Builds only the Labrador binary, skips the installer.
    [Switch]$SKIP_BUILD_INSTALLER = $False,
    # Builds only the installer, skips the Labrador binary. Use this if the Labrador
...
if ($ARCH -eq 'arm64') {
    ...
    # If x64, then we just use the filename "LabradorSetup.exe" for example
    $FILE_ENDING = 'Setup'
...
$BUNDLE_ID = "dev.labrador.$app_name"
...
#
# LABRADOR_BIN is the name of the binary produced by cargo;
# BINARY_NAME is the desired name of the binary in the final package.
if ("$CHANNEL" -eq 'local') {
    $LABRADOR_BIN = 'labrador-local'
    $BINARY_NAME = 'labrador-local.exe'
    $APP_NAME = 'LabradorLocal'
    $FEATURES = "$FEATURES,nld_improvements"
} elseif ("$CHANNEL" -eq 'dev') {
    $LABRADOR_BIN = 'dev'
    $BINARY_NAME = 'labrador-dev.exe'
    $APP_NAME = 'LabradorDev'
    $FEATURES = "$FEATURES,agent_mode_debug,nld_improvements"
} elseif ("$CHANNEL" -eq 'preview') {
    $LABRADOR_BIN = 'preview'
    $BINARY_NAME = 'labrador-preview.exe'
    $APP_NAME = 'LabradorPreview'
    $FEATURES = "$FEATURES,preview_channel,nld_improvements"
} elseif ("$CHANNEL" -eq 'stable') {
    $LABRADOR_BIN = 'stable'
    $BINARY_NAME = 'labrador.exe'
    $APP_NAME = 'Labrador'
...
$PDB_PATH = "$CARGO_TARGET_OUTPUT_DIR\$LABRADOR_BIN.pdb"
...
if ($CHECK_ONLY) {
    cargo check -p labrador --profile "$CARGO_PROFILE" --bin "$LABRADOR_BIN" --features "$FEATURES" --target $PLATFORM_TARGET
    if (-Not $?) {
        Write-Error "Failed to verify Labrador $LABRADOR_BIN compilation with profile $CARGO_PROFILE"
        exit 1
    }
    exit 0
}
...
if (-Not $SKIP_BUILD_BINARY) {
    Write-Output "Building Labrador for channel $CHANNEL and bundle id $BUNDLE_ID"
    $env:CARGO_BIN_NAME = $CHANNEL
    $env:LABRADOR_APP_NAME = $APP_NAME
    cargo build -p labrador --profile "$CARGO_PROFILE" --bin "$LABRADOR_BIN" --features "$FEATURES" --target $PLATFORM_TARGET
    if (-Not $?) {
        Write-Error "Failed to build Labrador $LABRADOR_BIN binary with profile $CARGO_PROFILE"
        exit 1
    }

    # If we desire an executable name different from the cargo bin, rename it.
    if ("$LABRADOR_BIN.exe" -ne $BINARY_NAME) {
        $binarySource = "$CARGO_TARGET_OUTPUT_DIR\$LABRADOR_BIN.exe"
        Write-Output "Renaming executable $LABRADOR_BIN.exe to $BINARY_NAME"
...
Write-Output 'Building Labrador installer'
$ISCC_ARGS = @(
