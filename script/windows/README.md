...
a configuration file for building a Labrador installer.
...
* Run `cargo build` to ensure the installer uses the latest version of Labrador.
...
```shell
.\script\windows\Output\Labrador-Windows-Setup.exe`.
```
...
* `MyAppExeName` (default: `labrador-dev.exe`)
