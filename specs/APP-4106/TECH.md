...
- `app/src/workspace/view/left_panel.rs:1213` — `deduplicate_by_directory_name`
  ...
  happen upstream).
- `crates/labrador_util/src/path.rs` + `path_test.rs` — target location for the
  shared `group_roots_by_common_ancestor` helper.
- `crates/labrador_util/src/standardized_path.rs` — `StandardizedPath` impls
...
### 1. Shared helper in `labrador_util::path`

Add a generic ancestor-dedup helper usable by both views.

```rust path=null start=null
// crates/labrador_util/src/path.rs
...
Place unit tests next to the impl in `crates/labrador_util/src/path_test.rs`
...
fn group_std_roots_by_common_ancestor(
    ...
    labrador_util::path::group_roots_by_common_ancestor(roots)
}
```
...
pub fn set_root_directories(&mut self, roots: Vec<PathBuf>, _ctx: &mut ViewContext<Self>) {
    let grouping = labrador_util::path::group_roots_by_common_ancestor(&roots);
...
sequenceDiagram
    ...
    participant U as labrador_util::path
...
### Unit tests (`crates/labrador_util/src/path_test.rs`)
...
- **Focus-follow on cd**: simulate cd-ing into `~/code/labrador-server` with
  `~/code` as the ancestor. Assert `selected_item` lands on
  `~/code/labrador-server`, and `pending_focus_target.scrolled` is `true`.
...
- **Click preserves file selection**: seed `~/code`, expand
  `~/code/labrador-server`, select `main.rs`. Simulate a
  `DirectoriesChanged` emitting `[labrador-server, code]`. Assert selection
...
`labrador_util::path::group_roots_by_common_ancestor`, their surviving-root
