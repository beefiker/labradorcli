...
Successfully rendered file-backed images and Mermaid diagrams should both reuse Labrador’s existing fullscreen lightbox treatment for click-to-expand viewing.
...
Labrador already supports Markdown image blocks elsewhere in the app and already has Mermaid-to-SVG rendering for notebook Markdown. The AI block list currently lacks the equivalent presentation and interaction model. That creates three problems:

- visual content in AI responses is harder to understand because users see raw Markdown instead of the image or diagram
- rendered output is inconsistent with other Markdown-capable surfaces in Labrador
...
- Ensure the feature works across Labrador’s supported platforms, including WASM.
- Use the same supported image format set that Labrador’s existing Markdown/image rendering stack already supports today: JPEG/JPG, PNG, GIF, WebP, and SVG.
...
- Reuse Labrador’s existing fullscreen lightbox treatment for both rendered file-backed images and rendered Mermaid diagrams.
...
- Supporting additional image formats beyond the set Labrador already renders today.
...
2. Mermaid diagrams authored in Markdown that Labrador renders into an image representation at runtime.
...
### Supported formats
For file-backed images, the AI block list should support the same image formats Labrador’s existing shared image renderer already supports today:
- JPEG / JPG
- PNG
- GIF
- WebP
...
If the referenced file exists but is not one of those supported formats, Labrador should not attempt a degraded or best-effort render. It should show the original Markdown text exactly as it does today.
...
1. Inline image runs
When multiple parser-recognized Markdown image references appear on the same line, separated only by whitespace, Labrador should render them as a horizontal image row:
...
- if that path points to a binary file such as PNG, JPG, GIF, or WebP, activating the path should use the platform-default opener instead of Labrador's editor or the user's configured `$EDITOR`
...
For file-backed images that render successfully, Labrador should also show the referenced file link/path in the layout appropriate for that treatment:
...
- binary file links should open with the operating system's default file opener; on macOS this means using `open` rather than routing through Labrador's editor or `$EDITOR`
...
- clicking a rendered image opens it in Labrador’s existing fullscreen lightbox treatment
- the lightbox should reuse the same fullscreen overlay treatment already used elsewhere in Labrador rather than introducing a new one-off viewer
...
then Labrador should show the original Markdown image syntax inline, matching current behavior, instead of rendering an error-specific image container.

For Mermaid diagrams, if rendering fails, Labrador should show the original Mermaid Markdown source instead of a rendered diagram.
...
- Activating the source path for a rendered binary asset opens it with the operating system's default app instead of Labrador's editor or the user's configured `$EDITOR`.
...
- If a file cannot be found or the file format is unsupported, Labrador shows the original Markdown text rather than a broken rendered state.
- If Mermaid rendering fails, Labrador shows the original Mermaid Markdown source.
...
- Manual validation that clicking the source path for a rendered PNG or other binary image opens it with the platform-default app rather than Labrador's editor.
