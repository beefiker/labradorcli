...
- Verify `@` opens the context menu with files/folders and code symbols (no Labrador-specific categories) in CLI agent rich input.
...
- Verify no regressions in normal Labrador agent input (`@` context still works as before).
