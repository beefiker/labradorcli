...
2. **Multiplexing**: multiple Labrador tabs SSH-ing to the same host must share a single
...
- `crates/labrador_cli/src/lib.rs` — `WorkerCommand::RemoteServerProxy` / `RemoteServerDaemon`
...
connects to `~/.labrador[-channel]/remote-server/server.sock`.
...
task on LabradorUI's background executor with a dedicated `async_channel` sender.
...
`WorkerCommand::RemoteServerProxy` in `labrador_cli/src/lib.rs` dispatches to
`unix::run_proxy()`.
...
has access to LabradorUI's `ModelContext`. It binds an `async_io::Async<UnixListener>`,
then for each accepted connection spawns a task on the LabradorUI background executor
...
are dispatched through `ModelSpawner` onto the single-threaded LabradorUI main loop, so they
cannot interleave.
