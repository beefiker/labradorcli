# TECH.md — Rename Oz to Labrador Agent in settings and onboarding
...
This is a rename of the in-app agent from "Oz" to "Labrador Agent" across user-facing
...
1. `app/src/settings_view/mod.rs`
   - Rename `SettingsSection::Oz` variant to `SettingsSection::LabradorAgent`.
   - In the `Display` impl, the `LabradorAgent` arm writes `"Labrador Agent"`.
   - In the `FromStr` impl, accept both `"Oz"` (backward-compat legacy name)
     and `"Labrador Agent"` as parseable forms that map to
     `SettingsSection::LabradorAgent`, per Behavior #8 in `product.md`.
   - Update `is_ai_subpage`, `ai_subpages()`, and the two default-subpage
     fallbacks (`SettingsSection::AI => SettingsSection::LabradorAgent`) to use the
     new variant name.
...
2. `app/src/settings_view/ai_page.rs`
   ...
   - Replace all remaining user-visible description strings that referenced
     ...
     allowlist/denylist descriptions, Rules description, Labrador Drive context
...
3. `crates/onboarding/src/slides/agent_slide.rs`
   - `render_header`: change paragraph text from `"Customize your Agent, Oz"`
     to `"Customize your Labrador Agent"`. Keep font size, weight, layout, and
     surrounding subtitle unchanged.
   - `render_disable_oz_section`: change checkbox label from `"Disable Oz"` to
     `"Disable Labrador Agent"`. Keep styling, spacing, `disable_oz_mouse` state
...
4. Navigation call-sites (approximately 15 files)
   - All `SettingsSection::Oz` references in navigation actions, workspace
     ...
     `SettingsSection::LabradorAgent`. Affected files include:
...
- `cargo fmt` and `cargo clippy --workspace --all-targets --all-features --tests
  -- -D warnings` must pass (per `LABRADOR.md` PR workflow).
- `cargo nextest run -p labrador --no-fail-fast` or the relevant subset covering
  `settings_view::mod_test` must pass. The Display test asserts
  `SettingsSection::LabradorAgent.to_string() == "Labrador Agent"`. The `FromStr` test
  covers both `"Oz"` and `"Labrador Agent"` resolving to
  `SettingsSection::LabradorAgent`, exercising Behavior #8. All helper tests
  (`is_ai_subpage`, `ai_subpages_list_contains_all_ai_subpage_variants`,
  filter/visibility tests) are updated to reference `SettingsSection::LabradorAgent`.
...
- Behavior #1, #2, #3, #9: manually open the settings UI and confirm the
  sidebar entry reads "Labrador Agent", the subpage renders unchanged content, the
  heading above the global toggle reads "Labrador Agent", and the "Oz Cloud API
...
- Behavior #7: search within the settings modal using each of
  `"labrador agent"`, `"ai"`, `"agent"`, `"oz"` (should still reach the subpage) and
  `"oz cloud"` (should reach the cloud subpage only).
- Behavior #8: confirm both `"Oz"` and `"Labrador Agent"` resolve to
  `SettingsSection::LabradorAgent` via the `FromStr` round-trip test.
- Behavior #11: no automated accessibility test exists for these labels; manual
  ...
  accessible label.
- Behavior #12: toggle the `OpenLabradorNewSettingsModes` feature flag and confirm
  the disable row only appears when enabled and always reads "Disable Labrador
...
- Risk: external deep links or persisted telemetry strings reference `"Oz"` and
  break. Mitigation: `FromStr` accepts both `"Oz"` and `"Labrador Agent"` mapping to
  `SettingsSection::LabradorAgent`, and the legacy `"oz"` search term is preserved so
...
- Risk: stale comments inside `agent_slide.rs` that still reference "Disable Oz"
  ...
  comments describing them are acceptable to leave as-is per `LABRADOR.md`.
