...
Add install and update chip support for the Gemini CLI Labrador plugin (`gemini-warp`), matching the existing Claude Code chip UX. Enable the listener so Labrador processes structured notifications from Gemini sessions (session status, permission requests, task completion).

## Problem

The `gemini-warp` extension already exists and uses the same structured OSC 777 notification protocol as Claude Code (`labrador://cli-agent` title, JSON body with `agent: "gemini"`). However, Labrador currently ignores these notifications because:
...
- Changing the Gemini CLI Labrador plugin itself (it's maintained separately in `warpdotdev/gemini-cli-warp`).
...
- **Hooks**: Defined in `hooks/hooks.json` within the extension directory (not in the manifest). The Gemini CLI Labrador plugin uses `SessionStart`, `AfterAgent`, `Notification`, `BeforeAgent`, and `AfterTool` hooks.
...
- Title: "Install Labrador Plugin for Gemini CLI"
- Subtitle: "Run the following command, then restart Gemini CLI."
- Steps:
  1. "Install the Labrador extension" — command: `gemini extensions install https://github.com/warpdotdev/gemini-cli-warp --consent`
...
- Clicking triggers auto-update via `gemini extensions update gemini-warp`.
  - The installed extension name is `gemini-warp` (the directory name under `~/.gemini/extensions/`).
- On success, a toast confirms the update and tells the user to restart Gemini.
- On failure, a toast appears with an error, and the user can click the info (ⓘ) button to open manual instructions in a split pane.

**Manual update instructions (split pane):**
- Title: "Update Labrador Plugin for Gemini CLI"
- Subtitle: "Run the following command, then restart Gemini CLI."
- Steps:
  1. "Update the Labrador extension" — command: `gemini extensions update gemini-warp`
...
1. Starting a Gemini CLI session in Labrador when the plugin is not installed shows a green install chip.
