...
- `crates/labrador_features/src/lib.rs:760-769` — existing `HOANotifications`, `OpenCodeNotifications`, `CodexNotifications` flags
...
### 4a. Feature Flag (`crates/labrador_features/src/lib.rs`)
...
`installed_version()` — reads `~/.gemini/extensions/gemini-warp/gemini-extension.json`, parses the `version` field. The JSON structure is flat: `{"name": "labrador", "version": "1.0.0", ...}`.
...
### Install
1. User starts Gemini CLI in Labrador. `CLIAgent::Gemini` detected, session created.
...
- **Publish `warpdotdev/gemini-cli-warp` to GitHub** — must happen before shipping to external users.
...
- **Modified:** `crates/labrador_features/src/lib.rs` — add `GeminiNotifications` variant + `DOGFOOD_FLAGS`
