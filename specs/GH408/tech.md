...
- `crates/labrador_util/src/path.rs (149-181)` — `CleanPathResult::with_line_and_column_number()`, which strips line/column suffixes from the path string. Called before path resolution.
...
2. **Manual test:** Run Labrador, type `/open-file ~/.bashrc`, confirm it opens.
