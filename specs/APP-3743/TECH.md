...
`OpenWorktreeInRepo { repo_path }`: loads `~/.labrador/default-tab-configs/worktree.toml` (created from embedded template if missing), substitutes template variables, and opens the tab immediately.
...
Params substituted: `repo` (selected path), `branch_name` (auto-generated via `generate_worktree_branch_name()`), `pane_type` (from default session mode). On macOS, the data directory is channel-specific (`~/.labrador-local/` for Local, `~/.labrador/` for Stable).
...
- Build check: `cargo check -p labrador` passes with no errors.
