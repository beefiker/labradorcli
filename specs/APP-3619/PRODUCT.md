...
When the Labrador notification plugin can't be auto-installed (SSH session, or a previous install attempt failed), the user currently has no way to learn how to install it manually. We need a modal that shows step-by-step manual installation instructions.

## Current Behavior

- A green "Install Labrador plugin" chip appears in the CLI agent footer when the plugin isn't installed (`agent_input_footer/mod.rs:611-633`)
...
`should_show_install_plugin_button` hides the chip when `manager.is_installed()` returns true. But `is_installed()` reads the **local** filesystem (`~/.claude/plugins/installed_plugins.json`), not the remote machine's. In any remote session (labradorified SSH, legacy SSH, Docker via SSH) where Claude Code runs on the remote, this check is wrong:
...
- Chip label: "Install Labrador plugin"
- Chip tooltip: "Install the Labrador plugin to enable rich agent notifications within Labrador"
...
- Chip tooltip: "View instructions to install the Labrador plugin"
...
- Title from `PluginInstallInstructions.title` (e.g. "Install Labrador Plugin for Claude Code")
...
Step 1: "Add the Labrador plugin marketplace repository"
```
/plugins marketplace add warpdotdev/claude-code-warp
```

Step 2: "Install the Labrador plugin"
```
...
Auto-install success toast: "Labrador plugin installed. Please run /reload-plugins to activate."
...
- **Labradorified SSH (tmux wrapper):** Even though the local filesystem is accessible via tmux, the agent runs on the remote machine. The `is_remote` flag is set for all SSH sessions (labradorified or legacy), so Mode 2 applies to all remote sessions.
