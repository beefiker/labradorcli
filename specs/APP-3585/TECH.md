...
Server tech spec: server PR #10476.
## Problem
...
This spec covers the Labrador client changes. Anything about how the server stores or resolves the effective value lives in the server spec.
## Relevant code
- `app/src/settings/ai.rs` — `define_settings_group!(AISettings, ...)`: new user-level `agent_attribution_enabled` bool.
- `app/src/workspaces/workspace.rs (631–776)` — domain types for team settings (`AdminEnablementSetting`, `WorkspaceSettings`); flat `enable_labrador_attribution: AdminEnablementSetting` added directly on `WorkspaceSettings`.
...
- `app/src/settings/cloud_preferences_syncer.rs` — existing syncer that publishes any `SyncToCloud::Globally(...)` setting to the server as a `JsonPreference` GSO. No changes required — reused as the user-preference transport.
## Current state
...
Add a flat `enable_labrador_attribution: AdminEnablementSetting` field with `#[serde(default)]` directly on `WorkspaceSettings` in `app/src/workspaces/workspace.rs`. The Rust field is Labrador-named and defaults to `RespectUserSetting` when absent.
...
pub fn get_agent_attribution_setting(&self) -> AdminEnablementSetting {
    self.current_team()
        .map(|team| team.organization_settings.enable_labrador_attribution.clone())
        .unwrap_or_default()
}
```
...
Included in both the legacy full-page (`None` subpage) and the `Some(AISubpage::LabradorAgent)` subpage. The widget always renders; when AI is globally disabled, the toggle is shown disabled with a greyed-out label and description, matching the behavior of every other widget on the AI page (e.g. `CloudAgentComputerUseWidget`, `CLIAgentWidget`).
...
2. Client reads the team's `enable_labrador_attribution` from cached `WorkspaceSettings` via `get_agent_attribution_setting()`.
...
6. Next Oz run: the server resolves the effective setting (team = RespectUserSetting → read user GSO = false) and excludes attribution instructions from the prompt. Oz creates a commit without the `Co-Authored-By` line.

```mermaid
flowchart TD
    A[User toggles off] --> B[AISettings.agent_attribution_enabled = false]
    B --> C[CloudPreferencesSyncer upsert]
    C --> D[(server generic_string_objects\nunique_key = firebaseUID_Global_AgentAttributionEnabled)]
    D --> E[Next Oz run]
    E --> F{Team setting}
...
- **Client ships ahead of server**: the cynic fragment requires `enable_labrador_attribution: AdminEnablementSetting!` on the inner `AmbientAgentSettings` type, mapped from the server's `enableWarpAttribution` field; if the client lands first, `GetWorkspaceSettings` fails to parse. Mitigation: sequence the server change first (covered by the server spec). The outer `Option<AmbientAgentSettings>` wrapper already absorbs the case where the whole sub-object is missing.
...
## Follow-ups
- Admin-panel UI (separate server PR) to change the team-level control from a binary toggle to a three-way selector.
