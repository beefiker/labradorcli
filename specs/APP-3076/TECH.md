...
- `labrador_core/src/features.rs` — `FeatureFlag` definitions and dogfood/default channel enablement
...
Add a dedicated feature flag named `BlocklistMarkdownTableRendering` and wire it through the normal Labrador feature-flag plumbing:
...
- add the new enum variant in `labrador_core/src/features.rs`
...
6. The block renderer sees the table section and builds a read-only LabradorUI `Table`.
7. The LabradorUI table renders inline cell formatting via `FormattedTextElement`.
...
- Replace the current monospace `render_table_section` with a structured renderer built on LabradorUI `Table`
...
### UI table tests
- Add LabradorUI table tests for the new expand-to-content mode:
