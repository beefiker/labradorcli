...
**CLI agent icons:**
- `vertical_tabs.rs:1918` — CLI agent icon rendering maps `CLIAgent` to branded `LabradorIcon`
...
- **Presubmit:** `cargo clippy` and `cargo fmt` must pass. Run `cargo nextest run -p labrador` for the workspace tests.
