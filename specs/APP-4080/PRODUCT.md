...
Add a user setting named `Use latest user prompt as conversation title in tab names` that controls which conversation text Labrador shows in vertical tabs for agent conversations.
...
The desired outcome is that Labrador's default tab-name behavior uses stable, title-like conversation labels, while users who prefer prompt-based labels can explicitly opt into using the latest prompt.
...
Labrador exposes a setting in the AI settings area, near settings that affect agent input or agent conversation behavior.
...
- Oz agent conversations, including local agent conversations and cloud agent conversations shown in Labrador.
- Plugin-backed third-party CLI agent sessions, such as supported Claude Code, Codex, or similar sessions where Labrador receives structured session metadata from the agent plugin.
...
- Non-terminal panes such as code panes, notebooks, workflows, settings, files, or Labrador Drive objects.
...
- If the preferred text type is missing, Labrador falls back to the other text type when available.
- If both title-like metadata and prompt metadata are missing, Labrador uses the existing terminal fallback chain.
...
If Labrador normally syncs comparable AI display preferences across devices, this setting should follow that same sync behavior. If comparable AI display preferences are local-only, this setting should follow that local-only behavior instead.
...
- **Persistence**: Toggle the setting, restart Labrador, and verify the setting value and vertical-tabs behavior are preserved.
