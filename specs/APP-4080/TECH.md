...
- **Global AI disabled blocks a third-party setting**: make the UI toggle usable even when Labrador AI is disabled, since the setting also affects plugin-backed third-party CLI sessions.
...
- `cargo test -p labrador workspace::view::vertical_tabs_tests`
- `cargo test -p labrador terminal::cli_agent_sessions::mod_tests`
- `cargo test -p labrador settings::ai_tests`
