...
- `crates/labrador_ui_extras/src/user_preferences/toml_backed.rs` — TOML preferences backend (`new()`, `reload_from_disk()`)
- `app/src/settings/init.rs` — `init_public_user_preferences()`, `init()`, `handle_labrador_config_change()`
...
- `app/src/user_config/mod.rs` — `LabradorConfigUpdateEvent::SettingsErrors` / `SettingsErrorsCleared`
...
- Errors are propagated via `LabradorConfigUpdateEvent` to the workspace banner system.
...
`handle_labrador_config_change()` in `app/src/settings/init.rs`:
- On `reload_from_disk()` failure → emits `LabradorConfigUpdateEvent::SettingsErrors(FileParseFailed(...))`
...
Two new `LabradorConfigUpdateEvent` variants:
...
- `subscribe_to_settings_errors()` subscribes to `LabradorConfig` model for `SettingsErrors`/`SettingsErrorsCleared` events.
...
sequenceDiagram
    ...
    participant LabradorConfig
    participant Handler as handle_labrador_config_change
    participant Workspace

    FileWatcher->>LabradorConfig: Settings event
    LabradorConfig->>Handler: LabradorConfigUpdateEvent::Settings
    Handler->>Handler: reload_from_disk() → Err
    Handler->>LabradorConfig: emit SettingsErrors(FileParseFailed)
    LabradorConfig->>Workspace: SettingsErrors → banner appears
    ...
    Handler->>LabradorConfig: emit SettingsErrorsCleared
    LabradorConfig->>Workspace: SettingsErrorsCleared → banner disappears
```
...
### Unit tests (`crates/labrador_ui_extras/src/user_preferences/toml_backed_tests.rs`)
