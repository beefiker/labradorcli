...
`CloudPreferencesSyncer::handle_initial_load` uses `ForceCloudToMatchLocal::No` on normal startup, meaning cloud values overwrite local values for all synced keys. This is incorrect when the user has made local changes (file edit while Labrador was closed, or UI change while offline) that haven't been synced to cloud.
...
- `crates/labrador_ui_extras/src/user_preferences/toml_backed.rs` — `TomlBackedUserPreferences`, `flush()`, `reload_from_disk()`
...
- `SettingsFileError` type, `LabradorConfigUpdateEvent::SettingsErrors`/`SettingsErrorsCleared`
...
`crates/labrador_ui_extras/src/user_preferences/toml_backed.rs`:
...
sequenceDiagram
    ...
    Note over User,File: User edits file while Labrador is closed

    User->>File: Edit settings.toml

    Note over LibRs: Labrador starts up
