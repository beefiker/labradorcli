...
- `app/src/ai/agent_sdk/driver.rs:1242-1264` — `setup_harness`: subscribes to CLI session events, installs the Labrador plugin
...
- `warpdotdev/claude-code-warp-internal` — private repo containing both the existing notification plugin and the `warp-cloud` platform plugin
...
This installs the public, legacy-named notification plugin. There is no concept of a second platform plugin. The `CliAgentPluginManager` trait models a single plugin per CLI agent.

The internal Claude Code marketplace repo already defines the `warp-cloud` marketplace entry and contains the platform plugin, but nothing in the Labrador client installs it.
...
sequenceDiagram
    ...
    Note over Setup: Install Labrador plugin (existing)
    ...
    Note over CC: Both plugins active:<br/>notification plugin + platform plugin
```
...
**Plugin name collision.** The legacy-named notification plugin and `warp-cloud` platform plugin are in different marketplace repos. Claude Code's plugin system keys plugins by `<plugin_name>@<marketplace_name>`, so `warp@claude-code-warp` and `warp-cloud@claude-code-warp` are distinct. No collision risk.
