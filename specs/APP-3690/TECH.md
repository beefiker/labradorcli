...
- `install_instructions()` → static `PluginInstructions` with title "Install Labrador Plugin for OpenCode", steps to add `"opencode-warp"` to the `plugin` array in `opencode.json` + restart
- `update_instructions()` → static `PluginInstructions` with title "Update Labrador Plugin for OpenCode", steps to `rm -rf ~/.cache/opencode/node_modules/opencode-warp` + restart
...
- `update_plugin_button`: "Update Labrador plugin"
...
### Install
1. User starts OpenCode in Labrador. Command detection creates a `CLIAgentSession`. The footer's `Started` subscription fires, spawning a debounce timer (`plugin_chip_ready` starts `false`).
...
**Risk: Stale `MINIMUM_PLUGIN_VERSION`.** The minimum version is compiled into the Labrador binary. **Mitigation:** Same as Claude — by design. We only prompt updates when Labrador needs new plugin behavior.
