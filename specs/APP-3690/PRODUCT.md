...
Add install and update chip support for the OpenCode Labrador plugin (`opencode-warp`), matching the existing Claude Code chip UX. Clicking the chip opens a modal with manual installation/update instructions.

## Problem

When a user runs OpenCode inside Labrador, the Labrador plugin enriches the experience with native notifications and session status tracking. Today, Labrador has no way to prompt OpenCode users to install or update this plugin — `plugin_manager_for(CLIAgent::OpenCode)` returns `None`.
...
- Title: "Install Labrador Plugin for OpenCode"
- Subtitle: "Add the Labrador plugin to your OpenCode configuration, then restart OpenCode."
- Steps:
...
- Since the opencode-Labrador plugin has never been released, there is no legacy version that predates version reporting. Every installed version will report `plugin_version`.

**Update instructions modal:**
- Title: "Update Labrador Plugin for OpenCode"
...
1. Starting an OpenCode session in Labrador when the plugin has never connected shows a green "Notifications setup instructions" chip (after a brief debounce).
