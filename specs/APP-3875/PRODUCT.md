...
- If the user later returns to `View as = Tabs`, Labrador restores the last selected `Tab item` mode.
...
Labrador should not try to pick one "most representative" directory in Summary mode. The card should show the stable list of unique working directories instead.

Examples:

- `~/labrador-internal`
- `~/labrador-internal • ~/labrador-server • ~/labrador-terraform`
...
If multiple panes in the same coalesced group expose the same logical branch metadata, Labrador should simply coalesce them into that one rendered line rather than treating them as separate rows.
...
Labrador must not insert placeholder copy such as `No branch` or `No directory`.
...
- Click a Summary card and verify Labrador activates the tab and focuses its active pane.
