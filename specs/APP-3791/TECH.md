...
When a user SSHes into a remote host, Labrador's completions pipeline (autosuggestions, syntax highlighting, tab completions) needs to run generator commands on the remote machine. Today this is done by opening a new SSH session per command, which is constrained by the host's `MaxSessions` limit and cannot run commands in parallel.
...
4. LabradorifiedRemote + legacy SSH + !InBandForSSH   → RemoteCommandExecutor
