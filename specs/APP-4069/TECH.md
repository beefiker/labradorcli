...
## 1. Problem
When a user SSHes into a remote host, we want to introduce a choice block for users to choose between (1) installing and connecting to the remote server (2) falling back to the existing Labradorify behavior.

To do so, we'll need to block the current bootstrap and connect server flow. Today, the client has a race where: `PtyController` writes the legacy bootstrap script to the PTY synchronously on `InitShell`, while `TerminalView` in parallel kicks off an async background task in `RemoteServerManager` that checks for the remote-server binary, installs it if missing, and initializes the server. Because the bootstrap is written before the check completes, we cannot defer or cancel Labradorification based on the check result — by the time we know whether the remote server is available, the legacy Labradorification has already taken effect.
...
- **Yes, install** — flush the bootstrap, install the binary on the remote, launch + handshake the remote server. Session is fully labradorified via the remote-server path.
- **No, skip** — flush the stashed bootstrap (so the shell is properly initialized) but do not call `connect_session`. The session falls back to ControlMaster Labradorification without engaging the remote-server path.
...
- `crates/labrador_features/src/lib.rs:806` — `FeatureFlag::SshRemoteServer`.
...
Structure:
- **Header** — `HeaderConfig` from `inline_action_header` with title "Install Labrador remote server?" and a suitable icon.
- **Description** — `render_text_with_markdown_support` (the same helper `AskUserQuestionView` uses) briefly explaining that Labrador can install a helper binary on the remote host to enable remote-server features.
- **Two numbered buttons via `NumberShortcutButtons`**:
  - `1.` "Install Labrador's SSH extension"
